  ...
  function walk(_t, _nonceIn, _lax, _goal, _delta) {
    ...
    switch (rule) {
      ...
      case 'loli_match': {
        ...
        let groundBody;
        if (groundFacts.length === 0) {
          groundBody = Store.put('one', []);
        } else if (groundFacts.length === 1) {
          groundBody = groundFacts[0];
        } else {
          groundBody = groundFacts[groundFacts.length - 1];
          for (let i = groundFacts.length - 2; i >= 0; i--) {
            groundBody = Store.put('tensor', [groundFacts[i], groundBody]);
          }
        }
        const groundMonad = Store.put('monad', [groundBody]);
        const groundLoli = Store.put('loli', [c0, groundMonad]);

        // SubstChip: swap original → ground on context bus
        chipRows('subst').push([1, hash, groundLoli]);
        deltaRemove(delta, hash);
