...
const { binToInt } = require('../engine/ffi/convert');
...
function generateWitness(term, sequent, opts = {}) {
  ...
  // BabyBear prime: values >= this are not directly constrainable
  const BABY_BEAR_P = 2013265921n;

  /**
   * Extract predicate metadata from a ground predicate hash.
   * Returns { is_plus, is_mul, is_inc, arg0, arg1, arg2 } or null if unverifiable.
   */
  function extractPredMeta(predHash) {
    ...
    let is_plus = 0, is_mul = 0, is_inc = 0;
    let arg0 = 0, arg1 = 0, arg2 = 0;

    if (tag === 'plus' && arity === 3) {
      const a = argVal(0), b = argVal(1), c = argVal(2);
      if (a === null || b === null || c === null) return null;
      is_plus = 1; arg0 = a; arg1 = b; arg2 = c;
    } else if (tag === 'mul' && arity === 3) {
      const a = argVal(0), b = argVal(1), c = argVal(2);
      if (a === null || b === null || c === null) return null;
      is_mul = 1; arg0 = a; arg1 = b; arg2 = c;
    } else if (tag === 'inc' && arity === 2) {
      const a = argVal(0), b = argVal(1);
      if (a === null || b === null) return null;
      is_inc = 1; arg0 = a; arg1 = b; arg2 = 0;
    } else {
      // Unverified predicate (arr_get, mem_expand, etc.) — no selector set
      return null;
    }
    return { is_plus, is_mul, is_inc, arg0, arg1, arg2 };
  }
  ...
  function addPredRomEntry(predHash) {
    ...
    // Unrecognized predicates (arr_get, mem_expand, etc.) are NOT added to pred_rom.
    // Their verification is deferred to later phases (6-6b+). Only plus/mul/inc
    // get PRED_BUS entries — fact_axiom sets pred_active=0 for unrecognized predicates.
  }
      ...
      case 'copy': {
        ...
        if (customChips.size > 0 && Store.isTerm(hash) && Store.tag(hash) === 'loli') {
          ...
          if (Store.isTerm(monadH) && Store.tag(monadH) === 'monad') {
            ...
            // Phase 6-6a: only set pred_hash/pred_active for recognized predicates
            // (plus, mul, inc). Unrecognized predicates (arr_get, mem_expand) get
            // pred_hash=0, pred_active=0 — no PRED_BUS interaction.
            let predHash = 0;
            let predActive = 0;
  ...
  // --- Build pred ROM (Phase 6-6a: in-circuit predicate verification) ---
  // [pred_hash, is_active, num_lookups, is_plus, is_mul, is_inc, arg0, arg1, arg2]
  const pred_rom = [];
  for (const [hash, entry] of predRomEntries) {
    pred_rom.push([hash, 1, entry.count, entry.is_plus, entry.is_mul, entry.is_inc, entry.arg0, entry.arg1, entry.arg2]);
  }
  ...
  verifyBusBalance(chips, fact_rom, pred_rom);

  return { tags: ZK_TAGS, rule_specs: ruleSpecs, chips, formula_rom, gamma_rom, freevar_rom, fact_rom, pred_rom };
}
 ...
 */
function verifyBusBalance(chips, fact_rom, pred_rom) {
  ...
  // PRED_BUS: pred_rom supply count must equal fact_axiom pred_active demand count
  ...
  if (predRomSupply !== predDemand) {
    throw new Error(
      `PRED_BUS imbalance: pred_rom supply=${predRomSupply} but fact_axiom pred_active demand=${predDemand}`
    );
  }
}
