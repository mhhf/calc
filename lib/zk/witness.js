  ...
  function walk(_t, _nonceIn, _lax, _goal, _delta) {
    ...
    switch (rule) {
      ...
      case 'copy': {
        ...
        // Phase 6-4: custom chip short-circuit.
        // Clause proof subtrees for registered predicates have
        // principal = loli(ant, monad(Q)) where Q is the proved predicate.
        // Emit one fact_axiom row instead of walking the full clause proof
        // subtree (e.g., ~4600 nodes for arr_get → 1 row).
        if (customChips.size > 0 && Store.isTerm(hash) && Store.tag(hash) === 'loli') {
          ...
          if (Store.isTerm(monadH) && Store.tag(monadH) === 'monad') {
            const goalH = Store.child(monadH, 0);
            const predTag = Store.tag(goalH);
            if (predTag && customChips.has(predTag)) {
              // fact_axiom: [active, goal_hash, nonce_in, lax]
              chipRows('fact_axiom').push([1, goalH, nonceIn, lax]);
              factRomEntries.set(goalH, (factRomEntries.get(goalH) || 0) + 1);
              break processNode;
            }
          }
        }
