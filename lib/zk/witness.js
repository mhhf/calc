 ...
 * Handles plus and inc with values >= BabyBear prime (deferred from extractPredMeta).
 *
 * Returns { is_plus_256, is_inc_256, a_limbs, b_limbs, c_limbs, carries } or null.
 * Values must be non-negative and fit in 256 bits.
 *
...
function extractUint256PredMeta(predHash) {
  ...
  if (tag === 'plus' && arity === 3) {
    ...
    // Only handle if at least one value exceeds BabyBear
    if (a < BABY_BEAR_P && b < BABY_BEAR_P && c < BABY_BEAR_P) return null;
    const aL = bigintToLimbs(a), bL = bigintToLimbs(b), cL = bigintToLimbs(c);
    return { is_plus_256: 1, is_inc_256: 0, a_limbs: aL, b_limbs: bL, c_limbs: cL,
             carries: computeAdditionCarries(aL, bL) };
    ...
    return { is_plus_256: 0, is_inc_256: 1, a_limbs: aL, b_limbs: bL, c_limbs: cL,
             carries: computeIncrementCarries(aL) };
  }
  return null;
}
...
function generateWitness(term, sequent, opts = {}) {
  ...
  function addPredRomEntry(predHash) {
    ...
    if (u256meta) {
      ...
      // Accumulate byte lookup counts for all limbs
      for (let i = 0; i < NUM_LIMBS; i++) {
  ...
  // JSON format: all 133 cols concatenated
  const uint256_arith = [];
  for (const [hash, entry] of uint256ArithEntries) {
    const row = [hash, 1, entry.is_plus_256, entry.is_inc_256];
    ...
    row.push(...entry.carries); // 32 carries
...
function verifyBusBalance(chips, fact_rom, pred_rom, uint256_arith) {
  ...
  // uint256_arith: num_lookups is at index 100 (after 100 preprocessed cols)
  const uint256Supply = (uint256_arith || []).reduce((sum, entry) => sum + entry[100], 0);
...
module.exports = {
  ...
  extractUint256PredMeta, bigintToLimbs, computeAdditionCarries, computeIncrementCarries,
};
