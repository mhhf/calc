...
function deriveZkRuleSpecs(calculus, zkTags) {
  ...
  // Used for persistent goals resolved by FFI (arithmetic, etc.).
  // ZK-unsound — the verifier trusts these. Phase 4: replace with clause evidence.
  specs.ffi = {
...
function generateWitness(term, sequent, opts = {}) {
  ...
  function walk(_t, _nonceIn, _lax, _goal, _delta) {
    ...
    switch (rule) {
      ...
      case 'loli_match': {
        // Opaque loli match: the loli body may contain freevars that the match
        // resolves. Instead of decomposing via monad_l/tensor_l (which follows
        // Store.child and gets unresolved hashes), directly adjust delta with
        // the ground facts produced by the forward engine.
        const hash = t.principal;
        const c0 = Store.child(hash, 0); // antecedent
        const n0 = ++nonceCtr;

        // Emit loli_l chip row (c1 = Store.child for ZK formula verification)
        const c1 = Store.child(hash, 1);
        chipRows('loli_l').push([1, hash, c0, c1, nonceIn, lax, n0, 0, goal]);
        addFormulaLookup(hash);
        deltaRemove(delta, hash);

        // Process antecedent proof (sub0): consumes trigger facts from delta
        walk(t.subterms[0], n0, lax, c0, delta);

        // Add ground consequent facts directly (not from Store decomposition)
  ...
  return { tags: ZK_TAGS, rule_specs: ruleSpecs, chips, formula_rom, gamma_rom };
}
