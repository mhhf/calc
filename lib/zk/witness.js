...
function generateWitness(term, sequent, opts = {}) {
  ...
  if (linear.length > 0) {
    chipRows('init').push([linear[0], 1, succedent, 1, 0, 0]);
  } else {
    chipRows('init').push([0, 0, succedent, 1, 0, 0]);
  }
  // Additional context rows
  for (let i = 1; i < linear.length; i++) {
    chipRows('init').push([linear[i], 1, 0, 0, 0, 0]);
  }
  ...
  function addPredRomEntry(predHash) {
    ...
    // Unrecognized predicates (not handled by extractPredMeta) are NOT added.
    // Their verification is deferred to Phase 6-6b (256-bit values).
  }
  ...
  return {
    tags: ZK_TAGS, rule_specs: ruleSpecs, chips, formula_rom, gamma_rom, freevar_rom, fact_rom, pred_rom,
...
module.exports = { generateWitness, generateChunkedTreeWitness, deriveZkTags, deriveZkRuleSpecs };
