 ...
 * verification + SUBST_TREE_BUS body demands.
 * Uses 5 buses (CONTEXT + GAMMA + FORMULA + SUBST_TREE + FREEVAR).
 */
...
function generateFlatWitness(trace, sequent, opts = {}) {
  ...
  for (const step of trace) {
    ...
    row.push(groundCons);  // ground consequent tensor (reserved)
    ...
    // When body_diff[i]=0, body_leaf must equal produced[i] (anti-suppression constraint).
    for (let i = 0; i < MAX_PRODUCED; i++) {
      if (bodyDiff[i] === 1 && i < bodyLeaves.length) {
        row.push(bodyLeaves[i]);
      } else {
        row.push(i < step.produced.length ? step.produced[i] : 0);
      }
    }
