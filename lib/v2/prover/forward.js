 ...
 * @returns {{ [predicate: string]: number[] }}
 */
 ...
 *
 * Uses dual indexing:
 * 1. State index: facts grouped by predicate for fast pattern matching
 * 2. Rule index: rules grouped by trigger predicates to avoid trying irrelevant rules
 *
 * Strict filtering: only try rules where ALL linear antecedent predicates exist in state.
 * This is sound because if a rule needs predicate P but state has no P facts, the rule
 * cannot possibly match.
 *
 * @param {Object} state - { linear, persistent }
 * @param {Object[]} rules - Compiled rules (or { rules, ruleIndex } for cached index)
 * @param {Object} calc - { clauses, types } for backward proving
 * @returns {{ rule, theta, consumed } | null}
 */
function findMatch(state, rules, calc) {
  ...
  // Get rule list
  const ruleList = rules.ruleIndex ? rules.rules : rules;
  ...
  // Strict candidate filtering: only include rules where ALL trigger predicates exist in state
  // This is sound because if a rule needs (pc X) and (stack Y) but state has no stack facts,
  // the rule cannot possibly fire
  const orderedCandidates = ruleList.filter(rule => {
    const triggers = rule.triggerPreds || [];
    // Rule with no triggers goes to wildcard bucket - always try it
    if (triggers.length === 0) return true;
    // All trigger predicates must exist in state
...
function run(state, rules, opts = {}) {
  ...
  // Build rule index once for all steps
  const ruleIndex = buildRuleIndex(rules);
  const indexedRules = { rules, ruleIndex };
