...
function createProver(calculus) {
  ...
  const prove = (seq, opts = {}) => {
    ...
    const search = (seq, state, depth, delta) => {
      ...
      // Phase 0: Try identity FIRST (A ⊢ A for any A, not just atoms)
      // This handles cases like !A ⊢ !A before inversion decomposes them
      ...
      // Phase 0.5: Try copy from cartesian if succedent matches something there
      // This enables proofs like: A; · ⊢ A (copy A from cartesian to linear, then identity)
      ...
      if (cart.length > 0 && Context.isEmpty(delta) && ruleSpecs.copy) {
        for (let i = 0; i < cart.length; i++) {
          ...
          if (theta) {
            // Found a match in cartesian - apply copy to bring it to linear
            const copySpec = ruleSpecs.copy;
      ...
      if (state.isFocused()) {
        ...
        // For bang_l, also try absorption (moves to cartesian instead of linear)
