...
function createManualProofAPI(calculus) {
  const ruleSpecs = buildRuleSpecs(calculus);
  ...
  const isContextSplitting = (ruleName) => {
    return ['tensor_r', 'loli_l'].includes(ruleName);
  };
  ...
  function getApplicableActions(state, opts = {}) {
    ...
    if (!hasFocus) {
      ...
      if (succedent && !isAtomic(succedent)) {
        ...
        if (tag) {
          const rn = `${tag}_r`;
          if (ruleSpecs[rn] && prover.ruleIsInvertible(tag, 'r')) {
            const action = buildRuleAction(rn, 'R', -1, succedent, seq, state.delta);
            if (action) actions.push(action);
          }
        }
      ...
      for (let i = 0; i < linear.length; i++) {
        ...
        if (!isAtomic(formula)) {
          ...
          if (tag) {
            // Main rule
            const rn = `${tag}_l`;
            if (ruleSpecs[rn] && prover.ruleIsInvertible(tag, 'l')) {
              const action = buildRuleAction(rn, 'L', i, formula, seq, state.delta);
              if (action) actions.push(action);
            }
            // Alternative rules (with_l1, with_l2)
            for (const suffix of ['1', '2']) {
              const altName = `${tag}_l${suffix}`;
              if (ruleSpecs[altName] && prover.ruleIsInvertible(tag, 'l')) {
                const action = buildRuleAction(altName, 'L', i, formula, seq, state.delta);
                if (action) actions.push(action);
              }
            }
          }
    ...
    if (hasFocus) {
      ...
      if (isAtomic(focusFormula)) {
        ...
        if (tag) {
          ...
          // Main rule
          if (ruleSpecs[ruleName]) {
            const action = buildRuleAction(ruleName, focusPos, focusIdx, focusFormula, seq, state.delta);
            if (action) actions.push(action);
          }

          // Alternative rules
          for (const suffix of ['1', '2']) {
            const altName = `${tag}_${side}${suffix}`;
            if (ruleSpecs[altName]) {
              const action = buildRuleAction(altName, focusPos, focusIdx, focusFormula, seq, state.delta);
              if (action) actions.push(action);
            }
          }
        }
