...
/**
 * Build AST constructor functions (using Store.intern for hash-based terms)
 */
...
/**
 * Build renderer (handles both hashes and AST objects)
 */
function buildRenderer(constructors) {
  ...
  return function render(ast, format = 'ascii', parentPrec = 0) {
    // Handle hashes (numbers) - look them up in the Store
    if (typeof ast === 'number') {
      const node = Store.get(ast);
      if (!node) return String(ast);
      return render(node, format, parentPrec);
    }

    // Handle primitives (atom names, variable names)
