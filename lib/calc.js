/**
 * CALC abstraction - v1 calculus loader
 *
 * @deprecated This is v1 code. Use lib/v2 instead for new code.
 *   const v2 = require('./lib/v2');
 *   const calc = await v2.loadILL();
 */
const { inferAllPolarities, inferAllContextModes } = require('./polarity');
...
// Cache for inferred polarity (computed once per calc initialization)
let inferredPolarityCache = null;
let inferredContextModeCache = null;

const analyzeProperties = function (calc, ctxName, ruleName, o) {
  // Get polarity from explicit specification OR infer from rules
  let polMap = calc.calc_structure_rules_meta?.polarity || {};

  // If polarity not specified, use inference
  if (ctxName === "Formula") {
    if (typeof polMap[ruleName] === "string") {
      // Use explicit polarity if specified
      o.polarity = polMap[ruleName];
    } else if (inferredPolarityCache && inferredPolarityCache[ruleName]) {
      // Use inferred polarity
      o.polarity = inferredPolarityCache[ruleName];
    }
  }
...
Calc.init = function (calc) {
  // Initialize calculus database (should only be called once)
  ...
  // Infer polarity and context modes from rule structure
  // This allows removing explicit polarity specification from ll.json
  if (calc.rules) {
    inferredPolarityCache = inferAllPolarities(calc.rules);
    inferredContextModeCache = inferAllContextModes(calc.rules);
  }
