...
class Node {
  ...
  getPolarity(o = {positive: [], negative: []}) {
    ...
    let isPositiveFormula = (!isAtomic && polarityMap[type.ruleName] === "positive")
    let isPositive = isPositiveVar || isPositiveFormula;
    let isNegativeVar = (isAtomic && o.negative.indexOf(name) > -1);
    let isNegativeFormula = (!isAtomic && polarityMap[type.ruleName] === "negative")
  ...
  isTermType() {
    ...
    return type.ruleName === "Structure_Term_Formula"
        || type.ruleName === "Structure_Focused_Term_Formula"
  }
  isNegative(o) {
