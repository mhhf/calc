  ...
  switch (nodeType) {
    ...
    case 'expr': {
      ...
      if (text.includes('-o')) {
        ...
        if (text.includes('{')) {
          return AST.TermForward(left, right);
        }
        return AST.TermLoli(left, right);
      }
    ...
    case 'expr_with': {
      ...
      const left = cstToAst(namedChildren[0], source);
      const right = cstToAst(namedChildren[1], source);
      return AST.TermWith(left, right);
    }
    ...
    case 'expr_tensor': {
      ...
      for (let i = 1; i < namedChildren.length; i++) {
        result = AST.TermTensor(result, cstToAst(namedChildren[i], source));
      }
      return result;
    }

    case 'expr_bang': {
      // Check if this node has a '!' by looking at all children
      let hasBang = false;
      for (let i = 0; i < node.childCount; i++) {
        const child = node.child(i);
        if (!child.isNamed && source.slice(child.startIndex, child.endIndex) === '!') {
          hasBang = true;
          break;
        }
      }
      const namedChildren = getNamedChildren();
      if (!hasBang && namedChildren.length === 1) {
        return cstToAst(namedChildren[0], source);
      }
      // Has ! prefix
      return AST.TermBang(cstToAst(namedChildren[0], source));
    }
...
function isTypeExpr(node) {
  ...
  if (node.type === 'TypeAtom') return true;
