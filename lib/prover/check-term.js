...
function buildCheckerMap(calculus) {
  ...
  // checkers below. Default is ILL's unary monad; a graded calculus supplies
  // { tag, bodyIdx: 1, gradeIdx: 0 } via its roles and gets `<tag>_r`/`<tag>_l`
  // checkers that accept the graded node (the grade child is threaded
  // opaquely; subeffect checking arrives with the grade algebra, Phase 3).
  const comp = calculus.roles?.computation || { tag: 'monad', bodyIdx: 0, gradeIdx: null };
