...
function buildCheckerMap(calculus) {
  ...
  // Computation (lax monad) role record — tag + child layout for the monad
  // checkers below. Default is the graded monad (binary since the D6
  // merge-back, TODO_0157 — ILL is the trivial-grade instance); the grade
  // child is threaded opaquely; subeffect checking arrives with the grade
  // algebra (Phase 3).
  const comp = calculus.roles?.computation || { tag: 'monad', bodyIdx: 1, gradeIdx: 0 };
  // Linear implication role (TODO_0086): the focused/invertible left rule
  // and the guided-execution match checker are generic over ANY linear
  // implication — the tag comes from the derived roles, same discipline
  // as the computation checkers below.
  const imp = calculus.roles?.implication || 'loli';
  ...
  checkers[`${comp.tag}_r`] = (gamma, delta, term, type, lax) => {
    if (type.tag !== comp.tag) return fail(`${comp.tag}_r: type not monadic`);
    // Guided mode: evidence is a full proof term — recurse into it.
    // The lax flag becomes true: once inside the monad, the judgment is |-_lax
    // (Pfenning-Davies 2001). The left rule is only valid under |-_lax.
    if (term.evidence && term.evidence.rule) {
      const innerType = type.children[comp.bodyIdx];
  ...
  checkers[`${comp.tag}_l`] = (gamma, delta, term, type, lax) => {
    // Stickiness: the left rule is only valid under |-_lax (Pfenning-Davies
    // 2001). The prover enforces this via requiresSuccedentTag on the
    // descriptor. The checker independently verifies: once you enter the
    // monad (via the right rule setting lax=true), you stay — the left rule
    // outside lax mode is a proof error.
    if (!lax) return fail(`${comp.tag}_l: not in lax mode (must be inside ${comp.tag}_r)`);
    const p = term.principal;
    if (p === null) return fail(`${comp.tag}_l: no principal`);
    if (!delta.has(p)) return fail(`${comp.tag}_l: principal not in delta`);
    const pType = delta.get(p);
    if (pType.tag !== comp.tag) return fail(`${comp.tag}_l: principal not monadic`);
    // Remove principal, add body type as new linear binding (grade rides along)
    const inner = pType.children[comp.bodyIdx];
    const newDelta = mapDelete(delta, p);
    newDelta.set(inner.hash, inner);
    if (term.subterms.length !== 1) return fail(`${comp.tag}_l: expected 1 subterm`);
    return checkTerm(gamma, newDelta, term.subterms[0], type, lax, checkers);
  };
  ...
  checkers[`${imp}_l`] = (gamma, delta, term, type, lax) => {
    const p = term.principal;
    if (p === null) return fail(`${imp}_l: no principal`);
    if (!delta.has(p)) return fail(`${imp}_l: principal not in delta`);
    const pType = delta.get(p);
    if (pType.tag !== imp) return fail(`${imp}_l: tag mismatch`);
    ...
    if (term.subterms.length !== 1) return fail(`${imp}_l: expected 1 or 2 subterms`);
  ...
  // ── loli_match (linear loli consumption from guided execution) ──
  //
  // guided-term.js emits loli_match when the forward engine consumes a linear
  // loli (A -o {B}) via dynamic matching. Structurally identical to focused
  // loli_l, but the principal loli may contain freevars that were resolved by
  // the match — so we use the ground types from the term instead of
  // decomposing the original loli via Store.child.
  //
  // The ground types (groundAntecedent, groundFacts) are oracle outputs from
  // the forward engine's pattern matcher, same trust level as monad_r's
  // rightFocus check.

  checkers[`${imp}_match`] = (gamma, delta, term, type, lax) => {
    const p = term.principal;
    if (p === null) return fail(`${imp}_match: no principal`);
    if (!delta.has(p)) return fail(`${imp}_match: principal not in delta`);
    const pType = delta.get(p);
    if (pType.tag !== imp) return fail(`${imp}_match: principal not implication`);
    if (term.subterms.length !== 2) return fail(`${imp}_match: expected 2 subterms`);

    const workDelta = mapDelete(delta, p);

    // Ground antecedent: the resolved A after freevar substitution
    const A = expand(term.groundAntecedent);
    if (!A) return fail('loli_match: cannot expand groundAntecedent');

    // Ground consequent: {tensor(groundFacts...)} — for a graded computation
    // the grade is taken from the principal loli's own consequent node
    const facts = term.groundFacts;
    if (!facts || !Array.isArray(facts)) return fail('loli_match: missing groundFacts');
    const consBody = rTensor(facts);
    let consNode;
    if (comp.gradeIdx === null) {
      consNode = Store.put(comp.tag, [consBody]);
    } else {
      const bNode = pType.children[1];
      if (!bNode || bNode.tag !== comp.tag) return fail('loli_match: consequent not monadic');
      const kids = [];
      kids[comp.gradeIdx] = bNode.children[comp.gradeIdx].hash;
      kids[comp.bodyIdx] = consBody;
      consNode = Store.put(comp.tag, kids);
    }
    const B = expand(consNode);
    if (!B) return fail('loli_match: cannot expand ground consequent');

    // Check antecedent proof, then continuation with consequent in delta
    const r1 = checkTerm(gamma, workDelta, term.subterms[0], A, lax, checkers);
    if (!r1.valid) return r1;
    const newDelta = new Map(r1.remaining);
    newDelta.set(B.hash, B);
...
function createChecker(calculus) {
   ...
   *
   * @param {Object} term       - GenericTerm (from extractTerm or monadicTerm)
  ...
  function check(term, sequent) {
    ...
    const gamma = new Map();
    const delta = new Map();
    for (const h of Seq.getContext(sequent, ctxStruct.copySource))
      gamma.set(h, expand(h));
    for (const h of Seq.getContext(sequent, ctxStruct.consumableZone))
...
export { createChecker, expand };
export default { createChecker, expand };
