...
function buildCheckerMap(calculus) {
  ...
  checkers['loli_match'] = (gamma, delta, term, type, lax) => {
    const p = term.principal;
    if (p === null) return fail('loli_match: no principal');
    if (!delta.has(p)) return fail('loli_match: principal not in delta');
    const pType = delta.get(p);
    if (pType.tag !== 'loli') return fail('loli_match: principal not loli');
    if (term.subterms.length !== 2) return fail('loli_match: expected 2 subterms');
