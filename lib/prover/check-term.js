...
function buildCheckerMap(calculus) {
  ...
  checkers['monad_r'] = (gamma, delta, term, type, lax) => {
    ...
    // Full/opaque mode: evidence from bridge was verified separately
    return ok(delta);
  };
  ...
  // ── Focused loli_l (2-subterm variant for guided execution profile) ──
  // Invertible (1-subterm): handled by descriptor-generated rule below.
  // Focused (2-subterm): first proves antecedent, second continues with consequent.
  // Sequential context split: first subterm consumes antecedent resources,
  // remaining + consequent type passes to second subterm.
...
function buildRuleChecker(d, checkers) {
  return (gamma, delta, term, type, lax) => {
    ...
    // Quantifier binding: open body with witness/eigenvariable via Store-level substitution
    ...
    if (d.premises.length === 0) {
      ...
      // zero_l: from absurdity, anything follows — discard all linear resources
