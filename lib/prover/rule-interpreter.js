...
function interpretPremises(formula, seq, _index) {
  ...
  if (type === 'template') {
    ...
    const cart = Seq.getContext(seq, cs.copySource);
    return t.premises.map(p => {
      ...
      return Seq.seq({
        [cs.consumableZone]: lin,
        [cs.copySource]: xCart.length ? [...cart, ...xCart] : cart,
      }, succ);
    });
  }
  ...
  // Structural: copy from the copy-source zone into the consumable zone
  if (type === 'structural') {
    const cs = this._ctxStruct || Seq.DEFAULT_CONTEXT_STRUCTURE;
    const cart = Seq.getContext(seq, cs.copySource);
    const linear = Seq.getContext(seq, cs.consumableZone);
    return [Seq.seq({
      [cs.consumableZone]: [...linear, formula],
      [cs.copySource]: cart,
    }, seq.succedent)];
  }
