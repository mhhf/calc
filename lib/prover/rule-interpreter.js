...
function computeRuleSpecMeta(rules) {
  ...
  for (const [name, rule] of Object.entries(rules)) {
    ...
    // (e.g., zero_l: the entire linear context is discarded)
    if (rule.numPremises === 0 && d.side === 'l' && !d.emptyLinear) data.discardsContext = true;
    if (d.modeShift) data.modeShift = true;
    if (d.requiresMonadicSuccedent) data.requiresMonadicSuccedent = true;
...
function buildRuleSpecsFromMeta(meta, rules) {
  ...
  for (const [key, data] of Object.entries(meta.specData)) {
    ...
    if (data.requiresMonadicSuccedent) spec.requiresMonadicSuccedent = true;
...
module.exports = { buildRuleSpecs, computeRuleSpecMeta, buildRuleSpecsFromMeta, initRuleSpecs };
