  ...
  if (type === 'template') {
    ...
    if (t.steps.length) {
      ...
      for (const s of t.steps) {
        ...
        if (s.kind === 'def') {
          // `-` is the algebra's PARTIAL residual ⊖: null (out of fence) ⇒
          // the rule is inapplicable — partiality IS the guard (TODO_0273)
          const r = s.op === '+' ? g.effect.compose(a, b) : g.effect.residual(a, b);
