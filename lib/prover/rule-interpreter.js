...
function specMeta(rules) {
  ...
  for (const [name, rule] of Object.entries(rules)) {
    ...
    if (d.fireStep) data.fireStep = true;
 ...
 */
function specsFromMeta(meta, rules, theory = null) {
  const specs = {};
  ...
  for (const [key, data] of Object.entries(meta.specData)) {
    ...
    if (data.fireStep) spec.fireStep = true;
...
function buildRuleSpecs(calculus) {
  ...
  return specsFromMeta(meta, calculus.rules, calculus.theory);
}
...
function initRuleSpecs(calculus) {
  return calculus.ruleSpecMeta
    ? specsFromMeta(calculus.ruleSpecMeta, calculus.rules, calculus.theory)
