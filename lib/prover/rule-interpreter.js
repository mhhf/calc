 ...
 */
function specsFromMeta(meta, rules) {
  const specs = {};
...
function buildRuleSpecs(calculus) {
  ...
  return specsFromMeta(meta, calculus.rules);
}
...
function initRuleSpecs(calculus) {
  return calculus.ruleSpecMeta
    ? specsFromMeta(calculus.ruleSpecMeta, calculus.rules)
    : buildRuleSpecs(calculus);
}

module.exports = { buildRuleSpecs, specMeta, initRuleSpecs };
