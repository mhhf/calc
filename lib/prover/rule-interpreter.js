...
function specMeta(rules) {
  ...
  for (const [name, rule] of Object.entries(rules)) {
    ...
    // A @tick rule (whole-context next) generalizes the empty-linear base case,
    // so it must NOT be gated to an empty delta — it fires on an all-○ context.
    if (d.emptyLinear && rule.numPremises > 0 && !d.tick) data.requiresEmptyDelta = true;
    if (d.tick) { data.tick = true; data.tickConn = d.connective; }
    if (d.contextSplit) data.contextSplit = true;
    if (d.binding) data.binding = d.binding;
    if (d.affine) data.affine = true;
    // Zero-premise rules with a FREE linear context (no premise to thread it to,
    // not required empty) absorb it: zero_l (0L, left) and top_r (⊤R, right) —
    // the additive units. A zero-premise rule can only soundly discard a free Δ,
    // so this is side-agnostic. Template axioms (retiming at_l) consume ONLY their
    // principal — never discard.
    if (rule.numPremises === 0 && !d.emptyLinear && !d.template) data.discardsContext = true;
    // Zero-premise RIGHT rules (one_r, top_r) name a principal succedent
    // connective but generate no arity-checked premise, so the kernel/search must
    // re-validate the succedent tag — else a forged node proves an arbitrary
    // succedent (one_r ⊢ b; the far worse top_r a ⊢ b via context absorption).
    if (rule.numPremises === 0 && d.side === 'r' && d.connective) data.zeroSuccConn = d.connective;
...
function specsFromMeta(meta, rules, theory = null, ctxStruct = null) {
  ...
  for (const [key, data] of Object.entries(meta.specData)) {
    ...
    if (data.tick) { spec.tick = true; spec._tickConn = data.tickConn; }
    if (data.contextSplit) spec.contextSplit = true;
    if (data.discardsContext) spec.discardsContext = true;
    if (data.zeroSuccConn) { spec._zeroSuccConn = data.zeroSuccConn; spec.requiresSuccedentTag = data.zeroSuccConn; }
