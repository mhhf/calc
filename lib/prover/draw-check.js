...
function walkWitness(term, sort, program, out) {
  ...
  const isDs = program.sorts.isDatasort && program.sorts.isDatasort(sort);
  ...
  const memberPool = isDs
    ? program.sorts.datasortInfo(sort).members
    : program.sorts.membersOf(sort);
  let found = false;
  for (const m of memberPool) if (m === member) { found = true; break; }
  ...
  out.tokens.push(Store.put('drawn', [atom(member), atom(sort)]));
  const prior = (program.priors && program.priors.get(member)) || [1n, 1n];
  out.weight = [out.weight[0] * prior[0], out.weight[1] * prior[1]];
  if (!isAtom) {
    for (let i = 0; i < arity; i++) {
      const err = walkWitness(Store.child(term, i), sig.argSorts[i], program, out);
      if (err) return err;
    }
  }
  return null;
...
const drawChecker = Object.freeze({
  step(conclusion, state, { calculus, program }) {
    ...
    let lin = Context.fromArray(Seq.getContext(conclusion, 'linear'));
