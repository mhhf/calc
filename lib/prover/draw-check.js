...
function checkDrawTreeStep(node, childLeftovers, { calculus, program }) {
  ...
  const pool0 = Context.fromArray(Seq.getContext(seq, cs.consumableZone));
  {
    let need = pool0;
  ...
  const childLin = Context.fromArray(Seq.getContext(child.conclusion, cs.consumableZone));
...
const drawChecker = Object.freeze({
  step(conclusion, state, { calculus, program }) {
    ...
    let lin = Context.fromArray(Seq.getContext(conclusion, cs.consumableZone));
