...
import { guidedTerm } from './guided-term.js';
...
function modeSwitch(seq, engineCalc, opts = {}, ruleName = null) {
  ...
  // (THY_0018 §5); bridge success = settle-reachability. ILL's ungraded
  ...
  const comp = roles.computation || ILL_COMPUTATION;
  ...
  const proofNode = new ProofTree({
    conclusion: seq, rule: `${comp.tag}_r`, proven: true, premises: [],
    state: {
...
function timedSwitch(seq, engineCalc, opts, comp, ruleName) {
  ...
  const zero = engineCalc._timedConfig?.effect?.unit?.();
  const canon = (h) =>
  ...
  const proofNode = new ProofTree({
    conclusion: seq, rule: ruleName || `${comp.tag}_r`, proven: true, premises: [],
    state: {
