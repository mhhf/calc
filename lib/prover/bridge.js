...
function timedSwitch(seq, engineCalc, opts, comp, ruleName, calculus = null) {
  ...
  // TODO_0294 B2: elaborate the trace into a kernel-checkable @fire chain
  // — full verification (valid && !unverified) replaces the trusted
  // modeSwitch oracle node. Unsupported shapes (whole-bind antecedents,
  // counted/bang consequents or succedents) fall back to the oracle node
  // below; opts.elaborate === false forces the old behavior.
  if (calculus?.fire && opts.elaborate !== false) {
    const program = programFromCalc(engineCalc);
    const elab = elaborateTrace({
      sequent: seq, events: result.events || [], program, calculus,
    });
    if (elab.tree) return { proofNode: elab.tree, quiescent: true };
  }
