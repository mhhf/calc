...
import { programFromCalc, elaborateTrace } from './timed/elaborate-trace.js';
import forward from '../engine/forward.js';
import { monadicTerm } from './generic-term.js';

/** Guided-profile term builder — REGISTERED, not imported: the builder
 *  emits calculus-specific rule names (ILL: copy → loli_l → tensor_r →
 *  monad_l), so the calculus's module self-registers on import
 *  (calculus/ill/lib/guided-term.js) and the generic bridge stays free of
 *  ILL imports (TODO_0296 P2). */
let _guidedTerm = null;
function registerGuidedTerm(fn) { _guidedTerm = fn; }
/** Convert the sequent's zones to the forward engine's state keys: the
 *  consumable zone feeds `linear` (consumed-once resources), the
 *  copy-source zone feeds `persistent` (reusable facts). The LEFT names
 *  come from the calculus's declared contextStructure (TODO_0086); the
 *  RIGHT names are the engine's own state constants. */
function sequentToState(seq, ctxStruct) {
  const cs = ctxStruct || Seq.DEFAULT_CONTEXT_STRUCTURE;
  const linear = {}, persistent = {};
  // ALL consumable zones feed the linear pool (TODO_0285) — aux-zone
  // entries stay wrapper-tagged, so the forward engine matches them as
  // ordinary linear facts in their own tag group.
  for (const h of Seq.consumablePool(seq, cs))
...
function rightFocus(linear, persistent, hash, roles = {}) {
  ...
  // Reject async connectives by role (implication, choices, computation) and
  // the fixpoint connectives (roles.lfp = μ, roles.gfp = ν): a μ/ν succedent
  // is a backward-proof obligation (μMALL unfold / cyclic proof), NEVER closed
  // by a forward residual — treating it as an opaque atom to consume from
  // linear state would let a forward-produced fixpoint token close a
  // coinductive monad goal without proof (TODO_0009 soundness, audit 2026-09-11).
  if (tag === roles.implication || tag === roles.externalChoice ||
      tag === roles.internalChoice || tag === roles.computation?.tag ||
      tag === roles.lfp || tag === roles.gfp) return null;
 ...
 */
function modeSwitch(seq, engineCalc, opts = {}, ruleName = null, calculus = null) {
  ...
  // monad (gradeIdx null) never enters this branch.
  {
    ...
    if (comp && comp.gradeIdx != null && typeof engineCalc.settle === 'function') {
      return timedSwitch(seq, engineCalc, opts, comp, ruleName, calculus);
    }
  }

  const guided = opts.forward === 'guided';
  const initialState = sequentToState(seq, calculus?.contextStructure);
  ...
  if (guided) {
    // Guided profile: rightFocusTerm + the registered term builder →
    // complete calculus proof term (ILL: copy → loli_l → tensor_r →
    // monad_l). check-term.js can fully recurse — no unverified gap.
    if (!_guidedTerm) {
      throw new Error("bridge: guided profile has no registered term builder — import the calculus's guided-term module (e.g. calculus/ill/lib/guided-term.js self-registers)");
    }
    ...
    mTerm = _guidedTerm(result.trace || [], rfTerm);
    termVerified = true;
  } else if (opts.terms) {
 ...
 */
function timedSwitch(seq, engineCalc, opts, comp, ruleName, calculus = null) {
  ...
  // TODO_0294: elaborate the trace into a kernel-checkable @fire chain —
  // full verification (valid && !unverified) replaces the trusted
  // modeSwitch oracle node. Elaboration is TOTAL on legal traces
  // (THY_0018 §5 residual partiality; the full D4/Phase-6c surface is
  // covered), so with a bound checker a failure after rightFocus success
  // is an engine/elaborator disagreement — a found bug, thrown loudly.
  // The oracle node below survives ONLY for calculi that have not bound
  // a fire checker (calculus.fire absent — e.g. ad-hoc graded calculi).
  if (calculus?.fire) {
    const program = programFromCalc(engineCalc);
    const elab = elaborateTrace({
      sequent: seq, events: result.events || [], program, calculus,
    });
    if (!elab.tree) {
      throw new Error(`timed bridge: trace elaboration failed (${elab.unsupported}) — ` +
        'engine/elaborator disagreement (TODO_0294 totality)');
    }
    return { proofNode: elab.tree, quiescent: true };
  }

  const proofNode = bridgeNode(seq, ruleName || `${comp.tag}_r`, {
    modeSwitch: true, quiescent: true,
    // steps counts FIRES (RLE multiplicity expanded, TODO_0278 B1) —
    // display metadata only; verification is rightFocus on the state.
    steps: (result.events || []).reduce((a, e) => a + (e.multiplicity || 1), 0),
    residualState: result.state,
    trace: result.events || [],
...
export { sequentToState, stateToContext, rightFocus, rightFocusTerm, modeSwitch, registerGuidedTerm };
export default { sequentToState, stateToContext, rightFocus, rightFocusTerm, modeSwitch, registerGuidedTerm };
