...
import { hashString, hashCombine2 } from '../hash.js';
...
/**
 * Post-process a serialized tree, replacing subtrees at depth >= `d`
 * with elided stubs. The node keeps its `id`, `sequent`, `rule` and
 * `proven` flag; `premises` becomes empty and `elided: true` is added
 * so the renderer can emit a fetch-stub instead of a normal fold.
 *
 * nodeIds are preserved — elision is a pure view transform on already-
 * computed ids, so lazy-subtree fetches (`findSubtreeById`) continue to
 * match. The pool is not pruned in v1 because pool bytes are already
 * dwarfed by the node-tree bytes (~5%:95% for tensor128).
 */
function elideBelowDepth(tree, depthCap) {
  if (typeof depthCap !== 'number' || depthCap < 0) return tree;
  function walk(node, depth) {
    if (depth >= depthCap && node.premises && node.premises.length > 0) {
      return {
        id: node.id,
    ...
    if (!node.premises || node.premises.length === 0) return node;
    const kids = new Array(node.premises.length);
    for (let i = 0; i < node.premises.length; i++) kids[i] = walk(node.premises[i], depth + 1);
    return { ...node, premises: kids };
  }
  return { ...tree, root: walk(tree.root, 0) };
}

/** Depth-first lookup of a ProofNode by its stable content-hash id. */
function findSubtreeById(tree, nodeId) {
  function visit(node) {
...
export { FORMAT_VERSION, serializeTree, serializeFormula, serializeSequent, computeNodeId, elideBelowDepth, findSubtreeById, newContext as _newContext };
export default { FORMAT_VERSION, serializeTree, serializeFormula, serializeSequent, computeNodeId, elideBelowDepth, findSubtreeById, _newContext: newContext };
