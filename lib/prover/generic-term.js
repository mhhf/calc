 ...
 */
function findPrincipal(conclusion, connective, ctxStruct) {
  const linear = Seq.getContext(conclusion, ctxStruct.consumableZone);
 ...
 */
function idPrincipal(conclusion, ctxStruct) {
  const linear = Seq.getContext(conclusion, ctxStruct.consumableZone);
 ...
 */
function findCopyPrincipal(conclusion, premise, ctxStruct) {
  const cart = Seq.getContext(conclusion, ctxStruct.copySource);
  const premiseLinear = Seq.getContext(premise, ctxStruct.consumableZone);
  const conclLinear = Seq.getContext(conclusion, ctxStruct.consumableZone);
...
function extractTerm(proofTree, calculus) {
  ...
  if (rule === 'id' || rule === 'id_+' || rule === 'id_-') {
    return {
      rule: 'id',
      principal: idPrincipal(concl, ctxStruct),
      subterms: []
    };
  }
  ...
  if (rule === 'copy') {
    const principal = findCopyPrincipal(concl, premises[0].conclusion, ctxStruct);
    return {
      rule: 'copy',
      principal,
  ...
  // Mode switch (the computation right rule; monad_r in ILL) — the forward
  // engine ran to quiescence. Three evidence levels depending on execution profile:
  ...
  let principal = null;
  if (d.side === 'l') {
    principal = findPrincipal(concl, d.connective, ctxStruct);
  }
    ...
    if (formula && premises.length > 0) {
      ...
      if (d.binding === 'eigenvariable') {
        ...
        const instance = d.side === 'r'
          ...
          : findOpenedFormula(premConclusion, d.connective, ctxStruct);
        if (body && instance) {
 ...
 */
function findOpenedFormula(premConclusion, connective, ctxStruct) {
  const linear = Seq.getContext(premConclusion, ctxStruct.consumableZone);
...
export { sigMap, flattenedArity, formatSignature, extractTerm, monadicTerm, exploreTerm };
export default { sigMap, flattenedArity, formatSignature, extractTerm, monadicTerm, exploreTerm };
