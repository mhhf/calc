 ...
 *
 * Phase 1: Derives proof term constructor signatures from rule descriptors.
 * Phase 2: Extracts generic proof terms from completed ProofTrees.
 *
 * Each rule name IS a proof term constructor. Arity and binding
 * structure are determined mechanically from descriptor fields:
 *
 *   side='l'           → first arg is principal z
 *   binding='eigenvariable' → fresh variable arg a
 *   binding='metavar'  → witness term arg s
 *   premises[i].linear → bound variables scoped over sub-proof
 *   premises[i].cartesian → bound variables moved to Gamma
 *
 * See TODO_0068 §2.7 for the full generic term grammar.
 */
...
// Hardcoded signatures for rules outside the descriptor framework
 ...
 * Also registers spec-key aliases (e.g., bang_l → absorption's signature)
 * since the prover records spec keys in proof trees.
 *
 * @param {Object} rules - calculus.rules (name → rule)
 * @returns {{ [ruleName: string]: TermSignature }}
 */
function buildSignatureMap(rules) {
  const sigs = {};
  ...
  for (const [name, rule] of Object.entries(rules)) {
    ...
    const sig = genericTermSignature(rule);
    sigs[name] = sig;
 ...
 */
function findIdentityPrincipal(conclusion) {
  const linear = Seq.getContext(conclusion, 'linear');
...
function extractTerm(proofTree, calculus) {
  ...
  if (rule === 'id' || rule === 'id_+' || rule === 'id_-') {
    return {
      rule: 'id',
      principal: findIdentityPrincipal(concl),
      subterms: []
    };
  }
  ...
  // Mode switch (monad_r) — attach monadic term from bridge if available
  if (rule === 'monad_r') {
    const term = {
      rule: 'monad_r',
      principal: null,
  ...
  // Standard rules: look up descriptor (try direct name, then scan for spec-key match)
  let ruleObj = calculus.rules[rule];
  if (!ruleObj) {
    // Proof tree may use spec keys (e.g., bang_l) instead of rule names (absorption).
    // Find a rule whose connective_side matches.
  ...
  if (d.binding === 'metavar' || d.binding === 'eigenvariable') {
    ...
    if (formula && premises.length > 0) {
      ...
      if (d.binding === 'eigenvariable') {
        ...
        if (body && instance) {
          witness = extractBoundValue(body, instance);
        }
      }
    }
  }
 ...
 * @returns {number|null}
 */
function extractBoundValue(pattern, instance) {
  ...
  for (let i = 0; i < ar; i++) {
    const r = extractBoundValue(Store.child(pattern, i), Store.child(instance, i));
    if (r !== null) return r;
  }
  return null;
}
 ...
 *
 * Each forward step becomes a monadic let-binding (TODO_0068 §3.2):
 ...
 * @returns {GenericTerm}
 */
function buildMonadicTerm(trace, rfTerm) {
...
/**
 * Build proof terms from an explore tree (TODO_0068 §3.4).
 *
 ...
 *   bound  → null (incomplete)
 *   cycle  → null (future work)
 *   memo   → null (shared reference, future work)
 *
 * Multi-alt branches (oplus) become case splits:
 ...
 * @returns {GenericTerm|null}
 */
function buildExploreTreeTerm(tree, rfTermFn, roles) {
  if (!tree) return null;
    ...
    case 'branch': {
      ...
      if (hasChoice) {
        ...
        const subterms = tree.children.map(c =>
          c.child ? buildExploreTreeTerm(c.child, rfTermFn, roles) : null
        );
        return {
          rule: 'oplus_l',
          principal: null,
      ...
      for (let i = tree.children.length - 1; i >= 0; i--) {
        ...
        const childTerm = buildExploreTreeTerm(c.child, rfTermFn, roles);
...
module.exports = {
  genericTermSignature,
  buildSignatureMap,
  flattenedArity,
  formatSignature,
  extractTerm,
  buildMonadicTerm,
  buildExploreTreeTerm,
  SPECIAL_SIGNATURES
};
