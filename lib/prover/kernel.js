  ...
  function verifyStep(conclusion, ruleName, premises, state, opts = {}) {
    ...
    const checkWith = (formula, index) => {
      ...
      for (let i = 0; i < expectedPremises.length; i++) {
        ...
        const expectedLinear = Seq.getContext(expected, ctxStruct.consumableZone);
        const actualLinear = Seq.getContext(actual, ctxStruct.consumableZone);
  ...
  function verifyTree(tree, opts = {}) {
    ...
    const tryCandidate = (node, spec, formula, index, ctxL, side, childLeftovers) => {
      ...
      for (let i = 0; i < expected.length; i++) {
        ...
        const ci = Context.fromArray(Seq.getContext(child.conclusion, ctxStruct.consumableZone));
        const intro = Seq.getContext(exp, ctxStruct.consumableZone);
    ...
    function walk(node) {
      ...
      const seq = node.conclusion;
      const linearArr = Seq.getContext(seq, ctxStruct.consumableZone);
      ...
      if (idRules.has(rule)) {
        ...
        for (const zone of ctxStruct.zones) {
          if (zone === ctxStruct.consumableZone) continue;
