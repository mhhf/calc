...
import { checkFireData, checkFireTreeStep } from './fire-check.js';
/**
...
function createKernel(calculus) {
  ...
  function verifyStep(conclusion, ruleName, premises, state, opts = {}) {
    ...
    // @fire (TODO_0294): one timed firing, checked against the program's
    // declared rule data — data-level here; resource threading is
    // verifyTree's job. Requires opts.program and calculus.fire.
    if (spec && spec.fireStep) {
      if (!calculus.fire) {
        return { valid: false, error: `${ruleName}: calculus declares no fire config` };
      }
      const r = checkFireData(conclusion, state && state.fire,
        { program: opts.program || null, theory: calculus.theory, config: calculus.fire });
  ...
  function verifyTree(tree, opts = {}) {
    ...
    function walk(node) {
      ...
      // @fire (TODO_0294): full re-derivation against the program's rule
      // data + theory, WITH resource threading — never enters `unverified`
      if (spec.fireStep) {
        if (!calculus.fire) {
          errors.push(`At rule ${rule}: calculus declares no fire config`);
          return null;
        }
        const r = checkFireTreeStep(node, childLeftovers,
          { program: opts.program || null, theory: calculus.theory, config: calculus.fire });
