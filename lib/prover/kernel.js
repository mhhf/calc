...
function createKernel(calculus) {
  ...
  // Union pool over all consumable zones (TODO_0285): resource accounting
  // is pool-based and zone-blind (wrapper tags make membership
  // self-describing); zone columns are presentation. With one consumable
  // zone this is the CZ array unchanged.
  const poolOf = (s) => Seq.consumablePool(s, ctxStruct);
  const consumableSet = new Set(ctxStruct.consumableZones || [ctxStruct.consumableZone]);
  ...
  function verifyStep(conclusion, ruleName, premises, state, opts = {}) {
    ...
    if (ruleName === 'copy') {
      ...
      // Premise should have one extra consumable formula copied from
      // copySource (pool view — the copy may route to an aux zone)
      const premiseTarget = poolOf(premises[0]);
      const conclusionTarget = poolOf(conclusion);
    ...
    // Explicit cut (TODO_0309) — shape check: two premises, premise 1
    // concludes the cut formula, premise 2 carries it and concludes the
    // sequent's succedent. Resource threading is verifyTree's job.
    if (spec._premiseType === 'cut') {
      if (premises.length !== 2) {
        return { valid: false, error: `${ruleName}: cut expects 2 premises, got ${premises.length}` };
      }
      const X = premises[0].succedent;
      if (!poolOf(premises[1]).includes(X)) {
        return { valid: false, error: `${ruleName}: cut premise 2 does not carry the cut formula` };
      }
      if (premises[1].succedent !== conclusion.succedent &&
          !unify(premises[1].succedent, conclusion.succedent)) {
        return { valid: false, error: `${ruleName}: cut premise 2 succedent mismatch` };
      }
      return { valid: true };
    }

    // Find the principal formula and apply the rule
    const linear = poolOf(conclusion);
    ...
    const checkWith = (formula, index) => {
      ...
      const madeStep = spec.makePremises(formula, conclusion, index);
      if (!madeStep) {
      ...
      // consume } — check companion presence in the pool (threading is
      // verifyTree's job).
      const expectedPremises = Array.isArray(madeStep) ? madeStep : madeStep.premises;
      const stepConsume = Array.isArray(madeStep) ? null : madeStep.consume;
      if (stepConsume) {
        for (const h of stepConsume) {
          if (!linear.includes(h)) {
            return { valid: false, error: `Rule ${ruleName}: companion formula not in context` };
          }
        }
      }
      ...
      for (let i = 0; i < expectedPremises.length; i++) {
        ...
        // Expected consumable formulas must be present in actual (pool view)
        const expectedLinear = poolOf(expected);
        const actualLinear = poolOf(actual);
  ...
  function verifyTree(tree, opts = {}) {
    ...
    let sawCycle = false;   // any nu_cycle bud → run the GTC post-walk (Inc-4)
    ...
    const tryCandidate = (node, spec, formula, index, ctxL, side, childLeftovers) => {
      ...
      const made = spec.makePremises(formula, seq, index);
      if (!made) {
        return { error: `Rule ${node.rule}: makePremises returned null` };
      }
      // Extended contract (TODO_0309): exact axioms return { premises,
      // consume } — companions are consumed from the pool like the
      // principal.
      const expected = Array.isArray(made) ? made : made.premises;
      ...
      if (consume) {
        pool = subtractIntro(pool, consume);
        if (pool === null) {
          return { error: `Rule ${node.rule}: companion formula not in context` };
        }
      }
      // Zero-premise RIGHT rules (one_r, top_r) name a principal succedent
      // connective but produce no arity-checked premise to re-derive it. Without
      // this guard a forged node proves an arbitrary succedent: `one_r ⊢ b`
      // (empty-context, latent) or — far worse — `top_r a ⊢ b`, since ⊤R absorbs
      // the context and would otherwise accept ANY succedent b.
      if (side === 'R' && expected.length === 0 && spec._zeroSuccConn) {
        if (!Store.isTerm(seq.succedent) || Store.tag(seq.succedent) !== spec._zeroSuccConn) {
          return { error: `Rule ${node.rule}: succedent is not ${spec._zeroSuccConn}` };
        }
      }
      ...
      if (spec.discardsContext) {
        // zero_l-style (0L) and top_r (⊤R): absorb the whole remaining linear
        // pool into the additive unit; leftover empty. For ⊤R a partial
        // absorption is recorded by the node carrying only the absorbed subset
        // as its context (the search's don't-know split), which this same rule
        // handles — the parent threads pool∖(absorbed) to the sibling.
        return { leftover: Context.empty() };
      }
      if (expected.length === 0) {
        // zero-premise: left consumes its principal + companions
        // (template axioms — at_l, the SAX X-rules), right consumes its
        // companions — leftover threads on
      ...
      // hash-identically; witness-mode templates AND μ/ν unfold (TODO_0009)
      // re-derive DETERMINISTICALLY (the witness is a resolved term / the
      // whole fixpoint), so they get NO such degradation — the exact-match
      // subtractIntro must hold or the step is a forgery. Only 'eigenvariable'
      // and 'metavar' introduce fresh, non-reproducible variables.
      const dbind = spec._descriptor?.binding;
      const isBinding = dbind === 'eigenvariable' || dbind === 'metavar' ||
      ...
      for (let i = 0; i < expected.length; i++) {
        ...
        const ci = Context.fromArray(poolOf(child.conclusion));
        const intro = poolOf(exp);
    ...
    function walk(node) {
      ...
      const seq = node.conclusion;
      const linearArr = poolOf(seq);
      const ctxL = Context.fromArray(linearArr);
      const rule = node.rule;

      // Cyclic-proof bud (TODO_0009 Inc-4): a coinductive back-edge closing to
      // an ancestor companion. Terminal (0 premises), closes its branch like an
      // axiom (consumes its consumable pool → empty leftover). Per-node soundness
      // cannot be judged locally — the GLOBAL trace condition over the whole
      // tree decides it, so record the sighting and run checkCyclicProof (TCB)
      // after the walk.
      if (rule === 'nu_cycle') {
        if (node.premises.length !== 0) {
          errors.push(`At rule ${rule}: cyclic bud expects 0 premises, got ${node.premises.length}`);
          return null;
        }
        sawCycle = true;
      ...
      if (idRules.has(rule)) {
        ...
        for (const zone of ctxStruct.zones) {
          // ALL consumable zones are in the pool already — only genuinely
          // reusable zones (copy source) allow consumption-free identity
          if (consumableSet.has(zone)) continue;
      ...
      // Whole-context "next" tick (TODO_0203, THY_0043): `G ; ○Δ ⊢ ○C` from
      // `G ; Δ ⊢ C`. Fully re-derived here (NOT a trusted mode switch): the
      // succedent must be ○-headed, EVERY consumable formula must be ○-wrapped
      // (a non-○ resource cannot advance — the non-collapse guard), the premise
      // pool must be exactly the stripped context, the premise succedent the
      // ○-body, and the persistent zone may not be enlarged (no smuggled facts).
      // Leftover: an unconsumed stripped X is re-wrapped to ○X, so a genuine leak
      // still fails root emptiness.
      if (spec.tick) {
        if (node.premises.length !== 1) {
          errors.push(`At rule ${rule}: tick expects 1 premise, got ${node.premises.length}`);
          return null;
        }
        const conn = spec._tickConn;
        if (!seq.succedent || !Store.isTerm(seq.succedent) || Store.tag(seq.succedent) !== conn) {
          errors.push(`At rule ${rule}: succedent is not ${conn}-headed`);
          return null;
        }
        const C = Store.child(seq.succedent, 0);
        const stripped = [];
        for (const h of linearArr) {
          if (!Store.isTerm(h) || Store.tag(h) !== conn) {
            errors.push(`At rule ${rule}: consumable formula is not ${conn}-wrapped — cannot advance across a tick`);
            return null;
          }
          stripped.push(Store.child(h, 0));
        }
        const premise = node.premises[0].conclusion;
        const premisePool = Context.fromArray(poolOf(premise));
        if (!Context.eq(premisePool, Context.fromArray(stripped))) {
          errors.push(`At rule ${rule}: premise context is not the stripped ${conn}-context`);
          return null;
        }
        // EXACT match (audit 2026-09-11): the search always builds the premise
        // succedent as exactly the ○-body C, so require hash-equality. A `unify`
        // fallback here was a SOUNDNESS HOLE — a metavar premise succedent (e.g. a
        // forged id leaf `a ⊢ X`) unifies with C independently of the metavar's
        // binding under the id check, admitting the false `○a ⊢ ○b`. Exact
        // equality closes it (a metavar body stays exact: same hash on both sides).
        const ps = premise.succedent;
        if (ps !== C) {
          errors.push(`At rule ${rule}: premise succedent is not the ${conn}-body`);
          return null;
        }
        if (ctxStruct.copySource) {
          const gConc = Seq.getContext(seq, ctxStruct.copySource);
          const gPrem = Seq.getContext(premise, ctxStruct.copySource);
          const gSet = new Set(gConc);
          if (!gPrem.every(h => gSet.has(h))) {
            errors.push(`At rule ${rule}: premise persistent zone smuggles a fact not present in the conclusion`);
            return null;
          }
        }
        return Context.fromArray(Context.toArray(childLeftovers[0]).map(h => Store.put(conn, [h])));
      }
      ...
      if (rule === 'copy') {
        ...
        // Premise pool = conclusion pool + one copied formula (the copy
        // may route to an aux zone — compare pools, not one column)
        const ci = Context.fromArray(poolOf(node.premises[0].conclusion));
      ...
      // Explicit cut (TODO_0309): re-derive the split from the recorded
      // premises — the cut formula is premise 1's succedent; premise 2
      // must carry it as an intro. Threading mirrors the generic loop:
      // each premise's context decomposes as intro ⊎ D with D from the
      // pool; an unconsumed cut formula flows into the leftover and
      // fails at the root like any other leak.
      if (spec._premiseType === 'cut') {
        if (node.premises.length !== 2) {
          errors.push(`At rule ${rule}: cut expects 2 premises, got ${node.premises.length}`);
          return null;
        }
        const X = node.premises[0].conclusion.succedent;
        const p2succ = node.premises[1].conclusion.succedent;
        if (p2succ !== seq.succedent && !unify(p2succ, seq.succedent)) {
          errors.push(`At rule ${rule}: cut premise 2 succedent mismatch`);
          return null;
        }
        let cpool = ctxL;
        const c1 = Context.fromArray(poolOf(node.premises[0].conclusion));
        if (!Context.contains(cpool, c1)) {
          errors.push(`At rule ${rule}: cut premise 1 carries formulas not in the available context`);
          return null;
        }
        cpool = Context.merge(Context.subtract(cpool, c1), childLeftovers[0]);
        const d2 = subtractIntro(Context.fromArray(poolOf(node.premises[1].conclusion)), [X]);
        if (d2 === null) {
          errors.push(`At rule ${rule}: cut premise 2 does not carry the cut formula`);
          return null;
        }
        if (!Context.contains(cpool, d2)) {
          errors.push(`At rule ${rule}: cut premise 2 carries formulas not in the available context`);
          return null;
        }
        return Context.merge(Context.subtract(cpool, d2), childLeftovers[1]);
      }
    ...
    // Cyclic proofs: every nu_cycle back-edge must satisfy the global trace
    // condition (context conservation + νR/μL progress). The GTC checker
    // reconstructs the back-edges from the tree and validates them (TCB).
    if (sawCycle) {
      ...
      if (!gtc.valid) for (const e of gtc.errors) errors.push(`GTC: ${e}`);
    }
