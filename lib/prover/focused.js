...
function createProver(calculus) {
  ...
  const prove = (seq, opts = {}) => {
    ...
    // NOTE: with CPS, sibling premises are searched while an earlier premise's
    // frames are still live, so `seenOnPath` can hold a completed sibling's keys.
    // That only ever ADDS a recurrence detection (a spurious nu_cycle bud or a
    // conservative fail) — never removes one — and every emitted back-edge is
    // re-derived and re-checked from the final tree by the TCB GTC checker
    // (checkCyclicProof), so soundness is unaffected; at worst a valid proof is
    // missed. Exhaustive mode is fill-only (experimental), where that is fine.
    const searchK = seenOnPath
      ? (seq, state, depth, delta, sk) => {
          ...
          seenOnPath.add(key);
          try { return searchCoreK(seq, state, depth, delta, sk); }
          finally { seenOnPath.delete(key); }
        }
      : searchCoreK;
