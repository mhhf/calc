...
function createProver(calculus) {
   ...
   */
  const applyAndRecurse = (seq, rName, spec, position, index, state, searchFn, depth, delta, opts) => {
    // Mode switch: monad_r bypasses standard premise computation
    if (spec.modeShift) {
      const engineCalc = opts?.engineCalc || null;
      const switchResult = bridge.executeModeSwitch(seq, engineCalc, opts);
    ...
    // Merge thetas from children (e.g., metavar bindings from identity leaves)
    const mergedTheta = childResults.flatMap(r => r.theta);
  ...
  /**
   * Apply a unification substitution to every sequent in a proof tree.
   * Resolves metavars left behind by identity-via-unification
   * (e.g., forall_l opens with _X, identity binds _X := a).
   * After resolution, identity nodes have hash-equal linear/succedent.
   */
  const applyThetaToTree = (tree, theta) => {
    return new ProofTree({
      conclusion: Seq.substitute(tree.conclusion, theta),
      premises: tree.premises.map(p => applyThetaToTree(p, theta)),
      rule: tree.rule,
      proven: tree.proven,
      state: tree.state
    });
  ...
  const prove = (seq, opts = {}) => {
    ...
    const search = (seq, state, depth, delta) => {
      ...
      if (inv) {
        ...
        if (spec) {
          const result = applyAndRecurse(seq, rName, spec, inv.position, inv.index, state, search, depth, delta, opts);
      ...
      if (state.isFocused()) {
        ...
        for (const tryName of ruleNames) {
          ...
          const result = applyAndRecurse(seq, tryName, trySpec, state.position, focusIdx, state, search, depth, delta, opts);
          if (result) return result;
        }
      }
    ...
    if (result && Context.isEmpty(result.delta_out)) {
      ...
      // Apply accumulated substitution from identity unification
      // (e.g., metavar _X := a after forall_l + id)
      if (result.theta.length > 0) {
        tree = applyThetaToTree(tree, result.theta);
      }
