...
function createProver(calculus) {
  ...
  const stripToken = (tree, token) => {
    const lin = Seq.getContext(tree.conclusion, CZ);
    const i = lin.indexOf(token);
    if (i < 0) return tree;
    const conclusion = Seq.seq({
      [CZ]: [...lin.slice(0, i), ...lin.slice(i + 1)],
      [SZ]: Seq.getContext(tree.conclusion, SZ),
  ...
  const prove = (seq, opts = {}) => {
      ...
      if (cart.length > 0 && Context.isEmpty(delta) && ruleSpecs.copy) {
        for (let i = 0; i < cart.length; i++) {
          ...
          if (theta) {
            ...
            const premise = Seq.seq({ [CZ]: newLinear, [SZ]: cart }, seq.succedent);
    ...
    const initialDelta = Context.fromArray(Seq.getContext(seq, CZ));
