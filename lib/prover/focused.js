...
function createProver(calculus) {
   ...
   */
  // Whole-context "next" tick (TODO_0203, THY_0043): from `G ; ○Δ ⊢ ○C` build the
  // premise `G ; Δ ⊢ C` — strip one ○ from EVERY consumable formula, succedent ○C
  // → C, persistent G unchanged. Returns null (rule inapplicable) unless the
  // succedent is ○-headed AND every consumable formula is ○-wrapped — a non-○
  // linear resource cannot advance, which is exactly what keeps ○ non-collapsing
  // (○a ⊬ a because the succedent is a; a ⊬ ○a because the context a is not ○).
  // The connective is read from the spec (spec._tickConn), so this stays
  // calculus-agnostic. delta is the authoritative consumable pool.
  const tickPremise = (seq, delta, conn) => {
    const succ = seq.succedent;
    if (!Store.isTerm(succ) || Store.tag(succ) !== conn) return null;
    const C = Store.child(succ, 0);
    const stripped = [];
    for (const h of Context.toArray(delta)) {
      if (!Store.isTerm(h) || Store.tag(h) !== conn) return null;
      stripped.push(Store.child(h, 0));
    }
    const cols = Seq.routeContexts(ctxStruct, stripped);
    if (ctxStruct.copySource) cols[ctxStruct.copySource] = Seq.getContext(seq, ctxStruct.copySource);
    return { premiseSeq: Seq.seq(cols, C), strippedDelta: Context.fromArray(stripped) };
  };
  // Re-wrap the premise's leftover: an unconsumed stripped X corresponds to an
  // unconsumed ○X at the conclusion, so a genuine leak still fails root emptiness.
  const wrapTick = (deltaOut, conn) =>
    Context.fromArray(Context.toArray(deltaOut).map(h => Store.put(conn, [h])));

  const applyAndRecurse = (seq, rName, spec, position, index, state, searchFn, depth, delta, opts, mc, affine) => {
    // Whole-context tick: advances the entire ○-context one step (bypasses
    // standard premise computation, like modeShift).
    if (spec.tick) {
      const tp = tickPremise(seq, delta, spec._tickConn);
      if (!tp) return null;
      const child = searchFn(tp.premiseSeq, inversion(), depth + 1, tp.strippedDelta);
      if (!child) return null;
      return {
        proofTree: new ProofTree({
          conclusion: mc.hasBindings() ? mc.resolveSeq(seq) : seq,
          premises: [child.proofTree],
          rule: rName, proven: true, state: state.copy(),
        }),
        delta_out: wrapTick(child.delta_out, spec._tickConn),
      };
    }
  ...
  const applyAndRecurseK = (seq, rName, spec, position, index, state, searchFn, depth, delta, opts, mc, affine, sk) => {
    // Whole-context tick (CPS twin of the committed branch above).
    if (spec.tick) {
      const tp = tickPremise(seq, delta, spec._tickConn);
      if (!tp) return null;
      return searchFn(tp.premiseSeq, inversion(), depth + 1, tp.strippedDelta, (child) => sk({
        proofTree: new ProofTree({
          conclusion: mc.hasBindings() ? mc.resolveSeq(seq) : seq,
          premises: [child.proofTree],
          rule: rName, proven: true, state: state.copy(),
    ...
    // ⊤R (context-absorbing right rule, discardsContext + side R): ⊤ is the top
    // element, so it absorbs an ARBITRARY SUBSET of the pool. The committed path
    // absorbs everything (fine for `Δ ⊢ ⊤` and the additive fragment), but a
    // multiplicative split `Δ ⊢ ⊤ ⊗ b` where ⊤ comes first needs the don't-know
    // choice: try each absorbed subset so the sibling b can claim its share.
    // The node conclusion carries EXACTLY the absorbed subset, so the kernel
    // threads pool∖(absorbed) to the sibling and re-derives ⊤'s leftover as empty
    // (soundness is the kernel's, not the search's). The absorb-all case (subset
    // = whole pool) is offered first, matching the committed reading.
    if (spec.discardsContext && position === 'R') {
      if (spec._zeroSuccConn && (!Store.isTerm(seq.succedent) ||
          Store.tag(seq.succedent) !== spec._zeroSuccConn)) return null;
      const poolArr = pool(seq);
      const n = poolArr.length;
      const buildNode = (absorbed) => {
        const contexts = { ...seq.contexts };
        for (const z of ctxStruct.consumableZones) contexts[z] = [];
        for (const h of absorbed) contexts[Seq.routeZone(ctxStruct, h)].push(h);
        const concl = Seq.seq(contexts, seq.succedent);
        return new ProofTree({
          conclusion: mc.hasBindings() ? mc.resolveSeq(concl) : concl,
          premises: [], rule: rName, proven: true, state: state.copy(),
        });
      };
      const offer = (absorbed, kept) => {
        const mark = mc.save();
        const acc = sk({ proofTree: buildNode(absorbed), delta_out: Context.fromArray(kept) });
        if (acc) return acc;
        mc.restore(mark);
        return null;
      };
      // Common case first: absorb everything (kept = ∅). Enumerate proper
      // subsets only when the pool is small enough to be a genuine don't-know
      // choice (2^n bitmask over pool indices; deduped for multiset repeats).
      const all = offer(poolArr, []);
      if (all) return all;
      if (n === 0 || n > 12) return null;
      const seen = new Set(['']);   // absorb-all (kept = ∅) already tried
      for (let keep = 1; keep < (1 << n); keep++) {
        const kept = [], absorbed = [];
        for (let i = 0; i < n; i++) ((keep >> i) & 1 ? kept : absorbed).push(poolArr[i]);
        const key = kept.slice().sort().join(',');   // dedup multiset repeats
        if (seen.has(key)) continue;
        seen.add(key);
        const acc = offer(absorbed, kept);
        if (acc) return acc;
      }
      return null;
    }
