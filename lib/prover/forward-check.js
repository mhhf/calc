...
function linMap(obj) {
  const m = new Map();
  for (const k in obj || {}) { const c = obj[k]; if (c > 0) m.set(Number(k), c); }
  return m;
}
function perSet(obj) {
  const s = new Set();
  for (const k in obj || {}) s.add(Number(k));
  return s;
}
...
function stepForward(pr, step, lin, per, deps) {
  ...
  const ground = (h) => { const g = subst(h, bind); return hasUnbound(g) ? null : g; };
  const groundAll = (hs, what) => {
    const out = [];
    for (const h of hs || []) {
      const g = ground(h);
      if (g === null) return { error: `unbound slot in ${what} pattern` };
      out.push(g);
    }
    return { out };
  };
 ...
 */
function checkForwardTree(tree, { program, roles, initial }) {
  const errors = [];
  const unsupported = new Set();
  let leaves = 0;

  const deps = { program, roles };
  ...
  const checkTerminal = (kind, node, lin, per) => {
    ...
    const ldiff = mapEqDiff(lin, linMap(node.state.linear));
    if (ldiff) errors.push(`${here()} ${kind}: linear state mismatch (${ldiff})`);
    const pdiff = setEqDiff(per, perSet(node.state.persistent));
  ...
  walk(tree, linMap(initial.linear), perSet(initial.persistent));
