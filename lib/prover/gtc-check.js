...
function checkGTC(backEdges, { roles, contextStructure, canonicalize } = {}) {
  ...
  for (let idx = 0; idx < (backEdges || []).length; idx++) {
    ...
    // (A) context conservation — the consumable pool must be EMPTY at the
    // back-edge (bud and companion), and succedents equal modulo theory. The
    // persistent zone is excluded by consumablePool and stays unconstrained.
    //
    // EMPTINESS is the soundness condition, not mere bud==companion equality
    // (audit 2026-09-11): the cyclic bud closes its branch like an axiom, and the
    // kernel treats it as consuming its pool. But a linear resource that is
    // CONSERVED (unchanged) around an infinite cycle is never actually consumed —
    // admitting a non-empty conserved pool would discharge it for free, i.e.
    // coinductive WEAKENING (`a ⊢ νX.X`, `!!_5 a ⊢ νX.(!!_2 X)` would wrongly
    // prove). Only PERSISTENT resources (excluded from consumablePool) can sustain
    // a coinductive proof, so the consumable pool must be empty. Valid signals
    // (`!a ⊢ νX.(a & X)`, bang absorbed to the cartesian zone) already satisfy this.
    ...
    if (budPool.length > 0) {
      errors.push(`${where}: coinductive back-edge has a non-empty consumable pool ` +
        `(${budPool.length} formula(s)) — a conserved linear resource around an infinite ` +
        `cycle is never consumed; coinduction does not discharge linear resources`);
    }
