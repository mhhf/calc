...
function checkFireData(seq, fire, { program, theory, config }) {
  ...
  // decompose a recorded fact map {atHash: count} → bag of inners + stamps
  const splitAt = (obj, what) => {
    const inners = new Map();
    const stamps = new Set();
    const pairs = new Map();
    for (const k in obj || {}) {
      const h = Number(k);
      if (Store.tag(h) !== ST) return { error: `${what} fact is not ${ST}-stamped` };
      bagAdd(inners, Store.child(h, 0), obj[k]);
      stamps.add(Store.child(h, 1));
      bagAdd(pairs, h, obj[k]);
    }
    return { inners, stamps, pairs };
  };

  const consumed = splitAt(fire.consumed, 'consumed');
  if (consumed.error) return consumed;
  const reserved = splitAt(fire.reserved, 'reserved');
  if (reserved.error) return reserved;

  // antecedent correspondence: consumed inners = consume patterns (bags)
  const gc = groundAll(pr.consume, 'consume');
  if (gc.error) return gc;
  const wantConsume = new Map();
  for (const g of gc.out) bagAdd(wantConsume, g);
  if (!bagEq(wantConsume, consumed.inners)) {
    return { error: `consumed multiset ≠ rule antecedent: want {${bagShow(wantConsume)}}, got {${bagShow(consumed.inners)}}` };
  }
  const gr = groundAll(pr.read, 'read');
  if (gr.error) return gr;
  const wantRead = new Map();
  for (const g of gr.out) bagAdd(wantRead, g);
  if (!bagEq(wantRead, reserved.inners)) {
    return { error: `read multiset ≠ rule reads: want {${bagShow(wantRead)}}, got {${bagShow(reserved.inners)}}` };
  }
  ...
  const candidates = new Set([...consumed.stamps, ...reserved.stamps, ...ga.out]);
  ...
  // produced: consequent patterns stamped at u
  const gp = groundAll(pr.produce, 'produce');
  if (gp.error) return gp;
  const wantProduced = new Map();
  for (const g of gp.out) bagAdd(wantProduced, Store.put(ST, [g, u]));
  const gotProduced = new Map();
  for (const k in fire.produced || {}) bagAdd(gotProduced, Number(k), fire.produced[k]);
  if (!bagEq(wantProduced, gotProduced)) {
    return { error: `produced multiset ≠ rule consequent at the done stamp` };
  }

  // persistent conclusions
  const gpp = groundAll(pr.producePers, 'producePers');
  ...
  // persistent goals: state membership (cartesian zone) or theory derivation
  ...
  return {
    consumed: consumed.pairs,
    reserved: reserved.pairs,
    produced: wantProduced,
...
export { checkFireData, checkFireTreeStep };
export default { checkFireData, checkFireTreeStep };
