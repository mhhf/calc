 ...
 * @param {Object} deps - { program, theory, config, ctxStruct }
 * @returns {{ error?: string, consumed?, reserved?, produced?, producedPers? }}
 *   the ground bags (Map hash→count) on success
 */
function checkFireData(seq, fire, { program, theory, config, roles = {}, ctxStruct = null }) {
  ...
  if (!pr) {
    ...
    // loli token — the token's structure is the justification. The record
    // name must BIND the token ('loli:' + inner hash — the engine's
    // minting, timed-api.js): without the equation any consumed
    // implication token would justify an arbitrarily-named record
    // (forgery hole, fuzz-till forgery arm, found 2026-09-04), and with
    // two consumed loli tokens the checker could verify against the
    // wrong one.
    for (const k in fire.consumed || {}) {
      const h = Number(k);
      const inner = Store.tag(h) === config.stampTag ? Store.child(h, 0) : h;
      if (fire.rule !== 'loli:' + inner) continue;
  ...
  if (candidates.size > 0) {
    if (config.join) {
      // Declared join predicate (TODO_0285 P6a): under a PRODUCT stamp
      // order the componentwise join may equal NO single input, so
      // attainment-by-membership is wrong — fold the candidates through
      // the theory's join clauses and compare the result by value. Still
      // clause-only: the join is derived, never computed by the checker.
      const cs2 = [...candidates];
      let j = cs2[0];
      for (let i = 1; i < cs2.length; i++) {
        const M = Store.put('metavar', ['_fire_join']);
        const sub = proveGoal(Store.put(config.join, [j, cs2[i], M]));
        let jv = null;
        if (sub !== null) for (const [k2, v2] of sub) if (k2 === M) jv = v2;
        if (jv === null) return { error: 'activation join underivable from the theory' };
        j = jv;
      }
      if (!valEq(j, a)) {
        return { error: 'activation is not the join of its inputs' };
      }
    } else if (![...candidates].some(sc => valEq(sc, a))) {
  ...
  const cart = new Set(Seq.getContext(seq, cs.copySource));
  ...
  for (const g of gg.out) {
    ...
    // theory.has is REQUIRED (audit 2026-09-02): without it the fallback
    // silently widened to every prelude-provable goal — a theory that
    // cannot name its scope certifies nothing.
...
function checkFireTreeStep(node, childLeftovers, deps) {
  const cs = deps.ctxStruct || Seq.DEFAULT_CONTEXT_STRUCTURE;
  ...
  // The checker reads the UNION pool (TODO_0285): elaboration currently
  // collapses all consumable facts into the primary zone, but the checker
  // must not depend on that — pool reads hold under any routing.
  let pool = Context.fromArray(Seq.consumablePool(seq, cs));
  ...
  let di = Context.fromArray(Seq.consumablePool(child.conclusion, cs));
  ...
  step(conclusion, state, { calculus, program }) {
    ...
    return checkFireData(conclusion, state && state.fire,
      { program, theory: calculus.theory, config: calculus.fire, roles: calculus.roles || {},
        ctxStruct: calculus.contextStructure || null });
  },
