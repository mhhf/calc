...
function checkFireData(seq, fire, { program, theory, config, roles = {} }) {
  ...
  const matchFacts = (groundPatterns, entries, what) => {
    const remaining = entries.map(e => ({ ...e }));
    const wantPlain = new Map();
    for (const p of groundPatterns) {
  ...
  const consumeWant = consumeGround.out.slice();
  for (const w of pr.wholeBind || []) {
    ...
    for (let i = 0n; i < k; i++) consumeWant.push(gb);
  ...
  const mr = matchFacts(readGround.out, reserved, 'read');
  ...
  for (const g of gp.out) {
    if (Store.tag(g) === expTag) {
      ...
      const b = Store.child(g, 1);
      for (let i = 0n; i < k; i++) wantProduced.push({ inner: b, stamp: u });
      continue;
    }
...
function checkFireTreeStep(node, childLeftovers, deps) {
  ...
  if (data.wholeBind && data.wholeBind.length) {
    ...
    const valEq = (x, y) => x === y ||
      (deps.theory.prove(Store.put(deps.config.le, [x, y])) !== null &&
       deps.theory.prove(Store.put(deps.config.le, [y, x])) !== null);
