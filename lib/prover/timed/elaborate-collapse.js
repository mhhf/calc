 ...
 * the bias discipline is program data).
 */

import Store from '../../kernel/store.js';
import Seq from '../../kernel/sequent.js';
import { debruijnSubst } from '../../kernel/substitute.js';
import { substEvarInTerm, splitBody } from '../../engine/decimate.js';
import { checkDrawData } from '../draw-check.js';
import {
...
function closeCollapse(ctx, calculus, skolems) {
  ...
  const residualSeq = Seq.fromArrays(bagList(ctx.pool), ctx.cart, ctx.succ);
  ...
  for (let i = peels.length - 1; i >= 0; i--) {
    node = new ProofTree({
      conclusion: Seq.fromArrays(fullCtx, ctx.cart, peels[i].goal),
...
function elaborateCollapse({ sequent, trace, sigma, program, calculus, skolems = [] }) {
  ...
  for (const entry of trace) {
    ...
    if (drawn) {
      ...
      draw = { sort, member, witness, wave: fact };
    } else {
    ...
    chain.push({
      conclusion: Seq.fromArrays(bagList(ctx.pool), ctx.cart, ctx.succ),
      rule: 'draw',
      state: { draw },
    });
...
function certifyCollapse({ engineCalc, calculus, kernel, state, horizonTerm, collapseOpts = {} }) {
  ...
  const tokens = trace.filter((t) => t.draw)
    .map((t) => Store.put('drawn', [atom(t.draw.member), atom(t.draw.sort)]));
