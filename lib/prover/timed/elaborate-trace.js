...
function programFromCalc(engineCalc) {
  ...
  return {
    ...
    // masses are the sampler's CLAIMED inside-mass table (slice 4:
    // verified by substitution, never re-solved)
    sorts: engineCalc.sorts || null, priors: engineCalc.priors || null,
    masses: engineCalc.masses || null,
  };
}
...
function adaptRule(r) {
  ...
  return {
    ...
    // Deliberately NOT resolveDelayValue (timed.js): the checker receives
    // the UNGROUNDED term and grounds it via its own bind map — handing it
    // a θ-substituted value would bypass the re-derivation.
    delay: r.delay
      ? (r.delay.ground !== undefined ? r.delay.ground
        : r.delay.term !== undefined ? r.delay.term   // checker grounds via its bind map
        : slots[r.delay.slot])
      : null,
...
/** Build the elaboration context — live pool (count map) + copy-source
...
function elabContext(sequent, calculus) {
  ...
  const pool = new Map();
  for (const h of Seq.getContext(sequent, CZ)) pool.set(h, (pool.get(h) || 0) + 1);
  return { tools, pool, cart: [...Seq.getContext(sequent, SZ)], succ: sequent.succedent, CZ, SZ };
}
...
function applyEvents(ctx, events, program, calculus) {
  ...
  for (const ev of events || []) {
    ...
    for (let i = 0; i < mult; i++) {
      ...
      const conclusion = Seq.seq({ [ctx.CZ]: bagList(pool), [ctx.SZ]: cart }, succ);
      // advance the pool
...
function closeResidual(ctx, calculus) {
  ...
  const residualSeq = Seq.seq({ [ctx.CZ]: bagList(pool), [ctx.SZ]: cart }, succ);
...
function decompose(formula, pool, cart, env) {
  // Zone names ride env (from elabContext's calculus-derived ctx) so every
  // emitted node's sequent uses the calculus's declared zones (TODO_0086).
  // Always populated by both callers — fail loud, not fall back (a silent
  // 'linear' default would mask a future refactor dropping the threading).
  if (!env.CZ || !env.SZ) throw new Error('decompose: env missing zone names (CZ/SZ)');
  const { CZ, SZ } = env;
  const mkSeq = (lin, succ) => Seq.seq({ [CZ]: lin, [SZ]: cart }, succ);
  ...
  if (tag === prodTag) {
    ...
    const ctx = [...ctxOf(left.node, CZ), ...ctxOf(right.node, CZ)];
    return {
      node: new ProofTree({
        conclusion: mkSeq(ctx, formula), rule: 'tensor_r',
  ...
  if (tag === unitTag) {
    return {
      node: new ProofTree({
        conclusion: mkSeq([], formula), rule: 'one_r', proven: true, premises: [],
      }),
      remaining: pool,
    };
  }
  ...
  if (tag === env.ST) {
    ...
    for (const admit of passes) {
      for (const [h, c] of pool) {
        ...
        return {
          node: new ProofTree({
            conclusion: mkSeq([h], formula), rule: 'at_l', proven: true, premises: [],
          }),
          remaining: pool,
        };
      }
  ...
  if (tag === expTag && countOf(Store.child(formula, 0)) !== null) {
    ...
    if (k === 0n) {
      return {
        node: new ProofTree({
          conclusion: mkSeq([], formula), rule: 'bang_r3',
    ...
    const ctx = [...ctxOf(head.node, CZ), ...ctxOf(rest.node, CZ)];
    return {
      node: new ProofTree({
        conclusion: mkSeq(ctx, formula), rule: 'bang_r2',
  ...
  if (tag === expTag) {
    ...
    const idN = new ProofTree({
      conclusion: mkSeq([inner], inner), rule: 'id', proven: true, premises: [],
    });
    const copyN = new ProofTree({
      conclusion: mkSeq([], inner), rule: 'copy', proven: true, premises: [idN],
    });
    return {
      node: new ProofTree({
        conclusion: mkSeq([], formula), rule: 'bang_r', proven: true, premises: [copyN],
      }),
      remaining: pool,
    };
  }
  ...
  if (pool.get(formula) > 0) {
    ...
    return {
      node: new ProofTree({
        conclusion: mkSeq([formula], formula), rule: 'id', proven: true, premises: [],
      }),
      remaining: pool,
    };
  }
...
const ctxOf = (node, cz) => Seq.getContext(node.conclusion, cz); // cz always passed explicitly
const take = (pool, h) => {
  const c = pool.get(h);
