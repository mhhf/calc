...
function createGenericProver(calculus) {
  ...
  const applyRule = (seq, position, index, ruleSpec) => {
    ...
    // Stickiness: monad_l only fires when succedent is monadic
    if (ruleSpec.requiresMonadicSuccedent) {
      const succTag = Store.isTerm(seq.succedent) ? Store.tag(seq.succedent) : null;
      if (succTag !== 'monad') return null;
    }
...
module.exports = { createGenericProver };
