/**
 * Manual Proof API — state management for interactive proof construction.
 *
 * Single source of truth for proof state (focus, delta, tree) and actions.
 * Rendering (renderSequent, getAbstractRule) delegated to manual-render.js
...
function createManualProofAPI(calculus) {
  ...
  function createProofState(seq) {
    return {
      ...
      // Delta: remaining consumable context to distribute
      // Starts with the full consumable pool (all consumable zones, TODO_0285)
      delta: Context.fromArray(Seq.consumablePool(seq, ctxStruct)),
  ...
  function getApplicableActions(state, opts = {}) {
    ...
    const linear = Seq.consumablePool(seq, ctxStruct);
  ...
  function buildRuleAction(ruleName, position, index, formula, seq) {
    ...
    if (needsContextSplit) {
      for (const hash of remainingDelta) {
        contextEntries.push({
          hash,
    ...
    // Compute full premises (with context added). Premise consumable
    // columns are read as the POOL and rebuilt by routing (TODO_0285) —
    // a CZ-only read would drop aux-zone premise formulas.
    const cart = Seq.getContext(seq, SZ);
    const fullPremises = barePremises.map((barePremise, i) => {
      const premiseLinear = Seq.consumablePool(barePremise, ctxStruct);

      if (spec.copyContext) {
        // Add full remaining delta to each premise
        const additions = Context.toArray(remainingDelta);
        return Seq.seq({ ...Seq.routeContexts(ctxStruct, [...additions, ...premiseLinear]), [SZ]: cart }, barePremise.succedent);
  ...
  function computePremisesWithSplit(action, split) {
    const cart = Seq.getContext(action.premises[0], SZ) || [];
  ...
  const renderer = createManualProofRenderer(calculus);
  ...
  return {
    ...
    // Rendering (delegated to manual-render.js)
    ...renderer,
...
export { createManualProofAPI };
export default { createManualProofAPI };
