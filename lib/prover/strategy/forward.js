...
const { match: _match, matchIndexed: _matchIdx, undoSave, undoRestore, undoDiscard } = require('../../kernel/unify');
const { apply: _subApply, applyFlat: _subApplyFlat, applyIndexed: _subApplyIdx } = require('../../kernel/substitute');
...
function subApply(hash, theta) {
  if (!PROFILE) return _subApplyFlat(hash, theta);
  const t0 = performance.now();
  const result = _subApplyFlat(hash, theta);
...
// Stage 7c: compiled substitution — direct Store.put from precomputed recipe.
...
// Reusable children buffer for Store.put (max arity ~4)
const _compiledChildren = [0, 0, 0, 0];

function subCompiled(recipe, theta) {
  if (recipe.isSlot) return theta[recipe.slot];
  const sources = recipe.sources;
  for (let i = 0; i < sources.length; i++) {
    const s = sources[i];
    _compiledChildren[i] = s.type === 'slot' ? theta[s.slot] : s.value;
  }
  // Store.put needs a proper array (it reads .length), so slice the buffer
  // For arity 1 and 2 (common), use direct array literals to avoid slice
  const n = sources.length;
  if (n === 1) return Store.put(recipe.tag, [_compiledChildren[0]]);
  if (n === 2) return Store.put(recipe.tag, [_compiledChildren[0], _compiledChildren[1]]);
  return Store.put(recipe.tag, _compiledChildren.slice(0, n));
}
...
function applyMatch(state, { rule, theta, slots, consumed, optimized }) {
  ...
  for (let i = 0; i < linPats.length; i++) {
    ...
    const h = recipe && recipe.compiled
      ...
      : (slots ? subApplyIdx(pattern, theta, slots) : subApply(pattern, theta));
