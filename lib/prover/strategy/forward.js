...
const { apply: _subApply, applyFlat: _subApplyFlat } = require('../../kernel/substitute');
const ffi = require('../../engine/ffi');
const { analyzeDeltas, analyzeBufferLimits } = require('./rule-analysis');
...
function tryMatch(rule, state, calc, matchOpts) {
  ...
  while (rpLen > 0 || persistentIdx < persistentList.length) {
    ...
    for (let pi = 0; pi < rpLen; pi++) {
      ...
      // Secondary index O(1) lookup for fingerprint predicate (e.g., code(PC, OPCODE))
      // Generalized from EVM-specific codeByPC — works for any predicate with
      // a discriminating ground child. See "Fingerprint indexing" in roadmap.
      if (pred === index._fpPred && index._byKey && meta.secondaryKeyPattern !== null) {
        const keyValue = subApply(meta.secondaryKeyPattern, theta);
    ...
    while (persistentIdx < persistentList.length) {
      ...
      // FFI returns pair format [[v,t]], convert to flat [v,t,...] on hot path
      ...
      // Backward prover returns pair format [[v,t]], convert to flat
