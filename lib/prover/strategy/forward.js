...
function tryMatch(rule, state, calc, matchOpts) {
  ...
  // Track how many preserved copies of each hash we've already reserved
  const preservedUsed = {};
  ...
  while (resourcePatterns.length > 0 || persistentIdx < persistentList.length) {
    ...
    for (const pattern of resourcePatterns) {
      ...
      const isPreserved = usePreserved &&
        preservedCount[pattern] > 0 &&
        (preservedUsed[pattern] || 0) < preservedCount[pattern];
          ...
          if (available > 0) {
            ...
            if (newTheta !== null) {
              if (isPreserved) {
                ...
                preservedUsed[pattern] = (preservedUsed[pattern] || 0) + 1;
              } else {
