...
function getPredicateHead(h) {
  ...
  if (!node) return null;
  if (!NON_PRED_TAGS.has(node.tag)) return node.tag;
...
function extractSecondArg(h) {
  const node = Store.get(h);
  if (!node || NON_PRED_TAGS.has(node.tag)) return null;
...
function compileRule(rule) {
  ...
  for (const p of (anteFlat.linear || [])) {
    ...
    if (pred === 'code') {
      const patternNode = Store.get(p);
      if (patternNode && patternNode.children.length >= 2) {
        pcSubPattern = patternNode.children[0];
      }
    }
...
function evmStrategy(state, stateIndex, opcodeIndex) {
  ...
  if (!pcNode || pcNode.children.length < 1) return null;
  const pcValue = pcNode.children[0];
  ...
  for (const codeHash of codeFacts) {
    ...
    if (!codeNode || codeNode.children.length < 2) continue;

    // Flat: {tag: 'code', children: [PC, OPCODE]}
    const codePC = codeNode.children[0];
...
function collectOutputVars(h) {
  ...
  if (NON_PRED_TAGS.has(node.tag) || node.children.length === 0) return vars;
  for (const v of collectFreevars(node.children[node.children.length - 1])) {
    vars.add(v);
  }
  return vars;
}
...
function tryFFIDirect(goal) {
  ...
  let head;
  if (!NON_PRED_TAGS.has(node.tag)) head = node.tag;
  else if (node.tag === 'atom') head = node.children[0];
  else return null;

  const meta = _ffiMeta[head];
  if (!meta) return null;

  if (!ffi.mode.checkMode(node.children, meta.mode)) return null;
