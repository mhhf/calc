...
const { applyFlat: subApply, applyIndexed: subApplyIdx } = require('../../kernel/substitute');
const {
  tryMatch,
  buildStateIndex,
  getPredicateHead,
...
function applyMatchChoice(state, { theta, consumed, optimized, rule, slots }, alt) {
  ...
  for (const pattern of alt.linear) {
    ...
    const h = slots ? subApplyIdx(pattern, theta, slots) : subApply(pattern, theta);
...
function mutateState(state, consumed, theta, linearPatterns, persistentPatterns, slots, rule) {
  ...
  // Stage 7c: when rule is provided, handle preserved-skip + compiled sub together.
  // This ensures recipe indices align with pattern indices (both use full consequent list).
  const cLinear = rule && rule.compiledConseqLinear;
  let skipCount = null;
  if (rule && rule.analysis && rule.analysis.preserved.length > 0) {
    skipCount = {};
    for (const h of rule.analysis.preserved) skipCount[h] = (skipCount[h] || 0) + 1;
  }
  const skipUsed = {};

  for (let i = 0; i < linearPatterns.length; i++) {
    const pattern = linearPatterns[i];
    // Preserved-skip (was done externally via filterPreserved, now inline)
    if (skipCount && skipCount[pattern] > 0 &&
        (skipUsed[pattern] || 0) < skipCount[pattern]) {
      skipUsed[pattern] = (skipUsed[pattern] || 0) + 1;
      continue;
    ...
    if (recipe && recipe.compiled) {
      h = recipe.isSlot ? theta[recipe.slot] : _subCompiledInline(recipe, theta);
    } else {
      h = slots ? subApplyIdx(pattern, theta, slots) : subApply(pattern, theta);
    }
...
// Stage 7c: inline compiled substitution for symexec (avoid importing from forward)
function _subCompiledInline(recipe, theta) {
  const sources = recipe.sources;
  const n = sources.length;
  if (n === 1) {
    const s0 = sources[0];
    return Store.put(recipe.tag, [s0.type === 'slot' ? theta[s0.slot] : s0.value]);
  }
  if (n === 2) {
  ...
  function go(depth, ctx) {
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        // Multi-alt: external preserved-skip, no compiled sub
        // (alt patterns differ from original, recipe indices don't align)
