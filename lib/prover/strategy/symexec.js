 ...
 */
function mutateState(state, consumed, theta, linearPatterns, persistentPatterns, slots) {
  const linearOrig = {};
  ...
  // Produce linear facts (Stage 6: use indexed substitute when slots available)
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        const undo = mutateState(state, m.consumed, m.theta,
          linPats, m.rule.consequent.persistent || [], m.slots);
        ...
        for (let i = 0; i < alts.length; i++) {
          const linPats = skipPreserved
            ...
            : alts[i].linear;
          const undo = mutateState(state, m.consumed, m.theta,
            linPats, alts[i].persistent, m.slots);
          const { ctx: childCtx, indexUndo } = makeChildCtx(ctx, state, undo);
          const child = go(depth + 1, childCtx);
          undoIndexChanges(ctx.stateIndex, indexUndo);
          undoMutate(state, undo);
