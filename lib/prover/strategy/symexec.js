...
function getCodePC(h) {
  const node = Store.get(h);
  if (!node || node.tag !== 'code' || node.children.length < 2) return null;
...
function findCurrentOpcode(stateIndex) {
  ...
  if (stateIndex.codeByPC) {
    ...
    if (codeFact) {
      ...
      if (codeNode && codeNode.children.length >= 2) return codeNode.children[1];
    }
  }
  ...
  for (const codeHash of codeFacts) {
    ...
    if (codeNode.children[0] !== pcValue) continue;
...
function findAllMatches(state, rules, calc, strategy, stateIndex) {
  ...
  for (const rule of candidates) {
    const m = tryMatch(rule, _indexedState, calc);
    if (m) matches.push(m);
  }
  return matches;
}
