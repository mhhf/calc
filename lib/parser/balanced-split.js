/**
 * Split string on separator respecting balanced parentheses.
 *
 * Shared by: declarations.js, sequent-parser.js, rules2-parser.js.
 * Tracks paren depth () and brace depth {} to avoid splitting inside
 * nested expressions.
 *
...
function balancedSplit(str, sep) {
  const parts = [];
  let depth = 0, braceDepth = 0, start = 0;
  for (let i = 0; i < str.length; i++) {
    ...
    else if (depth === 0 && braceDepth === 0 && str.slice(i, i + sep.length) === sep) {
