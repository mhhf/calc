...
function tokenize(input, config = {}) {
  ...
  const rats = config.rationals === true;
  const tokens = [];
  let pos = 0;
  ...
  while (pos < len) {
    ...
    if (nums && _isDigit(cc)) {
      if (cc === 48 && pos + 1 < len && input.charCodeAt(pos + 1) === 120) { // 0x
        ...
        // Rational literals (timed calculi): 3/2 and 0.5 lex as one RATNUM
        // token — parsed digit-wise to an exact rational, never via float.
        if (rats && pos + 1 < len &&
            (input.charCodeAt(pos) === 47 || input.charCodeAt(pos) === 46) && // '/' '.'
            _isDigit(input.charCodeAt(pos + 1))) {
          pos++;
          while (pos < len && _isDigit(input.charCodeAt(pos))) pos++;
          tokens.push({ type: 'RATNUM', value: input.slice(start, pos), pos: start });
          continue;
        }
        // Parcel sugar (TODO_0268 §5d): NUMBER immediately followed by an
        // ident-start char, NO whitespace, lexes as ONE token — `4wood` =
        // `!_4 wood`. Lexically unambiguous by construction (identifiers
        // can't start with a digit); the SPACED form `4 wood` stays
        // application juxtaposition (`f 4 wood` = f(4, wood)) — a spaced
        // parcel production is genuinely ambiguous with it (detector-
        // verified). Fractional counts don't fuse: RATNUM wins above and
        // ℚ parcels are post-v1 (D4).
        if (config.parcels && pos < len && _isIdentStart(input.charCodeAt(pos))) {
          while (pos < len && _isIdentCont(input.charCodeAt(pos))) pos++;
          tokens.push({ type: 'PARCEL', value: input.slice(start, pos), pos: start });
          continue;
        }
      }
...
// ─── Ambiguity Instrument (TODO_0268 §5a) ────────────────────────────────────
//
// Static CFG ambiguity is undecidable — per-input detection is the honest
// maximum (the decidable static layer is the generator-level collision
// throws in earley-grammar.js: duplicate circumfix, one graded prefix,
// mixfix-shape errors). Two per-parse signals:
//   1. dual-derivation items — add() sees a second DISTINCT back-pointer
//      for an item already in the chart (the parser silently kept the
//      first one before; that is exactly first-wins ambiguity),
//   2. multiple accepting items in the final chart set.
// Item-level dual-backs are an over-approximation: a detected duplicate
// may not affect the parse RESULT (it may reduce only inside an abandoned
// branch), but the grammar is still ambiguous and worth flagging — the
// instrument is conservative on purpose. The counter is always on; strict
// mode additionally throws so corpus sweeps can assert ambiguity-freedom.
// Precondition: sound for ε-free grammars. CALC grammar generators emit
// no nullable rules, so the duplicate-ε-rule false-negative (an 'n'-back
// item legitimately sharing a key with a real ε-completion) cannot arise.

let _strictAmbiguity = false;
const _ambig = { dupItems: 0, accepts: 0, firstSet: -1 };

/** Strict mode: any detected ambiguity becomes a parse error. */
function setStrictAmbiguity(on) { _strictAmbiguity = !!on; }

/** Ambiguity stats of the most recent earleyParse call (copy). */
function ambiguityStats() { return { ..._ambig }; }

function _sameBack(a, b) {
  return a.t === b.t && a.l === b.l && a.r === b.r && a.tok === b.tok && a.nt === b.nt;
}
...
function _ensureChart(n) {
  while (_chartPool.length <= n) {
    _chartPool.push({ seen: new Map(), items: [] });
  }
}
...
function earleyParse(g, tokens, extractFn) {
  ...
  function add(si, ruleIdx, dot, origin, back) {
    ...
    const set = chart[si];
    const existing = set.seen.get(key);
    if (existing !== undefined) {
      // §5a: a second, distinct derivation of a chart item — first-wins
      // ambiguity. 'n' backs are excluded: the Aycock-Horspool nullable
      // advance may legitimately coexist with the real ε-completion of
      // the same item (one derivation, seen twice).
      if (back !== null && existing.back !== null &&
          back.t !== 'n' && existing.back.t !== 'n' &&
          !_sameBack(back, existing.back)) {
        if (_ambig.dupItems++ === 0) _ambig.firstSet = si;
      }
      return;
    }
    const item = { ruleIdx, dot, origin, back };
    set.seen.set(key, item);
    set.items.push(item);
  }
  ...
  // All accepting items are counted (§5a signal 2); the FIRST one is
  // extracted, as before.
  let accept = null;
  ...
  if (_strictAmbiguity && (_ambig.dupItems > 0 || _ambig.accepts > 1)) {
    const tok = _ambig.firstSet > 0 ? tokens[_ambig.firstSet - 1] : null;
    const at = tok ? ` near '${tok.value}' (pos ${tok.pos})` : '';
    throw new Error(`Ambiguous parse${at}: ${_ambig.dupItems} dual-derivation item(s), ${_ambig.accepts} accepting item(s)`);
  }
 ...
 * No production caller — used only in tests/earley.test.js.
 *
...
export { T, NT, grammar, tokenize, earleyParse, createParser, sortOperators, setStrictAmbiguity, ambiguityStats };
export default { T, NT, grammar, tokenize, earleyParse, createParser, sortOperators, setStrictAmbiguity, ambiguityStats };
