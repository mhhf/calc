...
function earleyParse(g, tokens, extractFn) {
  ...
  // Item key: perfect hash over (ruleIdx, dot, origin).
  // Uses arithmetic packing instead of bit-shifting to avoid 32-bit overflow.
  // Supports up to 65536 rules, 256 RHS symbols, and ~2^37 tokens.
  const stride = n + 1;

  // Chart: S[0..n], each set = { items: Map<key, item>, queue: item[] }
...
module.exports = { T, NT, grammar, tokenize, earleyParse, createParser };
