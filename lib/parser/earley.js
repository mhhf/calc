...
/**
 * Check if a parse is ambiguous for a given nonterminal span.
 * Returns number of distinct completed derivations.
 *
 * @param {{ rules, start, nullable, byLhs }} g
 * @param {Object[]} chart - Chart from earleyParse (not currently exposed)
 * @param {number} ntId - Nonterminal ID
 * @param {number} from - Origin position
 * @param {number} to - End position
 * @returns {number} Count of completed items for (ntId, from) at position to
 */
function countCompletions(chart, rules, ntId, from, to) {
  let count = 0;
  for (const [, item] of chart[to].items) {
    const rule = rules[item.ruleIdx];
    if (rule.lhs === ntId && item.dot === rule.rhs.length && item.origin === from) {
      count++;
    }
  }
  return count;
