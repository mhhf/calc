...
function sequentParser(calculus) {
  ...
  function parseSequent(input) {
    ...
    // Wrapper-routed consumable columns (TODO_0285): an aux-zone formula
    // (e.g. located) self-describes via its wrapper tag; with a single
    // consumable zone this is { [CZ]: linearFormulas } unchanged.
    return Seq.seq({ ...Seq.routeContexts(ctxStruct, linearFormulas), [SZ]: [] }, succedent);
  }
  ...
  function formatSequent(seq, format = 'ascii') {
    // Consumable zones render merged (entries are self-describing via
    // their wrapper tags — TODO_0285)
