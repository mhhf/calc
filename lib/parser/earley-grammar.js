...
const { GRADE_0, GRADE_W } = require('../engine/grades');
const { T, NT, grammar, tokenize, earleyParse, sortOperators } = require('./earley');
...
function _buildGrammar(tables) {
  ...
  // ── Unary prefix ──
  if (binderDefs) {
    // Generic unary prefixes (excludes bang — handled below)
    ...
    // Bang (graded modality): !, !_0, !_ω — hardcoded grammar rules
    // ! A    → bang(GRADE_W, A)  (backward compatible)
    // !_0 A  → bang(GRADE_0, A)  (compile-time)
    // !_ω A  → bang(GRADE_W, A)  (explicit omega = same as !)
    rules.push({
      lhs: UNARY, rhs: [T('!'), NT(BINDER_UNARY)],
      action: c => Store.put('bang', [GRADE_W, c[1]]), tag: 'unary',
    });
    rules.push({
      lhs: UNARY, rhs: [T('!_0'), NT(BINDER_UNARY)],
      action: c => Store.put('bang', [GRADE_0, c[1]]), tag: 'unary',
    });
    rules.push({
      lhs: UNARY, rhs: [T('!_ω'), NT(BINDER_UNARY)],
      action: c => Store.put('bang', [GRADE_W, c[1]]), tag: 'unary',
    });

    // Preserved resource sugar: $P marks a linear resource as preserved across
    // a forward rule (consumed and re-produced identically). Desugars to P on
    // both LHS and RHS. Only available when forwardRules is enabled. (TODO_0149)
    if (hasForward) {
      rules.push({
        lhs: UNARY, rhs: [T('$'), NT(BINDER_UNARY)],
    ...
    rules.push({
      lhs: UNARY, rhs: [T('!_0'), NT(UNARY)],
      action: c => Store.put('bang', [GRADE_0, c[1]]), tag: 'unary',
    });
    rules.push({
      lhs: UNARY, rhs: [T('!_ω'), NT(UNARY)],
      action: c => Store.put('bang', [GRADE_W, c[1]]), tag: 'unary',
    });

    // Preserved resource sugar ($prefix) for forward rules (TODO_0149)
    if (hasForward) {
      rules.push({
        lhs: UNARY, rhs: [T('$'), NT(UNARY)],
        action: c => Store.put('preserved', [c[1]]), tag: 'unary',
...
function buildParserFromGrammar(spec) {
  ...
  // Expression memoization — cleared on Store.clear()
  const memo = new Map();
  Store.onClear(() => memo.clear());
    ...
    if (tokens.length === 1) {
      ...
      if (tok.type === 'IDENT') {
        ...
        if (raw.startsWith('0x') && raw.length - 2 > 64) {
          ...
          result = Store.put('arrlit', [elems]);
        } else {
          result = Store.put('binlit', [BigInt(raw)]);
        }
      } else {
