...
import { grade0, gradeW } from '../engine/grades.js';
...
function extractParserTables(constructors) {
  ...
  return { operators, nullary, unaryPrefix, circumfix, gradedPrefix };
}
...
function _buildGrammar(tables) {
  ...
  const hasTimed = tables.timedAnnotations || false;
  ...
  const compGradeUnit = tables.gradeUnit || null;
  ...
  const TIMED = hasTimed ? ntId++ : -1;
  const APP = hasApp ? ntId++ : -1;
  const ATOM = ntId++;
  const BVARS = binderDefs ? ntId++ : -1;
  const GRADE = hasTimed ? ntId++ : -1;
  const GEXPR = hasTimed ? ntId++ : -1;
  const GTERM = hasTimed ? ntId++ : -1;

  // Chain helpers. The TIMED level (postfix `@`, TODO_0265 Phase 3) sits
  // between UNARY and APP/ATOM — `A@t` binds tighter than every operator.
  const LOOSEST = precLevels.length > 0 ? precToNT.get(precLevels[0]) : UNARY;
  const appNext = hasApp ? APP : ATOM;
  const unaryNext = hasTimed ? TIMED : appNext;
  ...
  // Timed antecedent forms (TODO_0265 Phase 3, gated by timedAnnotations):
  // scheduling windows `after E` / `before E` (E = grade expression, D8)
  // and the read marker `read A` (test arc, E7.2). Wrapper nodes only —
  // convert.js desugars/validates, compile.js strips into rule metadata.
  if (hasTimed) {
    rules.push({
      lhs: UNARY, rhs: [T('after'), NT(GRADE)],
      action: c => Store.put('after', [c[1]]), tag: 'unary',
    });
    rules.push({
      lhs: UNARY, rhs: [T('before'), NT(GRADE)],
      action: c => Store.put('before', [c[1]]), tag: 'unary',
    });
    rules.push({
      lhs: UNARY, rhs: [T('read'), NT(unaryOperand)],
      action: c => Store.put('readPreserved', [c[1]]), tag: 'unary',
    });
  }
  ...
  // ── Timed level: postfix `@` (TODO_0265 Phase 3, E1/E2) ──
  // `A@t` (atomic formula) → at(A, t): availability stamp.
  // `{B}@d` (graded circumfix) → rebuild with grade d (replaces the elided
  // unit grade): duration effect. Double stamps (`A@s@t`, via parens) and
  // stamps on an ungraded computation are rejected.
  if (hasTimed) {
    rules.push({
      lhs: TIMED, rhs: [NT(appNext), T('@'), NT(GRADE)],
      action: c => {
        const h = c[0], g = c[2];
        const t = Store.tag(h);
        if (gradedCircumfix && t === gradedCircumfix.name) {
          return Store.put(t, [g, Store.child(h, 1)]);
        }
        if (t === 'at') throw new Error("Parse error: double stamp 'A@s@t'");
        if (circumfixes.some(cf => cf.name === t)) {
          throw new Error(`Parse error: '@' on an ungraded computation '${t}'`);
        }
        return Store.put('at', [h, g]);
      },
      tag: 'timed',
    });
    rules.push({ lhs: TIMED, rhs: [NT(appNext)], action: c => c[0], tag: 'pass' });
  ...
  // Rational literals as ordinary term arguments (timed calculi): a fact
  ...
  // Graded-prefix tokens (derived: OP_ω, OP_0, OP — longest first).
  // OP_ (bare suffix marker) is RESERVED with no grammar rule: `!_2 wood` /
  // `!_W wood` (count grades, D4 — Phase 4) must be a LOUD parse error today,
  // not a silent bang(ω, _2(wood)) misparse.
  if (gradedPrefix) {
  ...
  if (hasTimed) keywords.push('after', 'before', 'read');

  return {
    rules, start: START,
    lexerConfig: { operators: allOps, keywords, numbers: hasNumbers || hasTimed, rationals: hasTimed },
...
function _evalRule(rule, children, ctx) {
  ...
  // Timed postfix (`A@t`): finalize the stamped operand's app spine
  if (tag === 'timed') {
    children[0] = _appFinalize(children[0], ctx);
    return rule.action(children);
  }
