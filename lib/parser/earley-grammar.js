...
function extractParserTables(constructors) {
  ...
  for (const [name, constr] of Object.entries(constructors)) {
    ...
    if (cfx && ((arity === 1 && (cfx[2] === '_' || cfx[3] === '1')) ||
                (arity === 2 && cfx[3] === '2'))) {
    ...
    if (ann.category === 'exponential' && arity === 2) {
      ...
      if (gp) { gradedPrefix = { op: gp[1], name }; continue; }
    }
...
function _buildGrammar(tables) {
  ...
  if (gradedPrefix) {
    for (const t of [gradedPrefix.op + '_ω', gradedPrefix.op + '_0', gradedPrefix.op]) {
...
function _putRatToken(raw) {
  ...
  if (slash >= 0) {
    return putRat(BigInt(raw.slice(0, slash)), BigInt(raw.slice(slash + 1)));
  }
