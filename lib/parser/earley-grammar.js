...
function extractParserTables(constructors) {
  ...
  for (const [name, constr] of Object.entries(constructors)) {
    ...
    if (arity === 0) {
      ...
      const t = _parseTemplate(name, ascii, constr, precedence, assoc);
      // @grammar grade (TODO_0285): the declared OPT-IN routing this
      // template into the auxiliary GRADE chain (structured grade forms —
      // pairs, intervals). Explicit, never inferred: return sorts are
      // free-form (lnl's structure templates return 'structure'), so
      // sort-based inference would misroute.
      if (ann.grammar === 'grade') t.gradeChain = true;
      templates.push(t);
    }
  }
...
function _buildGrammar(tables) {
  ...
  // Grade-chain templates (@grammar grade, TODO_0285) are emitted into
  // GEXPR below, never into the expression-kind levels.
  const gradeTs = templates.filter(t => t.gradeChain);
  const exprTs = templates.filter(t => !t.gradeChain);
  const postfixTs = exprTs.filter(t => t.kind === 'postfix');
  const infixTs = exprTs.filter(t => t.kind === 'infix');
  const prefixTs = exprTs.filter(t => t.kind === 'prefix');
  const closedTs = exprTs.filter(t => t.kind === 'closed');
  ...
  function addBinderRules(lhs) {
    ...
    for (const [kw, tagName] of Object.entries(binderDefs)) {
      ...
      // ∃_ρ — the weight-graded existential (TODO_0292 M1, will P3):
      // `exists X: s @w. A` → superpose(atom(s), exists(A)). Opt-in per
      // calculus (tables.binderSorts — only will sets it); disjoint from
      // the plain rule (the ':' after the first IDENT ends it), so no
      // ambiguity. Plain `exists X. A` stays the skolem form (D4).
      if (tables.binderSorts && tagName === 'exists') {
        rules.push({
          lhs, rhs: [T(kw), T('IDENT'), T(':'), T('IDENT'), T('@'), T('IDENT'), T('.'), NT(START)],
          action: null, tag: 'binderSorted', binderSortedTag: tagName,
        });
      }
    }
  }
  ...
  if (hasAux) {
    ...
    // Grade-chain declared templates (@grammar grade, TODO_0285):
    // structured grade forms (`3 ~ 2` pairs, intervals) parse inside the
    // parenthesized grade position — `@(3 ~ 2)` — keeping the compound-
    // inside-parens discipline (D3/E7.1). Holes route to GRADE (they are
    // cross-sort by construction; a same-sort 'grade' hole would also
    // land on GRADE via the left/right NT arguments).
    for (const t of gradeTs) {
      const { rhs, mkArgs } = tmplRhs(t, GRADE, GRADE);
      rules.push({
        lhs: GEXPR, rhs,
        action: (c) => Store.put(t.name, mkArgs(c)), tag: 'grade_tmpl',
      });
    }
  }
...
function _extract(item, rules, ctx) {
  ...
  // ── Sorted binder (∃_ρ): kw IDENT ':' IDENT '@' IDENT '.' Body ──
  // `exists X: s @w. A` → superpose(atom(s), exists(A)) — the wave
  // marker the decimation driver opts in on (TODO_0292 M1/D4). Walk
  // back-pointers right→left: Body(7), '.'(6), ann(5), '@'(4),
  // sort(3), ':'(2), var(1), kw(0).
  if (rule.binderSortedTag) {
    let cur = item;
    const bodyItem = cur.back.r;                       // Body
    cur = cur.back.l;
    cur = cur.back.l;                                  // '.'
    const annTok = cur.back.tok; cur = cur.back.l;     // annotation ident
    cur = cur.back.l;                                  // '@'
    const sortTok = cur.back.tok; cur = cur.back.l;    // sort ident
    cur = cur.back.l;                                  // ':'
    const varTok = cur.back.tok;                       // binder variable
    if (annTok.value !== 'w') {
      throw new Error(`Parse error: binder annotation '@${annTok.value}' — only '@w' (the weight-graded existential) is defined`);
    }
    if (!/[A-Z]/.test(varTok.value[0])) {
      throw new Error(`Parse error: superposed binder variable '${varTok.value}' must be capitalized`);
    }
    ctx.binderStack.push(varTok.value);
    const body = _extract(bodyItem, rules, ctx);
    ctx.binderStack.length -= 1;
    return Store.put('superpose', [
      Store.put('atom', [sortTok.value]),
      Store.put(rule.binderSortedTag, [body]),
    ]);
  }
