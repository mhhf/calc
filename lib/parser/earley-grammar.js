      ...
      if (t.countGrade) {
        ...
        if (gt !== 'binlit' && gt !== 'metavar' && gt !== 'freevar') {
          throw new Error(gt === 'ratlit'
            ? `Parse error: fractional count grade '${t.pieces[0].lit}n/d' — ℚ parcels are post-v1 (D4)`
...
function parserFromGrammar(spec) {
  ...
  // terminal rule with an action is nullary whichever mechanism emitted
  // it ('nullary': the 'type' keyword; 'tmpl': folded nullary constants —
  // a template rule with one terminal has zero holes by construction).
  const nullaryMap = new Map();
  for (const rule of spec.rules) {
    if ((rule.tag === 'nullary' || rule.tag === 'tmpl') &&
...
function _evalRule(rule, children, ctx) {
  ...
  if (tag === 'parcel') {
    ...
    return Store.put(rule.parcelTag, [putRat(BigInt(count), 1n), _resolveName(raw.slice(count.length), ctx)]);
  }
