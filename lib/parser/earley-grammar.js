...
import { ILL_COMPUTATION } from '../engine/formula-utils.js';
...
function earleyGrammar(constructors, opts = {}) {
  ...
  for (const [name, constr] of Object.entries(constructors)) {
    ...
    if (constr.argTypes.length === 0) {
      ...
      // Bang is handled via hardcoded grammar rules (graded modality: !, !_0, !_ω)
      // Skip the generic classifier — arity 2 (grade, formula) doesn't fit unary or binary patterns
...
function _buildGrammar(tables) {
  ...
  // Computation (lax monad) construction for the `{ ... }` forms.
  // tables.computation is the role record { tag, bodyIdx, gradeIdx }
  // (default: ILL's unary monad). A graded computation (gradeIdx !== null)
  // needs tables.gradeUnit — an in-process () => hash hook supplying the
  // unit grade for bare `{B}` (the `@d` postfix syntax lands with the grade
  // algebra; TODO_0265 Phase 3). Serialized parser tables omit both fields
  // and get the ILL default.
  const comp = tables.computation || ILL_COMPUTATION;
  const compGradeUnit = tables.gradeUnit || null;
  const putComp = (body) => {
    if (comp.gradeIdx === null) return Store.put(comp.tag, [body]);
    if (!compGradeUnit) throw new Error('graded computation requires tables.gradeUnit');
    const kids = [];
    kids[comp.gradeIdx] = compGradeUnit();
    kids[comp.bodyIdx] = body;
    return Store.put(comp.tag, kids);
  };
    ...
    for (const op of opsAtPrec) {
      ...
      if (hasForward && op.name === 'loli') {
        rules.push({
          ...
          action: c => Store.put('loli', [c[0], putComp(c[3])]),
          tag: 'loli_monad',
        });
      }
  ...
  // Bang (graded modality): !, !_0, !_ω
  rules.push({
    lhs: UNARY, rhs: [T('!'), NT(unaryOperand)],
    action: c => Store.put('bang', [gradeW(), c[1]]), tag: 'unary',
  });
  rules.push({
    lhs: UNARY, rhs: [T('!_0'), NT(unaryOperand)],
    action: c => Store.put('bang', [grade0(), c[1]]), tag: 'unary',
  });
  rules.push({
    lhs: UNARY, rhs: [T('!_ω'), NT(unaryOperand)],
    action: c => Store.put('bang', [gradeW(), c[1]]), tag: 'unary',
  });
  ...
  rules.push({
    ...
    action: c => putComp(c[1]), tag: 'monad',
  });
  ...
  // Bang grade tokens (hardcoded, not in unaryPrefix)
  if (!allOps.includes('!_ω')) allOps.push('!_ω');
  if (!allOps.includes('!_0')) allOps.push('!_0');
  if (!allOps.includes('!')) allOps.push('!');
  allOps.push('|');
  allOps.push(':');
...
export { earleyGrammar, earleyGrammarFromTables, parserFromGrammar };
export default { earleyGrammar, earleyGrammarFromTables, parserFromGrammar };
