 ...
 *   sub <: sup.                          → SubsortDecl (TODO_0011 rung 1)
 *   name: (s <: q) sig.                  → TypeDecl with bounded sort variable
 *   name: (x: C) body.                   → Decl with classifier binder(s)
 ...
 */
function _eigenWrap(hash, eigenVars, Store) {
  ...
  let result = _subHashes(hash, subs, Store);
 ...
 */
function _subHashes(h, subs, Store) {
  ...
  if (t === 'arrlit') {
    ...
    for (let i = 0; i < elems.length; i++) {
      ne[i] = _subHashes(elems[i], subs, Store);
  ...
  for (let i = 0; i < a; i++) {
    ...
    if (Store.isTermChild(c)) {
      const r = _subHashes(c, subs, Store);
      if (r !== c) changed = true;
      nc.push(r);
    } else {
      nc.push(c);
 ...
 */
function parseDecls(source, parseExpr, opts = {}) {
  const declarations = [];
  ...
  while (pos < source.length) {
    ...
    if (source[pos] === '#') {
      ...
      if (sep) {
        ...
        if (eigenVars.length > 0) {
          lhsHash = _eigenWrap(lhsHash, eigenVars, Store);
          rhsHash = _eigenWrap(rhsHash, eigenVars, Store);
        }
        ...
        if (eigenVars.length > 0) {
          ...
          bodyHash = _eigenWrap(bodyHash, eigenVars, Store);
        }
    ...
    if (source[pos] === '@') {
      ...
      if (key === 'module') {
        declarations.push({
          type: 'directive',
          key: 'module',
          args: _moduleDef(argsText)
        });
      } else {
        declarations.push({
          type: 'directive',
          key,
          args: _directiveArgs(argsText)
        });
      }
      continue;
    }
    ...
    skipWs();
    // Subsort declaration `A <: B.` (TODO_0011 rung 1) — desugared by the
    // loader into a persistent sedge fact; this layer only records the edge.
    if (source[pos] === '<' && source[pos + 1] === ':') {
      pos += 2;
      skipWs();
      const sup = readIdentifier();
      if (!sup || pos >= source.length || source[pos] !== '.') {
        throw new Error(`Malformed subsort declaration '${name} <: …' at ${posInfo()} — expected '${name} <: sortname.'`);
      }
      pos++; // skip .
      declarations.push({ type: 'subsort', sub: name, sup });
      continue;
    }
    if (pos >= source.length || source[pos] !== ':') {
    ...
    const annotations = [];
    if (parseAnnotations) {
      const { body: cleanBody, anns } = _extractAnns(declBody);
      declBody = cleanBody;
      annotations.push(...anns);
    } else if (opts.memberPriors) {
      // Member-prior annotation `name: sortname @w Q.` (TODO_0292 D5/M6)
      // — the ONE annotation program files admit. Deliberately narrow:
      // the body must be EXACTLY `ident @w <numeral[/numeral]>`, so rule
      // bodies — where `@` is the stamp position (`food@Q`, `{...}@3`) —
      // can never match; full annotation parsing stays .calc/.family-only.
      let pm = declBody.match(/^\s*([A-Za-z_]\w*)\s*@w\s+(\d+)(?:\s*\/\s*(\d+))?\s*$/);
      // Rung 2 (P3): constructor members carry priors too —
      // `cons: (a: lst) -> lst @w 3.` A trailing `@w Q` is stripped from
      // an ARROW-CHAIN body only (contains `->`, no rule/monad syntax),
      // so timed rule bodies still can never match.
      if (!pm && declBody.includes('->') && !declBody.includes('-o') && !declBody.includes('{')) {
        const am = declBody.match(/^([\s\S]*->[\s\S]*?)\s*@w\s+(\d+)(?:\s*\/\s*(\d+))?\s*$/);
        if (am) pm = am;
      }
      if (pm) {
        declBody = pm[1];
        const d = BigInt(pm[3] || '1');
        if (d === 0n) throw new Error(`'${name}': @w prior with zero denominator at ${posInfo()}`);
        annotations.push({ key: 'w', value: { type: 'RatValue', n: BigInt(pm[2]), d } });
      }
    }

    // Strip inline comments from body before parsing
    declBody = _stripComments(declBody);

    // Leading binder groups (TODO_0011 rung 1): `(s <: q)` bounded sort
    // variables and `(x: C)` classifier binders. A `(x: C)` group is a
    // binder only when it does NOT head an arrow signature — `(n: bin) ->
    // type` stays a named-arg signature. Semantics (classifier existence,
    // schema expansion) live in the loader; this layer only splits.
    const binders = [];
    {
      let rest = declBody.trimStart();
      for (;;) {
        const m = rest.match(/^\(\s*([A-Za-z_][A-Za-z0-9_]*)\s*(<:|:)\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)\s*/);
        if (!m) break;
        const after = rest.slice(m[0].length);
        if (m[2] === ':' && (after.startsWith('->') || after.length === 0)) break;
        binders.push({ name: m[1], rel: m[2], sort: m[3] });
        rest = after;
      }
      if (binders.length > 0) {
        if (!rest.trim()) {
          throw new Error(`Declaration '${name}' at ${posInfo()}: binder group(s) with no body`);
        }
        declBody = rest;
      }
    }
  ...
  function readSettingValue() {
    // '.' and '/' allowed inside values: rational setting values like
    // `settle: 0.7` / `settle: 1/2` (TODO_0265 Phase 4).
...
export { parseDecls, _findSep, _subHashes as subHashes };
export default { parseDecls, _findSep, subHashes: _subHashes };
