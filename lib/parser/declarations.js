...
function parseDeclarations(source, parseExpr, opts = {}) {
  ...
  while (pos < source.length) {
    ...
    if (source[pos] === '#') {
      ...
      // Parse optional settings: (rules: [label, ...]) or (rules: module_name)
      // Disambiguator: only parse as settings if we see (rules: — not a normal expression
      let settings = null;
      if (pos < source.length && source[pos] === '('
          && /^\(\s*rules\s*:/.test(source.slice(pos))) {
    ...
    while (pos < source.length && source[pos] !== ')') {
      ...
      if (pos < source.length && source[pos] === '[') {
        ...
        while (pos < source.length && source[pos] !== ']') {
          ...
          values.push(readIdentifier());
        }
        if (pos < source.length) pos++; // skip ]
        settings[key] = values;
      } else {
        // Single identifier value
        settings[key] = readIdentifier();
      }
      skipWs();
