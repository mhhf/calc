...
function parseDeclarations(source, parseExpr, opts = {}) {
  ...
  while (pos < source.length) {
    ...
    if (source.slice(pos, pos + 8) === '#import(') {
      ...
      if (end === -1) throw new Error(`Unterminated #import at position ${pos}`);
    ...
    if (pos >= source.length || source[pos] !== ':') {
      throw new Error(`Expected ':' after declaration name '${name}' at position ${pos}`);
    }
    pos++; // skip :
    skipWs();
  ...
  function findDeclEnd() {
    ...
    throw new Error(`Unterminated declaration starting at position ${pos}`);
  }
