 ...
 *
 * Datasort conditioning (TODO_0011 fence B, THY_0030): waves may live
 * at datasort states (binder `exists X: even @w.`) or be restricted
 * dynamically by `within(e, S)` facts — states combine into canonical
 * product keys, restriction never rescales surviving masses (B4), and
 * the conditioned sampler draws mass-proportionally using the
 * calculus-bound inside-mass solver (calc._datasortMass).
 */

'use strict';

import Store from '../kernel/store.js';
import { STATE_ZONES } from '../engine/fact-set.js';
...
import { mix32, strHash, sampleIndex } from '../engine/prf.js';
import { ratParts } from '../kernel/rat-term.js';
import { _parseSignature } from '../engine/type-check.js';

// Reserved names live in lib/engine/reserved-preds.js (RES_0143 L12) —
// the loader's convert fence shares them without importing the driver;
// re-exported below unchanged.
import { DECIMATE_PREDS } from '../engine/reserved-preds.js';
const BIAS_PRED = DECIMATE_PREDS.BIAS;
const WITHIN_PRED = DECIMATE_PREDS.WITHIN;
const ONE = [1n, 1n];
const ZERO = [0n, 1n];
...
function _posterior(calc, state, e, sort) {
  ...
  if (typeof calc.proveAll === 'function') {
    ...
    for (const m of members) {
      ...
      let sols = [];
      // MAX+1 so the guard is LIVE: proveAll capped at MAX can never
      // exceed MAX, which silently truncated 256+ derivations (audit
      // 2026-09-02); one extra solution proves the overflow.
      try { sols = calc.proveAll([goal], { maxSolutions: MAX + 1 }); } catch { /* no bias clauses — fine */ }
      if (sols.length > MAX) {
...
function collapse(calc, initialState, opts = {}) {
  ...
  if (mode === 'exact' || mode === 'solve') {
    ...
    const goWaves = (state, waveMap, skolemSet, mass, depth) => {
      ...
      const { members, weights, drawWeights, childStates } = best.p;
      // solve explores highest DRAW weight first (ρ·bias·Πm(children) —
      // the probability-proportional order; a high mass FACTOR can lead
      // into a low-mass child language). Recorded masses stay weights[i].
      const order = members.map((m, i) => [m, weights[i], drawWeights[i]])
        .filter(([, w]) => w[0] > 0n);
      if (mode === 'solve') order.sort((a, b) => -ratCmp(a[2], b[2]) || (a[0] < b[0] ? -1 : 1));
...
export {
  ...
  collapseView, collapseDraw, DECIMATE_PREDS,
