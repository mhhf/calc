...
import { theoryClassTags } from '../kernel/unify.js';
...
// eq-theory-equal value classes: cross-tag matches inside a registered
// theory's value class are over-approximated to "unifiable" — sound
// (more edges → more refusals). The classes come from the REGISTERED
// theories (kernel theoryClassTags()) — a calculus without a numeric
// theory has no numeric class; no constructor names live here
// (RES_0143 L4: this used to hardcode ILL's binlit/i/o/e).
...
function unifiable(a, b) {
  ...
  const cls = theoryClassTags();
  if (cls.has(ta) || cls.has(tb)) return cls.has(ta) && cls.has(tb);
