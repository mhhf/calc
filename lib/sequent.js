...
class Sequent {
  ...
  toString(o = {}) {
    ...
    return `${prStr} ${vdash} ${this.succedent.toString(o)}`
  }

  // todo - refactor everything to focusID
  ffocus() {
    // this.focus = this.antecedent[id];
    let focusId = Object.keys(this.antecedent)
    .find(id => {
      return this.antecedent[id].val.ruleConstructor === "Structure_Focused_Term_Formula"
    });
...
Sequent.copy = function (seq) {
  // copy antecedent
  let antecedent = {};
  Object.keys(seq.antecedent)
  .forEach(id => {
    let r = seq.antecedent[id];
    antecedent[id] = {num: r.num, val: r.val.copy()};
  });
  // copy succedent
...
Sequent.add_to_antecedent = function (seq, delta) {
  Object.keys(delta)
  .forEach(id => {
    let r = delta[id]
    if(id in seq.antecedent) {
      seq.antecedent[id].num += r.num;
    } else {
      seq.antecedent[id] = {num: r.num, val: r.val}
    }
  })
}
...
Sequent.remove_from_antecedent = function (seq, delta) {
  ...
  Object.keys(seq.antecedent)
  .forEach(id => {
    let r1 = seq.antecedent[id];
    let r2 = delta[id];
    if(r2 && r1.num < r2.num) {
      antecedent_ = false;
    } else if(antecedent_ && r2 && r1.num > r2.num) {
      antecedent_[id] = {num: r1.num - r2.num, val: r1.val}
    } else if(antecedent_ && !r2) {
      antecedent_[id] = r1;
    }
  })
  if(antecedent_) {
