...
Proofstate.auto = function(pt, o) {
  ...
  // TODO - soooo hacky - make rules abstraction
  if("rules" in o && !custom_rules_added) {
    o.rules.forEach((r, i) => {
      calc.rules.RuleNat["custom_"+i] = r;
      rules["custom_"+i] = r;
    })
    custom_rules_added = true;
  }
  ...
  if(invertableId) {
    ...
    debug.technique += `bwd`;
    let fullname = Object.keys(calc.rules.RuleNat).find(ruleName => {
      let rule = calc.rules.RuleNat[ruleName];
      rule = rule
        .filter(f => f !== "")
    ...
    actions.push(() => Proofstate.apply(pt, fullname, "R"))
