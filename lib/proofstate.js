...
Proofstate.applyR = function (pt, rule) {
  ...
  pt.delta_in = pt.conclusion.linear_ctx;
  return true;
}
...
Proofstate.applyL = function (pt, rule, id) {
  ...
  let theta = compare(conclusionConnective, pt.conclusion.linear_ctx[id].val)
  ...
  // todo - Sequent. remove from linear_ctx function?
  Object.keys(pt.conclusion.linear_ctx)
  .forEach(id_ => {
    let r = pt.conclusion.linear_ctx[id_].val;
    let isNotStructure = r.ruleConstructor !== "Structure_Freevar";
    let isNotActive = id_ !== id;
    if(isNotStructure && isNotActive) {
      pt.delta_in[id_] = {
        val: r,
        num: pt.conclusion.linear_ctx[id].num
      }
    }
  })
  return true;
...
Proofstate.invertR = function (pt, id) {
...
Proofstate.invertL = function (pt, id) {
  let formula = pt.conclusion.linear_ctx[id].val.vals[1];
  let name = formula.ruleConstructor;
  let ruleName = name.match(/(Formula_(.*))/)[2];
  let fullname = ruleName + "_L";
  let rule = rules[fullname]
  console.log(`invert L: ${fullname}`);
  ...
  console.log(`\u03B8: ${theta.map(([k, v]) => `${k} -> ${v}`).join(", ")}`);
