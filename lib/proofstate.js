...
Proofstate.auto = function(pt, o) {
  ...
  if(invertableId) {
    ...
    debug.technique += `invert.${fullname}`;

    actions.push(() => Proofstate.apply(pt, fullname, invertableId, o))
    ...
    if(doBlur) {
      ...
      debug.technique += `.apply.${fullname}`;

      let id = pos === "R"
        ? "R"
        : pt.conclusion.focusId;

      actions = Proofstate.getPotRules(pt, id, o).map(fullname => () => Proofstate.apply(pt, fullname, id, o))
    } else { // Atom in focus
      let isRight = pos === "R";
      let isLeft = pos === "L";
    ...
    debug.technique += `bwd`;
    let fullname = Object.keys(o.bwd).find(ruleName => {
      let rule = o.bwd[ruleName];
      rule = rule
        .filter(f => f !== "")
        .map(f => parser.parse(f))
        .map(f => Sequent.fromTree(f))

      theta = Sequent.compare(pt.conclusion, rule[0], {debug:true});
      return !!theta;
    });
    debug.technique += "." + fullname;
    actions.push(() => Proofstate.apply(pt, fullname, "R", o))
    ...
    debug.technique += filter.map(f => f.slice(0, 4)).join("|")
  ...
  debug.theta = theta.map(([k, v]) => `${k} -> ${v}`).join(", ");
  return {
    delta_out,
    theta,
    success,
...
Proofstate.getPotRules = function (pt, id, o) {
  ...
  let pot_rules = (fullname+"2" in o.rules)
    ? [fullname, fullname+"2"]
    : [fullname]
...
Proofstate.apply = function (pt, fullname, id, o) {
  let formula = id === "R"
    ? pt.conclusion.succedent.vals[1]
    : pt.conclusion.linear_ctx[id].val.vals[1]
  let name = formula.ruleConstructor;
  let rule = o.rules[fullname];
