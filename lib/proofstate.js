...
Proofstate.getInvertableRule = function (pt, o) {
  ...
  // Helper to check if a formula is atomic (not a complex connective)
  ...
  let succNegative =
    ...
    && succedent.vals[1]
    && !isAtomicFormula(succedent.vals[1])
  ...
  .find(id => {
    ...
    if (!val.isTermType()) return false;
    let formula = val.vals[1];
    if (isAtomicFormula(formula)) return false;  // Skip atomic formulas
    return formula.isPositive(o);
  })
