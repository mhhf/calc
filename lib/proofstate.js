 ...
 *
 * Uses global content-addressed store for:
 * - O(1) term equality in unification (mgu)
 * - O(1) sequent equality
 * - Structural sharing of subterms
 */
...
Proofstate.auto = function(pt, o) {
  profiler.count('proofstate.auto');
  const endTime = profiler.time('proofstate.auto');
 ...
 *
 * @param {PT} pt - Proof tree node
 * @param {string} rule_name - Name of rule to apply
 * @param {string} id - Position to apply ('R' or context id)
 * @param {Array} rule - Rule sequents
 */
 ...
 *
 * @param {PT} pt - Proof tree node
 * @param {ProofSearchState} state - Current prover state
 */
Proofstate.tryIdPos = function (pt, state) {
  profiler.count('proofstate.tryIdPos');
...
Proofstate.tryIdNeg = function (pt, state) {
  profiler.count('proofstate.tryIdNeg');
  let focusId = state.focusId;
