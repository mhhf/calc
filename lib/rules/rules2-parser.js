...
function _ruleBlock(block, parseFormula, formulaVars) {
  ...
  if (!principal) {
    ...
    // @fireStep true — a timed firing step (@fire, THY_0018 §5): the node
    // is checked by the kernel's fire checker against the program's rule
    // data, never applied in tag-driven search (no principal, keyed by name).
    if (annotations.fireStep === true) descriptor.fireStep = true;
    return {
      name, descriptor,
