...
function _compileTemplate({ name, principal, side, succHash, premiseSeqs,
                            theoryLines, parseFormula, formulaVars, companionEntries = [] }) {
  const bound = new Set();
  ...
  // Companion formulas (TODO_0309): SAX-style exact axioms list further
  // antecedent formulas beside the principal (`A, B |- A * B`, `A, A -o
  // B |- B`) — consumed from the pool at apply time. Zero-premise only
  // (fenced below); every companion variable must be bound by the
  // conclusion match.
  const companions = companionEntries.map(entry => {
    const h = parseFormula(entry);
    const vars = new Set();
    _collectVars(h, vars);
    for (const v of vars) {
      if (!bound.has(v)) {
        throw new Error(`rule '${name}': companion formula variable '${Store.child(v, 0)}' is unbound (not in the principal or conclusion succedent)`);
      }
    }
    return h;
  });
  if (companions.length && premiseSeqs.length > 0) {
    throw new Error(`rule '${name}': companion formulas are only supported on zero-premise (axiom) rules`);
  }
  ...
  const t = { principal, succedent: succPattern, theoryGoals, premises };
  if (companions.length) t.companions = companions;
  return t;
}
...
function _ruleBlock(block, parseFormula, formulaVars) {
  ...
  let principalEntryIdx = -1;
  if (!principal) {
    for (let ei = 0; ei < concLinFormulas.length; ei++) {
      const h = parseFormula(concLinFormulas[ei]);
      const t = Store.tag(h);
      if (!_isRuleVar(t) && t !== 'atom') {
        principal = h;
        side = 'l';
        principalEntryIdx = ei;
        break;
      }
    }
  }
  ...
  if (!principal) {
    ...
    // Canonical-cut detection (TODO_0309): the ONE no-principal shape
    // the prover can dispatch —
    //     name: D, D' |- C  <-  D |- A  <-  D', A |- C
    // (producer premise first). Exactly one formula var new to the
    // premises (the cut formula), premise 1 concludes it, premise 2
    // consumes it and concludes the conclusion's succedent var. Any
    // other no-principal shape keeps today's behavior (loaded, never
    // dispatched by search — the @fire/@draw checker rules).
    if (premiseSeqs.length === 2 && concLinFormulas.length === 0 &&
        _isRuleVar(succTag)) {
      const [p1, p2] = premiseSeqs;
      const p1Succ = parseFormula(p1.succedent);
      const p2Succ = parseFormula(p2.succedent);
      const p1Formulas = p1.linear.filter(e => !_isCtxVar(e, formulaVars, parseFormula));
      const p2Formulas = p2.linear.filter(e => !_isCtxVar(e, formulaVars, parseFormula));
      if (_isRuleVar(Store.tag(p1Succ)) && p1Succ !== succHash &&
          p1Formulas.length === 0 &&
          p2Succ === succHash &&
          p2Formulas.length === 1 && parseFormula(p2Formulas[0]) === p1Succ) {
        descriptor.cut = true;
      }
    }
    return {
  ...
  // Exact axioms (TODO_0309): a zero-premise rule with a principal and
  // NO context vars is a SAX-style axiom — its conclusion is matched by
  // unification alone. Compiled as a template because the index flavor
  // can express neither companion consumption (`A, B |- A * B`) nor the
  // left-axiom succedent pattern (`A & B |- A` must CHECK the goal is
  // the component). zero_l-style discarding axioms carry a context var
  // and keep the index path.
  const isExactAxiom = premiseSeqs.length === 0 &&
    concLinContextVars.length === 0 && concCartContextVars.length === 0 &&
    concCartFormulas.length === 0 &&
    (side === 'l' || concLinFormulas.length > 0);

  // Template detection (D1): theory premises, an explicit @template true
  // (ω rules whose grade restriction lives in the pattern), a compound
  // premise formula (inexpressible as a child index), or an exact axiom.
  ...
  let descriptor;
  if (isTemplate) {
    if (side === 'l' && concLinFormulas.length !== 1 && !isExactAxiom) {
    ...
    // Companion entries: the conclusion's antecedent formulas beside the
    // principal (left axioms) or all of them (right axioms — the
    // principal is the succedent).
    const companionEntries = isExactAxiom
      ? (side === 'l'
        ? concLinFormulas.filter((_, i) => i !== principalEntryIdx)
        : concLinFormulas)
      : [];
    descriptor = {
      connective, side, arity,
  ...
  if (annotations.binding) {
    ...
    if (descriptor.template) {
      ...
      const walk = (h) => {
        ...
        const tag = Store.tag(h);
        if (Store.BINDER_TAGS.has(Store.tagId(h)) && _isRuleVar(Store.tag(Store.child(h, 0)))) {
      ...
      if (bodyVar === null) {
        throw new Error(`rule '${name}': @binding on a template rule needs a binder body variable (a binder connective — exists/forall/mu/nu — over a metavar) in the principal or conclusion-succedent pattern`);
      }
      t.bodyVar = bodyVar;
      if (parts[0] === 'witness') {
        if (parts.length !== 2) {
  ...
  // @tick true — the whole-context "next" tick (TODO_0203, THY_0043): a right
  // rule on a ○-succedent (`G ; ○Δ |- ○C <- G ; Δ |- C`) that advances the WHOLE
  // linear context one step, stripping one ○ from every linear formula. It only
  // fires when EVERY consumable formula is ○-wrapped (a non-○ linear resource
  // cannot advance — this is what keeps ○ non-collapsing), so its premise/leftover
  // computation is a whole-sequent transform, bypassed here and handled in
  // focused.js + kernel.js (the connective is read from the descriptor, so lib/
  // stays calculus-agnostic). Generalizes the empty-context base case (promotion
  // shape) to a non-empty all-○ context.
  if (annotations.tick === true) descriptor.tick = true;
