...
/** Whether guard-region rep `rv` (a BigInt over ℕ≥0) satisfies guard `g`. */
    ...
    for (const V of params) {
      ...
      if (undecidable) {
        warnings.push(
          ...
          `a non-constant (or via an uncertified predicate) — coverage is ` +
          `undecidable here (V2; §6.3).`);
        continue;
      }

      // Representative integers over ℕ≥0: 0 and each constant ± 1 (clamped).
      const reps = new Set([0n]);
      for (const gs of perAlt) for (const g of gs) {
        for (const d of [-1n, 0n, 1n]) { const x = g.c + d; if (x >= 0n) reps.add(x); }
      }
      let flagged = false;
