 ...
 *              Π drawn weights, importance = Π posterior totals.
 *              E[importance] = total mass PER ATTEMPT (failures count as
 *              zero) — the returned first-success value is conditioned
 *              on success; use `attempts` to Horvitz–Thompson correct
 *              when contradictions occur (THY_0026 §8 T3 caveat).
 *   'exact'  — enumerate the collapse tree; outcomes carry exact
 *              unnormalized masses (normalization is META, D2), deduped
 *              by final state. woplus draws inside settle stay
 *              seed-resolved (settleExplore composition is future work).
 ...
 * from the STATE (forward-derived monotone conditioning) plus at most
 * one clause-derived bias per (wave, member) via the backward prover.
  ...
  for (const [k, n] of Object.entries(state.linear || {})) {
    ...
    if (t === 'superpose') {
      ...
      if (!sorts || !sorts.isClassifier(sortName)) {
        throw new Error(`decimate: superposed sort '${sortName}' is not a classifier`);
      }
    } else if (t === 'exists') {
      ex = inner;
    } else {
...
function _witness(calc, m) {
  ...
  const argWaves = [];
  const args = sig.argSorts.map((s) => {
    if (!calc.sorts.isClassifier(s)) {
      throw new Error(`decimate: constructor member '${m}' has a non-classifier argument sort '${s}' — its argument cannot superpose`);
    }
    const e2 = freshEvar();
    argWaves.push([e2, s]);
    return e2;
 ...
 */
function _settleOpen(calc, state, horizon, sOpts, env, waveMap, skolemSet, trace = null) {
  for (;;) {
...
function collapse(calc, initialState, opts = {}) {
  ...
  if (mode === 'sample') {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      for (let step = 0; step < maxCollapses; step++) {
        state = _settleOpen(calc, state, horizon, sOpts, env, waveMap, skolemSet, trace);
        const waves = _waves(state, waveMap);
        if (waves.length === 0) {
          return { state, ground: true, skolems: skolemSet.size, attempts: attempt, mass, importance, collapses,
            ...(trace ? { trace } : {}) };
        }
        ...
        if (trace) {
          trace.push({ draw: { evar: best.w.e, sort: best.w.sort, member: members[idx], witness: wit.hash } });
        }
  ...
  if (mode === 'exact' || mode === 'solve') {
    ...
    let found = null;
    let truncated = false;
    const go = (state, waveMap, skolemSet, mass, depth) => {
      if (found) return;
      state = _settleOpen(calc, state, horizon, sOpts, env, waveMap, skolemSet);
      const waves = _waves(state, waveMap);
      if (waves.length === 0) {
...
export default { collapse, substituteEvar, openSuspended, splitBody };
