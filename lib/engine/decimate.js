...
function _posterior(calc, state, e, sort) {
  ...
  const isDs = calc.sorts.isDatasort && calc.sorts.isDatasort(sort);
  const dsInfo = isDs ? calc.sorts.datasortInfo(sort) : null;
  const base = isDs ? dsInfo.base : sort;
  const structured = isDs ? dsInfo.trans.size > 0 : _isStructured(calc, sort);
  // per-head child states: datasort transitions, or null (= the
  // constructor's declared arg sorts, the ⊤ reading)
  const childStates = isDs && dsInfo.trans.size > 0
    ? new Map(dsInfo.trans) : null;
  let members = isDs
    ? [...dsInfo.members, ...dsInfo.trans.keys()].sort()
    : [...calc.sorts.membersOf(sort)].sort();
  ...
  for (const k of Object.keys(state.persistent || {})) {
    ...
    if (structured || info.trans.size > 0) {
      throw new Error(`decimate: within over a recursive domain needs product states — fence B slice 3 (condition statically at the binder instead)`);
    }
    ...
    members = members.filter((m) => info.members.has(m));
  }
  ...
  if (childStates !== null) {
    if (!calc.masses) {
      throw new Error(`decimate: recursive datasort '${sort}' needs inside masses — load computed none (program without @w-armed masses?)`);
    }
  ...
  return { members, weights, total, drawWeights, drawTotal, childStates };
}
...
function _witness(calc, m, childStates = null) {
  ...
  const args = sig.argSorts.map((s, i) => {
    ...
    // STATES (datasort names) instead of the declared ⊤ arg sorts.
    const st = childStates ? childStates[i] : s;
    if (!(calc.sorts.isClassifier(st) ||
        (calc.sorts.isDatasort && calc.sorts.isDatasort(st)))) {
...
function _isStructured(calc, sort) {
  if (calc.sorts.isDatasort && calc.sorts.isDatasort(sort)) {
    return calc.sorts.datasortInfo(sort).trans.size > 0;
  }
