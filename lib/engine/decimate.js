...
function openSuspended(state, { stampTag, roles, sorts }) {
  ...
  return { state: out, opened, waves, skolems };
}
 ...
 */
function _settleOpen(calc, state, horizon, sOpts, env, waveMap, skolemSet) {
  for (;;) {
    const res = calc.settle(state, horizon, sOpts);
    if (!res.quiescent) {
      throw new Error('decimate: settle did not quiesce (raise settle maxSteps)');
    }
    const { state: opened, opened: n, waves, skolems } = openSuspended(res.state, env);
    state = opened;
...
export { collapse, substituteEvar, openSuspended, splitBody };
