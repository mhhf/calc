...
import { tryTimedMatch } from './timed.js';
import { timedSubset } from './timed-views.js';
...
function traceLines(events) {
  return events.map(e => {
    ...
    return `[${fmtStamp(e.activation)}] ${e.rule}: ${lhs} → ${rhs}${d}${alt}`;
  });
}
...
function timelineLines(events, initialState, T) {
  ...
  lines.push('  ' + (events.map(e => `${e.rule} [${fmtStamp(e.activation)}→${fmtStamp(e.done)}]`).join('  ') || '(none)'));
  ...
  for (const e of events) {
    ...
    for (const [hStr, c] of Object.entries(e.consumed || {})) {
      ...
      for (let i = 0; i < c; i++) arr.push({ by: e.rule, at: e.activation });
      deaths.set(h, arr);
    }
  }
...
function whyLines(events, initialState, factHash) {
  ...
  const producersOf = (h, bound) => {
    ...
    for (let i = 0; i < n; i++) {
      const c = events[i].produced && events[i].produced[h];
      for (let k = 0; k < (c || 0); k++) out.push(i);
    }
    return out;
  };
...
function whyNotLines(rule, state, opts) {
  ...
  if (m) {
    if (horizon !== undefined && tcfg.availability.cmp(m.activation, horizon) > 0) {
      lines.push(`  pending: fires at activation ${fmtStamp(m.activation)} > horizon ${fmtStamp(horizon)}`);
    } else {
      lines.push(`  it CAN fire — activation ${fmtStamp(m.activation)}, consuming ${_facts(m.consumed).join(', ')}`);
    }
    return lines;
  }
  if (diag.window) {
...
export { fmtStamp, fmtFact, traceLines, timelineLines, whyLines, whyNotLines, timedSubset };
export default { fmtStamp, fmtFact, traceLines, timelineLines, whyLines, whyNotLines, timedSubset };
