...
function composeSub(thetaMap, theta2, canonicalize) {
  ...
  // Domain set for O(1) overlap check below — lets us skip apply() when a
  // thetaMap value doesn't reference any theta2 metavar (common case).
  const theta2Domain = new Set();
  for (let i = 0; i < theta2.length; i++) theta2Domain.add(theta2[i][0]);

  const newMap = new Map();
  for (const [mv, val] of thetaMap) {
    if (isGround(val) || !hasMetavarInDomain(val, theta2Domain)) {
      newMap.set(mv, val);
    } else {
...
function resolve(goals, clauses, definitions, opts = {}) {
  ...
  function search(remainingGoals, thetaMap, depth) {
    ...
    if (ffiResult) {
      // FFI returned an authoritative answer (null = mode_mismatch fall-through,
      // handled in ffiDirect). success=true → commit bindings; success=false →
      // definitive failure (e.g. lt(A,A) returns false) — prune this branch
      // rather than re-proving via backchain.
...
export { resolve, _newProf as newProf };
export default { resolve, newProf: _newProf };
