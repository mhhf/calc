...
/**
 * Store a rational in canonical form: gcd-reduced, den > 0;
 * integers (den = 1, num ≥ 0) collapse to binlit.
 * @param {bigint} n @param {bigint} d
 * @returns {number} term hash
 */
function putRat(n, d) {
  const [n2, d2] = rat.norm(n, d);
...
const ratlitTheory = {
   ...
   *
   * Also folds the numeral constructors over rational leaves:
   * o(x) ≡ 2x and i(x) ≡ 2x + 1 hold over ℚ, not just ℕ — bin.ill's digit
   * algorithms are semiring-generic, so clause resolution of overloaded
   * predicates (e.g. mul (o X) Y Z with rational Y) legitimately derives
   * o/i-wrapped rationals. Folding them here makes every such derivation
   * canonically equal to the FFI result (confluence modulo the theory).
   */
  canonicalize(h) {
 ...
 * into the 'rat' clause bucket). Idempotent. Theory COMPOSITION stays
 * per-caller (calculusConfig.theories / backchain opts.theories) — only the
 * classification registry is global, like the tag registry itself.
 */
