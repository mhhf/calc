...
/**
 * Decode both inputs as rationals (bins coerce to n/1); null if either fails.
 * NEGATIVE numerators are rejected: the v1 contract is ℚ≥0 (rat(N, D) ranges
 * over bin = ℕ; ratlitTheory.rewrite refuses n < 0), so the clause path can
 * neither represent nor derive them. Accepting them here would let the FFI
 * succeed where clause resolution fails — violating the FFI principle
 * (audit round 11). Signed STORAGE stays (D14: no migration later); signed
 * SEMANTICS arrive with debt/dual grades.
 */
function _decode2(args) {
  const a = ratParts(args[0]);
  if (a === null || a[0] < 0n) return null;
  const b = ratParts(args[1]);
  if (b === null || b[0] < 0n) return null;
  return [a, b];
}
...
export { qplus, qsub, qmul, qdiv, qlt, qle, qeq, qneq, qeq_bool };
export default { qplus, qsub, qmul, qdiv, qlt, qle, qeq, qneq, qeq_bool };
