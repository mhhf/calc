...
/** DEFAULT unit grade of the graded lax monad (TODO_0157 D6 merge-back):
 *  binlit 0, the canonical putRat(0,1) form — ILL's `{A}` is
 *  monad(monadUnit(), A), hash-identical to till's zero-delay `{A}`.
 *  This is the DEFAULT, not the definition: callers override per-calculus via
 *  `opts.gradeUnit` (compileRule: `opts.gradeUnit || monadUnit`), sourced from
 *  `cc.gradeUnit` (ILL: monadUnit; till: tillGradeUnit). Recomputed on demand
 *  (captured Store IDs go stale across reindexing — see the module comment). */
export const monadUnit = () => Store.put('binlit', [0n]);

/**
 * Default grade configuration (ILL's {0, 1, ω} instance).
 *
 * The shape every grade-classifying site reads (TODO_0265 Phase 2):
 *   { grade0: () => hash, gradeOmega: () => hash }
 * Entries are FUNCTIONS, not captured hashes — the recompute-on-demand
 * invariant above applies to any calculus's grade atoms. A calculus with a
 * different grade algebra supplies its own record via calculusConfig;
 * absence means this default (bit-identical to pre-config behavior).
 */
export const defaultGradeConfig = { grade0, gradeOmega: gradeW };

export default { grade0, gradeW, defaultGradeConfig };
