...
function buildSortSystem({ definitions, clauses, sortVarsTable, calc, lit, connArgSorts, formulaSort }) {
  ...
  if (progEdges.length > 0 && !definitions.has(SORT_PREDS.EDGE)) {
    throw new Error(`subsort declarations need the sorts prelude (declares '${SORT_PREDS.EDGE}'/'${SORT_PREDS.SUB}') — import it (calculus/till/prelude/sorts.till)`);
  }
  ...
  // ── Closure (the compiled index over the sedge facts) ──
