...
function buildSortSystem({ definitions, clauses, sortVarsTable, calc, lit, connArgSorts, formulaSort }) {
  ...
  for (const [a, b] of progEdges) {
    if (!declaredAsSort(a) && declaredAsSort(b)) {
      ...
      datasorts.set(a, { base: b, members: new Set() });
      continue;
    }
    for (const op of [a, b]) {
  ...
  // Membership clauses: head = the datasort applied to ONE member of the
  // base (f1); no premises on nullary-member clauses (f2 — premises only
  // classify subterm variables, and atomic members have none); at most
  // one clause per (datasort, head) (f3 — membership tolerates overlap,
  // MASS does not). Constructor members are fence B2 (linear-recursive
  // masses) — refused here with a pointer.
  if (datasorts.size > 0) {
    const checkHead = (cname, ds, dname, hash, nPremises) => {
      if (Store.arity(hash) !== 1) {
        errors.push(`'${cname}': datasort membership clause must be unary ('${dname} <member>')`);
        return;
      }
      const arg = Store.child(hash, 0);
      if (Store.tag(arg) !== 'atom') {
        errors.push(`'${cname}': datasort head must be a nullary member of '${ds.base}' — constructor patterns are fence B2 (linear-recursive datasorts, not yet landed)`);
        return;
      }
    ...
    for (const [cname, hash] of definitions) {
      ...
      const hit = dsOf(hash);
      if (hit) checkHead(cname, hit.ds, hit.dname, hash, 0);
    }
    for (const [cname, c] of clauses) {
      const hit = dsOf(c.hash);
      if (hit) checkHead(cname, hit.ds, hit.dname, c.hash, (c.premises || []).length);
    }
    for (const [name, ds] of datasorts) {
      if (ds.members.size === 0) {
        errors.push(`datasort '${name}' has no membership clauses — an empty event is a load error (define members: '${name}/x: ${name} <member>.')`);
      }
    }
  }
  if (errors.length > 0) throw new Error(`Sort system: ${errors.join('; ')}`);
