...
import { lintProductivity } from './timed-lint.js';
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (sortSystem && definitions.has(SORT_PREDS.SUB)) {
    for (const [a, b] of sortSystem.closurePairs()) {
      clauses.set(`${SORT_PREDS.SUB}/${a}_${b}`, {
 ...
 */
function _snapToFile(cachePath, definitions, clauses, rawForwardRules, _compiledRules, queries, argNamesTable, sortVarsTable) {
  const metadata = {
    types: Object.fromEntries(definitions),
    clauses: Object.fromEntries(
      Array.from(clauses.entries()).map(([k, v]) => [k, v])
...
function _loadCached(topFilePath, sdkCachePath, sdkPaths, cc = _defaultCalcConfig) {
  ...
  return { definitions, clauses, rawForwardRules, compiledRules: allCompiled,
    queries, argNamesTable, sortVarsTable, querySettings, splitQueries, moduleDecls };
}
...
function _loadFresh(filePath, extraOpts) {
  ...
  const { definitions, clauses, forwardRules, queries, argNamesTable,
    sortVarsTable, querySettings, splitQueries, moduleDecls, importTree } =
  ...
  return _buildCalc(definitions, clauses, forwardRules, queries, {
    argNamesTable, sortVarsTable, rootLabel, labelDeps: ld, querySettings, splitQueries, moduleDecls,
    ...extraOpts
  });
}
...
function precompile(filePaths, cachePath, opts = {}) {
  Store.clear();
  const cc = opts.calculusConfig || _defaultCalcConfig;
  const { definitions, clauses, forwardRules, queries, argNamesTable, sortVarsTable } =
  ...
  const byteSize = _snapToFile(cachePath, definitions, clauses, forwardRules, compiledRules, queries, argNamesTable, sortVarsTable);
  return { definitions, clauses, forwardRules, queries, argNamesTable, sortVarsTable, byteSize };
}
...
function loadPrecompiled(cachePath, sellOpts) {
  ...
  const opts = { argNamesTable, sortVarsTable, ...(sellOpts || {}) };
