...
//
// Cache key: hashCombine(file tree hashes, bytecode hash, compose opts, version).
 ...
 */
function _saveComposeSnapshot(cachePath, calc, definitions, clauses, queries,
 ...
 */
function _loadComposeSnapshot(cachePath, sellOpts) {
  try {
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (!opts.skipCompose) {
    ...
    if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
      ...
      if (opts.residualResolver) {
        ...
        const { residualResolver: defaultResolver } = require('./ill/residual-resolver');
        composeOpts.residualResolver = defaultResolver;
      }
      // ILL defaults for compose pipeline — domain-specific knowledge injected here
      const { ILL_CHAIN_CONFIGS, ILL_SROA_CONFIG } = require('./ill/compose-config');
      ...
      composeOpts.chainFusionPredicates = opts.chainFusionPredicates || ILL_CHAIN_CONFIGS;
      composeOpts.linearFusionPredicate = opts.linearFusionPredicate || 'pc';
      composeOpts.sroaConfig = opts.sroaConfig || ILL_SROA_CONFIG;
      composeOpts.canonicalize = binlitTheory.canonicalize;
      const composeResult = composeGrade0(compiledRules, connectives, _illGetModeMeta, clauses, definitions, opts.extraGrade0Facts || null, opts.scopeGuard || null, composeOpts);
  ...
  const { buildClauseDispatch } = require('./opt/compiled-clauses');
  const { buildTheoryLookup, defaultTheories: _defaultTh } = require('../kernel/eq-theory');
  const { binlitTheory: _blTh } = require('./ill/binlit-theory');
  const { ffiParsedModes: _ffiPM } = require('./opt/ffi');
  const theoryLookup = buildTheoryLookup([..._defaultTh, _blTh]);
  const clauseDispatch = buildClauseDispatch(backchainIndex, _ffiPM);

  // Capture calc context for backward proving + engine dispatch
  const calcContext = { definitions, clauses, backchainIndex, connectives, clauseDispatch, theoryLookup };

  // Generate persistent step closures (after calcContext — compiled clause specs need dispatch table)
  for (const rule of compiledRules) compilePersistentSteps(rule, calcContext);

  // Compile existential chains (direct FFI for ∃-goals, bypasses provePersistent)
  const { compileExistentialChain } = require('./opt/existential-compile');
  for (const rule of compiledRules) {
    const chain = compileExistentialChain(rule);
...
function load(filePath, opts = {}) {
  ...
  if (composeDiskCache && !Array.isArray(filePath)) {
    ...
    const composeCacheKey = _composeDiskCacheKey(treeHashes, absPathForCache, extraGrade0Facts, fuseBasicBlocks);
    ...
    const cached = _loadComposeSnapshot(composeCachePath, { rootLabel, labelDeps });
...
function bytecodeToTrie(state) {
  const { arrToTrie } = require('./ill/ffi/array');
  const bcTagId = Store.TAG['bytecode'];
  if (bcTagId === undefined) return state;
