...
function _loadWithCachedImports(topFilePath, sdkCachePath, sdkPaths) {
  ...
  convert.loadFile(topFilePath, definitions, clauses, rawForwardRules, queries, {
    alreadyImported: new Set(sdkPaths), argNamesTable
  });
  ...
  return { definitions, clauses, rawForwardRules, compiledRules: allCompiled, queries, argNamesTable };
}
...
function _loadFresh(filePath) {
  const { definitions, clauses, forwardRules, queries, argNamesTable } = convert.load(filePath);
  return _buildCalc(definitions, clauses, forwardRules, queries, { argNamesTable });
}
      ...
      try {
        if (fs.existsSync(importsCachePath)) {
          ...
          return _buildCalc(result.definitions, result.clauses, result.rawForwardRules, result.queries, {
            compiledRules: result.compiledRules, argNamesTable: result.argNamesTable
          });
        }
      } catch (e) {
