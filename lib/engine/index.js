...
import { lintProductivity, lintChainCollapse } from './timed/timed-lint.js';
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (cc.grades) {
    ...
    const _advice = _lint.length ? [] : lintChainCollapse(compiledRules.filter(r => !r.hasGrade0), _tcfg);
