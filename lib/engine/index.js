...
const { cacheFlagFingerprint } = require('./cache-flags');
const { ensureVersionTag, lruEvict, DEFAULT_MAX_BYTES } = require('./cache-evict');
  ...
  if (!opts.skipCompose) {
    ...
    if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
      ...
      if (cc.ffi) {
        ...
        composeOpts.ffiDirect = require('./opt/ffi').ffiDirect;
      }
...
function load(filePath, opts = {}) {
  ...
  if (opts.bytecode) {
    ...
    const { loadBytecode, bytecodeArrGetGuard } = require('./ill/bytecode-loader');
  ...
  const composeOpts = extraGrade0Facts || scopeGuard || residualResolver || fuseBasicBlocks
      || chainFusionPredicates || linearFusionPredicate || sroaConfig || composeDiskCache || fusionBarriers || onPhase
    ? { extraGrade0Facts, scopeGuard, residualResolver, fuseBasicBlocks,
        chainFusionPredicates, linearFusionPredicate, sroaConfig,
        composeDiskCache, fusionBarriers, onPhase } : undefined;
