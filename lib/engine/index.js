...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  const { compileExChain } = require('./opt/existential-compile');
  ...
  return {
    ...
    exec: (state, execOpts = {}) => {
      ...
      return forward.run(state, rules, { ...execOpts, calc: calcContext, engine: eng });
    },

    // Exhaustive exploration (same filtering as exec) (T16)
    explore: (state, execOpts = {}) => {
      const rules = filterRules(execOpts);
      return explore.explore(state, rules, { ...execOpts, calc: calcContext, engine: eng });
    },
