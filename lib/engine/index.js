...
import os from 'os';
import _defaultCalcConfig from '../../calculus/ill/calculus-config.js';
...
function _getBytecodeLoader() {
  if (!_bytecodeLoader) _bytecodeLoader = _require('../../calculus/ill/lib/bytecode-loader.js');
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (cc.grades) {
    ...
    const _advice = [
      ...
      ...lintHypothesisS(_runtimeRules, _tcfg, { ..._rc, lintExempt: cc.lintExempt }),
...
function normalizeQuery(hash) {
  if (!_bcNorm) _bcNorm = _require('../../calculus/ill/lib/bytecode-normalize.js').default;
