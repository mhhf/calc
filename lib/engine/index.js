...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  // (cc.datasortMasses — will's config binds lib/engine/datasort-mass.js);
  ...
  if (sortSystem && sortSystem.datasorts && sortSystem.datasorts.size > 0) {
    ...
    if (stateMasses && definitions.has('mass')) {
      for (const [s, m] of stateMasses) {
        clauses.set(`mass/${s}`, {
          hash: Store.put('mass', [Store.put('atom', [s]), putRat(m[0], m[1])]),
          premises: [],
        });
      }
    }
