...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  // Skip when loading from cache (composed rules are already in the cached set).
  if (!opts.compiledRules && compiledRules.some(r => r.hasGrade0)) {
    const { composeGrade0 } = require('./compose');
    const composeResult = composeGrade0(compiledRules, connectives);
    ...
    if (composeResult.composedRules.length > 0) {
      const composedCompiled = composeResult.composedRules.map(
        r => forward.compileRule(r, { connectives, getModes: _illGetModes })
      );
