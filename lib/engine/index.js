...
import { programFromCalc as _programFromCalc } from '../prover/timed/elaborate-trace.js';
import { checkForwardTree as _checkForwardTree } from '../prover/forward-check.js';
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  const _attachers = [
    ...
    // Forward execution-tree certifier (TODO_0045, THY_0035): explore the
    // state, then MACHINE-CHECK the resulting tree — the forward twin of
    // certifyRun. explore is untrusted; forward-check re-derives every
    // step from the program rule data (θ the only witness) and threads Δ.
    // Terminal engine States are normalized to plain objects here (the
    // pure checker is fenced from the engine).
    ({ api, calc }) => ({
      certifyExplore: (state, execOpts = {}) => {
        const rules = filterRules(execOpts);
        const plainInit = state && state.linear && state.linear.group
          ? _fsToObject(state) : state;
        const tree = api.explore(state, { ...execOpts, evidence: true });
        // normalize every terminal node's engine State → plain object
        const norm = (node) => {
          if (!node) return node;
          if (node.type === 'branch') { (node.children || []).forEach(e => norm(e.child)); return node; }
          if (node.state && node.state.linear && node.state.linear.group) {
            node.state = _fsToObject(node.state);
          }
          return node;
        };
        norm(tree);
        const program = { ..._programFromCalc({ ...api, forwardRules: rules }) };
        const res = _checkForwardTree(tree,
          { program, roles: _rc, initial: plainInit, canonicalize: calc.canonicalize });
        return { ...res, tree };
      },
    }),
    ...(cc.apiExtensions || []),
