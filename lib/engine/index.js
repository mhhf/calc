...
import { serialize, deserialize, compact } from './cache/store-binary.js';
...
import { fuseBasicBlocks as _fbbPass, fuseChains as _fcPass } from './opt/compose-fuse.js';
import { sroa as _sroaPass } from './opt/compose-sroa.js';
const _fusePasses = { fuseBasicBlocks: _fbbPass, fuseChains: _fcPass, sroa: _sroaPass };
...
import { fpDetect as _fpDetect, fpLayer as _fpLayer, attachPred as _attachPred, autoLayers as _autoLayers } from './opt/fingerprint.js';
import { installAutoLayers as _installAutoLayers } from './strategy.js';
// Composition root installs the default opt-layer detection into the
// generic strategy module (RES_0143 M5): generic code never imports opt/.
_installAutoLayers(_autoLayers);
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (!opts.skipCompose) {
    ...
    if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
      ...
      // Optimization passes P5/P5.5/P6 injected from opt/ (RES_0143 M4)
      composeOpts.fusePasses = _fusePasses;
  ...
  const api = {
    ...
    explore: (state, execOpts = {}) => {
      ...
      if (_b2t && !execOpts.dangerouslyUseFFI && state.linear && !state.linear.group) {
        state = _b2t(state);
      }
  ...
  if (api.settle && sortSystem) {
    ...
    api.collapseView = (session, o = {}) =>
      decimate.collapseView(api, session, { stampTag: cc.stampTag || 'at', ...o });
    api.collapseDraw = (session, wave, o = {}) =>
      decimate.collapseDraw(api, session, wave, o);
    // certifyCI (TODO_0302 M5, THY_0031 §4): decide the separation
    // criterion on the computable cover — soundness-only; `separated`
    // certifies X ⊥ Y | Z, a refusal carries a witness walk and never
    // asserts dependence.
    api.certifyCI = (query = {}) =>
      ci.certifyCI(api, { stampTag: cc.stampTag || 'at', ...query });
  }
  return api;
}
 ...
 */
function _loadCached(topFilePath, sdkCachePath, sdkPaths, cc) {
  Store.clear();
 ...
 * Returns intermediate result (raw data for further caching).
 */
function _parseFresh(topFilePath, sdkNodes, importsCachePath, cc) {
  Store.clear();
...
function load(filePath, opts = {}) {
  ...
  if (cacheMode === true) {
    ...
    const fullHit = _tryLoadFromCache(fullCachePath,
      { rootLabel, labelDeps: ld, calculusConfig: cc });
