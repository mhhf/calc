...
const _illCompileOpts = { connectives: _defaultCalcConfig.connectives, getModes: _defaultCalcConfig.compile.getModes, discriminatorPreds: _defaultCalcConfig.compile.discriminatorPreds };
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  let compiledRules = opts.compiledRules || forwardRules.map(r => forward.compileRule(r, { connectives, getModes: cc.compile.getModes, discriminatorPreds: cc.compile.discriminatorPreds }));
  ...
  if (!opts.skipCompose) {
    ...
    if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
      ...
      if (composeResult.composedRules.length > 0) {
        const _compileOpts = { connectives, getModes: cc.compile.getModes, discriminatorPreds: cc.compile.discriminatorPreds };
