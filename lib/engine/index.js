...
const { getModes: _illGetModes, getModeMeta: _illGetModeMeta, getRange: _illGetRange } = require('./opt/ffi');
const _illCompileOpts = { connectives: ILL_CONNECTIVES, getModes: _illGetModes, getRange: _illGetRange };
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  let compiledRules = opts.compiledRules || forwardRules.map(r => forward.compileRule(r, { connectives, getModes: _illGetModes, getRange: _illGetRange }));

  // Grade-0 cut elimination: compose grade-0 rules before runtime (THY_0015)
  if (compiledRules.some(r => r.hasGrade0)) {
    const { composeGrade0 } = require('./compose');
    const composeResult = composeGrade0(compiledRules, connectives, _illGetModeMeta);
    ...
    if (composeResult.composedRules.length > 0) {
      const composedCompiled = composeResult.composedRules.map(
        r => forward.compileRule(r, { connectives, getModes: _illGetModes, getRange: _illGetRange })
      );
