...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (!opts.skipCompose) {
    ...
    if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
      ...
      if (opts.chainFusionPredicates) composeOpts.chainFusionPredicates = opts.chainFusionPredicates;
      if (opts.linearFusionPredicate) composeOpts.linearFusionPredicate = opts.linearFusionPredicate;
      if (opts.sroaConfig) composeOpts.sroaConfig = opts.sroaConfig;
...
function load(filePath, opts = {}) {
  ...
  // Default fuseBasicBlocks on when bytecode facts are provided — compose is
  // always beneficial with concrete bytecode (5x execution speedup).
