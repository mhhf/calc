...
import { checkPriors, solveMasses } from './priors.js';
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  // Inside masses (fence B slice 2): recursive datasorts need exact state
  // masses for the conditioned sampler — solved at load (f4 linearity +
  // divergence are load errors), null when no structured datasort exists.
  let stateMasses = null;
  if (sortSystem && sortSystem.datasorts && sortSystem.datasorts.size > 0) {
    const mr = solveMasses(sortSystem, priorsTable, definitions);
  ...
  const api = {
    ...
    // Inside masses (fence B slice 2): state name (classifier ⊤ or
    // datasort) → exact [n, d] mass — null unless the program declares
    // recursive datasorts (presence-gated; solved at load, priors.js).
    masses: stateMasses,
    engine: eng,
    labelIndex,
    modules,
