...
function load(filePath, opts = {}) {
  ...
  // Extract compose opts (extraGrade0Facts, scopeGuard, residualResolver, fuseBasicBlocks) to pass through
  const { extraGrade0Facts, scopeGuard, residualResolver, fuseBasicBlocks, ...restOpts } = opts;
  const composeOpts = extraGrade0Facts || scopeGuard || residualResolver || fuseBasicBlocks
    ? { extraGrade0Facts, scopeGuard, residualResolver, fuseBasicBlocks } : undefined;
