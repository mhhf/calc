...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
    ...
    const composeOpts = {};
    // Auto-enable residual resolution for bytecode specialization (resolves !inc, !plus, etc.)
...
function load(filePath, opts = {}) {
  ...
  const { extraGrade0Facts, scopeGuard, residualResolver, fuseBasicBlocks,
          chainFusionPredicates, linearFusionPredicate, sroaConfig, ...restOpts } = opts;
  const composeOpts = extraGrade0Facts || scopeGuard || residualResolver || fuseBasicBlocks
    ...
    ? { extraGrade0Facts, scopeGuard, residualResolver, fuseBasicBlocks,
        chainFusionPredicates, linearFusionPredicate, sroaConfig } : undefined;
