...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (cc.grades) {
    ...
    const _lint = timed.lintProductivity(compiledRules.filter(r => !r.hasGrade0), _tcfg);
    for (const f of _lint) {
      console.warn(f.kind === 'self-cycle'
        ? `till lint (D16): rule '${f.rule}' re-produces everything it consumes at zero delay — a Zeno self-cycle unless windows/guards break it`
        : `till lint (D16): zero-delay rule cycle ${f.via} (rules: ${f.rules.join(', ')}) — Zeno unless resources deplete`);
    }
    ...
    _timedApi = {
      ...
      observable: (state, T) => timed.observable(state, _tcfg.parseStamp(T), _tcfg),
      pending: (state, T) => timed.pending(state, _tcfg.parseStamp(T), _tcfg),
      inFlight: (events, T) => timed.inFlight(events, _tcfg.parseStamp(T), _tcfg),
      // with-projection: the host collapses an offered menu (external choice)
      choose: (state, factHash, index, chOpts = {}) =>
        timed.withProject(state, factHash, index, { ...chOpts, timedConfig: _tcfg, roles: _rc }),
      // per-alternative availability at horizon T (UI greying — pure query)
      menuStatus: (state, factHash, T, execOpts = {}) =>
        timed.menuStatus(state, factHash, filterRules(execOpts),
...
function _loadFresh(filePath, extraOpts) {
  ...
  const { definitions, clauses, forwardRules, queries, argNamesTable,
    querySettings, splitQueries, moduleDecls, importTree } =
...
function load(filePath, opts = {}) {
  ...
  const composeOpts = opts.calculusConfig || extraGrade0Facts || scopeGuard || residualResolver || fuseBasicBlocks
    ...
    ? { calculusConfig: opts.calculusConfig, extraGrade0Facts, scopeGuard, residualResolver, fuseBasicBlocks,
        chainFusionPredicates, linearFusionPredicate, sroaConfig,
        composeDiskCache, fusionBarriers, onPhase, skipSpecialize } : undefined;
...
// Only normalizeQuery is re-exported (decompose + codeToArrlit +
// bytesToSemantic); the individual passes live in ill/ (import directly).
import _bcNorm from './ill/bytecode-normalize.js';
