...
const _illCompileOpts = { connectives: _defaultCalcConfig.connectives, getModes: _defaultCalcConfig.compile.getModes, discriminatorPreds: _defaultCalcConfig.compile.discriminatorPreds, cacheEpoch: _defaultCalcConfig.compile.cacheEpoch };
...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  let compiledRules = opts.compiledRules || forwardRules.map(r => forward.compileRule(r, { connectives, getModes: cc.compile.getModes, discriminatorPreds: cc.compile.discriminatorPreds, cacheEpoch: cc.compile.cacheEpoch }));
  ...
  if (!opts.skipCompose) {
    ...
    if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
      ...
      if (composeResult.composedRules.length > 0) {
        const _compileOpts = { connectives, getModes: cc.compile.getModes, discriminatorPreds: cc.compile.discriminatorPreds, cacheEpoch: cc.compile.cacheEpoch };
...
function _loadFresh(filePath, extraOpts) {
  ...
  const { definitions, clauses, forwardRules, queries, argNamesTable,
    querySettings, splitQueries, moduleDecls, importTree } = convert.load(filePath, { onPhase });
...
function load(filePath, opts = {}) {
  ...
  if (useComposeCache) {
    ...
    const composeCacheKey = _composeCacheKey(treeHashes, absPathForCache, bytecodeHex, {
      fuseBasicBlocks, cacheVersion: opts.cacheVersion,
    });
