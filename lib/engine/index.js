...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  const { compilePSAll } = require('./opt/ffi');
  ...
  const _ffiPM = ffiContext ? ffiContext.parsedModes : require('./opt/ffi').ffiParsedModes;
  ...
  for (const rule of compiledRules) compilePSAll(rule, calcContext, ffiContext);
  ...
  // ── Composition root: resolve layer callbacks for matchOpts injection ──
  ...
  // receive a fully-configured matchOpts and never need their internal
  // call-time fallback requires — achieving the LCF kernel property that
  // inner layers are parameterized by, not dependent on, outer layers.
  const { resolveConn: _resolveConn } = require('./compile');
  ...
  function _buildMatchOpts(execOpts) {
    return _match.buildMatchOpts({
      useFFI: execOpts.dangerouslyUseFFI || false,
      ...
      backchainUseFFI: execOpts.useFFI !== undefined ? execOpts.useFFI : true,
  ...
  return {
    ...
    explore: (state, execOpts = {}) => {
      const rules = filterRules(execOpts);
      const matchOpts = execOpts.matchOpts || _buildMatchOpts(execOpts);
      return explore.explore(state, rules, { ...execOpts, calc: calcContext, engine: eng, matchOpts });
    },
 ...
 */
function _snapToFile(cachePath, definitions, clauses, rawForwardRules, compiledRules, queries, argNamesTable) {
  const metadata = {
    types: Object.fromEntries(definitions),
    clauses: Object.fromEntries(
      Array.from(clauses.entries()).map(([k, v]) => [k, v])
 ...
 * Returns intermediate result (raw data for further caching).
 */
function _parseFresh(topFilePath, sdkNodes, sdkPaths, importsCachePath) {
  Store.clear();
...
function load(filePath, opts = {}) {
  ...
  if (cacheMode === true) {
    ...
    const result = _parseFresh(absPath, sdkNodes, sdkPaths, importsCachePath);
    _snapToFile(fullCachePath, result.definitions, result.clauses,
      result.rawForwardRules, result.compiledRules, result.queries, result.argNamesTable);
    return _buildCalc(result.definitions, result.clauses, result.rawForwardRules, result.queries,
      _sellOpts(result));
...
function codeToArrlit(state) { return _bcNorm.codeToArrlit(state); }
