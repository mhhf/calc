...
function _buildCalc(types, clauses, forwardRules, queries, opts = {}) {
  ...
  // Regenerate compiled matcher closures (closures can't be serialized)
  const { generateMatcher } = require('./compile');
  for (const rule of compiledRules) generateMatcher(rule);
 ...
 */
function _snapshotToFile(cachePath, types, clauses, rawForwardRules, compiledRules, queries) {
  const metadata = {
...
function _loadWithCachedImports(topFilePath, sdkCachePath, sdkPaths) {
  ...
  const types = new Map(Object.entries(meta.types));
  ...
  convert.loadFile(topFilePath, types, clauses, rawForwardRules, queries, {
  ...
  return { types, clauses, rawForwardRules, compiledRules: allCompiled, queries };
}
...
function _parseFreshWithSdk(topFilePath, sdkNodes, sdkPaths, importsCachePath) {
  Store.clear();
  const types = new Map(), clauses = new Map(), rawForwardRules = [], queries = new Map();
  const alreadyImported = new Set();

  // Phase 1: Parse SDK imports bottom-up (topo order)
  for (const node of sdkNodes) {
    convert.loadFile(node.path, types, clauses, rawForwardRules, queries, {
      alreadyImported
    });
  ...
  if (importsCachePath && sdkNodes.length > 0) {
    _snapshotToFile(importsCachePath, types, clauses,
    ...
    if (importsCachePath) {
      try {
        if (fs.existsSync(importsCachePath)) {
          ...
          _snapshotToFile(fullCachePath, result.types, result.clauses,
            result.rawForwardRules, result.compiledRules, result.queries);
          return _buildCalc(result.types, result.clauses, result.rawForwardRules, result.queries, {
...
function precompile(filePaths, cachePath) {
  ...
  const byteSize = _snapshotToFile(cachePath, types, clauses, forwardRules, compiledRules, queries);
  return { types, clauses, forwardRules, queries, byteSize };
}
