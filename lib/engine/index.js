...
function _buildCalc(definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  if (hasGrade0Rules || hasGrade0Clauses || opts.extraGrade0Facts) {
    ...
    const composeResult = composeGrade0(compiledRules, connectives, _illGetModeMeta, clauses, definitions, opts.extraGrade0Facts || null, opts.scopeGuard || null);
...
function load(filePath, opts = {}) {
  ...
  // Extract compose opts (extraGrade0Facts, scopeGuard) to pass through
  const { extraGrade0Facts, scopeGuard, ...restOpts } = opts;
  const composeOpts = extraGrade0Facts || scopeGuard
    ? { extraGrade0Facts, scopeGuard } : undefined;
