 ...
 *
 * Display and analysis functions for execution trees from explore.explore().
 * Separated from explore.js for clean module boundaries.
 */

function isTerminal(tree) {
  return tree.type === 'leaf' || tree.type === 'bound' || tree.type === 'cycle' || tree.type === 'memo' || tree.type === 'dead';
}
...
function toDot(tree, opts = {}) {
  ...
  const colors = { leaf: 'green', bound: 'yellow', cycle: 'red', memo: 'grey', branch: 'white', dead: 'orange' };
...
export { countLeaves, getAllLeaves, maxDepth, countNodes, toDot };
export default { countLeaves, getAllLeaves, maxDepth, countNodes, toDot };
