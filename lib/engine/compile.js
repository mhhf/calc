...
function compileRule(rule, opts = {}) {
  ...
  // Also handles range-constrained metavars: !arr_get B PC OP * !is_push OP N
  // where is_push has FFI range metadata → expand to multi-value discriminator.
  const getRange = opts.getRange || null;
  if (!discriminator) {
    for (const p of (anteFlat.persistent || [])) {
      ...
      if (pred === 'arr_get' && Store.arity(p) >= 3) {
        ...
        // Range-constrained metavar: check if another persistent goal
        // constrains valueChild to a known range (e.g. !is_push OP N)
        if (getRange && !isGround(valueChild)) {
          for (const q of (anteFlat.persistent || [])) {
            if (q === p) continue;
            const qPred = getPredicateHead(q);
            if (!qPred || Store.arity(q) < 1) continue;
            if (Store.child(q, 0) !== valueChild) continue;
            const range = getRange(qPred);
            if (!range) continue;

            // Enumerate all ground values in [lo, hi]
            const { intToBin } = require('./ill/ffi/convert');
            const groundValues = [];
            for (let v = range.lo; v <= range.hi; v++) {
              groundValues.push(intToBin(BigInt(v)));
            }
            discriminator = {
              type: 'virtual',
              pred: pred,
