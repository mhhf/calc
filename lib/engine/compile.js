...
const { deltaAnalysis, patternRoles, compileSub } = require('./rule-analysis');
const { isGround, collectMetavars, collectFreevars } = require('./pattern-utils');
const { resolveConn, flattenAnte, unwrapComp, expandChoice, expandConsqChoices } = require('./formula-utils');
...
function compileRule(rule, opts = {}) {
  ...
  // Cache hit: same hash → same compilation (just patch name/sourceLabel)
  const cached = _compileCache.get(rule.hash);
  if (cached) {
  ...
  _compileCache.set(rule.hash, compiled);
  return compiled;
}
...
function compileKey(hash, localSlots) {
  ...
  if (t === 'binlit') {
    ...
    if (v === 0n) return { predHead: head, firstArgSlot: null, firstArgKey: 'e' };
    return { predHead: head, firstArgSlot: null, firstArgKey: v % 2n === 1n ? 'i' : 'o' };
  }
...
module.exports = {
  // Formula utilities (canonical source: formula-utils.js, re-exported for convenience)
  resolveConn,
  flattenAnte,
  unwrapComp,
  expandChoice,
  ...
  // Rule compilation
  compileRule,
  // Compiled pattern matching (PM = pattern match GET, PUT = premise construction)
  compilePM,
  execPM,
  PM_BIND,
  PM_GROUND,
