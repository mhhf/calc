...
function compileRule(rule, opts = {}) {
  ...
  if (rule.isFused && opts.getModeMeta) {
    ...
    if (union.length > 0) {
      ...
      // Prefer the bind order CARRIED by fusion (Track 1) over reconstructing
      // it: rank each resolver goal by its position in rule.bodyOrder. Use the
      // carried order only when it covers every goal — passes that transform
      // goals AFTER fusePair set bodyOrder (chain fusion, SROA) can leave it
      // stale, in which case the dataflow reconstruction (scheduleGoals) is the
      // safe fallback until those passes maintain bodyOrder too.
      let order = null;
      let _carried = false;
      if (rule.bodyOrder && rule.bodyOrder.length) {
        const rank = new Map();
        for (let k = 0; k < rule.bodyOrder.length; k++) {
          if (!rank.has(rule.bodyOrder[k])) rank.set(rule.bodyOrder[k], k);
        }
        if (union.every(g => rank.has(g))) {
          order = union.map((_, i) => i).sort((a, b) => rank.get(union[a]) - rank.get(union[b]));
          _carried = true;
        }
      }
      if (!order) order = scheduleGoals(union, seed, gmm);
      ...
      resolutionBody = { steps, freshenSlots: bodyFreshenSlots, carried: _carried };
    }
  }
