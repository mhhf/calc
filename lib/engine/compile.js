...
function compileRule(rule, opts = {}) {
  ...
  for (const h of (anteFlat.linear || [])) {
    ...
    const pred = isAt ? predHead(Store.child(h, 0)) : predHead(h);
    if (pred && !triggerPreds.includes(pred)) triggerPreds.push(pred);
    // Stamped patterns don't drive fingerprint discriminators — the child
    // layout below indexes the at-node, not P (stamp fingerprints: Phase 4).
    if (isAt) continue;
  ...
  for (const p of (anteFlat.linear || [])) {
    ...
    const isAtP = Store.tag(p) === 'at';
    const pred = isAtP ? predHead(Store.child(p, 0)) : predHead(p);
    ...
    if (!isAtP && discriminator && pred === discriminator.pred) {
