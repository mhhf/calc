...
// Default window-arithmetic lowering targets. A calculus overrides via
// loaderConfig.qexprPreds — till points add/mul at the SHARED numeric names
// (TODO_0011 §3 collapse) while sub/div stay q-specific (checked vs monus,
// field vs Euclidean — never collapsed).
const _QEXPR_PRED = {
  qexpr_add: 'qplus', qexpr_sub: 'qsub', qexpr_mul: 'qmul', qexpr_div: 'qdiv',
};
 ...
 */
function desugarTimed(bodyHash, connTags = DEFAULT_LOADER_CONFIG.connTags, qexprPreds = _QEXPR_PRED) {
  const _impl = connTags.implication, _prod = connTags.product;
  ...
  function lower(e) {
    const pred = qexprPreds[Store.tag(e)];
    if (!pred) return e;
...
function loadFile(filePath, definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  for (const decl of decls) {
    ...
    if (decl.type === 'subsort') {
      const cname = `${SORT_PREDS.EDGE}/${decl.sub}_${decl.sup}`;
  ...
  for (const decl of decls) {
    ...
    const convertOne = (cName, cBodyHash, cPremises) => {
      ...
      if (_lc.timed) cleanBodyHash = desugarTimed(cleanBodyHash, _ct, _lc.qexprPreds || _QEXPR_PRED);
...
function load(filePaths, loadOpts = {}) {
  ...
  return { definitions, clauses, forwardRules, queries, argNamesTable,
    sortVarsTable, querySettings, splitQueries, moduleDecls, importTree: tree };
}
