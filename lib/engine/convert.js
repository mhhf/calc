 ...
 */
function desugarTimed(bodyHash, connTags = DEFAULT_LOADER_CONFIG.connTags) {
  const _impl = connTags.implication, _prod = connTags.product;
  ...
  function lower(e) {
    const pred = _QEXPR_PRED[Store.tag(e)];
    if (!pred) return e;
...
function _validateTimed(h, location, connTags) {
  ...
  if (t === connTags.exponential) {
    ...
    if (Store.tag(inner) === 'at') {
      throw new Error('stamped persistents are not allowed: !A@t (D15)');
    }
    if (Store.tag(inner) === 'readPreserved') {
      throw new Error('! read P is meaningless: persistent facts are never consumed');
    }
  }
  if (t === 'readPreserved' && Store.tag(Store.child(h, 0)) === connTags.exponential) {
    throw new Error('read !P is meaningless: persistent facts are never consumed');
  }
...
function loadFile(filePath, definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  for (const decl of decls) {
    ...
    if (_lc.timed) cleanBodyHash = desugarTimed(cleanBodyHash, _ct);
