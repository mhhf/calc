 ...
 */
function desugarPreserved(bodyHash, computation = { tag: 'monad', bodyIdx: 0, gradeIdx: null }) {
  ...
  return Store.put('loli', [cleanAnte, newConseq]);
}
...
function loadFile(filePath, definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  const _parserWasBuilt = _exprParser !== null;
  const _parser = _getExprParser();
  ...
  const decls = parseDecls(source, _parser);
  ...
  for (const decl of decls) {
    ...
    if (Store.tag(bodyHash) === 'bang' && Store.child(bodyHash, 0) === grade0()) continue;
  ...
  for (const decl of decls) {
    ...
    if (!hasMonad(bodyHash) && premises.length === 0) {
      if (!(Store.tag(bodyHash) === 'bang' && Store.child(bodyHash, 0) === grade0())) continue;
    }
    ...
    if (hasMonad(cleanBodyHash)) {
      ...
      const isGrade0 = Store.tag(cleanBodyHash) === 'bang' &&
                        Store.child(cleanBodyHash, 0) === grade0();
...
function load(filePaths, loadOpts = {}) {
  ...
  for (const p of paths) {
    loadFile(p, definitions, clauses, forwardRules, queries, {
      argNamesTable, querySettings, splitQueries, moduleDecls, alreadyImported, onPhase,
    });
  }
...
export { load, loadFile, parseExpr, hasMonad, decomposeQuery, desugarPreserved, buildImportTree, computeTreeHashes, extractTopLevelImports, stripNamedArgsFromArrowChain, resolveNamedArgSentinels };
export default { load, loadFile, parseExpr, hasMonad, decomposeQuery, desugarPreserved, buildImportTree, computeTreeHashes, extractTopLevelImports, stripNamedArgsFromArrowChain, resolveNamedArgSentinels };
