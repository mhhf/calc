...
function loadFile(filePath, definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  for (const decl of decls) {
    ...
    if (hasMonad(bodyHash)) {
      ...
      // Definition (axiom/constructor) — check for duplicate
      if (definitions.has(name)) {
        throw new Error(`Duplicate definition '${name}' (already defined)`);
      }
      definitions.set(name, bodyHash);
    }
  }
}
 ...
 * @returns {{ definitions: Map, clauses: Map, forwardRules: Array, queries: Map }}
 */
function load(filePaths) {
  ...
  return { definitions, clauses, forwardRules, queries };
}
