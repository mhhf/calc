...
function desugarPreserved(bodyHash) {
  ...
  // Validate: no preserved() nodes in the consequent
  _assertNoPreserved(conseq);
...
function _assertNoPreserved(h) {
  const t = Store.tag(h);
  if (!t) return;
  if (t === 'preserved') {
    throw new Error('$ (preserved) can only be used on antecedent resources, not in the consequent');
  }
  ...
  for (let i = 0; i < a; i++) {
    ...
    if (typeof c === 'number' && Store.isTerm(c)) _assertNoPreserved(c);
  }
}
