...
import { DECIMATE_PREDS } from './reserved-preds.js';
...
import { DEFAULT_COMPUTATION } from './formula-utils.js';
// ─── Expression parser ─────────────────────────────────────────────────────
...
// Expression parser: EVERY calculus supplies loaderConfig.buildParser
// (ILL: calculus/ill/lib/forward-parser.js; till/gill/will: kit.js
// makeForwardParserBuilder). The engine holds no calculus parser and
// knows no calculus location (TODO_0086). Lazy: built on first use and
// memoized on the config object itself.
function _getExprParser(loaderConfig = DEFAULT_LOADER_CONFIG) {
  if (!loaderConfig.buildParser) {
    throw new Error(
      'convert: loaderConfig.buildParser is required — the engine has no ' +
      'default parser. Import your calculus facade (e.g. calculus/ill/' +
      'index.js) or pass opts.loaderConfig = cc.loader.'
    );
  }
 ...
 */
function desugarPreserved(bodyHash, computation = DEFAULT_COMPUTATION,
...
function loadFile(filePath, definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  for (const decl of decls) {
    ...
    // build time (lib/measure/priors.js).
  ...
  for (const decl of decls) {
    ...
    const convertOne = (cName, cBodyHash, cPremises) => {
      ...
      if (hasMonad(cleanBodyHash, _ct.computation.tag)) {
        // Forward rule.
        // Soundness fence (TODO_0009, audit 2026-09-11): μ/ν are backward-proof
        // obligations (μMALL unfold rules / cyclic proofs), NEVER forward linear
        // resources. A rule that produced or matched a fixpoint as a fact would
        // let the lax-monad bridge (bridge.js rightFocus) close a coinductive
        // goal by mere forward token production. 'mu'/'nu' are kernel tags, so a
        // fixpoint-free calculus never trips this (like the 'at'/'drawn' fences).
        if (_containsTag(cleanBodyHash, 'mu') || _containsTag(cleanBodyHash, 'nu')) {
          throw new Error(`forward rule '${cName}' mentions a fixpoint connective ` +
            `(mu/nu): μ/ν are backward-proof (μMALL) connectives, not forward ` +
            `linear resources — they may not appear in a rule's antecedent or consequent`);
        }
 ...
 */
function decomposeQuery(hash, connTags = null) {
  // Connective tag names for the walk: optional override for a calculus
  // whose multiplicative/exponential/unit tags differ from the shared
  // defaults (all current calculi use tensor/bang/one — TODO_0086 R4).
  const _ct = connTags || {};
  const T_UNIT = _ct.unit || 'one';
  const T_PRODUCT = _ct.product || 'tensor';
  const T_EXP = _ct.exponential || 'bang';
 ...
 * @returns {number} hash
 */
function parseExpr(source, loaderConfig = DEFAULT_LOADER_CONFIG) {
  return _getExprParser(loaderConfig)(source);
}
