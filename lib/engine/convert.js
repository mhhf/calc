...
/**
 * Convert metavars in a term to freevars (eigenvariables).
 * Query/initial-state variables like Sender, Deadline are symbolic values,
 * not unification pattern variables — they must be freevar for correct
 * groundness checks (FFI dispatch, constraint solving).
 * @param {number} h - Term hash
 * @returns {number} New term hash with metavars replaced by freevars
 */
function _metavarToFreevar(h) {
  const t = Store.tag(h);
  if (!t) return h;
  if (t === 'metavar') return Store.put('freevar', [Store.child(h, 0)]);
  if (t === 'atom' || t === 'freevar' || t === 'binlit') return h;
  ...
  if (t === 'arrlit') {
    ...
    for (let i = 0; i < elems.length; i++) {
      const ne = _metavarToFreevar(elems[i]);
      newElems[i] = ne;
 ...
 * Converts metavars to freevars — query variables are eigenvariables.
 * @param {number} hash
...
function decomposeQuery(hash) {
  ...
  function walk(h) {
    ...
    if (node.tag === 'tensor') {
      ...
      const fv = _metavarToFreevar(node.children[0]);
      persistent[fv] = true;
    } else {
      const fv = _metavarToFreevar(h);
      linear[fv] = (linear[fv] || 0) + 1;
    }
  }
  walk(hash);
