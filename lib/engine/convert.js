...
const { extractMacros, expandMacros } = require('../parser/macros');
...
function desugarPreserved(bodyHash) {
  ...
  function stripPreserved(h) {
    ...
    if (t === 'bang' && Store.tag(Store.child(h, 0)) === 'preserved') {
      throw new Error(
        '!$P is not allowed: persistent resources (!) are never consumed, ' +
        'so $ (preserved) is meaningless. Use !P instead.'
      );
...
function loadFile(filePath, definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  // Extract @def macros (pre-pass, before structural parsing)
  const { macroTable, cleanSource } = extractMacros(source);
  const exprParser = macroTable.size > 0
    ? text => _exprParser(expandMacros(text, macroTable))
    : _exprParser;

  const decls = parseDeclarations(cleanSource, exprParser);
...
function load(filePaths) {
  ...
  for (const p of paths) {
    loadFile(p, definitions, clauses, forwardRules, queries, {
      argNamesTable, querySettings, moduleDecls
    });
  }
  ...
  return { definitions, clauses, forwardRules, queries, argNamesTable,
    querySettings, moduleDecls, importTree: tree };
}
