...
function loadFile(filePath, definitions, clauses, forwardRules, queries, opts = {}) {
  ...
  // ── Pass 2: queries + forward rules + backward clauses (with named arg resolution) ──
  for (const decl of decls) {
    if (decl.type === 'query') {
    ...
    const cleanBodyHash = resolveNamedArgSentinels(bodyHash, argNamesTable);
 ...
 * @returns {{ definitions: Map, clauses: Map, forwardRules: Array, queries: Map, argNamesTable: Map }}
 */
function load(filePaths) {
  ...
  for (const p of paths) {
    loadFile(p, definitions, clauses, forwardRules, queries, { argNamesTable });
  }
