...
const { buildStrategyStack } = require('./strategy');
const { detectFingerprintConfig, makeFingerprintLayer, attachPredictions } = require('./opt/fingerprint');
const { makeDiscTreeLayer } = require('./opt/disc-tree-opt');
...
function resolveProfile(input) {
  const envProfile = process.env.CALC_PROFILE;
 ...
 */
function createEngine(profile, rules) {
...
function _buildStrategy(profile, rules) {
  ...
  const fpConfig = profile.fingerprint ? detectFingerprintConfig(rules) : null;
  if (fpConfig) {
    layers.push(makeFingerprintLayer(fpConfig));
    if (profile.prediction) attachPredictions(rules, fpConfig);
  }
