 ...
 *  trivial-grade instance of the graded monad till also uses. Used as the
 *  ILL-path default (modes.js, convert.js, backchain-ill.js); the BRIDGE
 *  deliberately does NOT fall back to it (TODO_0274 item 8 — a roleless
 *  engine calculus is a loud error there, not silently ILL). */
// The default computation role: tag 'monad', arity 2 — the name every
// current calculus (ill/till/gill/will) shares for its graded lax monad.
// A calculus with a different computation tag overrides via cc.loader /
// roles; this is a convenience default, not an ILL binding (TODO_0086).
const DEFAULT_COMPUTATION = Object.freeze(computationRole('monad', 2));
...
function resolveConn(ct, gradeConfig) {
  ...
  for (const tag in ct) {
    ...
    else if (c === 'additive' && a === 0 && p === 'negative') r.additiveUnit = tag;
...
export { computationRole, DEFAULT_COMPUTATION, resolveConn, connTagsFrom, flattenAnte, unwrapComp, expandChoice, expandConsqChoices, factKeyOf };
export default { computationRole, DEFAULT_COMPUTATION, resolveConn, connTagsFrom, flattenAnte, unwrapComp, expandChoice, expandConsqChoices, factKeyOf };
