 ...
 *  (ratlit or binlit only — the weight positions of `woplus`). Local
 *  decoder to keep this module off the theory-registry import graph. */
function _ratLit(h) {
  const t = Store.tag(h);
...
/** Multiply an alternative's path weight ([n,d] pair; absent = 1). */
function _weighted(alt, w) {
  return { ...alt, weight: alt.weight ? _ratMul(alt.weight, w) : w };
}
...
function expandChoice(h, ct) {
  ...
  if (ct.weightedChoice && t === ct.weightedChoice) {
    ...
    const q = _ratLit(Store.child(h, 0));
    if (!q) {
      throw new Error(`woplus weight must be a ground rational literal (got '${Store.tag(Store.child(h, 0))}')`);
    }
    if (q[0] < 0n || q[0] > q[1]) {
  ...
  if (t === ct.product) {
    ...
    for (const l of lefts) {
      for (const r of rights) {
        ...
        const w = l.weight && r.weight ? _ratMul(l.weight, r.weight)
          : (l.weight || r.weight);
        if (w) alt.weight = w;
        out.push(alt);
      }
  ...
  for (const h of (consequent.linear || [])) {
    ...
    for (const acc of alts) {
      for (const ia of itemAlts) {
        ...
        const w = acc.weight && ia.weight ? _ratMul(acc.weight, ia.weight)
          : (acc.weight || ia.weight);
        if (w) alt.weight = w;
        next.push(alt);
      }
    }
