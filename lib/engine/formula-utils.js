...
import { grade0 as grade0Label } from './grades.js';
...
function flattenAnte(h, ct) {
  ...
  function walk(hash) {
    ...
    if (t === productTag) {
      ...
      if (grade === grade0Label()) {
        grade0.push(inner);
      } else {
        // gradeW() (or any future grade with structural rules) → persistent
...
export { resolveConn, flattenAnte, unwrapComp, expandChoice, expandConsqChoices };
export default { resolveConn, flattenAnte, unwrapComp, expandChoice, expandConsqChoices };
