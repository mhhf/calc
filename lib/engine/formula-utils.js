...
/** Decode a canonical rational-literal hash to a [num, den] BigInt pair
 *  (ratlit or binlit only — the weight positions of `woplus`). Kept here
 *  (not in the theory registry) so this module stays import-light; the
 *  ONE shared decoder for weight positions (timed.js reuses it —
 *  round-15 F7). A per-calculus decode slot arrives with the first
 *  second-timed-logic demand (recorded decoupling rider), not before. */
function ratLit(h) {
  const t = Store.tag(h);
  if (t === 'ratlit') return [Store.child(h, 0), Store.child(h, 1)];
  if (t === 'binlit') return [Store.child(h, 0), 1n];
  return null;
}
...
export { computationRole, ILL_COMPUTATION, resolveConn, connTagsFrom, flattenAnte, unwrapComp, expandChoice, expandConsqChoices, factKeyOf, ratLit };
export default { computationRole, ILL_COMPUTATION, resolveConn, connTagsFrom, flattenAnte, unwrapComp, expandChoice, expandConsqChoices, factKeyOf, ratLit };
