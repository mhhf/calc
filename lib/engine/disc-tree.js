...
import { isMetavar, theoryFor } from '../kernel/unify.js';
...
function _queryFlat(node, pos, len, results) {
  ...
  // rules (D13: an index may never change WHICH matches exist). Follow any
  // child branch the fact's theory can rewrite to; over-approximate with
  // collectAll — candidates are verified by tryMatch anyway.
  if (node.children && typeof sym === 'string') {
    const tid = Store.TAG[sym];
    const th = tid !== undefined ? theoryFor(tid) : null;
    if (th) {
      for (const k in node.children) {
        if (k === sym) continue;
        const dstTid = Store.TAG[k];
        if (dstTid !== undefined && th.canRewrite(tid, dstTid)) {
