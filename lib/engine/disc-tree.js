...
import { isMetavar, theoryFor, canCrossMatch } from '../kernel/unify.js';
...
function _queryFlat(node, pos, len, results) {
  ...
  // Theory branch: a compact literal (binlit, ratlit) at this position can
  // cross-tag match structural pattern forms (i/o/e, rat(N,D)) — the
  // matcher's unify handles the rewrite, so the INDEX must not hide those
  // rules (canCrossMatch is the contract; D13: an index may never change
  // WHICH matches exist). Follow any child branch the fact's theory can
  // rewrite to; over-approximate with collectAll — candidates are verified
  // by tryMatch anyway.
  if (node.children && typeof sym === 'string') {
    const tid = Store.TAG[sym];
    if (tid !== undefined && theoryFor(tid)) {
      for (const k in node.children) {
        if (k === sym) continue;
        const dstTid = Store.TAG[k];
        if (dstTid !== undefined && canCrossMatch(tid, dstTid)) {
          collectAll(node.children[k], results);
        }
      }
    }
  }
 ...
 * Uses state.linear.group(tagId) instead of stateIndex[pred].
 */
function makeDiscTreeLayer() {
  return {
    ...
    build: (rules) => {
      ...
      // Collect the set of relevant tag IDs (first trigger pred of each rule).
      // At query time, only iterate facts from these tag groups.
      const relevantTagIdSet = new Set();
      for (const rule of rules) {
        ...
        if (triggers && triggers.length > 0) {
          ...
          if (t && t.length > 0) {
            // the pattern's OWN head tag decides the group — the global
            // name table is ambiguous across loaded programs (an atom
            // trigger must walk the atom group even when a same-named
            // predicate tag exists); name fallback for stale caches
            const meta = rule.linearMeta && rule.linearMeta[triggers[0]];
            const tid = meta && meta.predTag !== undefined
              ? meta.predTag : Store.TAG[t[0]];
            if (tid !== undefined && tid >= Store.PRED_BOUNDARY) {
              relevantTagIdSet.add(tid);
            } else {
              // Atom predicate — add atom tag group
              relevantTagIdSet.add(Store.TAG.atom);
            }
          }
        }
      }
      ...
      const relevantTagIds = [...relevantTagIdSet];
      return {
        getCandidateRules(state) {
          const results = [];
          const seen = new Set();
          if (hasWildcardRoot) {
            // Must scan all tag groups for wildcard-rooted patterns
            for (let t = 0; t < state.linear.maxTagId; t++) {
              const grp = state.linear.group(t);
              for (let fi = 0; fi < grp.length; fi++) {
                const factLen = flattenFact(grp[fi]);
                queryFlat(root, factLen, results);
              }
            }
          } else {
            // Only scan facts from tag groups that appear in indexed patterns
            for (let pi = 0; pi < relevantTagIds.length; pi++) {
              const grp = state.linear.group(relevantTagIds[pi]);
              for (let fi = 0; fi < grp.length; fi++) {
                const factLen = flattenFact(grp[fi]);
                queryFlat(root, factLen, results);
              }
          ...
          for (let i = 0; i < results.length; i++) {
            ...
            for (let j = 0; j < t.length; j++) {
              if (!state.hasPredicate(t[j])) { ok = false; break; }
            }
...
export { createNode, insert, flattenPattern, flattenFact, subtreeSize, queryFlat, collectAll, makeDiscTreeLayer, WILDCARD };
export default { createNode, insert, flattenPattern, flattenFact, subtreeSize, queryFlat, collectAll, makeDiscTreeLayer, WILDCARD };
