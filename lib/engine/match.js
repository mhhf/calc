...
function _resolveValue(root, resolved) {
  ...
  const LEAF_TAGS = { atom: 1, binlit: 1, strlit: 1, charlit: 1, arrlit: 1 };
  const tag0 = Store.tag(root);
  if (tag0 === 'freevar') { const r = resolved.get(root); return r !== undefined ? r : root; }
  if (LEAF_TAGS[tag0]) return root;

  // Stack-based post-order: each frame = { h, tag, arity, children[], childIdx }
  const stack = [{ h: root, tag: tag0, arity: Store.arity(root), children: [], childIdx: 0 }];
  while (stack.length > 0) {
    ...
    if (frame.childIdx < frame.arity) {
      ...
      const ctag = Store.tag(c);
      if (ctag === 'freevar') {
        const r = resolved.get(c);
        frame.children.push(r !== undefined ? r : c);
        continue;
      }
      if (LEAF_TAGS[ctag]) { frame.children.push(c); continue; }
      // Push child for processing
      stack.push({ h: c, tag: ctag, arity: Store.arity(c), children: [], childIdx: 0 });
    } else {
