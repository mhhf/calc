...
// Pre-allocated argument buffer for FFI calls (avoids per-call allocation).
// Sized from registered FFI metadata — no hardcoded arity limit.
const _maxFfiArity = Math.max(4, ...Object.values(_ffiParsedModes).map(m => m.length));
const _ffiArgs = new Array(_maxFfiArity).fill(0);
 ...
 *
 * Three-level resolution (FFI-first order):
 *   1. FFI — O(1) optimization for arithmetic predicates
 *   2. State lookup — match pattern against state.persistent facts
...
function provePersistentGoals(patterns, startIdx, theta, slots, state, calc) {
  ...
  while (idx < patterns.length) {
    ...
    // 1. FFI — try O(1) arithmetic first (skips state scan for FFI-backed predicates)
    ...
    // 2. State lookup — check if fact already exists in state.persistent
    {
      ...
      if (pPred) {
        ...
        // O(1) guard: only scan if predicate group is non-empty
...
function matchOnePattern(pattern, origPos, rule, state, theta, slots,
  ...
  if (role && role.type === 'delta' && role.flat && !isPreserved) {
    ...
    for (let ci = 0; ci < candidates.length; ci++) {
      ...
      const available = state.linear.count(hTag, h) - (consumed[h] || 0) - (reserved[h] || 0);
  ...
  if (pred === state._fpPred && state._byKey && meta.secondaryKeyPattern !== null) {
    ...
    if (codeFact) {
      ...
      const available = state.linear.count(cfTag, codeFact) - (consumed[codeFact] || 0) - (reserved[codeFact] || 0);
      if (available > 0) {
        const savedUndo = undoSave();
...
function tryMatch(rule, state, calc, matchOpts) {
  ...
  const result = matchAllLinear(rule, state, theta, slots, consumed, reserved,
                                preservedCount, usePreserved, persistentList, calc);
