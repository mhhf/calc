...
const { clearBWCache, proveNaive } = require('./lnl/persistent');
const { resolveEx } = require('./lnl/existential');
const { matchLoli } = require('./lnl/loli');

// ─── FFI Direct Bypass ───────────────────────────────────────────────

const { proveWithFFI, execPS, compilePS } = require('./opt/ffi');

/**
 * Prove persistent patterns — dispatches to FFI-enhanced pipeline.
 * For bare/noFFI profile, callers compose proveNaive via matchOpts.provePersistent.
 */
const provePersistent = proveWithFFI;
...
function tryMatch(rule, state, calc, matchOpts) {
  ...
  if (rule.existentialSlots && rule.existentialSlots.length > 0) {
    resolveEx(_poolTheta, slots, rule, state, calc, matchOpts);
  }
 ...
 */
function buildMatchOpts({ useFFI, evidence, rc, ffiCtx, canonicalize, onProveFail, onProveSuccess, backchainUseFFI, optimizePreserved }) {
  const opts = {
  ...
  opts.provePersistent = useFFI
    ...
    : (patterns, startIdx, theta, slots, state, calc, evidenceOut) =>
        proveNaive(patterns, startIdx, theta, slots, state, calc, evidenceOut, opts);
  return opts;
}
