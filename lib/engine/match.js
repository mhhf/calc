 ...
 *   - lnl/persistent.js — provePersistentNaive, clearBackwardCache, executePersistentStep
...
const { provePersistentNaive, clearBackwardCache, executePersistentStep } = require('./lnl/persistent');
...
function matchAllLinear(rule, state, theta, slots, consumed, reserved,
  ...
  while (rpLen > 0 || persistentIdx < persistentList.length) {
    ...
    const proveFn = (matchOpts && matchOpts.provePersistent) || provePersistentGoals;
...
module.exports = {
  ...
  clearBackwardCache,
  // Persistent proving (re-exported)
