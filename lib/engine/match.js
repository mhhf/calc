...
// Empty persistent list for fused-block rules (their goals resolve in the
// body pass, not at match time — TODO_0307 P3).
const _EMPTY_PERSISTENT = Object.freeze([]);
...
function matchLinear1(pattern, origPos, rule, state, theta, slots,
                         consumed, reserved, preservedCount, usePreserved, deltaBypassFn) {
  ...
  // Strategy A: Delta bypass — direct child extraction for flat delta
  // patterns. The bypass is an INJECTED optimization (matchOpts opt
  // protocol, profile-gated at the composition root — RES_0143 F4);
  // absent, matching falls through to Strategy B/C unchanged.
  if (deltaBypassFn && deltaBypassFn(pattern, origPos, rule, state, theta,
 ...
 *
 * Contract: matchOpts is always the frozen 21-field record produced by
...
function tryMatch(rule, state, calc, matchOpts = EMPTY_MATCH_OPTS) {
  ...
  // Fused-block rules resolve ALL persistent goals (ante asks + conseq tells)
  // in the dataflow-ordered body pass (resolveEx below, TODO_0307 P3), NOT at
  // match time — proving an ante ask here in isolation would run it before the
  // conseq tell that produces its input. Match only the linear entry pattern.
  const persistentList = rule.resolutionBody ? _EMPTY_PERSISTENT : (rule.antecedent.persistent || []);
  ...
  // Resolve existentials. A plain rule's ∃ resolution never blocks (freshEvar
  // fallback). A fused-block body pass (TODO_0307 P3) resolves the whole
  // ordered body and returns false on a ground guard failure (out-of-gas,
  // infeasible constraint) — that aborts the match exactly as a failed
  // antecedent goal would, undoing the linear consumption.
  if ((rule.existentialSlots && rule.existentialSlots.length > 0) || rule.resolutionBody) {
    const _resolveEx = matchOpts.resolveEx;
    if (_resolveEx) {
      if (_resolveEx(_poolTheta, slots, rule, state, calc, matchOpts) === false) {
        undoRestore(_poolTheta, topUndo);
        return null;
      }
    } else if (rule.resolutionBody) {
      // No resolver bound (family absent) but the rule needs body resolution —
      // cannot fire it soundly. Refuse.
      undoRestore(_poolTheta, topUndo);
      return null;
    }
  }
 ...
 */
/** @param {{optimizePreserved?: boolean, evidence?: boolean, canonicalize?: Function|null, onProveFail?: Function|null, onProveSuccess?: Function|null, provePersistent?: Function|null}} [o] */
...
/** @param {{matchDynamicRule?: Function|null, resolveEx?: Function|null, drainDynamicRules?: Function|null, rc?: object|null, backchainUseFFI?: boolean}} [o] */
...
/** Opt layer protocol: compiled fast-path callbacks. deltaBypass is the
 *  Strategy A matcher fast path (opt/delta-bypass.js), profile-gated and
 *  injected by the composition root (RES_0143 F4) — null falls through
 *  to the general matching strategies, same semantics. */
/** @param {{execPS?: Function|null, execExStep?: Function|null, tryCCDispatch?: Function|null, deltaBypass?: Function|null, useCompiledSteps?: boolean}} [o] */
function buildOptProtocol({ execPS, execExStep, tryCCDispatch, deltaBypass, useCompiledSteps = false } = {}) {
  return {
    execPS: execPS || null,
    execExStep: execExStep || null,
    tryCCDispatch: tryCCDispatch || null,
    deltaBypass: deltaBypass || null,
    useCompiledSteps,
  };
}
