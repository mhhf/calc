...
// ─── Protocol Factories ─────────────────────────────────────────────
// Each factory returns its layer's contribution to matchOpts.
// The composition root (index.js) spreads them flat + freezes.
// ML functor analogy: each factory = functor body, spread = include.
//
// Each factory always returns the same keys (shape stability). Missing inputs
// default to null/undefined. This guarantees every matchOpts has identical
// hidden class (V8 monomorphism).

/** Generic layer protocol: mode flags + cross-cutting hooks.
 *  Boolean fields are passed through as-is so explicit `false` is distinguishable
 *  from `undefined` (consumers read `!== undefined` to detect explicit opt-out).
 */
function buildGenericProtocol({ optimizePreserved, evidence, canonicalize, onProveFail, onProveSuccess } = {}) {
  return {
    optimizePreserved,
    evidence,
...
function buildLnlProtocol({ matchLoli, resolveEx, drainLolis, rc, backchainUseFFI } = {}) {
  return {
    ...
    // Composition root overrides when execOpts.useFFI is explicitly set.
    backchainUseFFI: backchainUseFFI !== undefined ? backchainUseFFI : true,
  };
}

/** Opt layer protocol: compiled fast-path callbacks. */
function buildOptProtocol({ execPS, execExStep, tryCCDispatch, useCompiledSteps } = {}) {
  return {
    execPS: execPS || null,
...
/**
 * Proof-routing protocol: selects the persistent-proving implementation.
 * useFFI routes to the FFI-accelerated prover; otherwise, the naive clause prover.
 * Consumer-defined interface (generic/match.js), provider-implemented at composition root.
 */
function buildProofProtocol({ useFFI, proveWithFFI, proveNaive } = {}) {
  return {
    provePersistent: useFFI
      ? (proveWithFFI || null)
      : (proveNaive || null),
  };
}
...
const PROOF_FIELDS = Object.freeze(Object.keys(buildProofProtocol()));

/**
 * Monoidal identity: the matchOpts with all fields present but all callbacks null.
 * Same hidden class as fully-populated matchOpts — preserves V8 IC monomorphism.
 * Used by direct callers (tests, benchmarks) that bypass the orchestrator and
 * want default no-op semantics. Not a fallback kludge: the principled zero.
 */
