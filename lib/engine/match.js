...
function matchAllLinear(rule, state, theta, slots, consumed, reserved,
  ...
  while (rpLen > 0 || persistentIdx < persistentList.length) {
    ...
    // Skip compiled FFI fast path when collecting evidence (it doesn't capture evidence)
    if (!evidenceOut) {
    ...
    const newIdx = provePersistentGoals(persistentList, persistentIdx, theta, slots, state, calc, evidenceOut);
 ...
 * instead of empty placeholders, enabling proof term reconstruction.
 *
...
function matchLoli(h, state, calc, matchOpts) {
  ...
  if (persistentTriggers.length > 0) {
    const proved = provePersistentGoals(
