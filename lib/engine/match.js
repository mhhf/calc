...
// Tag ID caches for hot paths (numeric comparison, no string alloc).
// Built-in tags are stable; predicate tags (i, o) resolve lazily since
// they're registered dynamically by the calculus loader.
...
const _TAG_ARRLIT2 = Store.TAG.arrlit;

function _normalizeBin(h) {
  if (!Store.isTerm(h)) return h;
  const tid = Store.tagId(h);
  // Check binary constructor tags (i/o are predicate tags, resolved lazily)
  ...
  if (tid === _TAG_ATOM || tid === _TAG_BINLIT || tid === _TAG_STRLIT ||
      tid === _TAG_CHARLIT || tid === _TAG_FREEVAR) return h;
  if (tid === _TAG_ARRLIT2) {
...
// Monotonic counter for unique probe freevar names in backward cache.
// Prevents content-addressed aliasing: each probe gets fresh freevars
...
function _tryBackwardCache(goal, slots, theta, calc) {
  ...
  // Collect input args for cache key, output positions (direct accessors, no alloc)
  let key = head;
  ...
  for (let i = 0; i < pm.length && i < arity; i++) {
    if (pm[i] === '+') {
      ...
      if (Store.isTerm(argH) && Store.tagId(argH) === Store.TAG.freevar) return undefined;
      key += ':' + argH;
    } else if (pm[i] === '-') {
  ...
  if (cached !== undefined) {
    ...
    for (let oi = 0; oi < outputPositions.length; oi++) {
      ...
      if (Store.isTerm(argHash) && Store.tagId(argHash) === Store.TAG.freevar) {
  ...
  for (const oi of outputPositions) {
    const fv = Store.put('freevar', [`_bwc_${_bwCacheProbeId++}`]);
    probeArgs[oi] = fv;
  ...
  // Extract output values from resolved theta (direct accessors, no alloc)
  ...
  for (const fv of probeFreevars) {
    ...
    if (val === undefined) {
      ...
      const fvName = Store.child(fv, 0);
      for (const [k, v] of resolved) {
        if (Store.isTerm(k) && Store.tagId(k) === Store.TAG.freevar && Store.child(k, 0) === fvName) {
          val = v; break;
        }
      }
    }
...
// Leaf tag bitset: 1 = leaf (no term children to recurse into).
// Indexed by tagId for O(1) lookup (replaces 5 string comparisons).
const _LEAF_TAGS = new Uint8Array(256);
_LEAF_TAGS[_TAG_ATOM] = 1;
_LEAF_TAGS[_TAG_BINLIT] = 1;
_LEAF_TAGS[_TAG_STRLIT] = 1;
_LEAF_TAGS[_TAG_CHARLIT] = 1;
_LEAF_TAGS[_TAG_ARRLIT2] = 1;

function _resolveValue(root, resolved) {
  // Iterative post-order traversal to resolve freevars in deep terms.
  // Avoids stack overflow on deep clause proof terms (e.g. arr_get O(n) chains).
  if (!Store.isTerm(root)) return root;
  const tid0 = Store.tagId(root);
  if (tid0 === _TAG_FREEVAR) { const r = resolved.get(root); return r !== undefined ? r : root; }
  if (_LEAF_TAGS[tid0]) return root;

  // Stack-based post-order: each frame = { h, tag, arity, children[], childIdx }
  // tag stored as string (needed for Store.put reconstruction)
  const tag0str = Store.tag(root);
  const stack = [{ h: root, tag: tag0str, arity: Store.arity(root), children: [], childIdx: 0 }];
  while (stack.length > 0) {
    const frame = stack[stack.length - 1];
    if (frame.childIdx < frame.arity) {
      // Process next child
      const c = Store.rawChild(frame.h, frame.childIdx);
      frame.childIdx++;
      if (!Store.isTerm(c)) { frame.children.push(c); continue; }
      const ctid = Store.tagId(c);
      if (ctid === _TAG_FREEVAR) {
        const r = resolved.get(c);
        frame.children.push(r !== undefined ? r : c);
        continue;
      }
      if (_LEAF_TAGS[ctid]) { frame.children.push(c); continue; }
      // Push child for processing
      stack.push({ h: c, tag: Store.tag(c), arity: Store.arity(c), children: [], childIdx: 0 });
    } else {
      // All children processed — build result
      stack.pop();
      let changed = false;
      for (let i = 0; i < frame.arity; i++) {
        if (frame.children[i] !== Store.rawChild(frame.h, i)) { changed = true; break; }
      }
      const result = changed ? Store.put(frame.tag, frame.children) : frame.h;
