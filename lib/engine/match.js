...
function tryMatch(rule, state, calc, matchOpts) {
  ...
  if (rule.existentialSlots && rule.existentialSlots.length > 0) {
    const _resolveEx = matchOpts && matchOpts.resolveEx;
 ...
 */
function buildMatchOpts({ useFFI, evidence, rc, ffiCtx, canonicalize, onProveFail, onProveSuccess, backchainUseFFI, optimizePreserved, execExStep, execPS, matchLoli: matchLoliCb, resolveEx: resolveExCb, drainLolis: drainLolisCb, provePersistentFFI, provePersistentNaive, tryCCDispatch }) {
  const opts = {
    optimizePreserved: optimizePreserved || undefined,
    evidence: evidence || undefined,
    useCompiledSteps: useFFI,
    dynamicRuleTag: rc.implication || null,
    matchDynamicRule: matchLoliCb || null,
    resolveEx: resolveExCb || null,
    connectives: rc,
  ...
  opts.provePersistent = (useFFI && provePersistentFFI)
    ? provePersistentFFI
    : provePersistentNaive
      ? (patterns, startIdx, theta, slots, state, calc, evidenceOut) =>
          provePersistentNaive(patterns, startIdx, theta, slots, state, calc, evidenceOut, opts)
      : null;
  return opts;
}
