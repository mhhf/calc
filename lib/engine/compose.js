...
function _sroaStackDecomposition(pool, rc, getModeMeta, sroaConfig) {
  ...
  for (const rule of pool) {
    // For fused mega-rules, create an SROA'd copy with stricter structural
    // requirements (needs concrete array depth) but no arr_get/arr_set goals.
    // Place SROA'd copy BEFORE the original so findMatch tries the more
    // specific pattern first; if the stack is too shallow, falls through
    // to the original which uses FFI.
    if (rule.name.includes('+')) {
      const transformed = _trySROA(rule, rc, cfg);
      if (transformed) {
        result.push(transformed);
        sroaCount++;
      }
    }
    // Always keep the original rule (after SROA'd copy).
    result.push(rule);
  }

  return { rules: result, sroaCount };
}
...
function _trySROA(rule, rc, sroaConfig) {
  ...
  for (const h of ante.linear) {
    ...
    const child = Store.child(h, 0);
    if (Store.tag(child) === 'acons' && Store.arity(child) === 2) {
      if (arrBaseVars.has(Store.child(child, 1))) { baseVar = Store.child(child, 1); break; }
    }
...
function composeGrade0(compiledRules, connectives, getModeMeta, clauses, definitions, extraGrade0Facts, scopeGuard, opts) {
  ...
  // ── Pass 6: SROA — stack decomposition ──────────────────────────────
  // Expand array patterns, eliminate arr_get/arr_set persistent goals.
  // Creates SROA'd copies alongside originals (additive, not replacing).
  if (doFuse && specializedPool.length > 0) {
