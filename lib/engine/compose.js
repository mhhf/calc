...
function _buildRuleHash(ante, conseq) {
  const anteParts = [
    ...ante.linear,
    ...ante.persistent.map(p => Store.put('bang', [gradeW(), p])),
    ...(ante.grade0 || []).map(p => Store.put('bang', [grade0(), p])),
  ];
  const anteHash = rTensor(anteParts);
  const conseqParts = [
    ...conseq.linear,
    ...conseq.persistent.map(p => Store.put('bang', [gradeW(), p])),
    ...(conseq.grade0 || []).map(p => Store.put('bang', [grade0(), p])),
  ];
...
function _openEx(h, rc) {
  ...
  if (tag === rc.exponential) {
    ...
    if (grade === grade0()) return { linear: [], persistent: [], grade0: [inner] };
