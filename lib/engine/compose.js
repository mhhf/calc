 ...
 */
function composePair(producer, consumer, cutPredHead, rc) {
  ...
  const combinedAntePersistent = applyAll([...pAnte.persistent, ...cAnte.persistent]);
...
/**
 * Grade-0 cut elimination: compose all grade-0 rules in a single pass (v1).
 *
 ...
 * @returns {{ composedRules: Object[], predicateMap: Map, diagnostics: Object }}
 */
function composeGrade0(compiledRules, connectives) {
  ...
  if (predicateMap.size === 0) {
    return { composedRules: [], predicateMap, diagnostics };
  }
  ...
  // Compose: for each grade-0 predicate, pair all producers with all consumers.
  const composedRules = [];
  for (const [pred, entry] of predicateMap) {
    for (const producer of entry.producers) {
      for (const consumer of entry.consumers) {
        diagnostics.pairsAttempted++;
        const result = composePair(producer, consumer, pred, rc);
        if (result) {
          composedRules.push(result);
  ...
  // Defense-in-depth: filter out composed rules with grade-0 residuals.
  // This catches multi-predicate edge cases that single-pass can't resolve
  // (requires multi-stage composition, see TODO_0158).
  const validComposed = [];
  for (const raw of composedRules) {
    ...
    if (anteFlat.grade0.length > 0 || conseqFlat.grade0.length > 0) {
      ...
      validComposed.push(raw);
    }
  }

  return { composedRules: validComposed, predicateMap, diagnostics };
}

module.exports = { composePair, buildPredicateMap, composeGrade0 };
