...
function _sroaStackDecomposition(pool, rc, getModeMeta, sroaConfig) {
  ...
  for (const rule of pool) {
    // For fused mega-rules, replace with SROA'd version that expands the cons
    // pattern to expose array positions statically.  If the stack is too shallow
    // for the expanded pattern, the original would also stall (arr_get OOB on
    // the same array), so the fallback is unreachable — replacement is safe.
