...
function _specializeFromContext(ctx, factHash, factName) {
  const theta0 = unify(ctx.goalMatchHash, factHash);
  if (theta0 === null) return null;

  const { ante, conseq, remainingPersistent, sortedIdx } = ctx;

  // Pool-disjointness fix (TODO_0307 P3): every candidate is specialized from
  // ONE shared rename context (`ctx`), so the fact-unification θ grounds only
  // the matched goal's own vars — the rule's OTHER metavars (gas thread,
  // stack) survive IDENTICALLY into every sibling. Those siblings then carry
  // the disjointInPool tag while SHARING metavars, and a later fusion that
  // trusts the tag (skipping the producer rename) welds a thread output to its
  // own input (`checked_sub(G,c,G)`, unsatisfiable). Compose θ with a fresh
  // α-renaming of the residual (non-θ) vars so each candidate's output is
  // genuinely globally fresh — the invariant is true by construction.
  const domain = new Set();
  for (let i = 0; i < theta0.length; i++) domain.add(theta0[i][0]);
  const residual = new Set();
  const _collect = arr => { for (const h of arr) collectMetavars(h, residual); };
  _collect(ante.linear); _collect(ante.persistent); _collect(ante.grade0 || []);
  _collect(conseq.linear); _collect(conseq.persistent); _collect(conseq.grade0 || []);
  const theta = theta0.slice();
  for (const m of residual) {
    if (!domain.has(m)) theta.push([m, freshMetavar()]);
  }
...
// pass; compose0 receives it via composeOpts.optPasses.)
...
/** Run a pass pipeline over a rule pool (RES_0143 F3). Uniform
 *  enable-gating, empty-pool skip, and phase profiling live HERE —
 *  passes hold only their transformation. */
function _runPipeline(pool, passes, ctx, _pStart, _pEnd) {
  for (const pass of passes) {
    const t = _pStart();
    const on = (pass.enabled ? pass.enabled(ctx) : true) && pool.length > 0;
    const poolIn = pool.length;
    let meta = null;
    if (on) {
      const r = pass.run(pool, ctx);
      pool = r.pool;
      meta = r.meta || null;
    }
    if (pass.phase) {
      _pEnd(pass.phase, t, { enabled: on, poolIn, poolOut: pool.length, ...(meta || {}) });
    }
  }
  return pool;
}

/** Intrinsic pipeline pass: residual persistent resolution — the
 *  safety net catching goals that became resolvable (most work happens
 *  during transitive resolution in the specialization loop). The
 *  primary instance profiles and records diagnostics; silent instances
 *  (appended after resolveAfter passes) just resolve. */
function _residualPass(silent) {
  return {
    name: 'residual',
    phase: silent ? null : 'load/compose/residual',
    enabled: (ctx) => !!ctx.residualResolver,
    run(pool, ctx) {
      const out = pool.map((r) => _resolveOnce(r, ctx.rc, ctx.getModeMeta, ctx.residualResolver));
      if (!silent) {
        ctx.diagnostics.residualResolutions = out.reduce(
          (n, r) => n + ((r.name.match(/resolved:/g) || []).length), 0);
        return { pool: out, meta: { resolutions: ctx.diagnostics.residualResolutions, hasResolver: true } };
      }
      return { pool: out };
    },
  };
...
function compose0(compiledRules, connectives, getModeMeta, clauses, definitions, extraGrade0Facts, scopeGuard, opts) {
  ...
  // Optimization passes (P5/P5.5/P6) are INJECTED via o.optPasses from
  ...
  // ── Pool-transform pipeline (RES_0143 F3 — the PassBuilder shape) ──
  // Everything after specialization is a fold of PASSES over the
  // specialized pool. A pass is data:
  //   { name, phase: string|null, enabled(ctx) → bool,
  //     run(pool, ctx) → { pool, meta? }, resolveAfter?: bool }
  // Intrinsic: the residual-resolution safety net (below). Optimization
  // passes (fusion, SROA) are INJECTED by the composition root via
  // o.optPasses (opt/compose-fuse.js + opt/compose-sroa.js) and gated
  // twice — the doFuse flag AND injection. A pass declaring
  // resolveAfter gets a silent residual pass appended (fusion/SROA
  // ground new variables, enabling further resolution). The driver owns
  // the uniform timing/pool-count profiling.
  const pipelineCtx = {
    rc, getModeMeta, residualResolver, chainConfigs,
    linearFusionPredicate, fusionBarriers, sroaConfig, onPhase, diagnostics,
  };
  const pipeline = [_residualPass(false)];
  if (doFuse && o.optPasses) {
    for (const pass of o.optPasses) {
      pipeline.push(pass);
      if (pass.resolveAfter) pipeline.push(_residualPass(true));
    }
  }
  specializedPool = _runPipeline(specializedPool, pipeline, pipelineCtx, _pStart, _pEnd);
