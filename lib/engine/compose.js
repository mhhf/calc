...
const { flattenAnte, unwrapComp, resolveConn, expandConsqChoices } = require('./compile');
...
const { resolve: _resolveAll } = require('./resolve-all');
...
function fusePair(producer, consumer, cutPred, rc, getModeMeta) {
 ...
 */
function fusePairEx(producer, consumer, cutPred, rc, getModeMeta) {
  ...
  if (!hasChoice) {
    ...
    const fused = fusePair(producer, consumer, cutPred, rc, getModeMeta);
  ...
  for (let ai = 0; ai < alts.length; ai++) {
    ...
    const fused = fusePair(projected, consumer, cutPred, rc, getModeMeta);
  ...
  for (const chain of chains) {
    ...
    for (let i = 1; i < limit; i++) {
      ...
      for (const branch of branches) {
        const fused = fusePairEx(branch.rule, next, cutPred, rc, getModeMeta);
        if (fused) {
