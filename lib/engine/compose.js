...
import { flattenAnte, unwrapComp, resolveConn, expandConsqChoices } from './formula-utils.js';
...
//
// Default on; set CALC_0216_POOL_DISJOINT=0 to disable for A/B comparison.
const _POOL_DISJOINT_ENABLED = process.env.CALC_0216_POOL_DISJOINT !== '0';
...
function _buildRuleHash(ante, conseq, rc) {
  // Structural tags come from the resolved connective table (rc) so compose0
  // is not hardwired to ILL's bang/monad/loli — a calculus with grade-0 rules
  // supplies its own. Fallbacks keep ILL (and rc-less internal call paths)
  // bit-identical. The grade values (gradeW/grade0/monadUnit) are ILL's; a
  // third calculus wiring compose would add a grade-unit slot to rc.
  const bangTag = (rc && rc.exponential) || 'bang';
  const monadTag = (rc && rc.computation && rc.computation.tag) || 'monad';
  const loliTag = (rc && rc.implication) || 'loli';
  const anteParts = [
    ...ante.linear,
  ...
  const conseqHash = Store.put(monadTag, [monadUnit(), conseqBody]);
...
function _mccarthy(rule, rc, sroaConfig) {
  ...
  for (const p of newAnte.persistent) {
    if (!appliedEliminated.has(p)) anteParts.push(Store.put('bang', [gradeW(), p]));
  }
  for (const ng of newGoals) {
    anteParts.push(Store.put('bang', [gradeW(), apply(ng, theta)]));
  }
  for (const p of (newAnte.grade0 || [])) anteParts.push(Store.put('bang', [grade0(), p]));
...
function _trySROA(rule, rc, sroaConfig) {
  ...
  for (const h of newAnte.linear) anteParts.push(h);
  for (const p of newAnte.persistent) {
    if (!eliminatedGoals.has(p)) anteParts.push(Store.put('bang', [gradeW(), p]));
  }
  for (const p of (newAnte.grade0 || [])) anteParts.push(Store.put('bang', [grade0(), p]));
  const newAnteHash = rTensor(anteParts);
...
function compose0(compiledRules, connectives, getModeMeta, clauses, definitions, extraGrade0Facts, scopeGuard, opts) {
  ...
  const skipSpecialize = opts && opts.skipSpecialize === true;
  ...
  const ffiContext = opts && opts.ffiContext || null;
  const ffiDirect = opts && opts.ffiDirect || null;
...
export { cutPair, specialize, predMap, elimOrder, compose0, _fuseChains, _resolveOnce, _resolveBatch, _sroa, _tablingCacheKey, _composeFullKey, sortGoals, fusePair, fusePairEx };
export default { cutPair, specialize, predMap, elimOrder, compose0, _fuseChains, _resolveOnce, _resolveBatch, _sroa, _tablingCacheKey, _composeFullKey, sortGoals, fusePair, fusePairEx };
