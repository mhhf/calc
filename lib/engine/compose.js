...
function _fuseBasicBlocks(pool, rc, getModeMeta, linearFusionPredicate, fusionBarriers, onPhase) {
  ...
  if (onPhase) {
    ...
    // Top-K hottest chains as individual sub-paths (cosmetic — useful for sunburst).
    // Names are sanitized (slashes → '-') so they don't corrupt the path hierarchy.
    const TOP_K_CHAIN_PATHS = 6;
    const topChainPaths = chainRecords.slice()
      .sort((a, b) => b.ms - a.ms)
      .slice(0, TOP_K_CHAIN_PATHS);
    let accountedChainMs = 0;
    for (const r of topChainPaths) {
      const safeName = `${r.head}_${r.tail}`.replace(/[\s/]/g, '-').slice(0, 60);
      accountedChainMs += r.ms;
      _pEmit(`load/compose/fuse-blocks/fuse/chain/${safeName}`, r.ms, {
        fromChainHead: r.head,
        toChainTail: r.tail,
        chainLen: r.len,
        fusedChainLen: r.fusedLen,
        pairsAttempted: r.pairsAttempted,
        pairsSucceeded: r.pairsSucceeded,
        branchMults: r.branchMults,
      });
    }
    const otherChainMs = Math.max(0, chainMsArr.reduce((s, v) => s + v, 0) - accountedChainMs);
    if (chainRecords.length > TOP_K_CHAIN_PATHS) {
      _pEmit('load/compose/fuse-blocks/fuse/chain/other', otherChainMs, {
        chains: chainRecords.length - TOP_K_CHAIN_PATHS,
      });
    }
...
function compose0(compiledRules, connectives, getModeMeta, clauses, definitions, extraGrade0Facts, scopeGuard, opts) {
  ...
  if (!_grade0CacheHit && onPhase) {
    ...
    const TOP_K_PATHS = 5;
    const sortedClauses = [..._tabPerClause].sort((a, b) => b.ms - a.ms);
    const topKPaths = sortedClauses.slice(0, TOP_K_PATHS);
    const otherClauses = sortedClauses.slice(TOP_K_PATHS);
    ...
    // Top-K hottest clause families as their own leaves (surfaces the specific
    // named clauses in the sunburst arc, with an "other" catch-all for the
    // long tail). Only emitted if there are clauses to report.
    if (topKPaths.length > 0) {
      for (let i = 0; i < topKPaths.length; i++) {
        const c = topKPaths[i];
        if (c.ms <= 0) continue;
        // Sanitize name for path segment (strip slashes, keep alnum/underscore)
        const safe = String(c.name).replace(/[^A-Za-z0-9_\-.:]/g, '_');
        _pEmit(`load/compose/grade0-facts/tabling/clause/${safe}`, c.ms, {
          rank: i + 1,
          solutions: c.solutions,
          premises: c.premises,
          searchNodes: c.searchNodes,
      ...
      if (otherClauses.length > 0) {
        const otherMs = otherClauses.reduce((s, c) => s + c.ms, 0);
        const otherSols = otherClauses.reduce((s, c) => s + c.solutions, 0);
        _pEmit('load/compose/grade0-facts/tabling/clause/other', otherMs, {
          clauses: otherClauses.length,
          solutions: otherSols,
          note: `long tail of ${otherClauses.length} clauses`,
        });
      }
    }
  }
