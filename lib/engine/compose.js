 ...
 */
function _sroaStackDecomposition(pool, rc, getModeMeta) {
  const result = [];
  let sroaCount = 0;
  ...
  for (const rule of pool) {
    ...
    if (rule.name.includes('+')) {
      const transformed = _trySROA(rule, rc);
      if (transformed) {
 ...
 */
function _trySROA(rule, rc) {
  const anteHash = Store.child(rule.hash, 0);
  const ante = flattenAntecedent(anteHash, rc);

  // Phase 1: Find arr_get/arr_set persistent goals and the array holder.
  const arrGoals = _collectArrGoals(ante.persistent);
  if (arrGoals.length === 0) return null;

  const arrBaseVars = new Set();
  for (const g of arrGoals) arrBaseVars.add(g.arrVar);

  // Find a linear fact holding an array var (stack([TOP | REST]) or stack(S))
  let baseVar = null;
  for (const h of ante.linear) {
    const pred = getPredicateHead(h);
 ...
 */
function _collectArrGoals(persistent) {
  const goals = [];
  for (const p of persistent) {
    ...
    } else if (pred === 'arr_set' && Store.arity(p) === 4) {
      goals.push({
        type: 'set',
        hash: p,
        arrVar: Store.child(p, 0),
...
function composeGrade0(compiledRules, connectives, getModeMeta, clauses, definitions, extraGrade0Facts, scopeGuard, opts) {
  ...
  if (doFuse && specializedPool.length > 0) {
    const sroaResult = _sroaStackDecomposition(specializedPool, rc, getModeMeta);
...
module.exports = { composePair, specializePersistent, fuseLinearPair, _fuseIncChains, _sroaStackDecomposition, buildPredicateMap, buildEliminationOrder, composeGrade0 };
