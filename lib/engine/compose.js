 ...
 */
function _makeRule(name, ante, conseq, sourceLabel) {
  const { hash, antecedent, consequent } = _buildRuleHash(ante, conseq);
  return { name, hash, antecedent, consequent, sourceLabel: sourceLabel || null };
}
 ...
 *
 * When a fused rule has persistent array-access goals with ground indices on
...
// the McCarthy array axioms (McCarthy 1962; Stump et al. 2001) reduce them:
//   get([H|T], 0, V)  →  V = H                    (read-head / select-store)
