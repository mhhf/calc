 ...
 *
 * Two patterns:
 * - Unary step: pred(input, output), e.g. inc(X,Y). Chain of N → fusedPred(X, N, Y).
 * - Binary accumulate: pred(input, constant, output), e.g. checked_sub(G,3,G2).
 *   Chain → pred(input, sum_of_constants, output).
 *
...
/** Default chain configs: inc(X,Y)→plus(X,N,Y), checked_sub and plus self-accumulate. */
 ...
 *
 * Generic version of the old _fuseIncChains. Handles any predicate family where
 ...
 * @param {ChainConfig[]|null} chainConfigs - chain descriptors (null = DEFAULT_CHAIN_CONFIGS)
...
function _fuseAdditiveChains(pool, rc, getModeMeta, chainConfigs) {
  ...
  const configs = chainConfigs || DEFAULT_CHAIN_CONFIGS;
  ...
  for (const rule of pool) {
    ...
    for (const { chain, config } of safeChains) {
      ...
      const totalHash = intToBin(total);
    ...
    result.push({
      name: `${rule.name}[${fusedLabels.join(',')}]`,
      hash: fullHash,
      antecedent: anteHash,
      consequent: conseqHash,
      sourceLabel: rule.sourceLabel || null,
...
/** Backward-compatible wrapper: inc-only chain fusion (for existing tests). */
function _fuseIncChains(pool, rc, getModeMeta) {
  return _fuseAdditiveChains(pool, rc, getModeMeta, [DEFAULT_CHAIN_CONFIGS[0]]);
}
 ...
 *
 * No alpha-rename: resolver returns fully ground facts (zero metavars),
 * so no collision with rule metavars is possible.
 *
 * Uses a running theta to handle transitive dependencies:
 * !inc(0x5, ?Y) → theta += {?Y: 0x6} → !plus(?Y, 2, ?Z) becomes
 * !plus(0x6, 2, ?Z) → theta += {?Z: 0x8}.
 *
 * Also detects inc chains with ground inputs and resolves them directly
 * without creating intermediate !plus goals. E.g., !inc(X,?Y)*!inc(?Y,?Z)
 * with ground X → theta += {?Y: X+1, ?Z: X+2}.
 *
...
function _resolveResidualOnce(rule, rc, getModeMeta, resolver) {
  ...
  // Phase 1: Detect inc chains with ground start and resolve them directly.
  // This replaces _fuseIncChains + subsequent residual resolution for these chains.
  ...
  // Build inc chain maps (same logic as _fuseIncChains but we resolve directly)
  const incChainResolved = new Set(); // indices of inc goals resolved via chain walking
  let combinedTheta = []; // [[mvHash, valueHash], ...]

  if (incGoals.length >= 2) {
    // byInput: inputHash → incGoal
    const byInput = new Map();
    for (const g of incGoals) byInput.set(g.input, g);

    // byOutput: outputHash → incGoal (only metavar outputs)
    const byOutput = new Map();
    ...
    for (const head of heads) {
      // Check if chain head has ground input (can be resolved directly)
      const headInput = apply(head.input, combinedTheta);
      if (Store.tag(headInput) === 'metavar') continue; // non-ground, skip

      // Walk chain, resolving each link directly via FFI
      const chain = [head];
      let current = head;
      while (true) {
        const next = byInput.get(current.output);
      ...
      // Resolve entire chain via arithmetic
      let currentVal = headInput;
      for (const link of chain) {
        const factHash = resolver(apply(link.hash, combinedTheta));
        if (factHash === null) {
          // Fallback: compute directly. inc(X) → X+1.
          const inputInt = binToInt(currentVal);
          if (inputInt === null) break;
          const nextVal = require('./ill/ffi/convert').intToBin(inputInt + 1n);
          // Add to theta: link.output → nextVal
          if (Store.tag(link.output) === 'metavar') {
            const newEntry = [link.output, nextVal];
            // Compose: apply to existing theta values
            for (let j = 0; j < combinedTheta.length; j++) {
              combinedTheta[j][1] = apply(combinedTheta[j][1], [newEntry]);
            }
            combinedTheta.push(newEntry);
          }
          incChainResolved.add(link.index);
          currentVal = nextVal;
          continue;
        }
        // Use resolver result: unify to get the output binding
        const applied = apply(link.hash, combinedTheta);
        const theta_i = unify(applied, factHash);
        if (theta_i === null) break;
        // Compose theta_i into combinedTheta
        for (let j = 0; j < combinedTheta.length; j++) {
        ...
        incChainResolved.add(link.index);
        currentVal = Store.child(factHash, 1); // output value
      }
    }
  }

  // Phase 2: Resolve remaining goals with running theta (handles transitive deps).
  const resolvedIndices = new Set(incChainResolved);
  const resolvedPreds = [];

  for (let i = 0; i < ante.persistent.length; i++) {
    if (resolvedIndices.has(i)) continue;
    const goal = apply(ante.persistent[i], combinedTheta);
    const factHash = resolver(goal);
    if (factHash === null) continue;

    const theta_i = unify(goal, factHash);
    ...
    resolvedPreds.push(getPredicateHead(ante.persistent[i]) || 'unknown');
  }

  if (resolvedIndices.size === 0) return rule; // nothing resolved

  // Apply combined theta and reassemble — ONCE
  const applyAll = arr => arr.map(h => apply(h, combinedTheta));

  const newAnteLinear = applyAll(ante.linear);
  const newAntePersistent = sortPersistentGoals(
    ante.persistent
  ...
  const newAnteGrade0 = applyAll(ante.grade0);

  const newConseqLinear = applyAll(conseq.linear);
  const newConseqPersistent = applyAll(conseq.persistent);
  const newConseqGrade0 = applyAll(conseq.grade0);

  const anteParts = [
    ...newAnteLinear,
    ...newAntePersistent.map(p => Store.put('bang', [GRADE_W, p])),
    ...newAnteGrade0.map(p => Store.put('bang', [GRADE_0, p])),
  ];
  const anteNew = buildRightTensor(anteParts);

  const conseqParts = [
    ...newConseqLinear,
    ...newConseqPersistent.map(p => Store.put('bang', [GRADE_W, p])),
    ...newConseqGrade0.map(p => Store.put('bang', [GRADE_0, p])),
  ];
  const conseqBodyHash = buildRightTensor(conseqParts);
  const conseqNew = Store.put('monad', [conseqBodyHash]);
  const fullHash = Store.put('loli', [anteNew, conseqNew]);
  ...
  return {
    name: rule.name + resolvedSuffix,
    hash: fullHash,
    antecedent: anteNew,
    consequent: conseqNew,
    sourceLabel: rule.sourceLabel || null,
  };
}
 ...
 * Replaces the old _resolveResidualGoals fixpoint + _fuseIncChains.
 *
 ...
 */
function _fuseBasicBlocks(pool, rc, getModeMeta, linearFusionPredicate) {
  const cutPred = linearFusionPredicate || 'pc';
  ...
  for (let ri = 0; ri < pool.length; ri++) {
    ...
    // Scan for hidden producers inside oplus/with/exists
    ...
    // Producer: cutPred(GROUND) in consequent linear (only from choice-free consequents)
  ...
  for (const chain of chains) {
    ...
    for (let i = 1; i < chain.length; i++) {
      ...
      const fused = fuseLinearPair(current, next, cutPred, rc, getModeMeta);
      if (!fused) { break; }
      current = fused;
      fusedUpTo = i;
    }
...
// ─── Pass 6: SROA — stack decomposition ─────────────────────────────────────

/**
 * SROA (Scalar Replacement of Aggregates) for array-backed resources.
 *
 * When a fused rule has persistent arr_get/arr_set goals with ground indices
 * on an array held by a linear resource (e.g. stack([TOP | REST])), SROA
 * expands the array pattern into individual slots and eliminates the goals.
 *
 * Example: stack([X | REST]) * !arr_get(REST, 0, Y) * !arr_set(REST, 0, X, REST')
 *   becomes: stack([X, S0 | TAIL]) with Y=S0, REST'=[X | TAIL]
 *
 * @returns {{ rules: Object[], sroaCount: number }}
 */
function _sroaStackDecomposition(pool, rc, getModeMeta, sroaConfig) {
  const cfg = sroaConfig || DEFAULT_SROA_CONFIG;
  const result = [];
  let sroaCount = 0;
  let mccarthyCount = 0;
  ...
  for (const rule of pool) {
    if (rule.name.includes('+')) {
      // Phase A: McCarthy normalization — peel acons arr_get/arr_set goals.
      // arr_get([H|T], 0, V) → V=H (direct), arr_get([H|T], N, V) → arr_get(T, N-1, V) (reduced).
      const normalized = _mccarthyNormalize(rule, rc, cfg);
      const effective = normalized || rule;
      if (normalized) mccarthyCount++;

      // Phase B: SROA — expand stack cons pattern for remaining metavar arr goals.
...
//
// When arr_get/arr_set goals have an acons (cons cell) as their array argument,
// the McCarthy array axioms reduce them structurally:
//   arr_get([H|T], 0, V)  →  V = H                    (read-head)
//   arr_get([H|T], N, V)  →  arr_get(T, N-1, V)       (read-tail, N>0)
//   arr_set([H|T], 0, W, R)  →  R = [W|T]             (write-head)
//   arr_set([H|T], N, W, R)  →  arr_set(T, N-1, W, I), R = [H|I]  (write-tail)
//
 ...
 * Peels acons layers from arr_get/arr_set goals, resolving or reducing them.
...
function _mccarthyNormalize(rule, rc, sroaConfig) {
  ...
  for (const g of arrGoals) {
    ...
    if (!isGround(g.idx) || binToInt(g.idx) === null) continue;
    aconsGoals.push(g);
  }
  ...
  for (const g of aconsGoals) {
    ...
    const idxNum = Number(binToInt(g.idx));

    if (g.type === 'get') {
      if (idxNum < depth) {
        // Direct resolution: V = heads[idx]
        theta.push([g.outVar, heads[idxNum]]);
        eliminatedGoals.add(g.hash);
      } else {
        // Reduced goal: arr_get(base, idx-depth, V)
        const newIdx = intToBin(BigInt(idxNum - depth));
      ...
      if (idxNum < depth) {
        // Direct resolution: replace heads[idx] with val, rebuild chain
        ...
        // Reduced goal: arr_set(base, idx-depth, val, innerOut)
        // outVar = [heads... | innerOut]
...
function _trySROA(rule, rc, sroaConfig) {
  ...
  // nested several levels deep (e.g., stack([TOP | [H | REST]]) with arrVar=REST).
  ...
  // Phase 2: Collect version chain (fixpoint over arr_set outputs).
  const chainGoals = [];
  ...
  while (changed) {
    ...
    for (const g of arrGoals) {
      ...
      if (!isGround(g.idx) || binToInt(g.idx) === null) return null;
      chainGoals.push(g);
  ...
  let maxIdx = 0n;
  for (const g of chainGoals) { const i = binToInt(g.idx); if (i > maxIdx) maxIdx = i; }
  ...
  // (b) Trace version chain: arr_get(V, idx, X) → X = V_slots[idx]
  //                           arr_set(V, idx, val, V') → V' = modified slots
  ...
  while (processQueue.length > 0) {
    ...
    for (const g of chainGoals) {
      ...
      theta.push([g.outVar, curSlots[Number(binToInt(g.idx))]]);
    }
    for (const g of chainGoals) {
      if (g.arrVar !== curVar || g.type !== 'set') continue;
      const idx = Number(binToInt(g.idx));
 ...
 * Handles structured output patterns from arr_set (e.g. [?H | ?T] from fusion).
...
/**
 * Collect arr_get and arr_set persistent goals with their components.
 */
/** Default SROA configuration: arr_get/arr_set on stack resource. */
const DEFAULT_SROA_CONFIG = { arrayPreds: ['arr_get', 'arr_set'], resourcePred: 'stack' };
...
function composeGrade0(compiledRules, connectives, getModeMeta, clauses, definitions, extraGrade0Facts, scopeGuard, opts) {
  ...
  const chainConfigs = opts && opts.chainFusionPredicates || null; // null = DEFAULT_CHAIN_CONFIGS
  const linearFusionPredicate = opts && opts.linearFusionPredicate || null; // null = 'pc'
  const sroaConfig = opts && opts.sroaConfig || null; // null = DEFAULT_SROA_CONFIG
    ...
    for (const pred of elimOrder) {
      ...
      for (const rule of pool) {
        ...
        for (const fact of candidates) {
          ...
          if (result) {
            // Transitive resolution: resolve inc/plus goals that became ground
            if (residualResolver) {
  ...
  // Replaces separate inc chain fusion + fixpoint residual resolution.
  // Most work already done during transitive resolution in specialization loop.
  ...
  if (doFuse && specializedPool.length > 0) {
    const fuseResult = _fuseBasicBlocks(specializedPool, rc, getModeMeta, linearFusionPredicate);
  ...
  // Fused mega-rules accumulate threading chains (e.g. N checked_sub goals).
  // Collapse them: checked_sub(G,3,G2)*checked_sub(G2,5,G3) → checked_sub(G,8,G3).
  if (doFuse && specializedPool.length > 0) {
  ...
  // Fusion may create newly-ground goals (e.g. arr_get with ground inputs).
  // Resolve them before SROA.
  if (residualResolver && doFuse && specializedPool.length > 0) {
    specializedPool = _resolveResidualBatch(specializedPool, rc, getModeMeta, residualResolver);
  }

  // ── Pass 6: McCarthy normalization + SROA — stack decomposition ─────
  // McCarthy: peel acons arr_get/arr_set goals (read-head, read-tail axioms).
  // SROA: expand stack cons pattern, eliminate remaining metavar arr goals.
  ...
  // McCarthy + SROA ground variables (arr slot bindings), enabling resolution
  // of dependent goals (inc, plus, checked_sub) that were previously blocked.
  if (residualResolver && doFuse && specializedPool.length > 0) {
    specializedPool = _resolveResidualBatch(specializedPool, rc, getModeMeta, residualResolver);
  }
...
module.exports = { composePair, specializePersistent, fuseLinearPair, _fuseIncChains, _fuseAdditiveChains, DEFAULT_CHAIN_CONFIGS, _resolveResidualOnce, _resolveResidualBatch, _sroaStackDecomposition, DEFAULT_SROA_CONFIG, buildPredicateMap, buildEliminationOrder, composeGrade0 };
