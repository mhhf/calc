...
function _tablingCacheKey(clauses, definitions) {
  const parts = [];
  if (clauses) for (const [, cl] of clauses) parts.push(cl.hash);
  if (definitions) for (const [, dh] of definitions) parts.push(dh);
  return parts.join(',');
}
...
function _composeFullKey(compiledRules, clauses, definitions, extraGrade0Facts, hasResolver) {
  ...
  for (let i = 0; i < compiledRules.length; i++) parts.push(compiledRules[i].hash);
  if (extraGrade0Facts) {
    for (const [, facts] of extraGrade0Facts) {
      for (const f of facts) parts.push(f.hash);
    }
  }
