...
import { hasMetavars as _hasMV } from './pattern-utils.js';
// Debug soundness fence (TODO_0307 P3): CALC_FENCE_MV=1 makes any raw metavar
// in a PRODUCED fact a loud error at the produce funnel — a fired rule must
// never emit an open pattern slot into a live state (doc/def/0046). One
// short-circuited boolean per produced fact when off; a diagnostic for the
// fused-block resolver, not a hot-path check.
const _FENCE_MV = typeof process !== 'undefined' && process.env.CALC_FENCE_MV === '1';
/**
...
function produce(linear, patterns, theta, slots, rule, optimized, arena, canon, canonPats) {
  ...
  for (let i = 0; i < patterns.length; i++) {
    ...
    if (_FENCE_MV && _hasMV(h)) throw new Error(`CALC_FENCE_MV: metavar in produced linear fact — rule=${rule && rule.name} pattern=${pattern} h=${h}`);
...
function producePers(persistent, patterns, theta, slots, rule, arena, canon, canonPats) {
  ...
  for (let i = 0; i < patterns.length; i++) {
    ...
    if (_FENCE_MV && _hasMV(h)) throw new Error(`CALC_FENCE_MV: metavar in produced persistent fact — rule=${rule && rule.name} pattern=${patterns[i]} h=${h}`);
