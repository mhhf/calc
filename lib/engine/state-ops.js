...
import { filterPres } from './preserved.js';
/**
...
function produce(linear, patterns, theta, slots, rule, optimized, arena) {
  ...
  for (let i = 0; i < patterns.length; i++) {
    ...
    const h = compiledSub(pattern, i, theta, slots, cLinear, subApplyIdx);
 ...
 * @param {Arena|null} arena - undo arena
 */
function producePers(persistent, patterns, theta, slots, rule, arena) {
  const cPersistent = rule && rule.compiledConseqPersistent;
  for (let i = 0; i < patterns.length; i++) {
    const h = compiledSub(patterns[i], i, theta, slots, cPersistent, subApplyIdx);
...
function mutateState(state, consumed, theta, linearPatterns, persistentPatterns, slots, rule, linArena, perArena) {
  ...
  produce(state.linear, linearPatterns, theta, slots, rule, !!rule, linArena);
  producePers(state.persistent, persistentPatterns, theta, slots, rule, perArena);
}

export { filterPres, compiledSub, consume, produce, producePers, mutateState };
export default { filterPres, compiledSub, consume, produce, producePers, mutateState };
