...
import { hashString, hashCombine2, hashBigInt } from '../../hash.js';
const MAGIC = 0x43414C43; // "CALC"
const VERSION = 9; // v9: μMALL fixpoint tags 'mu'/'nu' appended (TODO_0009 rung 3) — PRED_BOUNDARY shifted 35→37.
// v8: monad became binary (D6 merge-back) and 'gmonad' is gone —
                   // PRED_BOUNDARY shifts and every monad hash changes; v7: till kernel tags —
                   // pre-shift snapshots must be rejected, so the version gates them out.
                   // (v6: flat childOff + childBuf, replacing fixed-width child0-4 + overflow)
...
function compact(snap) {
  ...
  // ── Tag classification (which tags have non-ID children) ──
  // Store contract: child KIND is determined by the tag alone and applies to
  // EVERY child (see reconstructChild) — e.g. ratlit has two bigint children.
  const STRING_CHILD = new Set(['atom', 'freevar', 'metavar', 'strlit']);
  const BIGINT_CHILD = new Set(['binlit', 'bound', 'evar', 'ratlit']);
  const ARRAY_CHILD = new Set(['arrlit']);

  const nonIdChild = new Uint8Array(256); // indexed by tagId
  const strChild = new Uint8Array(256);
  const bigChild = new Uint8Array(256);
  const arrChild = new Uint8Array(256);
  const charlitTag = tagNames.indexOf('charlit');
  for (let i = 0; i < tagNames.length; i++) {
    if (STRING_CHILD.has(tagNames[i])) { nonIdChild[i] = 1; strChild[i] = 1; }
    if (BIGINT_CHILD.has(tagNames[i])) { nonIdChild[i] = 1; bigChild[i] = 1; }
    if (ARRAY_CHILD.has(tagNames[i])) { nonIdChild[i] = 1; arrChild[i] = 1; }
    if (i === charlitTag) nonIdChild[i] = 1;
  }
  ...
  while (stack.length > 0) {
    ...
    if (!nonIdChild[tag]) {
      for (let ci = 0; ci < ar; ci++) mark(childBuf[off + ci]);
    }
    // Walk array elements (arrlit child0 is array table index)
    if (arrChild[tag] && ar >= 1) {
...
export { serialize, deserialize, compact, crc32 };
export default { serialize, deserialize, compact, crc32 };
