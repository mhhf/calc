...
const { createPredictionCtx, predictNext } = require('./opt/prediction');
...
function explore(initialState, rules, opts = {}) {
  ...
  // Prediction context (Opt_H) — threaded code dispatch
  const predCtx = createPredictionCtx(ruleList, fpConfig, state);
  ...
  function go(depth, predicted) {
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        const pred = predictNext(m, predCtx);
