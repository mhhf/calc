 ...
 * Uses stateIndex for O(1) access to pc and sh facts.
 *
 ...
 * PC and SH execute the same instruction sequence. Other differences
 * (concrete stack values, code fact counts from loop iterations, persistent
 * constraints) don't affect subtree STRUCTURE when branching is driven
 * by symbolic values.
 *
 ...
 * excluded from this hash (member-specific eq/neq constraints).
 */
function computeControlHash(stateIndex) {
  const pcArr = stateIndex['pc'];
  const shArr = stateIndex['sh'];
  const pcVal = (pcArr && pcArr.length > 0) ? Store.child(pcArr[0], 0) : 0;
  const shHash = (shArr && shArr.length > 0) ? shArr[0] : 0;
 ...
 *     When enabled, subtrees starting from states with the same PC, SH,
 *     and predicate histogram are memoized. Sound when branching depends
 *     only on symbolic (evar/freevar) values, not concrete argument values.
 */
function explore(initialState, rules, opts = {}) {
  ...
  function go(depth, ctx) {
    ...
    if (globalControl) {
      const ch = computeControlHash(ctx.stateIndex);
      if (globalControl.get(ch) === true) {
        return { type: 'memo', state: snapshotState(state) };
      }
      // Store control hash + bound counter for later recording
      ctx._controlHash = ch;
      ctx._boundBefore = boundCount;
    }
    ...
    if (globalControl && ctx._controlHash !== undefined) {
      const bc0 = ctx._boundBefore;
      if (bc0 !== undefined && boundCount === bc0) {
        globalControl.set(ctx._controlHash, true);
      }
    }
