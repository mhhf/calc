...
function explore(initialState, rules, opts = {}) {
  ...
  // Auto-convert code facts to bytecode arrlit (plain objects only)
  let effectiveState = initialState;
  if (initialState.linear && !initialState.linear.group) {
    const { codeToArrlit } = require('./index');
    effectiveState = codeToArrlit(initialState);
  }

  // Accept both plain objects and State objects
  const state = effectiveState.linear && effectiveState.linear.group
    ? effectiveState.clone()  // Clone so we don't modify caller's state
    : fromObject(effectiveState.linear, effectiveState.persistent);
