...
function explore(initialState, rules, opts = {}) {
  ...
  // Constraint solver for branch pruning (eq/neq) — only when oplus rules exist
  const hasMultiAlt = ruleList.some(r => r.consequentAlts.length > 1);
  const solver = hasMultiAlt ? new EqNeqSolver() : null;
  if (solver) {
  ...
  function go(depth) {
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        const scp = solver ? solver.checkpoint() : 0;
