...
const { makeDiscTreeLayer } = require('./disc-tree.js');
 ...
 *
 * Flat log eliminates 3 object allocations per step ({ linearOrig, persistentOrig, wrapper })
 * and avoids for-in + Number() conversion overhead during undo/makeChildCtx iteration.
 *
...
function mutateState(state, consumed, theta, linearPatterns, persistentPatterns, slots, rule) {
  ...
  // This dedup is required for makeChildCtx correctness: it reads each entry's oldValue
  // to compute hash deltas, so duplicate entries would corrupt the hash.
  const seenLinear = {};
 ...
 * @param {Object} state - Mutable { linear, persistent }
 * @param {number[]} log - Flat undo log [type, hash, oldValue, ...]
 */
...
function findAllMatches(state, rules, calc, strategy, stateIndex) {
  ...
  // Also find fireable loli continuations in state
  if (matches.length === 0) {
    const loliMatches = findAllLoliMatches(state);
    for (const lm of loliMatches) {
      matches.push({
        rule: {
          name: lm.name,
          consequent: lm.produced,
          consequentAlts: [lm.produced],
          analysis: null,
