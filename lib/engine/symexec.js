...
const { fromObject, toObject, Arena, INSERT_OP } = require('./fact-set');
...
function explore(initialState, rules, opts = {}) {
  ...
  function go(depth) {
    ...
    if (pathVisited.has(sh)) {
      return { type: 'cycle', state: toObject(state) };
    }

    if (globalVisited.has(sh)) {
      return { type: 'memo', state: toObject(state) };
    }
    ...
    if (depth >= maxDepth) {
      boundCount++;
      return { type: 'bound', state: toObject(state) };
    }

    const matches = findAllMatches(state, indexedRules, calc, strategy);

    if (matches.length === 0) {
      return { type: 'leaf', state: toObject(state) };
    }
