...
function serialize(snapshot) {
  const { nodeCount, tags, arities, child0, child1, child2, child3,
          dedupHashes, strings, bigints, tagNames, metadata } = snapshot;
  ...
  totalSize += 4 * nodeCount * 4; // child0-3
...
function deserialize(buffer) {
  ...
  // SoA arrays
  const tags = new Uint8Array(buffer.buffer, buffer.byteOffset + pos, nodeCount); pos += nodeCount;
  const arities = new Uint8Array(buffer.buffer, buffer.byteOffset + pos, nodeCount); pos += nodeCount;
  const soaPad = (2 * nodeCount) % 4 === 0 ? 0 : 4 - (2 * nodeCount) % 4;
  pos += soaPad;
  ...
  return {
    nodeCount,
    tags: new Uint8Array(tags),  // copy so it's independent of buffer
    arities: new Uint8Array(arities),
...
module.exports = { serialize, deserialize, crc32, MAGIC, VERSION };
