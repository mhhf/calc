...
const { resolveConnectives, expandChoiceItem, expandConsequentChoices } = require('./compile');
const { buildFingerprintIndex } = require('./forward');
const match = require('./match');
const { tryMatch } = match;
const { clearBackwardCache: lnlClearCache } = require('./lnl/persistent');
...
const { drainPersistentLolis } = require('./lnl/loli-drain');
...
const { feedPersistent, filterAltsBySAT } = require('./constraint-feed');
...
function explore(initialState, rules, opts = {}) {
  ...
  const rc = ct ? resolveConnectives(ct) : {};
  ...
  if (!useFFI && initialState.linear && !initialState.linear.group) {
    const _bytecodeToTrie = calc?.domainConfig?.bytecodeToTrie || require('./ill/bytecode-normalize').bytecodeToTrie;
  ...
  const predictNext = createPredictNext(ruleList, fpConfig, state, _predOpts);
  ...
  const _predMatchOpts = match.buildMatchOpts({
  ...
  function go(depth, predicted) {
    ...
    let boundBefore;
    if (memoCtx) {
      controlHash = computeControlHash(state, controlOpts);
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        drainPersistentLolis(state, linArena, perArena, calc, null, _predMatchOpts);
