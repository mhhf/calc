...
const { resolveConn, expandChoice, expandConsqChoices } = require('./compile');
...
const { defaultTheories, buildCanonicalizer } = require('../kernel/eq-theory');

// ─── Opt module lazy accessors (eliminate Generic→opt import-time coupling) ───
let _structuralMemo = null, _prediction = null;
function _getStructuralMemo() { return _structuralMemo || (_structuralMemo = require('./opt/structural-memo')); }
function _getPrediction() { return _prediction || (_prediction = require('./opt/prediction')); }
...
function explore(initialState, rules, opts = {}) {
  ...
  const onProveFail = opts.onProveFail || null;
  const onProveSuccess = opts.onProveSuccess || null;
  ...
  if (!useFFI && initialState.linear && !initialState.linear.group) {
    const _bytecodeToTrie = calc?.domainConfig?.bytecodeToTrie
      || require('./ill/bytecode-normalize').bytecodeToTrie;
    effectiveInitial = _bytecodeToTrie(initialState);
  }
  ...
  const _ffiCtx = calc?.ffiContext || null;
  ...
  require('./backchain').ensureIndex(calc);
  ...
  const predictor = _getPrediction().predictNext(ruleList, fpConfig, state, _predOpts);
  const canonicalize = opts.canonicalize || calc?.canonicalize || buildCanonicalizer(defaultTheories);
  const backchainUseFFI = opts.useFFI !== undefined ? opts.useFFI : true;
  // Prefer orchestrator-injected matchOpts (all layer callbacks pre-resolved).
  // Fallback builds locally — only for direct callers that bypass orchestrator.
  let _predMatchOpts = opts.matchOpts;
  if (!_predMatchOpts) {
    const { execExStep: _execExStep } = require('./opt/existential-compile');
    const { drainLolis: _drainLolisFn } = require('./lnl/loli-drain');
    _predMatchOpts = match.buildMatchOpts({
      useFFI, evidence, rc, ffiCtx: _ffiCtx, canonicalize,
      onProveFail, onProveSuccess, backchainUseFFI,
      optimizePreserved: true, execExStep: _execExStep,
      drainLolis: _drainLolisFn,
    });
  }

  // Get drainLolis from matchOpts (injected by orchestrator or fallback above)
  const _drainLolis = _predMatchOpts.drainLolis;
  ...
  const memoCtx = useStructuralMemo ? _getStructuralMemo().createMemoCtx() : null;
  ...
  function go(depth, predicted) {
    ...
    let boundBefore;
    if (memoCtx) {
      ctrlHash = _getStructuralMemo().controlHash(state, controlOpts);
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        _drainLolis(state, linArena, perArena, calc, null, _predMatchOpts);
    ...
    if (memoCtx && ctrlHash !== undefined) {
      _getStructuralMemo().recordMemo(ctrlHash, boundBefore, memoCtx);
    }
...
module.exports = {
  ...
  mutateState,
  // Re-exported via lazy accessor — callers should import from opt/structural-memo.js directly
  get controlHash() { return _getStructuralMemo().controlHash; },
};
