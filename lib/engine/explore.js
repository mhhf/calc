...
const { controlHash, createMemoCtx, recordMemo } = require('./opt/structural-memo');
const { predictNext } = require('./opt/prediction');
...
function explore(initialState, rules, opts = {}) {
  ...
  const predictor = predictNext(ruleList, fpConfig, state, _predOpts);
  ...
  const _predMatchOpts = opts.matchOpts || match.buildMatchOpts({
  ...
  // Get drainLolis from matchOpts (injected by orchestrator or buildMatchOpts)
  const _drainLolis = _predMatchOpts.drainLolis || _drainLolisFn;
  ...
  function go(depth, predicted) {
    ...
    let boundBefore;
    if (memoCtx) {
      ctrlHash = controlHash(state, controlOpts);
    ...
    if (memoCtx && ctrlHash !== undefined) {
      recordMemo(ctrlHash, boundBefore, memoCtx);
    }
