...
import { fromObject, toObject, Arena } from './fact-set.js';
import { filterPres, mutateState, canonObject } from './state-ops.js';
import { ruleSetDigest, initialStateDigest } from './certify-confluence.js';
// --- Cycle detection ---
...
function explore(initialState, rules, opts = {}) {
  ...
  // (Backward-cache lifecycle lives at the composition root — index.js
  // clears before delegating here; see opt/backward-cache.js for the
  // soundness argument.)
  ...
  // Accept both plain objects and State objects. Plain objects cross the
  // state-entry boundary: canonicalize each fact (state-canonicity
  // invariant — live State objects are canonical already).
  const _entryCanon = (opts.matchOpts && opts.matchOpts.canonicalize) || null;
  const state = effectiveInitial.linear && effectiveInitial.linear.group
    ...
    : fromObject(canonObject(effectiveInitial.linear, _entryCanon),
                 canonObject(effectiveInitial.persistent, _entryCanon));
  ...
  // Confluence certificate (TODO_0309 P2, THY_0036): under a VALID
  // certificate for exactly this rule set + initial state, all
  // interleavings converge — explore commits to one (rule-choice
  // branching collapses; ⊕/existential branching is certificate-
  // excluded). Digest mismatch is a loud error, never a silent fall
  // through: a stale certificate must not prune a genuinely branching
  // exploration.
  const _cert = opts.confluence || null;
  if (_cert) {
    if (_cert.confluent !== true) {
      throw new Error('explore: opts.confluence is not a confluent certificate (certifyConfluence refused)');
    }
    if (_cert.rulesDigest !== ruleSetDigest(ruleList)) {
      throw new Error('explore: confluence certificate rule-set digest mismatch — re-certify against these rules');
    }
    const _plainInit = initialState.linear && initialState.linear.group
      ? toObject(initialState) : initialState;
    if (_cert.stateDigest !== initialStateDigest(_plainInit)) {
      throw new Error('explore: confluence certificate initial-state digest mismatch — re-certify against this state');
    }
  }
  ...
  // Order guards (task #84): the declared order comparators (cc.domain
  // .constraintPreds.order) INTERSECTED with the certified total decision
  // procedures (calc.decidablePreds, §6.1′). A declared-but-uncertified guard
  // is refused here — never pruned on an unbacked comparator semantics.
  let _orderPreds;
  const _declaredOrder = _domCfg?.constraintPreds?.order;
  if (_declaredOrder) {
    // Refuse by default: absent decidablePreds ⇒ certify NOTHING (an order
    // guard is pruned only on a §6.1′-certified comparator), so soundness does
    // not rest on the invariant that decidablePreds is always populated.
    const _dec = calc?.decidablePreds || new Set();
    _orderPreds = {};
    for (const name in _declaredOrder) {
      if (_dec.has(name)) _orderPreds[name] = _declaredOrder[name];
    }
  }
  const solver = new EqNeqSolver({
    evalNumeric: _evalNumeric, predNames: _domCfg?.constraintPreds, order: _orderPreds,
  });
  ...
  function go(depth, predicted) {
    ...
    // Certified-confluent: every interleaving converges (THY_0036) —
    // commit to the first enabled step instead of branching per rule.
    if (_cert && matches.length > 1) matches = [matches[0]];
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        mutateState(state, m.consumed, m.theta,
          ...
          matchOpts.canonicalize || null, m.rule.canonPatterns || null);
        // Per-edge step witness for the forward-tree checker (TODO_0045):
        // the θ (+ fired loli token) is the ONLY thing forward-check trusts;
        // consumed/produced are re-derived from the rule. alt = 0 (base
        // consequent). Evidence-only — zero cost otherwise.
        const _step0 = evidence ? {
          theta: m.theta.slice(), alt: 0, loliHash: m.loliHash || null,
        } : null;
        ...
        const g2Added = feedPers(solver, perArena, perCp);

        // G2 tell-consistency (THY_0039 §4): a ground tell of a false
        // constraint atom (e.g. !eq 1 0) drives this leaf's denotation to ∅
        // — a zombie the current single-alt path would otherwise count as a
        // reachable world. The accumulated solver already knows it is UNSAT
        // (multi-alt ⊕ guards are satFilter-pruned; only this single-alt tell
        // path skipped the check). Prune it — a sound refinement, never loss.
        // checkSAT runs only when a constraint was actually added, so
        // unflagged steps (the corpus's hot path) cost nothing.
        if (g2Added && !solver.checkSAT()) {
        ...
        children.push({ rule: m.rule.name, child, ...(_step0 ? { step: _step0 } : {}) });
      } else {
        ...
        if (satAlts.length === 0) {
          ...
          // G2 tell-consistency (THY_0039 §4): satFilter validated this alt's
          // OWN guards pre-mutation, but mutateState/drainDynamicRules can add
          // FRESH ground constraint tells (e.g. a drained loli asserting a false
          // !lt) that would leave a zombie leaf counted as a live world. The
          // sibling single-alt and multi-survivor paths prune it; this collapsed
          // path must too. checkSAT runs only when a constraint was added.
          const g2Added = feedPers(solver, perArena, perCp);
          if (g2Added && !solver.checkSAT()) {
            if (traceStack) traceStack.pop();
            state.persistent.undo(perArena, perCp);
          ...
          children.push({ rule: m.rule.name, child,
            ...(evidence ? { step: { theta: m.theta.slice(), alt: i, loliHash: m.loliHash || null } } : {}) });
        } else {
