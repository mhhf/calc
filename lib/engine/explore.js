...
const { provePersistentNaive, clearBackwardCache: lnlClearCache } = require('./lnl/persistent');
const { ffiParsedModes: _ffiParsedModes } = require('./opt/ffi');
...
const { drainPersistentLolis } = require('./ill/loli-drain');
...
function explore(initialState, rules, opts = {}) {
  ...
  const { ILL_CONNECTIVES: _defaultCT } = require('./ill/connectives');
  const ct = calc?.connectives || _defaultCT;
  ...
  if (!useFFI && initialState.linear && !initialState.linear.group) {
    const { bytecodeToTrie } = require('./index');
    effectiveInitial = bytecodeToTrie(initialState);
  }
  ...
  const canonicalize = buildCanonicalizer([...defaultTheories, binlitTheory]);
  const backchainUseFFI = opts.useFFI !== undefined ? opts.useFFI : true;
  const _predMatchOpts = {
    optimizePreserved: true,
    evidence: evidence || undefined,
    provePersistent: useFFI
      ? match.provePersistent
      : (patterns, startIdx, theta, slots, state, calc, evidenceOut) =>
          provePersistentNaive(patterns, startIdx, theta, slots, state, calc, evidenceOut, _predMatchOpts),
