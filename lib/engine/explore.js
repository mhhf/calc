...
function explore(initialState, rules, opts = {}) {
  ...
  const _predMatchOpts = opts.matchOpts || null;
  const _drainLolis = _predMatchOpts ? _predMatchOpts.drainLolis : null;
  ...
  function go(depth, predicted) {
    ...
    for (let mi = 0; mi < matches.length; mi++) {
      ...
      if (alts.length <= 1) {
        ...
        if (_drainLolis) _drainLolis(state, linArena, perArena, calc, null, _predMatchOpts);
