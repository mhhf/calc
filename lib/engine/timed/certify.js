...
import { apply, applyIndexed as subApplyIdx } from '../../kernel/substitute.js';
  ...
  while (changed) {
    ...
    for (const r of ruleList) {
      ...
      const attempt = () => {
        ...
        let done = a;
        if (r.delay) {
          let d = r.delay.ground !== undefined ? r.delay.ground
            : r.delay.term !== undefined ? subApplyIdx(r.delay.term, theta, r.slots || [])
            : theta[r.delay.slot];
