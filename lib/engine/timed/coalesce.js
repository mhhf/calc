 ...
 */
const _fObsCache = new Map();          // formula hash -> { preds, all }
Store.onClear(() => _fObsCache.clear());

function formulaObservers(h, tcfg, into) {
  let c = _fObsCache.get(h);
  if (c === undefined) {
    c = { preds: new Set(), all: false };
...
function _walkFormula(h, tcfg, out, depth) {
  ...
  if (depth > 64) { out.all = true; return; }
  const t = Store.tag(h);
  if (!t || t === 'atom') return;
...
function contribOf(rule, tcfg) {
  ...
  // rule can PRODUCE may observe stamps once it materializes.
...
export { stampObservers, stampObserversCached, coalesce, collectObservers, effectiveExclusion, formulaObservers };
export default { stampObservers, stampObserversCached, coalesce, collectObservers, effectiveExclusion, formulaObservers };
