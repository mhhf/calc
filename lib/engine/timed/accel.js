 ...
 */
function sinkPreds(ruleList, stateLolis, compileLoli, tcfg, state) {
...
function makeAccel(ruleList, tcfg, opts) {
  ...
  return {
    ...
    checkpoint(state, frontierHash, step, lastNondet, horizonHash, stateLolis, eff, extraKey) {
      ...
      curMin = new Map();
      const { inAnte, wildcard, caps, aliveRules } = sinkPreds(ruleList, stateLolis, opts.compileLoli, tcfg, state);
      ...
      const cycles = num >= den ? Number(num / den) : 0;
