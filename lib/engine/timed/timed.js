...
import { stampObserversCached, coalesce } from './coalesce.js';
// ─── Timed config ───────────────────────────────────────────────────
...
function settle(inputState, rules, opts = {}) {
  ...
  const doCoalesce = !!opts.coalesce;
  ...
  while (steps < maxSteps) {
    if (doCoalesce && steps > 0 && (steps & 63) === 0 && lastA !== undefined) {
      _coalesceNow(lastA, true);
    }
    let cands;
    if (sched) {
    ...
    if (cands.length === 0) {
      ...
      return _exit({ state: opts.raw ? state : toObject(state), quiescent: true, steps, events, trace, next: null });
    }
    ...
    if (cmp(aMin, horizon) > 0) {
      ...
      if (doCoalesce) _coalesceNow(horizon, false);
      return _exit({ state: opts.raw ? state : toObject(state), quiescent: true, steps, events, trace, next: aMin });
    }
