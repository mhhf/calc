...
function settle(inputState, rules, opts = {}) {
  ...
  const _rebaseNow = () => {
    ...
    const eff = effectiveExclusion(observers, stateLolis, opts.compileLoli, tcfg);
  ...
  // bound is the last fired activation (STRICT — activations fire in
  // nondecreasing order, so lastA ≤ the next frontier); at return it is
  // the horizon (every pending activation exceeds it).
  const doAccel = !!opts.accelerate;
  ...
  let _accAntePreds = null, _accWildcard = false;
  if (doAccel) {
    _accAntePreds = new Set();
    for (const r of ruleList) {
      for (const p of (r.antecedent.linear || [])) {
        const pred = factKeyOf(p, tcfg.expTag);
        if (pred) _accAntePreds.add(pred);
        else _accWildcard = true;
      }
    }
  }
  let lastNondet = -1;
  ...
  while (steps < maxSteps) {
    ...
    if (cands.length === 0) {
      ...
      return _exit({ state: opts.raw ? state : toObject(state), quiescent: true, steps, events, trace, next: null, ...(accelerated ? { accelerated } : {}) });
    }
    ...
    if (cmp(aMin, horizon) > 0) {
      ...
      if (doCoalesce) _coalesceNow(horizon, false);
      return _exit({ state: opts.raw ? state : toObject(state), quiescent: true, steps, events, trace, next: aMin, ...(accelerated ? { accelerated } : {}) });
    }
    // ── Coalesce/accelerate checkpoint (TODO_0277) — every 64 firings.
    // The frontier is aMin (the next event's instant): stamps strictly
    // below it are dead for every pending activation max. Rewriting
    // invalidates `cands` (hashes change), so a touched state re-enters
    // the loop; the dirty scheduler's caches survive (activation VALUES
    // are coalesce-invariant).
    if (doCoalesce && steps > 0 && (steps & 63) === 0 && steps !== lastCheckStep) {
      lastCheckStep = steps;
      const stateLolis = opts.compileLoli ? _stateLolis(state, tcfg) : null;
      const eff = effectiveExclusion(observers, stateLolis, opts.compileLoli, tcfg);
      ...
      if (accel && !eff.all) {
        ...
        // to the frontier. Ground (absolute) window bounds shift relative
        // to a moving frontier, so time-inhomogeneous rules can never
        // alias two sightings — the false-cycle guard falls out for free.
      ...
      if (rewrote > 0 || jumped) continue;   // cands hold pre-rewrite hashes
    }
    const tied = cands.filter(mm => cmp(mm.activation, aMin) === 0);
    if (doAccel && tied.length > 1 &&
        (_conflicts(tied, state) ||
         tied.some(mm => _feedsInstant(mm, tcfg, _accAntePreds, _accWildcard)))) {
