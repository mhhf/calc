...
function settle(inputState, rules, opts = {}) {
  ...
  while (steps < maxSteps) {
    ...
    if (cands.length === 0) {
      ...
      return { state: opts.raw ? state : toObject(state), quiescent: true, steps, events, trace, next: null };
    }
    ...
    if (cmp(aMin, horizon) > 0) {
      ...
      if (doCoalesce) _coalesceNow(horizon, false);
      return { state: opts.raw ? state : toObject(state), quiescent: true, steps, events, trace, next: aMin };
    }
