...
function lintHypothesisS(ruleList, tcfg, rc = {}) {
  ...
  const flag = (ruleName, persList, via) => {
    for (const q of persList) {
      ...
      // bias exemption (TODO_0298): `bias` is the decimation driver's M8
      // machinery predicate — derived conditioning for wave posteriors is
      // its PURPOSE, not a Hypothesis-S hazard (the driver re-settles
      // between draws; settle-optimality is not the collapse contract).
      if (Store.tag(q) === 'bias') continue;
