...
function observable(inputState, T, tcfg) {
  ...
  for (const hStr in (plain.linear || {})) {
    const h = Number(hStr);
    if (tcfg.availability.cmp(stampOf(h, unit, tcfg.stampTag), T) <= 0) {
      const i = innerOf(h, tcfg.stampTag);
 ...
 *  remaining? }] (remaining = stamp ⊖ T when the effect algebra has a
 *  residual; stamp > T here, so the residual is always defined). */
function pending(inputState, T, tcfg) {
  ...
  for (const hStr in (plain.linear || {})) {
    const h = Number(hStr);
    const s = stampOf(h, unit, tcfg.stampTag);
    if (tcfg.availability.cmp(s, T) > 0) {
      const e = { fact: innerOf(h, tcfg.stampTag), stamp: s, count: plain.linear[hStr] };
      if (tcfg.effect.residual) e.remaining = tcfg.effect.residual(s, T);
      out.push(e);
    }
  }
...
function inFlight(events, T, tcfg) {
  ...
  return events
    ...
    .map(e => {
      ...
      if (tcfg.effect.residual) r.remaining = tcfg.effect.residual(e.done, T);
      return r;
    });
}
