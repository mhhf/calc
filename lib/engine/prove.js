...
function normalizeBinRec(h) {
  ...
  if (tag === 'binlit' || tag === 'strlit' || tag === 'charlit' || tag === 'freevar' || tag === 'metavar') return h;
...
function prove(goal, clauses, types, opts = {}) {
  ...
  // Tag ID for metavar (stable, pre-registered). Metavars are chased through theta.
  ...
  function _chaseFreevar(h) {
    ...
    while (true) {
      ...
      if (!Store.isTerm(v) || Store.tagId(v) !== _MV_TAG) {
  ...
  function _resolveHash(startH) {
    ...
    if (Store.isTerm(startH) && Store.tagId(startH) === _MV_TAG && result !== startH) {
  ...
  function _resolveRec(h) {
    ...
    if (tid === _MV_TAG) {
      h = _chaseFreevar(h);
      if (!Store.isTerm(h) || Store.tagId(h) === _MV_TAG) return h;
      tid = Store.tagId(h);
    }
...
function makeFreshener(counter, prefix) {
  ...
  return function freshen(h) {
    ...
    if (t === 'metavar') {
      if (!renamed.has(h)) {
        renamed.set(h, Store.put('metavar', [Store.child(h, 0) + suffix]));
      }
      return renamed.get(h);
    }
    if (t === 'freevar') return h; // eigenvariable — don't rename
