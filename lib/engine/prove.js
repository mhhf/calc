...
class BackwardProver {
  ...
  _groundMatch(expected, goalH) {
    ...
    if (eTid !== gTid) {
      ...
      if (this._eqRewrite) {
        const rGoal = this._eqRewrite(gTid, goalH, eTid, Store.arity(expected));
        if (rGoal !== null) return this._groundMatch(expected, rGoal);
        const rExp = this._eqRewrite(eTid, expected, gTid, Store.arity(goalH));
  ...
  _matchHead(insts, goalHash, base) {
    ...
    const eqRewrite = this._eqRewrite;
    let sp = 0;
    mStack[sp++] = 0;
    mStack[sp++] = goalHash;
      ...
      switch (inst.op) {
        ...
        case PM_COMPOUND: {
          ...
          if (tid !== inst.tagId || Store.arity(resolved) !== inst.arity) {
            ...
            if (eqRewrite) {
              const rewritten = eqRewrite(tid, resolved, inst.tagId, inst.arity);
  ...
  _search(goalHash, depth) {
    ...
    if (tryFFIFn) {
      ...
      if (ffiResult && ffiResult.success) {
        ...
        return { term: buildTerm ? buildFFITermFn() : null };
      }
    }
    ...
    for (const [name, compiled] of candTypes) {
      ...
      if (this._matchHead(compiled.compiledHead, goalHash, typeBase)) {
        ...
        if (buildTerm) {
          ...
          return { term: buildTypeTermFn(groundGoal) };
        }
        return { term: null };
      }
   ...
   *   - eqRewrite {Function|null} - Equational normalization hook
   * @returns {{ success: boolean, theta: Array|null, term: Object|null, trace: string[]|null }}
   */
  prove(goal, clauses, definitions, opts = {}) {
    ...
    this._eqRewrite = opts.eqRewrite !== undefined
      ? opts.eqRewrite : illDefaults.eqRewrite;
...
function prove(goal, clauses, definitions, opts) {
  return _defaultProver.prove(goal, clauses, definitions, opts);
}
