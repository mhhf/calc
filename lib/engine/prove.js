...
function getCandidates(idx, goalHash) {
  ...
  // When first arg is a variable (fa === '_'), must check ALL buckets —
  // the variable could unify with any constructor.
  if (fa === '_') {
...
function _groundMatch(expected, goalH, goalSlots) {
  ...
  if (gTid === _MV_TAG_ID) {
    // Check goalSlots first
    if (goalSlots) {
      const gs = goalSlots[goalH];
      if (gs !== undefined) {
        const chased = _chaseSlot(gs);
        if (chased.value !== undefined) return _groundMatch(expected, chased.value, goalSlots);
        _bind(chased.slot, expected);
        return true;
      }
    }
...
function prove(goal, clauses, types, opts = {}) {
  ...
  function search(goalHash, goalSlotsCtx, depth) {
    ...
    const { types: candTypes, clauses: candClauses } = getCandidates(idx, goalHash);
