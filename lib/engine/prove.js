...
class BackwardProver {
  ...
  _undo(savedTrail, savedBase) {
    while (this._trailLen > savedTrail) this._theta[this._trail[--this._trailLen]] = undefined;
  ...
  _chaseSlot(startSlot) {
    ...
    for (let hops = 0; hops < 32; hops++) {
      const val = this._theta[slot];
      if (val === undefined) { this._chaseValue = undefined; this._chaseFinalSlot = slot; return; }
      const ns = val < this._mvSlotsLen ? this._mvSlots[val] - 1 : -1;
  ...
  _groundMatch(expected, goalH) {
    ...
    if (gTid === _MV_TAG_ID) {
      const gs = goalH < this._mvSlotsLen ? this._mvSlots[goalH] - 1 : -1;
      if (gs >= 0) {
        this._chaseSlot(gs);
    ...
    if (eTid === _MV_TAG_ID) {
      const eSlot = expected < this._mvSlotsLen ? this._mvSlots[expected] - 1 : -1;
      if (eSlot >= 0) {
  ...
  _matchHead(insts, goalHash, base) {
    let sp = 0;
    this._mStack[sp++] = 0;
    this._mStack[sp++] = goalHash;

    while (sp > 0) {
      const h = this._mStack[--sp];
      const ip = this._mStack[--sp];
      const inst = insts[ip];

      let resolved = h;
      let goalSlot = -1;
      const gs = h < this._mvSlotsLen ? this._mvSlots[h] - 1 : -1;
      if (gs >= 0) {
        this._chaseSlot(gs);
        if (this._chaseValue !== undefined) {
          ...
          switch (inst.op) {
            ...
            case PM_CHECK: {
              ...
              const existing = this._theta[cSlot];
      ...
      switch (inst.op) {
        ...
        case PM_CHECK: {
          ...
          const existing = this._theta[slot];
          if (existing !== undefined) {
        ...
        case PM_COMPOUND: {
          ...
          if (tid !== inst.tagId || Store.arity(resolved) !== inst.arity) {
            if (tid === _ARRLIT_TAG && inst.tagId === _ACONS_TAG && inst.arity === 2) {
              ...
              this._mStack[sp++] = child1Ip;
              this._mStack[sp++] = tailHash;
              this._mStack[sp++] = childIp2;
              this._mStack[sp++] = elems[0];
              break;
            }
            if (this._ephemeralRewrite) {
              const rewritten = this._ephemeralRewrite(tid, resolved, inst.tagId, inst.arity);
          ...
          for (let ci = inst.arity - 1; ci >= 0; ci--) {
            ...
            this._mStack[sp++] = skipIp;
            this._mStack[sp++] = Store.child(resolved, ci);
          }
          break;
        }
      }
  ...
  _executePut(insts, base) {
    ...
    for (let ip = 0; ip < insts.length; ip++) {
      ...
      switch (inst.op) {
        case PUT_GROUND:
          this._putStack[sp++] = inst.hash;
          break;
        case PUT_SLOT: {
          const val = this._theta[base + inst.slot];
          this._putStack[sp++] = val !== undefined ? val : this._getSlotMV(base + inst.slot);
          break;
        }
        case PUT_COMPOUND: {
          const children = new Array(inst.arity);
          sp -= inst.arity;
          for (let i = 0; i < inst.arity; i++) children[i] = this._putStack[sp + i];
          this._putStack[sp++] = Store.put(inst.tagName, children);
          break;
        }
        case PUT_ARRLIT: {
          const elems = new Uint32Array(inst.count);
          sp -= inst.count;
          for (let i = 0; i < inst.count; i++) elems[i] = this._putStack[sp + i];
          this._putStack[sp++] = Store.putArray(elems);
          break;
        }
      }
    }
    return this._putStack[0];
  }
  ...
  _resolveSlots(root) {
    ...
    if (Store.isTerm(h) && Store.tagId(h) === _MV_TAG_ID) {
      const slot = h < this._mvSlotsLen ? this._mvSlots[h] - 1 : -1;
      if (slot >= 0) {
    ...
    while (work.length > 0) {
      ...
      if (Store.isTerm(ph) && Store.tagId(ph) === _MV_TAG_ID) {
        const slot = ph < this._mvSlotsLen ? this._mvSlots[ph] - 1 : -1;
        if (slot >= 0) {
          this._chaseSlot(slot);
  ...
  _search(goalHash, depth) {
    ...
    if (this._tryFFIFn) {
      const ffiResult = this._tryFFIFn(goalHash);
      if (ffiResult && ffiResult.success) {
        if (this._trace) this._trace.push(`${'  '.repeat(depth)}FFI: ${getHead(goalHash)} ✓`);
        if (ffiResult.theta) {
          for (const [varHash, val] of ffiResult.theta) {
            const slot = varHash < this._mvSlotsLen ? this._mvSlots[varHash] - 1 : -1;
            if (slot >= 0 && this._theta[slot] === undefined) {
              this._bind(slot, val);
            }
          }
        }
        return { term: this._buildTerm ? { rule: 'ffi', principal: null, subterms: [] } : null };
      }
    }

    const { types: candTypes, clauses: candClauses } = getCandidates(this._idx, goalHash, this._allBuckets);

    if (this._trace) this._trace.push(`${'  '.repeat(depth)}Goal: ${formatTerm(goalHash)} [${candTypes.length}t/${candClauses.length}c]`);
    ...
    for (const [name, compiled] of candTypes) {
      ...
      if (this._matchHead(compiled.compiledHead, goalHash, typeBase)) {
        if (this._trace) this._trace.push(`${'  '.repeat(depth)}  ✓ ${name}`);
        if (this._buildTerm) {
          const groundGoal = this._normalize(this._resolveSlots(goalHash));
          return {
    ...
    for (const [name, compiled] of candClauses) {
      ...
      for (let pi = 0; pi < compiled.premises.length; pi++) {
        ...
        if (this._buildTerm) premTerms.push(r.term);
      }

      if (ok) {
        if (this._trace) this._trace.push(`${'  '.repeat(depth)}  ✓ ${name}`);
        if (this._buildTerm) {
          const groundHead = this._normalize(this._resolveSlots(
            this._executePut(compiled.headPut, clauseBase)));
          const groundPrems = compiled.premisePuts.map(pp => this._normalize(this._resolveSlots(
            this._executePut(pp, clauseBase))));
          return { term: this._buildClauseTermFn(groundPrems, premTerms, groundHead) };
        }
        return { term: null };
      }
    ...
    if (this._trace) this._trace.push(`${'  '.repeat(depth)}  ✗`);
    return null;
  }
