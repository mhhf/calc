 ...
 *
 * Iterative implementation with explicit stack to avoid call stack overflow
 * on deeply nested terms (e.g., 1000+ deep acons chains from arrlit decomposition).
 */
function _resolveSlots(root) {
  return _resolveOne(root);
}

// Sentinel for rebuild markers on the work stack.
const _REBUILD = {};

// Maximum iterations for _resolveOne to prevent runaway resolution
// (e.g., cycles in metavar bindings across stale module-level state).
const MAX_RESOLVE_ITER = 500_000;

function _resolveOne(root) {
  ...
  if (Store.isTerm(h) && Store.tagId(h) === _MV_TAG_ID) {
    ...
    if (slot >= 0) {
      _chaseSlot(slot);
      if (_chaseValue !== undefined) h = _chaseValue;
      else return h;
    } else return h;
  }
  ...
  // Work stack: interleaved Store hashes and rebuild markers.
  //   - Store hash (number): needs processing
  //   - [_REBUILD, origH, tag, childCount]: all children resolved on results stack
  // Children are pushed in REVERSE order so they're popped left-to-right.
  const work = [];
  const results = [];
  work.push(h);
  let iter = 0;
  ...
  while (work.length > 0) {
    if (++iter > MAX_RESOLVE_ITER) return root; // bail on runaway resolution
    const item = work.pop();

    // Rebuild marker — reconstruct from resolved children
    ...
    // Process a Store hash
    let ph = item;

    // Chase metavar chains
    if (Store.isTerm(ph) && Store.tagId(ph) === _MV_TAG_ID) {
      const slot = ph < _mvSlotsLen ? _mvSlots[ph] - 1 : -1;
      if (slot >= 0) {
        _chaseSlot(slot);
        if (_chaseValue !== undefined) ph = _chaseValue;
    ...
    if (tid === _ARRLIT_TAG) {
      ...
      // Push rebuild marker, then children in reverse order (popped left-to-right)
    ...
    // Push rebuild marker, then ALL children in reverse order.
    // Non-term children (strings, BigInts) pass through to results in processing.
...
// Reset module-level state when Store is cleared (hash IDs get reused).
...
module.exports = { prove, formatTerm, buildIndex, getCandidates, clearState };
