...
/**
 * Get head functor of a term. O(1).
 * Flat: {tag: 'plus', children: [A,B,C]} → 'plus'
...
function getFirstArgCtor(hash) {
  ...
  if (NON_PRED_TAGS.has(node.tag) || node.children.length === 0) return null;
  ...
  if (!NON_PRED_TAGS.has(argNode.tag)) return argNode.tag;
...
function getArgs(hash) {
  const node = Store.get(hash);
  if (!node) return [];
  if (!NON_PRED_TAGS.has(node.tag)) return node.children;
  return [];
}
