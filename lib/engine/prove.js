...
function buildIndex(clauses, types) {
  ...
  for (const [name, clause] of clauses) {
    ...
    const compiled = {
      ...
      // WAM PUT instructions: flat post-order sequences for premise construction.
      // Eliminates recursive _materializePremise tree walk during search.
      ...
      // Avoids getPredicateHead()/getFirstArgHead() on materialized hashes.
...
/**
 * Execute compiled PUT instructions to construct a materialized premise.
 *
 ...
 * @returns {number} Materialized premise Store hash (top of result stack)
 */
...
/**
 * Resolve a precomputed premise index key at runtime.
 *
 * Uses the compile-time key to determine predicate head and first-arg
 * index bucket without calling getPredicateHead()/getFirstArgHead()
 * on the materialized Store hash.
 *
 * @param {Object} key - From compilePremiseKey: { predHead, firstArgSlot, firstArgKey }
 * @param {number} base - Frame base for slot resolution
 * @returns {string} First-argument index key (constructor name or '_')
 */
function _resolvePremiseFA(key, base) {
  if (key.firstArgKey !== null) return key.firstArgKey;
  // First arg is a slot metavar — resolve at runtime
  const val = _theta[base + key.firstArgSlot];
  if (val !== undefined) return getFirstArgHead(val) || '_';
  return '_';
}
