...
function prove(goal, clauses, types, opts = {}) {
  ...
  function _resolveHash(h) {
    ...
    if (!tag) return h;
    if (tag === 'freevar') {
      const v = _thetaMap.get(h);
      if (v === undefined) return h;
      const rv = _resolveHash(v);
      if (rv !== v) _thetaMap.set(h, rv); // memoize
      return rv;
    }
    if (tag === 'atom' || tag === 'binlit' || tag === 'strlit' || tag === 'charlit') return h;
    if (tag === 'arrlit') {
      ...
      for (let i = 0; i < elems.length; i++) {
        const ne = _resolveHash(elems[i]);
        if (ne !== elems[i]) {
          if (!changed) { changed = true; newElems = new Uint32Array(elems.length); for (let j = 0; j < i; j++) newElems[j] = elems[j]; }
          newElems[i] = ne;
        } else if (changed) { newElems[i] = elems[i]; }
      }
