 ...
 *      one linear fact; per single-writer key at most one cell; no
 *      cell coexisting with its guard. Equality here is MODULO the
 *      registered equational theories: the matcher unifies cross-tag
 *      (binlit 3 ~ i(i e)), so hash-distinct theory-equal keys are the
 *      SAME destination and hash-keyed uniqueness would under-check.
 *
 ...
 *
 * PRECONDITIONS (documented, not checked — hold for every shipped
 * calculus by construction):
 *   - Backward proving of persistent goals is MONOTONE: goals are
 *     proved against the persistent component only, with no linear
 *     side effects. A family whose hooks consume linear resources
 *     during backward proofs is outside the discipline.
 *   - Theory-awareness (D2 values, D6 uniqueness) is evaluated against
 *     the theories registered AT CERTIFICATION TIME. Theories ride the
 *     kernel's global table; certify and explore under the same loaded
 *     calculus (the digests pin rules + state, not the theory table).
 */
 ...
 * @param {Object} [opts]
 * @param {Object} [opts.rc] - resolved connectives (implication/existential
 *        tags). REQUIRED: without it D5's dynamic-rule/existential refusals
 *        cannot fire, so its absence is itself a refusal (no-connective-info).
 *        A calculus that lacks those connectives passes rc with the tags
 *        undefined — D5 is then vacuous BY THE CALCULUS, not by omission.
...
function certifyConfluence(rules, initialState, opts = {}) {
  if (!opts.rc) return _refuse('no-connective-info', {});
  const rc = opts.rc;
  ...
  // Ground-term equality MODULO the registered equational theories:
  // the kernel unifier is theory-aware (cross-tag rewriting), so on
  // ground inputs it decides exactly the equality the matcher uses.
  const _thEq = (a, b) => a === b || unify(a, b) !== null;
  ...
  for (const r of rules) {
    ...
    // Compiled existentials: compile.js STRIPS ∃ from the alt and
    // records the fresh slots at rule level (existentialSlots) — the
    // tag-level scan below never sees them, so check the compiled
    // marker first. (The tag scan stays as defense for rule data that
    // did not pass through compile.)
    if (r.existentialSlots && r.existentialSlots.length > 0) {
    ...
    for (let j = i + 1; j < ruleArr.length; j++) {
      ...
      for (const gR of ri.goals) {
        ...
        for (const gS of si.goals) {
          ...
          for (const vi of pu.values) {
            // A contradiction needs values distinct MODULO THEORY:
            // hash-distinct but theory-equal grounds (binlit 3 vs
            // i(i e)) are the same cell value to the matcher — treating
            // them as contradicting would grant a false certificate.
            const vR = Store.child(aR, vi), vS = Store.child(aS, vi);
            if (isGround(vR) && isGround(vS) && !_thEq(vR, vS)) {
              excluded = true;
              break outer;
            }
          }
  ...
  // ── D6: initial-state invariants (equality modulo theory) ──────────
  // Hash-keyed maps would under-check: two hash-distinct theory-equal
  // destinations are ONE destination to the matcher, and admitting both
  // breaks the per-destination uniqueness the diamond argument leans
  // on. Per-pred lists + _thEq scans; states are small at certification.
  const linDests = new Map(); // pred → [destHash, count][]
  for (const hStr of Object.keys(initialState.linear || {})) {
    ...
    let row = linDests.get(pred);
    if (!row) { row = []; linDests.set(pred, row); }
    let entry = row.find(([d0]) => _thEq(d0, d));
    if (!entry) { entry = [d, 0]; row.push(entry); }
    entry[1] += count;
    if (entry[1] > 1) {
      return _refuse('duplicate-destination', { pred, dest: d });
    }
  }
  const cellRows = new Map(); // pred → keyHash[]
  for (const hStr of Object.keys(initialState.persistent || {})) {
    ...
    let row = cellRows.get(pred);
    if (!row) { row = []; cellRows.set(pred, row); }
    // Any second cell at a theory-equal key is a conflict: distinct
    // fact hashes there mean either distinct values (write conflict)
    // or distinct representations (θ picks one nondeterministically —
    // literal final-state equality breaks either way).
    if (row.some((k0) => _thEq(k0, k))) {
      return _refuse('duplicate-persistent-value', { pred, key: k });
    }
    row.push(k);
    const guard = guards[pred];
    const gRow = guard ? linDests.get(guard) : undefined;
    if (gRow && gRow.some(([d0]) => _thEq(d0, k))) {
