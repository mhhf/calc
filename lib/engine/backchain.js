...
import { isPredTag, predHead as getHead } from '../kernel/ast.js';
import { collectMetavars } from './pattern-utils.js';
import { compilePM, PM_BIND, PM_GROUND, PM_COMPOUND, _skipInstruction, compilePut, compileKey, PUT_GROUND, PUT_SLOT, PUT_COMPOUND, PUT_ARRLIT } from './compile.js';
import { buildTheoryLookup, defaultTheories as _defaultTheories, classifyFirstArg as _classifyCompact } from '../kernel/eq-theory.js';
// ============================================================================
// SHARED CONSTANTS
// ============================================================================
...
/**
 * Register an additional leaf tag (non-decomposable in resolution).
 * The static entries above are the kernel's own compact literals; a theory
 * that adds a compact leaf tag registers it here at install time
 * (e.g. ratlit via installRatlitTheory — TODO_0265 Phase 2b).
 */
function registerLeafTag(tagId) {
  _LEAF_TAGS[tagId] = 1;
}
...
function getFirstArgHead(hash) {
  ...
  return _classifyCompact(Store.tagId(firstArg), firstArg);
}
...
class Backchainer {
  constructor() {
    ...
    // Tabling / loop detection (TODO_0009 Axis 3 rung 1, opt-in). When on,
    // _pathGoals[depth] holds the resolved goal hash at each ancestor depth
    // on the current root-to-node path; a goal that recurs identically on
    // its own path is an inductive loop with no finite proof → fail. OFF by
    // default: the hot path pays one predicted branch and nothing else.
    this._loopCheck = false;
    this._pathGoals = [];
    // Coinductive logic programming (TODO_0009 Inc-5b, opt-in). A Set of
    // predicate-head keys declared coinductive: a goal that recurs identically
    // on its path SUCCEEDS (greatest-fixpoint / co-Herbrand semantics) instead
    // of failing — provided the whole cycle lies within coinductive predicates.
    this._coindPreds = null;
  }
  ...
  _bind(slot, value) {
    this._theta[slot] = value;
    // Out-of-bounds writes on a Uint32Array are silently DROPPED — a
    // derivation with more live bindings than the trail can hold would
    // lose undo entries and leave theta poisoned after backtracking
    // (round-14 fuzz find: qnorm on ~2300-iteration divmod chains failed
    // value-dependently). Grow instead of dropping.
    if (this._trailLen >= this._trail.length) {
      const t = new Uint32Array(this._trail.length * 2);
      t.set(this._trail);
      this._trail = t;
    }
    this._trail[this._trailLen++] = slot;
  }
  ...
  _search(initialGoal, initialDepth) {
    ...
    const loopCheck = this._loopCheck, pathGoals = this._pathGoals;
    const coindPreds = this._coindPreds;
    const theta = this._theta, mvSlots = this._mvSlots, mvSlotsLen = this._mvSlotsLen;
    ...
    outer: while (sp > 0) {
      ...
      switch (frame.phase) {
        case _PH_INIT: {
          ...
          // Tabling loop check (opt-in): if the resolved goal already sits on
          // the current path (a strict ancestor), this is an inductive loop —
          // no finite proof descends through it, so fail THIS branch (the
          // parent still tries its other clauses/FFI). Sound and
          // completeness-preserving: an identical goal can yield no proof its
          // ancestor occurrence did not already have. Coinductive success on
          // a guarded loop is rung 2 (the global trace condition).
          if (loopCheck) {
            const rg = this._resolveSlots(frame.goalHash);
            for (let d = 0; d < frame.depth; d++) {
              if (pathGoals[d] === rg) {
                // co-LP coinductive success (Inc-5b): if the recurring goal's
                // predicate is coinductive AND the entire cycle (companion→bud)
                // stays within coinductive predicates, the loop witnesses
                // membership in the greatest fixed point (Simon–Gupta co-LP,
                // sound over the persistent SLD fragment — no linear resource
                // crosses the back-edge). A cycle touching an inductive
                // predicate has no such justification → fail (the rung-1
                // inductive default).
                if (coindPreds && coindPreds.has(getHead(rg))) {
                  let allCoind = true;
                  for (let i = d; i < frame.depth; i++) {
                    if (!coindPreds.has(getHead(pathGoals[i]))) { allCoind = false; break; }
                  }
                  if (allCoind) {
                    if (trace) trace.push(`${'  '.repeat(frame.depth)}⟲ coinductive loop on ${formatTerm(rg)} → success`);
                    sp--; childResult = { term: null }; continue outer;
                  }
                }
                if (trace) trace.push(`${'  '.repeat(frame.depth)}⟲ loop on ${formatTerm(rg)} → fail`);
                sp--; childResult = null; continue outer;
              }
            }
            pathGoals[frame.depth] = rg;
          }
        ...
        case _PH_PREMISE: {
          ...
          // Tail-premise frame reuse breaks the depth-indexed path (a reused
          // frame overwrites its own ancestor slot), so it is disabled under
          // loopCheck — a per-goal frame keeps the path exact.
          if (!buildTerm && !loopCheck && frame.candClauses.length === 1
   ...
   *   - theories {Array} - Equational theories (default: kernel defaultTheories
   *     only — strlit; calculus theories like binlit arrive via opts from the
   *     config, never as an engine default)
  ...
  backchain(goal, clauses, definitions, opts = {}) {
    ...
    // co-LP (Inc-5b) needs the same path tracking as loopCheck; enabling
    // coindPreds implies loopCheck.
    this._coindPreds = opts.coindPreds || null;
    this._loopCheck = !!opts.loopCheck || !!this._coindPreds;
    if (this._loopCheck) this._pathGoals.length = 0;
    ...
    // Initialize slot machinery. Clear every _mvSlots[h] entry written by
    // the previous call — BOTH the queryMV cleanup list AND the _getSlotMV-
    // created slot hashes (stored in _slotMV). A wholesale `.fill(0)` on
    // the 4M-entry _mvSlots array costs ≈16 MB zero-fill per backchain
    // call, which dominates small proofs on benchmarks; walking the two
    // per-call dirty lists is O(dirty). Correctness: `_slotMV[i] = h` is
    // always paired with `_mvSlots[h] = i+1`, so clearing via _slotMV is
    // exhaustive. (Originally reproduced as "no derivation found" on the
    // 2nd identical proveSource call — fixed here by targeted cleanup.)
    this._trailLen = 0;
    this._nextBase = 0;
    // A deep derivation may legally grow theta past MAX_SLOTS (_reserveFrame
    // has no cap; JS arrays extend on write). Truncate the spill before the
    // wipe, or the NEXT call's frames above MAX_SLOTS read the previous
    // call's stale bindings — spurious failures or wrong theta (round-14
    // fuzz find: qmul clause path failed after any div-by-one > ~2100).
    if (this._theta.length > MAX_SLOTS) this._theta.length = MAX_SLOTS;
    for (let i = 0; i < MAX_SLOTS; i++) this._theta[i] = undefined;
    for (let i = 0; i < this._queryMVCleanup.length; i++) {
      this._mvSlots[this._queryMVCleanup[i]] = 0;
    }
    for (let i = 0; i < this._slotMV.length; i++) {
      this._mvSlots[this._slotMV[i]] = 0;
    }
    this._slotMV.length = 0;
...
function clearState() {
  ...
  if (_defaultBackchainer._theta.length > MAX_SLOTS) _defaultBackchainer._theta.length = MAX_SLOTS;
  for (let i = 0; i < MAX_SLOTS; i++) _defaultBackchainer._theta[i] = undefined;
}
Store.onClear(clearState);

export { backchain, prove, buildIndex, registerLeafTag };
export default { backchain, prove, buildIndex, registerLeafTag };
