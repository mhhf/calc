...
class Backchainer {
  ...
  backchain(goal, clauses, definitions, opts = {}) {
    ...
    // Initialize slot machinery. Clear _mvSlots / _slotMV wholesale, not
    // just the previous queryMVCleanup set — slot-metavar hashes created
    // by `_getSlotMV` persist in _slotMV across calls, and their stale
    // _mvSlots[h]→slot mappings from the previous call would alias the
    // current call's fresh queryMV slots, silently steering search into
    // a dead end. (Reproduced: two identical proveSource calls in a row;
    // first ok, second returns "no derivation found".)
    this._trailLen = 0;
    this._nextBase = 0;
    for (let i = 0; i < MAX_SLOTS; i++) this._theta[i] = undefined;
    this._mvSlots.fill(0);
