...
const { buildTheoryLookup } = require('../kernel/eq-theory');
// ─── Lazy ILL defaults (backward compatibility for direct callers) ───
//
// The orchestrator (index.js) injects backwardOpts from calculus config,
// so _getIllDefaults() is NEVER called on the orchestrator path.
...
// opts.theories is absent.
let _illDefaults = null;
function _getIllDefaults() {
  if (!_illDefaults) _illDefaults = require('./ill/backchain-ill');
  return _illDefaults;
}
...
class Backchainer {
          ...
          if (tryFFIFn) {
            ...
            if (ffiResult && ffiResult.success) {
              ...
              if (ffiOk) {
                ...
                sp--; childResult = { term: buildTerm ? buildFFITermFn() : null }; continue outer;
              }
          ...
          for (let ti = 0; ti < candTypes.length; ti++) {
            ...
            if (this._matchHead(compiled.compiledHead, frame.goalHash, typeBase)) {
              ...
              if (buildTerm) {
                ...
                sp--; childResult = { term: buildTypeTermFn(groundGoal) }; continue outer;
              }
          ...
          while (frame.clauseIdx < cc.length) {
            ...
            if (compiled.premises.length === 0) {
              ...
              if (buildTerm) {
                ...
                childResult = { term: buildClauseTermFn(groundPrems, [], groundHead) };
              } else {
        ...
        case _PH_PREMISE: {
          ...
          if (frame.premIdx >= compiled.premises.length) {
            ...
            if (buildTerm) {
              ...
              const groundPrems = compiled.premisePuts.map(pp => normalize(this._resolveSlots(
                this._executePut(pp, frame.clauseBase))));
              childResult = { term: buildClauseTermFn(groundPrems, frame.premTerms, groundHead) };
            } else {
              childResult = { term: null };
            }
            continue outer;
   ...
   *   - buildClauseTerm {Function} - Proof term builder for clause resolution
   *   - buildFFITerm {Function} - Proof term builder for FFI success
   *   - buildTypeTerm {Function} - Proof term builder for type/axiom match
  ...
  backchain(goal, clauses, definitions, opts = {}) {
    ...
    // When opts provide theories, use them directly (config-driven path from index.js).
    // Otherwise, fall back to lazy ILL defaults (direct callers: tests, bridge.js).
    if (opts.theories) {
      // Config-driven: theories come from calculus config
      this._theoryLookup = buildTheoryLookup(opts.theories);
    } else {
      // ILL defaults — lazy loaded on first use
      const illDef = _getIllDefaults();
      illDef.initILL();
      const { defaultTheories } = require('../kernel/eq-theory');
      const { binlitTheory } = require('./ill/binlit-theory');
      this._theoryLookup = buildTheoryLookup([...defaultTheories, binlitTheory]);
    }

    // Resolve normalize + term builders: explicit opts > ILL defaults
    if (opts.normalize !== undefined) {
      this._normalize = opts.normalize || (x => x);
    } else {
      const illDef = _getIllDefaults();
      this._normalize = illDef.normalize;
    }
    this._buildClauseTermFn = opts.buildClauseTerm !== undefined ? opts.buildClauseTerm
      : (opts.theories ? null : _getIllDefaults().buildClauseTerm);
    this._buildFFITermFn = opts.buildFFITerm !== undefined ? opts.buildFFITerm
      : (opts.theories ? null : _getIllDefaults().buildFFITerm);
    this._buildTypeTermFn = opts.buildTypeTerm !== undefined ? opts.buildTypeTerm
      : (opts.theories ? null : _getIllDefaults().buildTypeTerm);

    // Resolve tryFFI: explicit opts > ILL defaults
    if (opts.tryFFI !== undefined) {
      if (opts.tryFFI && opts.getFFIMeta) {
        // tryFFI from calculus config takes (goal, ffiMeta) — bind ffiMeta
        const ffiMeta = opts.getFFIMeta();
        this._tryFFIFn = useFFI ? (g) => opts.tryFFI(g, ffiMeta) : null;
      } else {
        this._tryFFIFn = opts.tryFFI;
      }
    } else {
      // ILL default: use backchain-ill.tryFFI with ILL FFI meta
      const illDef = _getIllDefaults();
      const ffiMeta = opts.ffiMeta || (useFFI ? illDef.getFFIMeta() : {});
      this._tryFFIFn = useFFI ? (g) => illDef.tryFFI(g, ffiMeta) : null;
    }
...
/** Ensure calc.backchainIndex exists (lazy build). */
function ensureIndex(calc) {
  if (calc && calc.clauses && calc.definitions && !calc.backchainIndex) {
    calc.backchainIndex = buildIndex(calc.clauses, calc.definitions);
  }
}

module.exports = { backchain, prove, buildIndex, ensureIndex, Backchainer };
