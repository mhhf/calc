...
const illDefaults = require('./ill/backchain-ill');
...
class Backchainer {
  ...
  backchain(goal, clauses, definitions, opts = {}) {
    ...
    this._buildFFITermFn = opts.buildFFITerm || illDefaults.buildFFITerm;
    this._buildTypeTermFn = opts.buildTypeTerm || illDefaults.buildTypeTerm;
    this._tryFFIFn = opts.tryFFI !== undefined ? opts.tryFFI
      : (useFFI ? (g) => illDefaults.tryFFI(g, ffiMeta) : null);
    // Build theory lookup from opts or ILL defaults
    if (opts.theories) {
      this._theoryLookup = buildTheoryLookup(opts.theories);
    } else {
      // Default: ILL theories (strlit + binlit)
...
module.exports = { backchain, prove, buildIndex, Backchainer };
