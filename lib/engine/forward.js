...
import { consume, produce, producePers, canonObject } from './state-ops.js';
 ...
 */
function applyMatchInPlace(state, { rule, theta, slots, consumed, optimized }, canon) {
  consume(state.linear, consumed, null);
  const cp = canon ? rule.canonPatterns : null;
  produce(state.linear, rule.consequent.linear || [], theta, slots, rule, optimized, null, canon, cp);
  producePers(state.persistent, rule.consequent.persistent || [], theta, slots, rule, null, canon, cp);
...
function run(inputState, rules, opts = {}) {
  ...
  // clears before delegating here; RES_0143 F4. Direct callers proving
  // clause goals across runs must clear via opt/backward-cache.js.)
  let steps = 0;
  let solver = null; // Lazy EqNeqSolver for multi-alt rules

  // Accept both plain objects and State objects. Plain objects cross the
  // state-entry boundary: canonicalize each fact (State objects are live
  // engine states — canonical by the invariant already).
  const _canon = (opts.matchOpts && opts.matchOpts.canonicalize) || null;
  let state = inputState.linear && inputState.linear.group
    ? inputState  // Already a State object
    : fromObject(canonObject(inputState.linear || {}, _canon),
                 canonObject(inputState.persistent || {}, _canon));
  ...
  while (steps < maxSteps) {
    ...
    applyMatchInPlace(state, m, matchOpts.canonicalize || null);
    steps++;
    if (onStep) onStep({
