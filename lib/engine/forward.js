...
/** Reject rules whose window guards the untimed engine cannot enforce. */
function _rejectWindowRules(ruleList) {
  for (const r of ruleList) {
    if (r.windows) {
      throw new Error(`Rule '${r.name}' carries after/before window guards — these require the timed matcher (TODO_0265 Phase 4); the untimed engine would ignore them`);
    }
  }
}
