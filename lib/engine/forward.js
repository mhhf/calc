...
function run(inputState, rules, opts = {}) {
  ...
  let effectiveState = inputState;
  if (inputState.linear && !inputState.linear.group) {
    const { codeToArrlit } = require('./index');
    effectiveState = codeToArrlit(inputState);
  }

  // Accept both plain objects and State objects
  let state = effectiveState.linear && effectiveState.linear.group
    ? effectiveState  // Already a State object
