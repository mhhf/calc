...
import { clearBWCache as lnlClearCache } from './backward-cache.js';
...
import { satFilter } from './constraint-feed.js';
...
function run(inputState, rules, opts = {}) {
  ...
  while (steps < maxSteps) {
    ...
    if (m.rule.consequentAlts && m.rule.consequentAlts.length > 1) {
      if (!solver) solver = new EqNeqSolver({ evalNumeric: _domCfg?.evalNumeric });
...
/** Reject rules the untimed engine cannot execute faithfully: window
 *  guards, non-unit duration grades, and count-grade parcels all need the
 *  timed matcher (calc.settle) — firing them here would silently ignore
 *  their semantics. */
function _rejectWindowRules(ruleList) {
  for (const r of ruleList) {
    const what = r.windows ? 'after/before window guards'
      : r.delay ? 'a non-unit duration grade ({B}@d)'
      : r.counted ? 'count-grade parcels (!_k / !_W)'
      : r.weighted ? 'weighted-choice consequents (woplus)'
      : null;
    if (what) {
      throw new Error(`Rule '${r.name}' carries ${what} — these require the timed matcher (calc.settle, TODO_0265 Phase 4); the untimed engine would ignore them`);
    }
  }
}

export { compileRule, run, createState, buildFingerprintIndex, _rejectWindowRules };
export default { compileRule, run, createState, buildFingerprintIndex };
