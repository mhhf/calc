...
const { resolveConnectives, compileRule, flattenAntecedent, unwrapComputation, expandChoiceItem, expandConsequentChoices } = require('./compile');
...
const { filterAltsBySAT } = require('./constraint-feed');
  ...
  const matchOpts = {
    ...
    provePersistent: useFFI
      ...
      : (patterns, startIdx, theta, slots, state, calc, evidenceOut) =>
          provePersistentNaive(patterns, startIdx, theta, slots, state, calc, evidenceOut, matchOpts),
