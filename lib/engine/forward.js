...
const { compileRule, flattenAnte, unwrapComp, expandChoice, expandConsqChoices } = require('./compile');
...
const { ensureIndex: _ensureBackchainIndex } = require('./backchain');
...
function run(inputState, rules, opts = {}) {
  ...
  const matchOpts = opts.matchOpts || null;

  _ensureBackchainIndex(calc);
