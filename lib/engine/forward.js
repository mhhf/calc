...
const { compileRule, flattenTensor, unwrapMonad, expandItem, expandConsequentChoices, collectMetavars, isGround } = require('./compile');
...
function tryFFIDirect(goal) {
  ...
  const skipModeCheck = meta.multiModal;
  for (let i = 0; i < goalArity; i++) {
    const c = Store.child(goal, i);
    if (!skipModeCheck && modes[i] === '+' && !ffi.convert.isGround(c)) return null;
    _ffiArgs[i] = c;
  }

  const result = fn(_ffiArgs);
  // For multi-modal: mode_mismatch is non-definitive (try other methods)
  if (skipModeCheck && result && !result.success && result.reason) return null;
  return result;
}
...
function tryMatch(rule, state, calc, matchOpts) {
  ...
  while (rpLen > 0 || persistentIdx < persistentList.length) {
    ...
    while (persistentIdx < persistentList.length) {
      ...
      // Check persistent state facts (handles symbolic/freevar inputs)
      {
        const persistPattern = persistentList[persistentIdx];
        const pPred = getPredicateHead(persistPattern);
        let foundInState = false;
        for (const h in state.persistent) {
          const hn = Number(h);
          if (getPredicateHead(hn) !== pPred) continue;
          const savedUndo = undoSave();
          if (matchIdx(persistPattern, hn, theta, slots)) {
            foundInState = true;
...
/**
 * Try to fire a loli(trigger, {body}) from state.
 *
 * Ground triggers: hash equality against state facts.
 * Parameterized triggers (with freevars): pattern matching against state facts,
 * then instantiate the body with the resulting binding.
 *
 * Returns { consumed, produced, name } or null.
 */
function _tryFireLoli(h, state) {
  const trigger = Store.child(h, 0);
  const body = Store.child(h, 1);
  const bodyInner = Store.tag(body) === 'monad' ? Store.child(body, 0) : body;
  const pred = getPredicateHead(trigger);

  // Fast path: ground trigger — hash equality
  if (isGround(trigger)) {
    if (!state.linear[trigger] || state.linear[trigger] <= 0) return null;
    return {
      consumed: { [h]: 1, [trigger]: 1 },
      produced: flattenTensor(bodyInner),
      name: 'loli:' + (pred || 'trigger'),
    };
  }

  // Slow path: parameterized trigger — pattern matching
  ...
  // Find state facts that match the trigger
  for (const factStr of Object.keys(state.linear)) {
    ...
    if (_matchIdx(trigger, fact, theta, slots)) {
      // Instantiate body with the matched bindings
      const instantiated = _subApplyIdx(bodyInner, theta, slots);
      undoDiscard(saved);
      return {
        consumed: { [h]: 1, [fact]: 1 },
        produced: flattenTensor(instantiated),
        name: 'loli:' + (pred || 'trigger'),
      };
    }
...
/** Find first fireable loli continuation in state */
function findLoliMatch(state) {
  for (const hStr of Object.keys(state.linear)) {
    const h = Number(hStr);
    if (Store.tag(h) !== 'loli') continue;
    const m = _tryFireLoli(h, state);
    if (m) return m;
  }
  return null;
}

/** Find ALL fireable loli continuations in state */
function findAllLoliMatches(state) {
  const matches = [];
  for (const hStr of Object.keys(state.linear)) {
    const h = Number(hStr);
    if (Store.tag(h) !== 'loli') continue;
...
/** Apply a loli match to state (immutable) */
function applyLoliMatch(state, loliMatch) {
  const newLinear = { ...state.linear };
  for (const hStr in loliMatch.consumed) {
    const hash = Number(hStr);
    newLinear[hash] -= loliMatch.consumed[hStr];
    if (newLinear[hash] <= 0) delete newLinear[hash];
  }
  for (const h of loliMatch.produced.linear) {
    newLinear[h] = (newLinear[h] || 0) + 1;
  }
  const newPersistent = { ...state.persistent };
  for (const h of loliMatch.produced.persistent) {
    newPersistent[h] = true;
  }
  return { linear: newLinear, persistent: newPersistent };
}
...
function run(state, rules, opts = {}) {
  ...
  while (steps < maxSteps) {
    ...
    if (!m) {
      ...
      const loliM = findLoliMatch(state);
      if (!loliM) return { state, quiescent: true, steps, trace };
      if (trace) trace.push(`[${steps}] ${loliM.name}`);
      state = applyLoliMatch(state, loliM);
      steps++;
      continue;
    }
