 ...
 * Generalized from EVM-specific evmStrategy — works for any program with
 * a pointer predicate and discriminator predicate.
 *
 ...
 *
 * Three-level resolution via provePersistentGoals:
 * 1. FFI direct → concrete result
 * 2. State lookup → match against persistent facts
 * 3. Backward chaining → prove via clauses
 * 4. All fail → freshEvar (symbolic witness, constraint accumulates in state)
 *
...
function resolveExistentials(theta, slots, rule, state, calc) {
  ...
  // Prove goals via three-level fallback (FFI → state → backward chaining)
  if (goals.length > 0) {
...
function findMatch(state, rules, calc, matchOpts) {
  ...
  if (opcodeIndex && rules.fpConfig) {
    const fpValue = findFingerprintValue(stateIndex, rules.fpConfig);
    const candidates = fpValue != null && opcodeIndex[fpValue];
    if (candidates) {
...
function run(state, rules, opts = {}) {
  ...
  const indexedRules = { rules, opcodeIndex, fpConfig };
