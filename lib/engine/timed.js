 ...
 *
 * Returns [{ formula, enabled }] in leaf order (formula renderable via show).
 */
function menuStatus(inputState, factHash, ruleList, opts) {
  ...
  const baseLinear = inputState.linear || {};
  return leaves.map((leaf, i) => {
    let projected;
    try {
      projected = withProject(inputState, factHash, i,
        { timedConfig: tcfg, roles: rc, atStamp: horizon });
    } catch {
      return { formula: leaf, enabled: false };   // e.g. grade-0 component
    }
    ...
    if (projKeys.size === 0) {
      // nothing linear projected (e.g. a sub-menu): opening is always allowed
      return { formula: leaf, enabled: true };
    }
    const state2 = normalizeTimedState(projected, tcfg);
    for (const r of ruleList) {
      const m = tryTimedMatch(r, state2, opts.calc || null, matchOpts, tcfg);
      if (!m || cmp(m.activation, horizon) > 0) continue;
      for (const hStr in m.consumed) {
        if (projKeys.has(factKeyOf(Number(hStr), tcfg.expTag))) {
          return { formula: leaf, enabled: true };
        }
      }
    }
    return { formula: leaf, enabled: false };
  });
}
