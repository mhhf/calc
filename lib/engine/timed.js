 ...
 */
function tryTimedMatch(rule, state, calc, matchOpts, tcfg) {
  ...
  function leaf(partialA) {
    ...
    if (persistentList.length > 0) {
      ...
      if (idx < persistentList.length) done = true;
    }
    if (!done) {
      let a = partialA;
      let ok = true;
      if (rule.windows) {
        ...
        for (const w of rule.windows.before) {
          if (cmp(a, evalWindow(w)) >= 0) { ok = false; break; }  // deadline: a < before
        }
      }
...
function fire(state, m, tcfg) {
  ...
  return { done, delay: d, producedPreds, producedPersistent };
}
 ...
 */
function _makeDirtySched(ruleList) {
  const predToRules = new Map();
  for (const r of ruleList) {
    const preds = new Set(r.triggerPreds || []);
...
function settle(inputState, rules, opts = {}) {
  ...
  const sched = opts.scheduler === 'dirty' ? _makeDirtySched(ruleList) : null;
  ...
  while (steps < maxSteps) {
    ...
    const m = choose(tied, state, seed, chooser);
...
// v1 scope guard: additive-choice consequents need the weighted-fork story
// (woplus, Phase 4b) — reject rather than silently commit to alternative 0.
function _rejectMultiAlt(ruleList) {
  for (const r of ruleList) {
    if (r.consequentAlts && r.consequentAlts.length > 1) {
      throw new Error(`Rule '${r.name}': additive-choice consequents under the timed scheduler await woplus (TODO_0265 Phase 4b)`);
    }
  }
}
 ...
 * a distinct world), or consumers starving a read of its last copies.
...
function settleExplore(inputState, rules, opts = {}) {
  ...
  function step(state, depth) {
    if (depth > maxSteps) {
      throw new Error(`settleExplore: depth ${depth} exceeded maxSteps=${maxSteps} (Zeno guard, D16)`);
    }
    const cands = [];
    for (const r of ruleList) {
      const m = tryTimedMatch(r, state, calc, matchOpts, tcfg);
    ...
    if (cands.length === 0) {
      const leafState = toObject(state);
      leaves.push(leafState);
      return { type: 'leaf', state: leafState };
    }
    let aMin = cands[0].activation;
    for (const m of cands) if (cmp(m.activation, aMin) < 0) aMin = m.activation;
    if (cmp(aMin, horizon) > 0) {
      const leafState = toObject(state);
      leaves.push(leafState);
      return { type: 'leaf', state: leafState, next: aMin };
    }
    const tied = cands.filter(mm => cmp(mm.activation, aMin) === 0);
    if (tied.length === 1 || !_conflicts(tied, state)) {
      // Independent (or singleton) tied set: any order reaches the same
      // state — commit without branching (partial-order reduction).
      const m = tied.slice().sort((a, b) => (_matchKey(a) < _matchKey(b) ? -1 : 1))[0];
      fire(state, m, tcfg);
      return step(state, depth + 1);
    }
    ...
    for (const m of sorted) {
      ...
      fire(branch, m, tcfg);
      children.push({ choice: { rule: m.rule.name, consumed: m.consumed }, tree: step(branch, depth + 1) });
    }
    return { type: 'conflict', activation: aMin, children };
  }

  const tree = step(normalizeTimedState(inputState, tcfg), 0);
  return { tree, leaves };
}
