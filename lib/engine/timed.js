 ...
 * QUIET rather than guessing. maxSteps remains the runtime backstop.
 *
...
/**
 * Strict-choice marker `now(X)`: a SYNCHRONOUS alternative — collapsing it
 * must be simultaneous with a rule consuming its projection (fireable at
 * the decision stamp), else the click is REFUSED and the state unchanged.
 * The PROGRAM marks strictness in the menu term itself (like $ or read,
 * an engine-interpreted annotation); menuStatus greys by the very same
 * relation, so UI greying and enforcement cannot drift apart. Unmarked
 * alternatives are asynchronous plans: their acts wait in the context.
 */
const NOW_TAG = 'now';
const _leafBody = (h) => (Store.tag(h) === NOW_TAG ? Store.child(h, 0) : h);
 ...
 */
function _projectionEnabled(base, projected, horizon, ruleList, calc, matchOpts, tcfg) {
  ...
  for (const r of ruleList) {
    ...
    if (!m || cmp(m.activation, horizon) > 0) continue;
    for (const hStr in m.consumed) {
...
function withProject(inputState, factHash, index, opts = {}) {
  ...
  const flat = flattenAnte(_leafBody(leaves[index]), rc);
  ...
  // Strict gate: a now-marked alternative must be fireable at the decision
  // stamp — refuse otherwise (nothing enters the state).
  if (Store.tag(leaves[index]) === NOW_TAG && !opts.noStrict) {
    if (!opts.rules) {
      throw new Error('choose: now-marked alternative needs rules in scope');
    }
    if (!_projectionEnabled(inputState, result, s, opts.rules,
        opts.calc || null, opts.matchOpts, tcfg)) {
      throw new Error(`choose: alternative ${index} is now-marked and cannot fire at the decision time`);
    }
  }
  return result;
}
...
function menuStatus(inputState, factHash, ruleList, opts) {
  ...
  if (Store.tag(menu) === 'at') menu = Store.child(menu, 0);
  if (Store.tag(menu) !== rc.externalChoice) {
  ...
  return leaves.map((leaf, i) => {
    const strict = Store.tag(leaf) === NOW_TAG;
    let projected;
    try {
    ...
    const enabled = _projectionEnabled(inputState, projected, horizon,
      ruleList, opts.calc || null, opts.matchOpts, tcfg);
...
function _altWeight(alt, m, tcfg) {
  ...
  for (const s of w.syms) {
    ...
    const t = q === undefined ? null : Store.tag(q);
    let nd = t === 'ratlit' ? [Store.child(q, 0), Store.child(q, 1)]
      : t === 'binlit' ? [Store.child(q, 0), 1n] : null;
    if (!nd) {
      throw new Error(`Rule '${m.rule.name}': woplus weight did not resolve to a ground rational at fire time (got '${t || 'unbound'}')`);
    }
