...
import { mul as _wMul, cmp as _wCmp, add as _wAdd } from '../rat.js';
// ─── Timed config ───────────────────────────────────────────────────
 ...
 */
function tryTimedMatch(rule, state, calc, matchOpts, tcfg, diag) {
  ...
  search(0, unit);                             // activation is at least the unit (ℚ≥0 v1)
  undoDiscard(topUndo);
...
function fire(state, m, tcfg) {
  ...
  for (let i = 0; i < pats.length; i++) {
    ...
    if (tcfg.implTag && Store.tag(h) === tcfg.implTag) {
      throw new Error(`Rule '${rule.name}': dynamic rules (lolis) in timed consequents are not supported (till v1)`);
    }
    if (count === 0) continue;
...
function _makeDirtySched(ruleList, tcfg) {
  // Pred keys MUST be computed by the same function on both sides of the
  // wake-up map: fire() reports touched preds via _predOf (lax — any tag
  // has a name), while compile's triggerPreds go through predHead, which
  // returns null for sub-boundary kernel tags (e.g. a bare binlit fact).
  // Keying the map on triggerPreds alone made literal-fact consumers
  // invisible to dirty tracking (round-14 fuzz find) — so derive the keys
  // from the antecedent patterns directly, and keep rules with unknowable
  // heads (metavar/freevar patterns) permanently dirty.
  const predToRules = new Map();
  const alwaysDirty = new Set();
  for (const r of ruleList) {
    const preds = new Set();
    for (const p of (r.antecedent.linear || [])) {
      let body = (tcfg.expTag && Store.tag(p) === tcfg.expTag) ? Store.child(p, 1) : p;
      if (Store.tag(body) === 'at') body = Store.child(body, 0);
      const t = Store.tag(body);
      const pred = (t === 'metavar' || t === 'freevar') ? null : _predOf(body);
      if (pred) preds.add(pred);
      else alwaysDirty.add(r);
    }
    for (const p of (r.antecedent.persistent || [])) {
      const pred = _persPred(p);
      if (pred) preds.add(pred);
    }
  ...
  return {
    ...
    markFired(m, fired) {
      ...
      for (const hStr in m.consumed) {
        const pred = _predOf(_inner(Number(hStr)));
...
function settle(inputState, rules, opts = {}) {
  ...
  while (steps < maxSteps) {
    ...
    if (m.rule.weighted && m.rule.consequentAlts.length > 1) {
      m = _withAlt(m, _sampleAlt(m, state, seed));   // woplus: PRF branch draw (4b)
    }
...
function nextActivation(inputState, rules, opts = {}) {
  ...
  for (const r of ruleList) {
    ...
    if (m && (aMin === null || cmp(m.activation, aMin) < 0)) aMin = m.activation;
  }
  return aMin;
}
 ...
 *   cycle: a cycle in the consumed→produced pred graph restricted to
 *     zero-delay, token-non-decreasing rules.
 *
 ...
 * produces one — token-decreasing, never flagged), and anything the lint
 * cannot ground (variable delays, !_W counts, metavar heads) stays QUIET
 * rather than guessing. maxSteps remains the runtime backstop.
 *
...
function lintProductivity(ruleList, tcfg) {
  ...
  const findings = [];
  const edges = new Map();      // pred -> Set(pred), zero-delay non-decreasing rules
  ...
  for (const r of ruleList) {
    ...
    for (const p of (r.antecedent.linear || [])) {
      ...
      if (Store.tag(body) === 'at') body = Store.child(body, 0);
      const t = Store.tag(body);
      if (!t || t === 'metavar' || t === 'freevar') { unknown = true; break; }
      const pred = _predOf(body);
    ...
    let maxTotal = 0, covers = false;
    const producedPredsAll = new Set();
    for (const alt of alts) {
      ...
      for (let pat of (alt.linear || [])) {
        ...
        const t = Store.tag(pat);
        if (!t || t === 'metavar' || t === 'freevar') { altUnknown = true; break; }
        const pred = _predOf(pat);
        produced.set(pred, (produced.get(pred) || 0) + count);
      }
      if (altUnknown) continue;                          // quiet on unknowable alts
      let total = 0;
      for (const c of produced.values()) total += c;
      if (total > maxTotal) maxTotal = total;
      for (const pred of produced.keys()) producedPredsAll.add(pred);
      let coversAlt = true;
    ...
    let consumedTotal = 0;
    for (const c of consumed.values()) consumedTotal += c;
    if (maxTotal < consumedTotal) continue;              // token-decreasing: depletes
    for (const a of consumed.keys()) {
      for (const b of producedPredsAll) {
        if (!edges.has(a)) edges.set(a, new Set());
        edges.get(a).add(b);
        edgeRules.set(`${a}→${b}`, r.name);
      }
    }
  }
 ...
 * (weights are exact rationals summing to 1 — compile validates).
 * Reproducible and horizon-split invariant by construction.
 */
function _sampleAlt(m, state, seed) {
...
function settleExplore(inputState, rules, opts = {}) {
  ...
  // Instant-feeding precomputation (round 13): the predicates any rule's
  // linear antecedent mentions, plus whether any pattern is a pred-less
  // wildcard (matches everything — then ALL instant production feeds).
  const antePreds = new Set();
  let wildcardPattern = false;
  for (const r of ruleList) {
    for (const p of (r.triggerPreds || [])) antePreds.add(p);
    for (const p of (r.antecedent.linear || [])) {
      const meta = r.linearMeta && r.linearMeta[p];
      if (!meta || !meta.pred) wildcardPattern = true;
    }
  }
  ...
  function _fireOrFork(state, m, depth, pathWeight) {
    if (m.rule.weighted && m.rule.consequentAlts.length > 1) {
      ...
      for (let i = 0; i < m.rule.consequentAlts.length; i++) {
        const w = m.rule.consequentAlts[i].weight;
        const branch = state.snapshot();
        _countFired();
...
function _feedsInstant(m, tcfg, antePreds, wildcardPattern) {
  ...
  for (const alt of alts) {
    ...
    for (let pat of (alt.linear || [])) {
      if (tcfg.expTag && Store.tag(pat) === tcfg.expTag) pat = Store.child(pat, 1);
      const t = Store.tag(pat);
      if (!t || t === 'metavar' || t === 'freevar') return true;   // unknown head
      const pred = t === 'atom' ? Store.child(pat, 0) : t;
...
export {
  buildTimedConfig, normalizeTimedState, tryTimedMatch, fire, choose,
  settle, nextActivation, settleExplore, lintProductivity,
  observable, pending, inFlight, timedSubset,
};
export default {
  buildTimedConfig, normalizeTimedState, tryTimedMatch, fire, choose,
  settle, nextActivation, settleExplore, lintProductivity,
