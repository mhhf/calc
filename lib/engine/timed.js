 ...
 * consequent: tensors flatten, !ω goes persistent, !_k yields k copies, a
 * nested & survives as one sub-menu fact.
 *
 * factHash may be the stamped hash as it appears in state.linear or the
 * bare menu formula (stamp defaults to the unit). Returns a NEW plain
...
function withProject(inputState, factHash, index, opts = {}) {
  ...
  if (!linear[key] && Store.tag(key) !== 'at') key = Store.put('at', [key, unit]);
  if (!linear[key]) throw new Error('choose: fact not present in state');
  const stamped = Store.tag(key) === 'at';
  const menu = stamped ? Store.child(key, 0) : key;
  const menuStamp = stamped ? Store.child(key, 1) : unit;
  ...
  if (opts.at !== undefined) {
    const at = tcfg.parseStamp(opts.at);
...
export {
  buildTimedConfig, normalizeTimedState, tryTimedMatch, fire, withProject,
