/**
 * ILL-specific defaults for the backward prover.
 *
 * Extracted from prove.js to keep the core prover calculus-agnostic.
 * Provides: normalization (via binlitTheory), proof term construction,
 * and FFI dispatch.
 *
 * Note: arrlit ↔ acons/ae expansion is handled inline in prove.js because
 * the Store normalizes acons(H, arrlit(T)) → arrlit([H, ...T]), making it
 * impossible to create acons nodes via Store.put. Array expansion is a
 * Store-level structural concern, not ILL-specific.
 */

import Store from '../../kernel/store.js';
import { isPredTag, predHead as getHead, rTensor } from '../../kernel/ast.js';
import { defaultTheories } from '../../kernel/eq-theory.js';
import { binlitTheory, installBinlitClassifier } from './binlit-theory.js';
import { setTheories } from '../../kernel/unify.js';
// Hoisted by tools/esm-hoist.js:
import { gradeW } from '../../engine/grades.js';

// ── Tag registration ────────────────────────────────────────────────
// Moved to initILL() — explicit initialization replaces import-time side effects.
// Callers: engine/index.js calls initILL() during ILL calc setup.
// Tests that need ILL call initILL() explicitly; tests that don't, get no binlit theory.

let _initialized = false;

/**
 * Explicitly initialize ILL-specific Store state and equational theories.
 * Idempotent: Store.put is content-addressed, setTheories replaces.
 * Must be called before any ILL backward chaining.
 */
function initILL() {
  if (_initialized) return;
  _initialized = true;
  // o, i are dynamic (>= PRED_BOUNDARY) — may change after Store.restore().
  const e = Store.put('atom', ['e']);
  Store.put('atom', ['ae']); // ensure ae atom exists for prove.js
  Store.put('o', [e]);       // ensure o tag is registered
  Store.put('i', [e]);       // ensure i tag is registered
  // Register ILL equational theories (binlit ↔ i/o/e) for all matchers.
  // Must happen after o/i tag registration above.
  setTheories([...defaultTheories, binlitTheory]);
  // First-arg classification for binlit goals (the 'i'/'o'/'e' clause
  // buckets) — registry entry, shared by backchain/compile/compiled-clauses.
  installBinlitClassifier();
}

// Lazy FFI import — never loaded for non-ILL callers
import * as _ffi from './ffi/index.js';
function _getFfi() { return _ffi; }

/** Get default FFI metadata (lazy). */
function getFFIMeta() { return _getFfi().defaultMeta; }

// ── Normalization ───────────────────────────────────────────────────

/**
 * Normalize binary constructor chains (i/o/e) to compact binlit form.
 * Delegates to binlitTheory.canonicalize — single source of truth.
 */
const normalize = binlitTheory.canonicalize;

// ── Proof Term Construction ─────────────────────────────────────────

function _tensorRSpine(terms) {
  if (terms.length === 0) return { rule: 'one_r', principal: null, subterms: [] };
  if (terms.length === 1) return terms[0];
  let acc = terms[terms.length - 1];
  for (let i = terms.length - 2; i >= 0; i--) {
    acc = { rule: 'tensor_r', principal: null, subterms: [terms[i], acc] };
  }
  return acc;
}

/**
 * Clause proof term builder factory.
 * Reconstructs: copy(loli(tensor(!P₁,...), {Q}), loli_l(antProof, monad_l(id(Q))))
 *
 * Parameterized by the computation role record (TODO_0265 Phase 2) so a
 * graded calculus can reuse the same LNL-generic term shape: a graded
 * computation node gets the unit grade (clause resolution is instantaneous)
 * from the gradeUnit hook, and the left-rule name follows the connective
 * (`<tag>_l`). The exported buildClauseTerm is the ILL instance — unary
 * monad, ω-graded premises — bit-identical to before.
 */
function makeClauseTermBuilder(opts = {}) {
  const computation = opts.computation || { tag: 'monad', bodyIdx: 0, gradeIdx: null };
  const gradeUnit = opts.gradeUnit || null;
  const gradeOmega = opts.gradeOmega || gradeW;

  return function buildClauseTerm(groundPrems, premTerms, groundHead) {
    const bangPrems = groundPrems.map(p => Store.put('bang', [gradeOmega(), p]));
    const groundAnt = rTensor(bangPrems);
    let groundMonad;
    if (computation.gradeIdx === null) {
      groundMonad = Store.put(computation.tag, [groundHead]);
    } else {
      if (!gradeUnit) throw new Error('makeClauseTermBuilder: graded computation requires a gradeUnit hook');
      const kids = [];
      kids[computation.gradeIdx] = gradeUnit();
      kids[computation.bodyIdx] = groundHead;
      groundMonad = Store.put(computation.tag, kids);
    }
    const groundLoli = Store.put('loli', [groundAnt, groundMonad]);

    const wrappedPrems = premTerms.map(pt =>
      ({ rule: 'promotion', principal: null, subterms: [pt] }));
    const antProof = _tensorRSpine(wrappedPrems);

    const monadBody = { rule: `${computation.tag}_l`, principal: groundMonad, subterms: [
      { rule: 'id', principal: groundHead, subterms: [] }
    ]};
    const loliApp = { rule: 'loli_l', principal: groundLoli,
                      subterms: [antProof, monadBody] };
    return { rule: 'copy', principal: groundLoli, subterms: [loliApp] };
  };
}

/** ILL clause proof term builder (default instance). */
const buildClauseTerm = makeClauseTermBuilder();

// ── FFI Dispatch ────────────────────────────────────────────────────

/** Default FFI dispatch for ILL predicates. */
function tryFFI(goal, ffiMeta) {
  const head = getHead(goal);
  if (!head) return null;
  const meta = ffiMeta[head];
  if (!meta || !meta.ffi) return null;
  if (!Store.isTerm(goal) || !isPredTag(Store.tag(goal))) return null;
  const a = Store.arity(goal);
  const args = new Array(a);
  for (let i = 0; i < a; i++) args[i] = Store.child(goal, i);
  const ffiMod = _getFfi();
  if (!meta.multiModal && !ffiMod.mode.checkMode(args, meta.mode)) return null;
  const fn = ffiMod.get(meta.ffi);
  if (!fn) return null;
  return fn(args);
}

/**
 * Build proof term for an FFI-resolved goal.
 */
function buildFFITerm() {
  return { rule: 'ffi', principal: null, subterms: [] };
}

/**
 * Build proof term for a type/axiom match (copy + identity).
 */
function buildTypeTerm(groundGoal) {
  return { rule: 'copy', principal: groundGoal,
           subterms: [{ rule: 'id', principal: groundGoal, subterms: [] }] };
}

/**
 * Build ILL-specific backward chaining opts for direct callers.
 * Bundles theories, normalization, FFI, and proof term builders.
 * Use this in tests/tools that call backward.prove() directly.
 *
 * @param {Object} [overrides] - Override individual opts
 * @returns {Object} Opts suitable for backward.prove()
 */
function makeILLBackchainOpts(overrides = {}) {
  initILL();
  return {
    theories: [...defaultTheories, binlitTheory],
    normalize,
    tryFFI,
    getFFIMeta,
    buildClauseTerm,
    buildFFITerm,
    buildTypeTerm,
    ...overrides,
  };
}

export { initILL, normalize, makeClauseTermBuilder, buildClauseTerm, buildFFITerm, buildTypeTerm, tryFFI, getFFIMeta, makeILLBackchainOpts };
export default { initILL, normalize, makeClauseTermBuilder, buildClauseTerm, buildFFITerm, buildTypeTerm, tryFFI, getFFIMeta, makeILLBackchainOpts };
