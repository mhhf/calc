...
function residualResolver(goalHash) {
  ...
  switch (pred) {
    ...
    // cd_read(+, +, -): Value = 32-byte read at Offset from ground binlit CD
    // NOTE: Same leading-zero limitation as calldata.js FFI — cdVal must not have
    // leading-zero bytes. For leading-zero calldata, use sconcat form so this
    // case returns null and backward clause resolution handles it correctly.
    case 'cd_read': {
      if (arity !== 3) return null;
      const cd = Store.child(goalHash, 0);
      const off = Store.child(goalHash, 1);
      if (!_isGroundArg(cd) || !_isGroundArg(off)) return null;
      const cdVal = binToInt(cd);
      const offVal = binToInt(off);
      if (cdVal === null || offVal === null) return null;
      const offN = Number(offVal);
      // Compute byte length from minimal BigInt representation.
      // Correct only when cdVal's most-significant byte is non-zero.
      let byteLen = 0;
      if (cdVal > 0n) {
        let v = cdVal;
      ...
      if (offN >= byteLen) {
        ...
        const avail = Math.min(32, byteLen - offN);
        const rShift = BigInt(Math.max(0, byteLen - offN - 32) * 8);
        const mask = (1n << BigInt(avail * 8)) - 1n;
        const extracted = (cdVal >> rShift) & mask;
        result = (extracted << BigInt((32 - avail) * 8)) & ((1n << 256n) - 1n);
      }
      return Store.put('cd_read', [cd, off, intToBin(result)]);
    }
