...
import { binToInt, intToBin, strToHash, hashToStr, isGround } from './convert.js';
import { computeArith } from './arith-core.js';
// Rational overloads (TODO_0265 Phase 1, D8.1): bin×bin keeps the fast path
// below; when bin decoding fails, ratFFI handles arguments carrying ratlit /
// rat(N,D) and returns null otherwise, so pure-bin semantics never change.
// Only value-coinciding operations are overloaded (plus/mul/comparisons);
// sub and div are NOT — bin sub is a saturating monus and bin div is
// Euclidean, neither lifts coherently to ℚ. Rational subtraction/division
// are the explicit qsub/qdiv predicates (see theories/rat-ffi.js).
...
function mul(args) {
  ...
  if (aInt === null || bInt === null) {
    return ratFFI.mul(args) || { success: false, reason: 'conversion_failed' };
  }
...
function lt(args) {
  ...
  if (aInt === null || bInt === null) {
    return ratFFI.lt(args) || { success: false, reason: 'conversion_failed' };
  }
...
function le(args) {
  ...
  if (aInt === null || bInt === null) {
    return ratFFI.le(args) || { success: false, reason: 'conversion_failed' };
  }
...
function eq(args) {
  ...
  if (aInt === null || bInt === null) {
    return ratFFI.eq(args) || { success: false, reason: 'conversion_failed' };
  }
...
function eq_bool(args) {
  ...
  if (aInt === null || bInt === null) {
    return ratFFI.eq_bool(args) || { success: false, reason: 'conversion_failed' };
  }
...
function neq(args) {
  ...
  if (aInt === null || bInt === null) {
    return ratFFI.neq(args) || { success: false, reason: 'conversion_failed' };
  }
...
export { plus, inc, mul, sub, div, mod, lt, le, eq, eq_bool, neq, gt, bitwiseAnd, bitwiseOr, bitwiseNot, bitwiseXor, to256, shr, shl, slt, sub256, div256, mod256, exp256, trim, sdiv256, smod256, addmod256, mulmod256, signextend256, byte256, byte_replace, sar256, checked_sub, byte_size256, fixed_mul, fixed_div, string_concat, string_length, sstore_gas, is_push, is_dup, is_swap };
export default { plus, inc, mul, sub, div, mod, lt, le, eq, eq_bool, neq, gt, bitwiseAnd, bitwiseOr, bitwiseNot, bitwiseXor, to256, shr, shl, slt, sub256, div256, mod256, exp256, trim, sdiv256, smod256, addmod256, mulmod256, signextend256, byte256, byte_replace, sar256, checked_sub, byte_size256, fixed_mul, fixed_div, string_concat, string_length, sstore_gas, is_push, is_dup, is_swap };
