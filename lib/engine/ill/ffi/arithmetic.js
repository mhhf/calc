...
/**
 * FFI for min/max: value-ordered selection over bin naturals.
 * min(A, B, M): M = smaller of A, B.  max(A, B, M): M = larger.
 * Mode: + + -
 *
 * Clause definitions in bin.ill (gt once + ground pick) — FFI principle.
 */
function min(args) {
  const [a, b, m] = args;
  if (!isGround(a) || !isGround(b)) return { success: false, reason: 'mode_mismatch' };
  ...
  const bInt = binToInt(b);
  if (aInt === null || bInt === null) return { success: false, reason: 'conversion_failed' };
  return { success: true, theta: [[m, intToBin(aInt <= bInt ? aInt : bInt)]] };
}
