...
// Static FFI import for binToInt/intToBin
import * as _convert from './ffi/convert.js';
function _getConvert() { return _convert; }
...
let _classifierInstalled = false;

/**
 * Register binlit's first-arg classifier: binlit goals index into the
 * 'e'/'i'/'o' clause buckets of the binary-numeral vocabulary (bin.ill
 * clause heads pattern-match e, i(X), o(X)). This is the eq-theory bridge
 * applied to clause indexing — one registry entry replaces the trichotomy
 * previously inlined at three sites (TODO_0265 Phase 2b). Idempotent;
 * called from initILL().
 */
function installBinlitClassifier() {
  if (_classifierInstalled) return;
  _classifierInstalled = true;
  registerFirstArgClassifier(_TAG_BINLIT, (h) => {
    const v = Store.child(h, 0);
    if (v === 0n) return 'e';
    return (v & 1n) === 1n ? 'i' : 'o';
  });
}

export { binlitTheory, installBinlitClassifier };
export default { binlitTheory, installBinlitClassifier };
