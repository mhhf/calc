...
function _sortOk(actual, expected, cx) {
  ...
  return cx.sorts ? cx.sorts.subsort(actual, expected) : false;
}
...
function _checkTerm(h, expectedSort, sortTable, metavarSorts, errors, path, cx = _LEGACY_CX) {
  ...
  if (t === 'atom') {
    ...
    const entry = sortTable.get(name);
    if (!entry) {
      // Calculus-level members (g0/gw : count from the .calc) and sort
      // names have no .ill declaration but are known to the sort system —
      // grade markers appear under every bare `!`, sedge facts mention
      // sort names as terms of the meta-sort.
      if (cx.sorts) {
        const ls = cx.sorts.leastSortOfName(name);
        if (ls !== null) {
          if (!_sortOk(ls, expectedSort, cx)) {
            errors.push(`${path}: expected sort '${expectedSort}', got '${ls}' for '${name}'`);
          }
          return;
        }
        if (cx.sorts.isSort(name)) {
    ...
    if (entry.argSorts.length !== 0) {
      // Dual-role datasort name (fence B): applied = membership predicate,
      // bare atom at expected sort 'sort' = the sort-NAME reading (subsort
      // closure facts, within facts).
      if (cx.sorts && expectedSort === SORT_PREDS.SORT && cx.sorts.isSort(name)) return;
  ...
  if (tid < Store.PRED_BOUNDARY) {
    ...
    // Grade positions live in the GRADE algebra (D2/D4), not the term
    // sorts — but they have their own closed grammar: counts (binlit),
    // delays (ratlit), rule variables, and the calculus's grade marker
    // atoms. WHICH connectives carry a leading grade child and WHICH
    // atoms are grade markers is calculus data, derived from
    // cc.connectives + cc.gradeConfig (deriveGradeMeta; RES_0143 L6 —
    // 'bang'/'monad'/'g0'/'gw' used to be hardcoded here).
    const start = (cx.gradeTags && cx.gradeTags.has(t)) ? 1 : 0;
    if (start === 1) {
      const g = Store.child(h, 0);
      const gt = Store.tag(g);
      const gradeOk = gt === 'binlit' || gt === 'ratlit' ||
        gt === 'metavar' || gt === 'freevar' ||
        (gt === 'atom' && cx.gradeAtoms && cx.gradeAtoms.has(Store.child(g, 0)));
      if (!gradeOk && cx.closedWorld) {
        errors.push(`${path}: invalid grade '${gt === 'atom' ? Store.child(g, 0) : gt}' on ${t} — grades are counts (integers), delays (rationals), rule variables, or the calculus's grade markers (D2/D4)`);
      }
    }
  ...
  // Calculus-declared connective on a DYNAMIC tag (declared in the .calc
  // without being a kernel tag — will's superpose): the connective
  // signature supplies per-child sorts exactly as the structural branch
  // does. Program declarations of the same name take precedence.
  const dynConn = cx.sorts && cx.sorts.connArgSorts ? cx.sorts.connArgSorts[predName] : null;
  if (dynConn && !sortTable.has(predName) && dynConn.length === Store.arity(h)) {
    for (let i = 0; i < dynConn.length; i++) {
      const ch = Store.child(h, i);
      if (typeof ch === 'number' && Store.isTerm(ch)) {
        const s = dynConn[i] === cx.sorts.formulaSort ? 'type' : dynConn[i];
        _checkTerm(ch, s, sortTable, metavarSorts, errors, path, cx);
      }
    }
    return;
...
function _checkBoundedApplication(h, predName, entry, svar, sortTable, metavarSorts, errors, path, cx) {
  ...
  if (!cx.sorts.subsort(solved, svar.sort)) {
  ...
  for (const inst of instances) {
    if (cx.sorts.subsort(solved, inst)) { hasInstance = true; break; }
  }
  if (!hasInstance) {
 ...
 * Premise-less facts live in `definitions` (loader pass 1) — they count as
 * instances too, so both stores are scanned.
 * @returns {Map<string, Set<string>>} predicate → instance sorts
 */
function _buildInstances(clauses, definitions, sorts, cx) {
  const instances = new Map();
  if (!cx.sorts) return instances;
  const heads = [];
  if (clauses) for (const [, clause] of clauses) heads.push(clause.hash);
  if (definitions) for (const [, hash] of definitions) heads.push(hash);
  for (const head of heads) {
    const tid = Store.tagId(head);
    if (tid < Store.PRED_BOUNDARY) continue;
    const clause = { hash: head };
    const predName = Store.TAG_NAMES[tid];
  ...
  // Declared instance signatures (loader promotion — bin.ill's
  // `plus: bin bin bin` under the bounded-var principal) count too:
  // they cover FFI-only instances with no local clause heads.
  for (const pred of cx.sorts.sortVarPreds()) {
    for (const s of cx.sorts.instSigSortsOf(pred)) {
      if (!instances.has(pred)) instances.set(pred, new Set());
      instances.get(pred).add(s);
    }
  }
  return instances;
}
...
function checkAll(definitions, compiledRules, clauses, opts = {}) {
  ...
  const errors = [];
  const warnings = [];
  const gm = opts.gradeMeta || null;
  const cx = {
    closedWorld: !!opts.closedWorld,
    sorts: opts.sorts || null,
    instances: null,
    gradeTags: gm ? gm.gradeTags : null,
    gradeAtoms: gm ? gm.gradeAtoms : null,
  };
  if (cx.sorts) cx.instances = _buildInstances(clauses, definitions, sorts, cx);
  ...
  // Premise-less facts land in `definitions` (loader pass 1) and escaped
  // checking historically. Sorted mode closes that hole — sortless mode
  // keeps the legacy blind spot (bit-identical ILL behavior).
  if (cx.sorts) {
    for (const [name, hash] of definitions) {
      if (Store.tagId(hash) >= Store.PRED_BOUNDARY) {
        errors.push(...checkClause(name, { hash, premises: [] }, sorts, cx));
      }
    }
  }
...
/**
 * Derive the grade-position metadata for _checkTerm from calculus data
 * (RES_0143 L6): which connective tags carry a leading grade child
 * (arity-2 exponential — bang(grade, A) — and arity-2 monad —
 * monad(grade, A)), and which atom names are the calculus's grade
 * markers (from the gradeConfig record's recompute-on-demand atoms).
 *
 * @param {Object} connectives - tag → { category, arity, polarity }
 * @param {Object} gradeConfig - { grade0: () => hash, gradeOmega: () => hash }
 * @returns {{ gradeTags: Set<string>, gradeAtoms: Set<string> }}
 */
function deriveGradeMeta(connectives, gradeConfig) {
  const gradeTags = new Set();
  for (const tag in connectives) {
    const { category: c, arity: a } = connectives[tag];
    if ((c === 'exponential' || c === 'monad') && a === 2) gradeTags.add(tag);
  }
  const gradeAtoms = new Set();
  if (gradeConfig) {
    for (const k of ['grade0', 'gradeOmega']) {
      const fn = gradeConfig[k];
      if (typeof fn !== 'function') continue;
      const h = fn();
      if (Store.tag(h) === 'atom') gradeAtoms.add(Store.child(h, 0));
    }
  }
  return { gradeTags, gradeAtoms };
}

export { _parseSignature, sortTable, inferSort, _checkTerm, checkForwardRule, checkClause, checkAll, deriveGradeMeta };
export default { _parseSignature, sortTable, inferSort, _checkTerm, checkForwardRule, checkClause, checkAll, deriveGradeMeta };
