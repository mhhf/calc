...
// Closed-world mode (set per checkAll run): undeclared atoms/predicates in
// rules and clauses are ERRORS, not silently skipped. The open-world skip
// below let every typo self-introduce a resource — "strongly typed" was
// only ever checked-if-declared (TODO_0265 Phase 6 post-mortem).
let _closedWorld = false;
...
function _checkTerm(h, expectedSort, sortTable, metavarSorts, errors, path) {
  ...
  // Rational / string literals: fixed sorts
  if (t === 'ratlit') {
    if (expectedSort !== '_' && expectedSort !== 'bin') {
      errors.push(`${path}: expected sort '${expectedSort}', got 'bin' (rational literal)`);
    }
    return;
  }
  if (t === 'strlit') {
    if (expectedSort !== '_' && expectedSort !== 'string') {
      errors.push(`${path}: expected sort '${expectedSort}', got 'string' (string literal)`);
    }
    return;
  }
  ...
  if (t === 'atom') {
    ...
    if (entry.argSorts.length !== 0) {
      const sig = entry.argNames
        ? entry.argSorts.map((s, i) => entry.argNames[i] ? `${entry.argNames[i]}: ${s}` : s).join(', ')
        : entry.argSorts.join(', ');
      errors.push(`${path}: '${name}' expects ${entry.argSorts.length} args (${sig}), got 0`);
      return;
    }
  ...
  if (tid < Store.PRED_BOUNDARY) {
    ...
    // Grade positions (bang/gmonad child 0) live in the GRADE algebra
    // (D2/D4: counts, delays, the internal gw/g0 markers) — they are not
    // term-sorted, so the closed-world token check does not apply there.
    const start = (t === 'bang' || t === 'gmonad') ? 1 : 0;
  ...
  if (a !== entry.argSorts.length) {
    const sig = entry.argNames
      ? entry.argSorts.map((s, i) => entry.argNames[i] ? `${entry.argNames[i]}: ${s}` : s).join(', ')
      : entry.argSorts.join(', ');
    errors.push(`${path}: '${predName}' expects ${entry.argSorts.length} args (${sig}), got ${a}`);
    return;
  }
...
function checkAll(definitions, compiledRules, clauses, opts = {}) {
  ...
  if (_closedWorld) {
    ...
    // honest path is a classifier-sort extension, not a string collision).
    const BUILTIN = new Set(['bin', 'string']);
    for (const [name, sig] of sorts) {
      for (const a of sig.argSorts) {
        if (a === 'type') {
          errors.push(`declaration '${name}': argument sort 'type' — quantification over propositions is not supported (needs a classifier-sort theory extension)`);
        } else if (!BUILTIN.has(a) && !sorts.has(a)) {
  ...
  for (const rule of compiledRules) {
    errors.push(...checkForwardRule(rule, sorts));
  }

  for (const [name, clause] of clauses) {
    errors.push(...checkClause(name, clause, sorts));
  }
  ...
  if (opts.queries) {
    for (const [name, q] of opts.queries) {
      ...
      for (const h of [q.lhsHash, q.rhsHash]) {
        if (typeof h === 'number') {
          _checkTerm(h, 'type', sorts, metavarSorts, errors, `directive '${name}'`);
        }
      }
    }
  }
