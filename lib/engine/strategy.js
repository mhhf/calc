...
function findAllMatches(state, rules, calc, strategy, stateIndex) {
  ...
  // Scan for fireable loli continuations via _loli index bucket
  const loliList = idx._loli || [];
  for (let li = 0; li < loliList.length; li++) {
    const lm = matchLoli(loliList[li], state, calc);
