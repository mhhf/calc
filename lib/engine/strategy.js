  ...
  return {
    ...
    build: (rules) => {
      ...
      for (const rule of rules) {
        const gvs = rule.discriminator.groundValues || [rule.discriminator.groundValue];
        for (const gv of gvs) {
...
function findMatch(state, rules, calc, matchOpts) {
  ...
  if (matchOpts && matchOpts.dynamicRuleTag && matchOpts.matchDynamicRule) {
 ...
 */
function findAllMatches(state, rules, calc, strategy, matchOpts) {
  const candidates = strategy
    ? strategy.getCandidateRules(state)
    : (rules.rules || rules);

  const opts = matchOpts || _matchOpts;
  const matches = [];
  for (const rule of candidates) {
    const m = tryMatch(rule, state, calc, opts);
    if (m) matches.push(m);
  }

  // Dynamic rules (e.g., loli continuations) — generic iteration
  if (opts.dynamicRuleTag && opts.matchDynamicRule) {
    const tagId = Store.TAG[opts.dynamicRuleTag];
    if (tagId !== undefined) {
      const group = state.linear.group(tagId);
      for (let li = 0; li < group.length; li++) {
        const lm = opts.matchDynamicRule(group[li], state, calc, opts);
