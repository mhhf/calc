...
// ── Auto-detection hook (RES_0143 M5) ──
// The DEFAULT layer detection (fingerprint + disc-tree) is optimization
// machinery living in opt/fingerprint.js; the composition root installs
// it here via installAutoLayers. Generic code never imports opt/.
// Uninstalled (direct unit callers of detectStrategy): no index layers —
// every rule flows through the predicate-filter catch-all, which is the
// same semantics, just O(R) selection.
let _autoLayers = null;
function installAutoLayers(fn) { _autoLayers = fn; }

/**
 * Auto-detect strategy stack based on rules, via the installed
 * opt-layer detection (fingerprint + disc-tree by default).
 *
 * @param {Object[]} rules - Compiled rules
 * @returns {{ getCandidateRules: function, fpConfig: Object|null }}
 */
function detectStrategy(rules) {
  const detected = _autoLayers ? _autoLayers(rules) : { layers: [], fpConfig: null };
  const stack = buildStack(rules, detected.layers);
  stack.fpConfig = detected.fpConfig || null;
  return stack;
}
...
export { buildStack, predicateLayer, detectStrategy, installAutoLayers, findMatch, findAllMatches };
export default { buildStack, predicateLayer, detectStrategy, installAutoLayers, findMatch, findAllMatches };
