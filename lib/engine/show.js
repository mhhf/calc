...
const DEFAULT_RENDER = {
  ...
  unit: 'one', additiveZero: 'zero', additiveUnit: 'top', grade0,
};

/**
 * Neutral leaf policy — no terminal atoms, no running predicate. Domain
 * policies (which atoms are terminals, which predicate marks a live
 * control point) are calculus data: they live on `cc.domain`
 * (classifyLeafPolicy / showExclude) and the calculus facade exports a
 * pre-bound classifyLeaf (RES_0143 L2 — the EVM instance used to be
 * the silent default here).
 */
const NEUTRAL_LEAF_POLICY = { terminals: {}, runningPred: null };
...
function show(h, argNamesTable, render = DEFAULT_RENDER) {
  ...
  if (n.tag === render.additiveUnit) return 'top';
 ...
 *   (default: neutral — no terminals, no running predicate; pass your
 *   calculus's policy from cc.domain.classifyLeafPolicy)
 * @returns {string} a terminal label, 'RUNNING', 'STUCK', or 'NO_STATE'
 */
function classifyLeaf(state, policy = NEUTRAL_LEAF_POLICY) {
 ...
 *   (default: none; pass your calculus's list from cc.domain.showExclude)
 * @returns {string[]}
 */
...
export { show, classifyLeaf, showInteresting, DEFAULT_RENDER, NEUTRAL_LEAF_POLICY };
export default { show, classifyLeaf, showInteresting, DEFAULT_RENDER, NEUTRAL_LEAF_POLICY };
