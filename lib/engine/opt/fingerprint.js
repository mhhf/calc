 ...
 *   autoLayers — the default strategy-stack detection installed into
 *                strategy.js by the composition root (index.js) via
 *                installAutoLayers; generic code never imports opt/.
 *
...
/**
 * Default strategy-stack auto-detection: fingerprint layer (when a
 * dominant discriminator exists) with the disc-tree as catch-all.
 * Installed into strategy.js by the composition root
 * (strategy.installAutoLayers(autoLayers)); without installation,
 * detectStrategy degrades to the predicate-filter catch-all — same
 * semantics, slower selection.
 */
function autoLayers(rules) {
  const layers = [];
  ...
  return { layers, fpConfig };
}

export { discIndex, fpDetect, fpValue, fpLayer, attachPred, buildFingerprintIndex, autoLayers };
export default { discIndex, fpDetect, fpValue, fpLayer, attachPred, buildFingerprintIndex, autoLayers };
