...
/**
 * Check structural memo — returns a memo node if the control hash was
 * already fully explored, null otherwise.
 *
 * @param {Object} state - FactSet-based State
 * @param {Object} memoCtx - { globalControl, boundCount }
 * @returns {{ type: 'memo', state: Object }|null}
 */
function checkMemo(state, memoCtx) {
  const controlHash = computeControlHash(state);
  if (memoCtx.globalControl.get(controlHash) === true) {
    return { type: 'memo', state: state.snapshotBulk(), _controlHash: controlHash };
  }
  return null;
}
