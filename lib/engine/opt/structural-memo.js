...
function controlHash(state, controlOpts) {
  // Control predicates are LOGIC POLICY: they come from
  // calculusConfig.domain.memoControlTags via controlOpts — the engine
  // holds no default names (RES_0143 L5: 'pc'/'stack' used to be silent
  // EVM defaults; a calculus without declared control tags now gets no
  // structural memo instead of a constant hash).
  if (!controlOpts || !controlOpts.pcPred) return null;
  const pcPred = controlOpts.pcPred;
  const stackPred = controlOpts.stackPred || null;
  const pcTagId = Store.TAG[pcPred];
  const stackTagId = stackPred !== null ? Store.TAG[stackPred] : undefined;
...
export { controlHash, createMemoCtx, recordMemo };
export default { controlHash, createMemoCtx, recordMemo };
