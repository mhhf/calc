...
function _mccarthy(rule, rc, sroaConfig) {
  ...
  const _reducedOf = new Map(); // original goal hash → reduced goal (bodyOrder)
  ...
  for (const g of aconsGoals) {
    ...
    if (g.type === 'get') {
      if (idxNum < depth) {
        ...
        _reducedOf.set(g.hash, newGoalHash);
      }
    } else if (g.type === 'set') {
      if (idxNum < depth) {
  ...
  // Maintain the carried bind order (Track 1): a read/write-head goal resolves
  // to a θ binding (drop it); a read/write-tail goal reduces to a smaller-index
  // goal (replace it in place). Everything else is θ-substituted.
  let newBodyOrder;
  if (rule.bodyOrder) {
    newBodyOrder = [];
    for (const g of rule.bodyOrder) {
      if (_reducedOf.has(g)) newBodyOrder.push(apply(_reducedOf.get(g), theta));
      else if (eliminatedGoals.has(g)) continue;
      else newBodyOrder.push(apply(g, theta));
    }
  }
  ...
  return _tagDisjoint({
    ...
    // McCarthy only runs on fused rules; preserve the flag so compileRule
    // computes the resolution body (TODO_0307 P3) and the downstream SROA
    // pass still recognises it as fused.
    isFused: true,
    ...(newBodyOrder ? { bodyOrder: newBodyOrder } : {}),
  });
}
...
function _trySROA(rule, rc, sroaConfig) {
  ...
  // Phase 3: Determine expansion depth. It must cover BOTH the deepest
  // accessed index AND the deepest STRUCTURED output — an arr_set whose output
  // is an explicit push chain `acons(v0, acons(v1, …, tail))` needs one slot
  // per pushed element, or _decompTheta cannot align that output against the
  // scalarized `acons(s0', …, tailVar)` and DROPS the tail var (leaving the
  // exit stack a dangling metavar → the resolver freshens it to an evar and
  // the whole stack collapses — TODO_0307 Bug B). Floor depth to the longest
  // acons prefix of any chained goal's array/output pattern.
  let maxIdx = 0n;
  for (const g of chainGoals) { const i = sroaConfig.parseIndex(g.idx); if (i > maxIdx) maxIdx = i; }
  let depth = Number(maxIdx) + 1;
  for (const g of chainGoals) {
    depth = Math.max(depth, _aconsPrefixLen(g.arrVar));
    if (g.type === 'set') depth = Math.max(depth, _aconsPrefixLen(g.outVar));
  }
  ...
  // Maintain the carried bind order (Track 1): the scalarized version-chain
  // goals are eliminated (inlined into θ), the survivors are θ-substituted in
  // place. Removing an eliminated goal keeps a valid order — its output is now
  // a concrete slot structure, so nothing downstream depends on it as a goal.
  let newBodyOrder;
  if (rule.bodyOrder) {
    newBodyOrder = rule.bodyOrder.map(g => apply(g, theta)).filter(g => !eliminatedGoals.has(g));
  }
  ...
  return _tagDisjoint({
    ...
    consequent: newConseqHash,
    sourceLabel: rule.sourceLabel || null,
    // SROA only runs on fused rules; preserve the flag so compileRule builds
    // the resolution body (TODO_0307 P3).
    isFused: true,
    ...(newBodyOrder ? { bodyOrder: newBodyOrder } : {}),
  });
}
...
/** Length of the explicit acons prefix of h (number of `acons` cells before a
 *  non-acons tail). `acons(a, acons(b, T))` → 2. Used to size SROA's slot
 *  expansion so structured push outputs decompose fully (TODO_0307 Bug B). */
function _aconsPrefixLen(h) {
  let n = 0;
  let cur = h;
  while (Store.tag(cur) === 'acons' && Store.arity(cur) === 2) { n++; cur = Store.child(cur, 1); }
  return n;
}
