...
// Separate buffer for Tier 3 outer loop (guards call tryCompiledClause
 ...
 */
function _analyzeTier3Clause(compiled, pred) {
  ...
  if (getPredicateHead(lastPrem) !== pred) return null;
  ...
  for (let i = 0; i < compiled.premises.length - 1; i++) {
    ...
    if (getPredicateHead(g) === pred) return null;
    guards.push(g);
  }
 ...
 */
function _buildTier2And3(backchainIndex, parsedModes) {
    ...
    for (const fa in allClauses[pred]) {
      for (const [, compiled] of allClauses[pred][fa]) {
        if (!compiled.premises || compiled.premises.length === 0) {
          ...
          const desc2 = _analyzeRecursiveClause(compiled, pred);
          if (desc2) {
            if (!rec2[fa]) rec2[fa] = [];
            rec2[fa].push(desc2);
            hasRec2 = true;
            ...
            const desc3 = _analyzeTier3Clause(compiled, pred);
            if (desc3) {
              if (!rec3[fa]) rec3[fa] = [];
              rec3[fa].push(desc3);
              hasRec3 = true;
...
function buildClauseDispatch(backchainIndex, parsedModes) {
  ...
  const { tier2, tier3 } = _buildTier2And3(backchainIndex, parsedModes);
...
function _tryRecursiveClause(desc, args, arity, modes, theories) {
  ...
  // Tier 1 base case match. Without this, _applyWrapStack reads stale bindings
 ...
 */
function _resolveGuard(guardGoal, outerLocalSlots, outerTheta, dispatch, theories, parsedModes, canonicalize) {
  // 1. Compiled clause dispatch (Tier 1+2 only — noTier3 prevents recursion)
  const ccResult = tryCompiledClause(dispatch, guardGoal, outerLocalSlots, outerTheta,
    canonicalize, theories, parsedModes, /* _noTier3 */ true);
  if (ccResult === true) return true;

  // 2. FFI fallback (lazy require: compiled-clauses.js ← ffi.js is circular at module level,
  //    but safe at call time since both are fully loaded by then)
  const { tryFFIDirect } = require('./ffi');
  const ffiResult = tryFFIDirect(guardGoal);
  if (ffiResult && ffiResult.success) {
    const ft = ffiResult.theta;
    for (let i = 0; i < ft.length; i++) {
      const s = outerLocalSlots[ft[i][0]];
      if (s !== undefined) {
        outerTheta[s] = canonicalize ? canonicalize(ft[i][1]) : ft[i][1];
      }
    }
    return true;
  }
...
function _tryTier3(t3entry, goal, slots, theta, canonicalize, theories, effectiveModes, dispatch, parsedModes) {
  ...
  for (let depth = 0; depth < MAX_DEPTH; depth++) {
    ...
    for (let ri = 0; ri < recCands.length; ri++) {
      ...
      // Resolve guard premises (uses _localTheta internally via tryCompiledClause)
      for (let gi = 0; gi < desc.guards.length; gi++) {
        const guardGoal = _resolve(desc.guards[gi], compiled.localSlots, _recTheta);
        const guardOk = _resolveGuard(guardGoal, compiled.localSlots, _recTheta,
 ...
 */
function tryCompiledClause(dispatch, goal, slots, theta, canonicalize, theories, parsedModes, _noTier3) {
