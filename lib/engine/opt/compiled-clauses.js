 ...
 */
function _resolveGuard(guardGoal, outerLocalSlots, outerTheta, dispatch, theories, parsedModes) {
  ...
  const ccResult = tryCompiledClause(dispatch, guardGoal, outerLocalSlots, outerTheta,
    null, theories, parsedModes, /* _noTier3 */ true);
  ...
  if (ffiResult && ffiResult.success) {
    ...
    for (let i = 0; i < ft.length; i++) {
      ...
      if (s !== undefined) outerTheta[s] = ft[i][1];
    }
    return true;
  }
  ...
  for (let depth = 0; depth < MAX_DEPTH; depth++) {
    ...
    for (let ri = 0; ri < recCands.length; ri++) {
      ...
      for (let gi = 0; gi < desc.guards.length; gi++) {
        ...
        if (!_resolveGuard(guardGoal, compiled.localSlots, _recTheta,
                           dispatch, theories, parsedModes)) {
          ok = false; break;
        }
      }
      if (!ok) continue;
