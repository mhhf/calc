 ...
 * PASS RECORDS ({ name, phase, enabled, run, resolveAfter } — the F3
 * pipeline shape) via composeOpts.optPasses from the composition root;
 * the semantic passes never import this module.
 */
...
import { scheduleGoals } from '../rule-analysis.js';
...
function _fuseChains(pool, rc, getModeMeta, chainConfigs) {
  ...
  for (const rule of pool) {
    ...
    const fusedLabels = [];
    const _chainToFused = new Map();  // chain goal hash → its collapsed goal
    for (const { chain, config } of safeChains) {
      ...
      const _fg = Store.put(cfg.fusedPred, args);
      newPersistent.push(_fg);
      for (const g of chain) _chainToFused.set(g.hash, _fg);
      fusedLabels.push(`${config.pred}-fused:${chain.length}`);
    }

    // Maintain the carried bind order (Track 1): replace each collapsed chain
    // IN PLACE at its FIRST goal's position, dropping the rest. The fused goal
    // produces the chain's final output, which LATER goals consume (e.g. the
    // MSTORE gas check reads the pushes' collapsed gas), so it must NOT be
    // moved to the end — its input is available at the first goal's position
    // and the chain's intermediates appear nowhere else (the safety check).
    let newBodyOrder;
    if (rule.bodyOrder) {
      newBodyOrder = [];
      const _emitted = new Set();
      for (const g of rule.bodyOrder) {
        const fg = _chainToFused.get(g);
        if (fg === undefined) { newBodyOrder.push(g); continue; }
        if (!_emitted.has(fg)) { _emitted.add(fg); newBodyOrder.push(fg); }
        // else: a later goal of an already-emitted chain — drop it.
      }
    }

    // Reassemble rule hash. Preserve isFused (TODO_0307 P3): chain fusion
    // runs AFTER block fusion, so its input may be a block-fused rule.
    // Dropping the flag hides that rule from the SROA pass downstream —
    // latent until the pool-disjoint fix let chain fusion actually reach
    // these rules (the metavar aliasing previously blocked the checked_sub
    // chains from forming).
    ...
    result.push(_makeRule(
      ...
      rule.sourceLabel,
      { ...(rule.isFused ? { isFused: true } : {}), ...(newBodyOrder ? { bodyOrder: newBodyOrder } : {}) }, rc
    ));
  }
...
function fusePair(producer, consumer, cutPred, rc, getModeMeta, prof) {
  ...
  const { hash: freshProdHash, theta: _renTheta } = _renameForCompose(producer, 'fusePair');
  ...
  let theta = /** @type {[number, number][]} */ (unify(pConseqFlat.linear[pCutIdx], cAnte.linear[cCutIdx]));
  ...
  for (let i = 0; i < pConseqLinear.length; i++) {
    ...
    _matchUnifyAttempts++;
    const theta2 = /** @type {[number, number][] | null} */ (unify(pApplied, cApplied));
    if (theta2 === null) {
  ...
  // CARRY THE BIND ORDER (TODO_0307 P3, Track 1). Fusion is sequential
  // composition on the pc thread: the producer's op(s) fire FULLY before the
  // consumer's, so the resolution order is
  //   [ producer.body … , consumer.body … ]
  // preserved by CONCATENATION, not by re-deriving the whole block's order.
  // What is carried is the ACROSS-op order (the expensive, error-prone part);
  // the WITHIN-op order is a fixed local property of each base op — and it is
  // NOT [ante, conseq]: a dynamic-gas op (MLOAD/MSTORE) computes its memory
  // cost in the consequent and reads it in the antecedent gas check, so ask-
  // before-tell would invert dataflow. So a base op's body is its OWN goals
  // dataflow-sorted once (small, unambiguous); a nested producer already
  // carries its ordered body. compileRule then reads this instead of sorting
  // the whole fused rule. (pool-disjoint: the producer rename is a no-op /
  // captured by _renTheta.)
  const _ren = (arr) => (_renTheta && _renTheta.length) ? arr.map(g => apply(g, _renTheta)) : arr;
  const _baseBody = (antePers, conseqPers, linSeed) => {
    const goals = [...antePers, ...conseqPers];
    if (goals.length <= 1) return goals;
    const seed = new Set();
    for (const p of linSeed) collectMetavars(p, seed);
    const ord = scheduleGoals(goals, seed, getModeMeta);
    return ord.map(i => goals[i]);
  };
  const pBody = producer.bodyOrder
    ? _ren(producer.bodyOrder)
    : _baseBody(pAnte.persistent, pConseqFlat.persistent, pAnte.linear);
  const cBody = consumer.bodyOrder
    ? consumer.bodyOrder
    : _baseBody(cAnte.persistent, cConseqFlat.persistent, cAnte.linear);
  const fusedBodyOrder = applyAll([...pBody, ...cBody]);
  ...
  const fused = _makeRule(
    ...
    { isFused: true, bodyOrder: fusedBodyOrder },
    rc
  );
...
function fusePairEx(producer, consumer, cutPred, rc, getModeMeta, prof) {
  ...
  for (let ai = 0; ai < alts.length; ai++) {
    ...
    // Carry the bind order into the projection: the producer's body, then
    // this branch's guards (which read values the body computed).
    const projFlags = producer.bodyOrder
      ? { ...(producer.isFused ? { isFused: true } : {}), bodyOrder: [...producer.bodyOrder, ...(alt.persistent || [])] }
...
// ── Pipeline pass records (RES_0143 F3) ──────────────────────────────
/** P5: fuse 1:1 producer→consumer pairs into mega-rules via the shared
 *  linear fusion predicate (cc.compose.linearFusionPredicate). */
const fuseBlocksPass = {
  name: 'fuse-blocks',
  phase: 'load/compose/fuse-blocks',
  enabled: (ctx) => !!ctx.linearFusionPredicate,
  run(pool, ctx) {
    const r = _fuseBasicBlocks(pool, ctx.rc, ctx.getModeMeta,
      ctx.linearFusionPredicate, ctx.fusionBarriers, ctx.onPhase);
    ctx.diagnostics.fusedRuleReduction = r.fusedCount;
    ctx.diagnostics.fuseChainLengths = r.chainLengths;
    return { pool: r.rules, meta: {
      fusedReduction: r.fusedCount,
      chains: r.chainLengths.length,
      chainLengths: r.chainLengths,
    } };
  },
  // Fusion substitutes ground cut values into consumers → newly-ground
  // goals; resolve them even when chain fusion is idle (the pre-F3 code
  // ran one residual after the whole fusion block).
  resolveAfter: true,
};

/** P5.5: collapse additive threading chains algebraically (cc-declared
 *  ChainConfigs). Grounds new variables → resolveAfter. */
const fuseChainsPass = {
  name: 'fuse-chains',
  phase: 'load/compose/fuse-chains',
  enabled: (ctx) => !!(ctx.chainConfigs && ctx.chainConfigs.length),
  run(pool, ctx) {
    return { pool: _fuseChains(pool, ctx.rc, ctx.getModeMeta, ctx.chainConfigs),
      meta: { chainConfigs: ctx.chainConfigs.length } };
  },
  resolveAfter: true,
};

export { _fuseChains as fuseChains, fusePair, fusePairEx, _fuseBasicBlocks as fuseBasicBlocks, _openEx, _openConseqEx, fuseBlocksPass, fuseChainsPass };
export default { fuseChains: _fuseChains, fusePair, fusePairEx, fuseBasicBlocks: _fuseBasicBlocks, fuseBlocksPass, fuseChainsPass };
