...
function execExStep(step, theta, slots) {
  ...
  for (let k = 0; k < ft.length; k++) {
    ...
    if (outSlot !== undefined) {
      theta[outSlot] = ft[k][1];
    } else if (ft[k][1] !== ft[k][0]) {
      // Output position was already bound (e.g. to an evar): the computed
      // value has nowhere to go and must not be discarded silently — defer
      // to provePersistent, whose FFI path unifies outputs properly.
      return false;
    }
  }
  return true;
...
export { compileExChain, execExStep };
export default { compileExChain, execExStep };
