 ...
 * goal is resolved without subApplyIdx, state lookup, or ffiDirect.
...
// Lazy ILL FFI defaults — loaded on first use, overridable via ffiContext param
let _defaultMeta = null;
let _defaultParsedModes = null;
let _defaultGet = null;

function _ensureDefaults() {
  if (_defaultMeta) return;
  const ffiRegistry = require('../ill/ffi');
  _defaultMeta = ffiRegistry.defaultMeta;
  _defaultParsedModes = ffiRegistry.parsedModes;
  _defaultGet = ffiRegistry.get;
}
 ...
 * @param {Object|null} ffiContext - { meta, parsedModes, get } — injected FFI config
 * @returns {Array|null} Array of {handler, argSlot, argMeta, inputMask, arity}|null
 */
function compileExChain(rule, ffiContext) {
  if (!rule.existentialSlots || rule.existentialSlots.length === 0) return null;
  if (!rule.existentialGoals) return null;

  // Extract FFI context or use lazy defaults
  const _meta = ffiContext ? ffiContext.meta : (_ensureDefaults(), _defaultMeta);
  const _get = ffiContext ? ffiContext.get : (_ensureDefaults(), _defaultGet);
  const _modes = ffiContext ? ffiContext.parsedModes : (_ensureDefaults(), _defaultParsedModes);

  const slots = rule.metavarSlots;

  // Collect goals in consequent-persistent order (mirrors resolveEx)
 ...
 * @returns {boolean}
 */
function execExStep(step, theta, slots) {
...
module.exports = { compileExChain, execExStep };
