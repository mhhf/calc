...
function proveWithFFI(patterns, startIdx, theta, slots, state, calc, evidenceOut, matchOpts) {
  ...
  const ffiMeta = matchOpts && matchOpts.ffiMeta;
  const ffiGet = matchOpts && matchOpts.ffiGet;
  const parsedModes = matchOpts && matchOpts.ffiParsedModes;
  const ffiIsGround = matchOpts && matchOpts.ffiIsGround;
