      ...
      if (ffiResult.success) {
        ...
        for (let fi = 0; fi < ft.length; fi++) {
          ...
          if (slot !== undefined) {
            ...
            // Output position is a ground literal — verify FFI result matches
            if (ft[fi][1] !== ft[fi][0]) { ffiOk = false; break; }
          }
        }
        if (ffiOk) {
  ...
  if (result && result.success) {
    ...
    for (let fi = 0; fi < ft.length; fi++) {
      ...
      if (s !== undefined) {
        ...
        // Output position is a ground literal — FFI computed a definite value
        // that doesn't match. This is a definitive failure (not advisory):
        // arr_get is functional, so the value at this index is certain.
        if (ft[fi][1] !== ft[fi][0]) return false;
      }
    }
    return true;
  }
...
/**
 * Get range metadata for a predicate (used for range-based discriminators).
 * @param {string} pred - predicate name
 * @returns {{ lo: number, hi: number }|null}
 */
function getRange(pred) {
  const meta = _ffiMeta[pred];
  return (meta && meta.range) || null;
}
