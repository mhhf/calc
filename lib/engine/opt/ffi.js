...
const { tryCompiledClause } = require('./compiled-clauses');
...
function provePersistentWithFFI(patterns, startIdx, theta, slots, state, calc, evidenceOut, matchOpts) {
  ...
  while (idx < patterns.length) {
    ...
    // 2. Compiled clause dispatch (structural predicates — compiled from ILL clauses)
    ...
    // 3. FFI direct (arithmetic predicates — compiled dispatch missed)
    ...
    if (ffiResult) {
      ...
      // FFI advisory failure — fall through to clause resolution
    }
...
function executePersistentStep(spec, theta) {
  // Compiled clause spec — resolve goal via tryCompiledClause
  if (spec._compiled) {
    const goal = subApplyIdx(spec._pattern, theta, spec._slots);
    const result = tryCompiledClause(
 ...
 * @returns {Object|null}
 */
function compilePersistentStep(pattern, slots, calcContext) {
 ...
 * @param {Object|null} calcContext - { clauseDispatch, theoryLookup } for compiled clause fallback
 */
function compilePersistentSteps(rule, calcContext) {
  const persistentPats = rule.antecedent.persistent || [];
  if (persistentPats.length === 0) return;

  const slots = rule.metavarSlots;
  const steps = persistentPats.map(p => compilePersistentStep(p, slots, calcContext || null));
