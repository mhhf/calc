...
// Lazy require: ffi.js ↔ compiled-clauses.js is circular at module level.
// Deferring to call-time avoids ESM bundler (Vite/Rollup) TDZ errors.
let _tryCCDispatch = null;
function _getCCDispatch() {
  if (!_tryCCDispatch) _tryCCDispatch = require('./compiled-clauses').tryCCDispatch;
  return _tryCCDispatch;
}

const PROFILE = typeof process !== 'undefined' && process.env.CALC_PERF_PROFILE === '1';

// ─── Lazy ILL FFI defaults (backward compatibility for direct callers) ───
//
// The orchestrator (index.js) injects FFI context via matchOpts/calcContext,
// so _ensureDefaults() is NEVER called on the orchestrator path.
// This exists solely for direct callers (tests, bridge.js) that bypass
// the orchestrator. It is a tolerated ILL dependency — not a layer violation
// because it's lazy (call-time, not import-time) and only activates when
// no config is provided.

let _defaultFfiMeta = null;
let _defaultFfiParsedModes = null;
let _defaultFfiGet = null;
let _defaultFfiIsGround = null;

function _ensureDefaults() {
  if (_defaultFfiMeta) return;
  const ffi = require('../ill/ffi');
  _defaultFfiMeta = ffi.defaultMeta;
  _defaultFfiParsedModes = ffi.parsedModes;
  _defaultFfiGet = ffi.get;
  _defaultFfiIsGround = ffi.convert.isGround;
  _validateArity(_defaultFfiParsedModes);
}
 ...
 * When called with only goal, uses lazy ILL defaults. */
function ffiDirect(goal, ffiMeta, ffiGet, parsedModes, ffiIsGround) {
  if (!ffiMeta) { _ensureDefaults(); ffiMeta = _defaultFfiMeta; ffiGet = _defaultFfiGet; parsedModes = _defaultFfiParsedModes; ffiIsGround = _defaultFfiIsGround; }
...
function proveWithFFI(patterns, startIdx, theta, slots, state, calc, evidenceOut, matchOpts) {
  // D2: extract FFI context from matchOpts at function entry (V8 IC-friendly locals)
  const ffiMeta = (matchOpts && matchOpts.ffiMeta) || (_ensureDefaults(), _defaultFfiMeta);
  const ffiGet = (matchOpts && matchOpts.ffiGet) || (_ensureDefaults(), _defaultFfiGet);
  const parsedModes = (matchOpts && matchOpts.ffiParsedModes) || (_ensureDefaults(), _defaultFfiParsedModes);
  const ffiIsGround = (matchOpts && matchOpts.ffiIsGround) || (_ensureDefaults(), _defaultFfiIsGround);
  ...
  while (idx < patterns.length) {
    ...
    // 1. State lookup (cheapest — array scan)
    {
    ...
    if (calc && calc.clauseDispatch) {
      ...
      const ccResult = _getCCDispatch()(
        calc.clauseDispatch, goal, slots, theta,
        (matchOpts && matchOpts.canonicalize) || null,
        calc.theoryLookup || null,
        (matchOpts && matchOpts.ffiParsedModes) || null
      );
      if (ccResult === true) {
        if (PROFILE) { profile.compiledCalls++; profile.compiledTime += performance.now() - _t1; _profileHit('compiled', goal); }
        if (evidenceOut) evidenceOut.push({ goal, method: 'compiled' });
        if (onProveSuccess) onProveSuccess(goal, 'compiled', { ground: _isGround, hasFfi: _hasFfi });
        idx++;
        continue;
      }
    }
...
function compilePS(pattern, slots, calcContext, ffiContext) {
  const _meta = ffiContext ? ffiContext.meta : (_ensureDefaults(), _defaultFfiMeta);
  const _get = ffiContext ? ffiContext.get : (_ensureDefaults(), _defaultFfiGet);
  const _modes = ffiContext ? ffiContext.parsedModes : (_ensureDefaults(), _defaultFfiParsedModes);
  const _isGr = ffiContext ? ffiContext.isFFIGround : (_ensureDefaults(), _defaultFfiIsGround);
 ...
 *
 * @param {Object} rule - Compiled forward rule
 * @param {Object|null} calcContext - { clauseDispatch, theoryLookup } for compiled clause fallback
 * @param {Object|null} ffiContext - { meta, parsedModes, get, isFFIGround } — injected FFI config
 */
function compilePSAll(rule, calcContext, ffiContext) {
  const persistentPats = rule.antecedent.persistent || [];
  if (persistentPats.length === 0) return;

  const slots = rule.metavarSlots;
  const steps = persistentPats.map(p => compilePS(p, slots, calcContext || null, ffiContext || null));
...
function getModeMeta(pred) {
  _ensureDefaults();
  const meta = _defaultFfiMeta[pred];
  if (!meta) return null;
  return { modes: _defaultFfiParsedModes[pred], multiModal: !!meta.multiModal };
}
...
module.exports = {
  ...
  compilePS,
  compilePSAll,
  get ffiParsedModes() { _ensureDefaults(); return _defaultFfiParsedModes; },
  getModes: (pred) => { _ensureDefaults(); return _defaultFfiParsedModes[pred] || null; },
  getModeMeta,
  getProfile,
  resetProfile,
  _validateArity,
