...
import { isPredTag, predHead, extFV, boundExt } from '../../kernel/ast.js';
...
import { tryCCDispatch as _tryCCDispatch } from './compiled-clauses.js';
  ...
  while (idx < patterns.length) {
    ...
    if (calc && calc.clauses && calc.definitions) {
      ...
      const result = backward.prove(goal, calc.clauses, calc.definitions, {
        ...
        // Match proveNaive (the FFI-off reference): search ALL first-arg
        // buckets. Without this the backchainer defaults allBuckets to
        // !useFFI = false, so a goal with an unbound first arg (fa='_')
        // whose clauses are keyed under specific first-args is silently
        // unprovable under FFI-on though FFI-off proves it (TODO_0311).
        allBuckets: true,
      });
      if (PROFILE) {
      ...
      if (result.success) {
        ...
        for (let ri = 0; ri < rt.length; ri++) {
          ...
          // Canonicalize like proveNaive: a clause-resolution output may be in
          // a non-canonical theory form (e.g. i(i(e)) ≡ binlit 2); storing it
          // raw would desync a later FFI tier keyed on the canonical tag.
          if (slot !== undefined) theta[slot] = canonicalize ? canonicalize(rt[ri][1]) : rt[ri][1];
        }
...
export { ffiDirect, proveWithFFI, execPS, compilePS, getProfile, resetProfile, _validateArity };
export default { ffiDirect, proveWithFFI, execPS, compilePS, getProfile, resetProfile, _validateArity };
