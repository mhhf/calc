...
function deltaBypass(pattern, origPos, rule, state, theta,
  ...
  // atom path is tag-aware (see match.js matchLinear1 / groupKeyForPredTag)
  const candidates = tagIdx >= 0 ? state.linear.group(tagIdx)
    : meta.predTag !== undefined ? state.groupForPredTag(meta.predTag, pred)
