 ...
 *
 * Used by symexec to prune infeasible branches at oplus expansion time:
 * - !eq X Y → union(X, Y)
 * - !neq X Y → forbid(X, Y)
...
const { isGround } = require('./ffi/convert');
...
class EqNeqSolver {
  ...
  _find(x) {
    while (this.parent[x] !== x) {
      // Path splitting (no compression to keep undo simple)
      x = this.parent[x];
    }
    return x;
  }
