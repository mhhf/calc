...
// Default ILL evalNumeric — lazy require to avoid import-time coupling
let _illBinToInt = null, _illIsGround = null;
function _defaultEvalNumeric(h) {
  if (!_illBinToInt) _illBinToInt = require('./ill/ffi/convert').binToInt;
  if (!_illIsGround) _illIsGround = require('./ill/ffi/convert').isGround;
  return _illIsGround(h) ? _illBinToInt(h) : null;
}
...
class EqNeqSolver {
  constructor(opts = {}) {
    this._evalNumeric = opts.evalNumeric || _defaultEvalNumeric;
  ...
  addConstraint(h) {
    ...
    const a0 = this._evalNumeric(arg0);
    const a1 = this._evalNumeric(arg1);
    if (a0 !== null && a1 !== null) {
      if (pred === 'eq') {
        if (a0 !== a1) {
