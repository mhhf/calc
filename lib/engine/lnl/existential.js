...
const { execExStep } = require('../opt/existential-compile');
 ...
 */
function resolveEx(theta, slots, rule, state, calc, matchOpts) {
  ...
  if (goals.length > 0) {
    ...
    if (useCompiled) {
      ...
      for (let i = 0; i < goals.length; i++) {
        ...
        if (step && execExStep(step, theta, slots)) continue;
        if (provePersistent) {
