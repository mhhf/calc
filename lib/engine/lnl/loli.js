...
const { flattenAntecedent, expandConsequentChoices, collectMetavars } = require('../compile');
...
function matchLoli(h, state, calc, matchOpts) {
  ...
  const { linear: linearTriggers, persistent: persistentTriggers } = flattenAntecedent(trigger, rc);
  ...
  const produced = flattenAntecedent(instantiated, rc);
  const consequentAlts = expandConsequentChoices(produced, rc);
