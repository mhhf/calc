...
const { proveNaive: _proveNaiveFallback } = require('./persistent');
...
function matchLoli(h, state, calc, matchOpts = EMPTY_MATCH_OPTS) {
  ...
  if (persistentTriggers.length > 0) {
    ...
    // Fallback to same-layer proveNaive (no FFI, clause resolution only).
    const proveFn = matchOpts.provePersistent || _proveNaiveFallback;
