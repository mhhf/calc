...
// Lazy accessor for compiled clause dispatch (avoids LNL→opt import-time coupling).
// tryCCDispatch is a pure function — given dispatch table + goal → true/false.
let _tryCCDispatch = null;
function _getCCDispatch() {
  if (!_tryCCDispatch) _tryCCDispatch = require('../opt/compiled-clauses').tryCCDispatch;
 ...
 */
function proveNaive(patterns, startIdx, theta, slots, state, calc, evidenceOut, matchOpts) {
