...
import { predHead as getPredHead } from '../kernel/ast.js';
import { isGround, collectMetavars } from './pattern-utils.js';

/**
 * Dataflow topological order over a set of goals (TODO_0307 P3).
 *
 * A fused basic block's antecedent asks + consequent tells form ONE dataflow
 * DAG (fusion erased the pc-thread order that sequenced them). Recover the
 * bind order: a goal is schedulable once its INPUT vars are produced (in the
 * grounded seed, or output by an already-scheduled goal). Mode-'-' positions
 * are outputs; multiModal predicates (any single arg computable from the rest)
 * schedule with ≤1 ungrounded input and then contribute ALL their vars. The
 * result orders every producer before its consumer, so the runtime resolver
 * never sees a blank input (the witness-capture hazard).
 *
 * @param {number[]} goals - goal hashes (ante ∪ conseq persistent)
 * @param {Set<number>} seed - metavars grounded before any goal runs (entry-linear vars)
 * @param {Function|null} getModeMeta - (pred) → { modes, multiModal } | null
 * @returns {number[]} permutation — goals[result[k]] is the k-th scheduled goal
 */
function scheduleGoals(goals, seed, getModeMeta) {
  const grounded = new Set(seed);
  const infos = goals.map((goal, originalIdx) => {
    const pred = getPredHead(goal);
    const meta = pred && getModeMeta ? getModeMeta(pred) : null;
  ...
  const isReady = (info) => {
    ...
    // multiModal ("compute any arg from the rest") licenses one ungrounded
    // INPUT — but ONLY for a predicate with no declared output position
    // (`plus`, all '+': the odd one out IS the output). A predicate with a
    // declared '-' output (`arr_set` +++-, `arr_get` ++-) has a FIXED
    // dataflow direction in a fused block; its '+' array/index inputs must be
    // produced first, or the topsort schedules it before its producer and the
    // resolver defers a value that should have been computed (TODO_0307 P3).
    const hasOutput = meta.modes.indexOf('-') >= 0;
    return (meta.multiModal && !hasOutput) ? ungroundedInputs <= 1 : ungroundedInputs === 0;
  };
  const order = [];
  ...
  while (progress && remaining.size > 0) {
    ...
    for (const idx of remaining) {
      if (isReady(infos[idx])) {
        order.push(infos[idx].originalIdx);
  ...
  // Unschedulable remainder (cycle / undetermined) in original order — the
  // resolver will defer these (their inputs never determined → eigenvariables).
  const rest = [...remaining].sort((a, b) => infos[a].originalIdx - infos[b].originalIdx);
  for (const idx of rest) order.push(infos[idx].originalIdx);
  return order;
}
/**
 ...
 */
function changedPos(anteHash, conseqHash) {
 ...
 * }}
 */
function deltaAnalysis(compiledRule) {
  ...
  for (const [pred, cList] of consumedByPred) {
    ...
    for (let i = 0; i < pairs; i++) {
      ...
      const changedPositions = changedPos(anteHash, conseqHash);
 ...
 * @param {Object} analysis - Output of deltaAnalysis()
 * @param {Object} slots - metavarSlots from compileRule
 * @returns {Array<{type: string, ...}>}
 */
function patternRoles(anteLinear, analysis, slots) {
 ...
 */
function compileSub(pattern, slots) {
  const tag = Store.tag(pattern);
  if (!tag) return null;
  if (tag === 'atom' || tag === 'freevar') return null; // literal, no sub needed
  if (tag === 'metavar') {
    if (slots[pattern] !== undefined) {
...
export { analyzeRule, deltaAnalysis, patternRoles, compileSub, scheduleGoals };
export default { analyzeRule, deltaAnalysis, patternRoles, compileSub, scheduleGoals };
