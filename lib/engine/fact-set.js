...
class FactSet {
  ...
  constructor(maxTagId, policy = null) {
    ...
    this._rl = !!(policy && policy.runLength);
  ...
  /** Resolve the group for a fact: policy groupKey wins over the passed tagIdx. */
  _key(tagIdx, hash) {
    return this._gk ? this._gk(hash) : tagIdx;
  }
...
class State {
  ...
  hasPredicate(predName) {
    ...
    // Atom check: scan atom group for matching name. Under a stamped index
    // policy the group may hold at(atom, t) wrappers — unwrap to the inner
    // atom before comparing (audit round 12, F1). The stamp tag comes from the
    // linear policy (cc.factSetPolicy.stampTag); default 'at', dead for
    // untimed calculi whose atom group holds no stamped facts.
