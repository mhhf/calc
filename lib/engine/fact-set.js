 ...
 * and arena-based undo for backtracking in symexec.
 *
...
class FactSet {
  ...
  insert(tagIdx, hash, arena) {
    ...
    if (oldCount > 0) this.hash ^= hashPair(hash, oldCount);
    this.hash ^= hashPair(hash, oldCount + 1);
  ...
  remove(tagIdx, hash, arena) {
    ...
    this.hash ^= hashPair(hash, oldCount);
    if (oldCount > 1) this.hash ^= hashPair(hash, oldCount - 1);
