...
// ─── Packed fact refs (label mode) ──────────────────────────────────
// The FactSet ROW ENCODING for labelled states (THY_0024): a fact ref
// packs (innerHash, stampId) into one float64-exact number — Map-keyable
// and splittable into two ints for arena records. The encoding is
// fact-set vocabulary (rows, arena records, group keys); the label
// INTERNING TABLE it indexes into (StampTable) is timed-layer machinery
// and arrives via the policy object: a policy with `labels` must carry
// `stampTable` (the constructor), supplied by lib/timed (RES_0143 M1 —
// fact-set imports no timed code; the untimed engine pays nothing).
const STAMP_BITS = 23;
...
class FactSet {
  ...
  constructor(maxTagId, policy = null) {
    ...
    if (this._lab && !policy.stampTable) {
      throw new Error('FactSet: a policy with `labels` must carry `stampTable` (the label-interning constructor, supplied by the timed layer — lib/timed/labels.js)');
    }
    this.stamps = this._lab ? new policy.stampTable(policy.labels) : null;
...
class State {
  ...
  hasPredicate(predName) {
    // UNION semantics: a name can be a predicate tag globally and an atom
    // in the current program (cross-program ambiguity — see
    // groupKeyForPredTag). hasPredicate is a presence PRE-FILTER, so it
    // must never under-approximate: check the tag group AND fall through
    // to the atom check.
    const tagId = Store.TAG[predName];
    if (tagId !== undefined && tagId >= Store.PRED_BOUNDARY) {
      if (this.linear.groupLen(tagId) > 0) return true;
    }
  ...
  groupForPred(predName) {
    ...
    const k = this.groupKeyForPred(predName);
    return k < 0 ? _emptyI32 : this.linear.group(k);
  }

  /** groupForPred with a KNOWN pattern head tag — see groupKeyForPredTag. */
  groupForPredTag(predTag, predName) {
    const k = this.groupKeyForPredTag(predTag, predName);
    return k < 0 ? _emptyI32 : this.linear.group(k);
  }

  /** Group INDEX for a predicate name (label-column iteration reads the
   *  parallel sid/count arrays through it) — same resolution as
   *  groupForPred; -1 when the predicate can have no group. */
  groupKeyForPred(predName) {
    if (!predName) return -1;
    const tagId = Store.TAG[predName];
    if (tagId !== undefined && tagId >= Store.PRED_BOUNDARY) return tagId;
    return this._atomGroupKey(predName);
  }

  /** Group key for a rule pattern with a KNOWN head tag (compile.js
   *  linearMeta.predTag). The global name→tag table is ambiguous across
   *  loaded programs (the same name can be a predicate tag in one and an
   *  atom in another); the pattern's own head tag is the truth — an
   *  atom-headed pattern (predTag 0) takes the atom path even when a
   *  same-named predicate tag exists globally. */
  groupKeyForPredTag(predTag, predName) {
    if (predTag >= Store.PRED_BOUNDARY) return predTag;
    if (!predName) return -1;
    return this._atomGroupKey(predName);
  }

  _atomGroupKey(predName) {
    // Atom predicate. Policy-keyed sets file each atom head in its own
    // group (TODO_0277) — exact candidates, no population scan.
    if (this.linear._gk) {
      const inner = Store.put('atom', [predName]);
      return this.linear._key(Store.TAG.atom,
        this.linear._lab ? packRef(inner, 0) : inner);
    }
    // Classic: the whole atom group (caller filters)
    return Store.TAG.atom;
  }
}
...
/**
 * The engine's canonical state zone keys, in State field order. The ONE
 * place the forward engine's zone vocabulary is defined (TODO_0086):
 * boundary plain-object states are { linear: {...}, persistent: {...} }.
 * These are ENGINE constants, distinct from a calculus's sequent zone
 * names (calculus.contextStructure) — the bridge maps between the two.
 */
const STATE_ZONES = Object.freeze(['linear', 'persistent']);

export { zobristMix, lowerBound, FactSet, Arena, State, fromObject, toObject, INSERT_OP, STATE_ZONES, packRef, refInner, refStamp, STAMP_BITS, STAMP_CAP, INNER_CAP };
export default { zobristMix, lowerBound, FactSet, Arena, State, fromObject, toObject, INSERT_OP, STATE_ZONES, packRef, refInner, refStamp, INNER_CAP };
