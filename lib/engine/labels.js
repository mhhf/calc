 ...
 *
 * The table is session-local and mortal: shiftAll (rebase) rewrites the
 * VALUES in place — O(#distinct labels), zero state writes, monotone so
 * group order is preserved — and compact() drops unreferenced entries at
 * rebase points. Contrast the append-only Store arena, where every
 * (inner × stamp) pair used to intern an immortal node.
 *
...
class StampTable {
  ...
  /**
   * Uniform shift of every VALUE by −b (rebase): ids stay, floats/mixes
   * recompute, reified terms invalidate, the intern map rebuilds. A
   * uniform shift is monotone, so any structure sorted by cmp stays
   * sorted. Unreferenced entries may go negative — harmless (nothing
   * reads them; compact() collects them at the same call sites).
   */
  shiftAll(bv) {
    const alg = this.alg;
    this.ids.clear();
    for (let i = 0; i < this.vals.length; i++) {
      const nv = alg.sub(this.vals[i], bv);
      this.vals[i] = nv;
      this.floats[i] = alg.float(nv);
      this.mixes[i] = alg.mix(nv);
      this.terms[i] = -1;
      this.ids.set(alg.key(nv), i);
    }
  }

  /**
   * Drop entries not marked live (unit always survives); returns a dense
   * remap Int32Array (old id → new id, dead → -1). The CALLER owns
   * rewriting its stampId columns through the remap.
   */
  compact(live) {
    const n = this.vals.length;
    const remap = new Int32Array(n).fill(-1);
    const vals = [], floats = [], mixes = [], terms = [];
    this.ids.clear();
    for (let i = 0; i < n; i++) {
      ...
      mixes.push(this.mixes[i]);
      terms.push(this.terms[i]);
      this.ids.set(this.alg.key(this.vals[i]), ni);
    }
    this.vals = vals; this.floats = floats; this.mixes = mixes; this.terms = terms;
    return remap;
  }
