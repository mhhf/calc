...
// checkPriors is INJECTED (opts.checkPriors) from the composition root
// (lib/measure/priors.js, RES_0143 M2) — the loader pass holds no
// measure-layer import; priors absent -> validation skipped (the same
// presence gate as before: priorsTable empty).
import { putRat } from '../kernel/rat-term.js';

/**
 * @returns {{ sortSystem, priorAdvice, stateMasses }}
 */
function materializeLoadTimeClauses({ checkPriors = null, cc, definitions, clauses, sortVarsTable,
  ...
  if (priorsTable.size > 0) {
    const pr = checkPriors ? checkPriors(priorsTable, sortSystem, definitions) : { priors: new Map(), errors: [], warnings: [] };
