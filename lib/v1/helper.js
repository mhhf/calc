// Browser-compatible: use crypto if available, otherwise use simple hash
let sha3;
if (typeof window === 'undefined') {
  // Node.js environment
  const crypto = require('crypto');
  sha3 = str => crypto.createHash('sha3-256').update(str).digest('hex');
} else {
  // Browser environment - use simple hash for unique IDs
  sha3 = str => {
    let hash = 0;
...
// cli-color may not be available in browser
let clc;
try {
