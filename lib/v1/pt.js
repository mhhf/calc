...
// const calcParser = require("../v1/parser.js");
...
class PT {
  constructor(params) {
    ...
    this.delta_out = {};
    this.proven = false;
    // Prover state (for focused proof search) - tracked externally, not in sequent
    this.proverState = params.proverState || null;
  }
  ...
  toString(o = {}) {
    ...
    switch (html || o.style) {
      ...
      // console.log(pre);
        ...
        // Escape rule type for KaTeX (replace _ with \_ for subscripts)
        let typeLatex = this.type.replace(/_/g, '\\_');
        let nameHtml;
        try {
          nameHtml = katex.renderToString(typeLatex);
        } catch (e) {
          nameHtml = `<code>${this.type}</code>`;
        }
        let name = `<span class="name">${nameHtml}</span>`;
