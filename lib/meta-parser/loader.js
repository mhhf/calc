...
function resolveExtends(extendsName, basePath) {
  ...
  // Self-resolution guard (TODO_0309): a calculus named after its family
  // (`calculus/sax/sax.calc` with `@extends sax`) must not resolve the
  // chain to ITSELF — skip any candidate that is the extending file and
  // let the sibling/family phases find the real parent.
  const self = path.resolve(basePath);
  for (const ext of ['.family', '.calc', '']) {
    const tryPath = path.join(dir, `${extendsName}${ext}`);
    if (path.resolve(tryPath) === self) continue;
