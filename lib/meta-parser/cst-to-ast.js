...
const BINARY_OPS = { 'expr_tensor': 'tensor', 'expr_with': 'with', 'expr_plus': 'oplus' };
...
function cstToAst(node, source) {
  ...
  switch (nodeType) {
    ...
    case 'expr': {
      ...
      if (text.startsWith('exists ') || text.startsWith('forall ')) {
        const kind = text.startsWith('exists') ? 'exists' : 'forall';
        const varAst = cstToAst(namedChildren[0], source);
        const bodyAst = cstToAst(namedChildren[1], source);
        return AST.TermApp(AST.TermApp(AST.TermIdent(kind), varAst), bodyAst);
      }
 ...
 */
/** Core parse: tree-sitter tree → AST result. Requires Parser to be initialized. */
function parseCore(source) {
  ...
  return { success: true, ast: cstToAst(tree.rootNode, source) };
}
