...
const saturate = function ({seq, atoms, negative, rules, bwd, getRule}) {
  let seq_ = run_chain({seq, atoms, negative, rules, bwd, getRule});
  ...
  while(success) {
    ...
    for(var i = 0; i < Object.keys(seq_.persistent_ctx).length; i++) {
      ...
      seq__ = run_chain({seq: seq__, atoms, negative, rules, bwd, getRule})
      if(seq__) {
