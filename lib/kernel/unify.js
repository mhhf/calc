...
function unifyBinlit(pattern, term, G) {
  ...
  if ((ptag === 'i' || ptag === 'o') && Store.arity(pattern) === 1) {
    ...
    if (ptag === 'o') {
      ...
      G.push([tailPattern, Store.put1('binlit', value / 2n)]);
      return true;
    }
    if (ptag === 'i') {
    ...
    if (tt === 'binlit') {
      ...
      if ((ptag === 'i' || ptag === 'o') && Store.arity(p) === 1) {
        ...
        if (ptag === 'o') {
          ...
          _Gp[gLen] = tailPattern; _Gt[gLen] = Store.put1('binlit', value / 2n); gLen++;
          continue;
        }
        if (ptag === 'i') {
  ...
  while (gLen > 0) {
    ...
    if (tpId === TAG.binlit) {
      ...
      if ((ttId === TAG.i || ttId === TAG.o) && Store.arity(t) === 1) {
        ...
        if (ttId === TAG.o) {
          if (value === 0n || value % 2n !== 0n) return false;
          _Gp[gLen] = Store.put1('binlit', value / 2n); _Gt[gLen] = tailTerm; gLen++;
          continue;
        }
        if (ttId === TAG.i) {
          if (value % 2n !== 1n) return false;
