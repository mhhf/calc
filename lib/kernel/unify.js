 ...
 *
 * Supports equational normalization for compact primitive literals:
...
// Hot-path tag ID caches for arrlit expansion
...
/**
 * Try to unify a pattern against a binlit term via equational normalization
 *
 ...
 * @returns {boolean} Whether equational normalization succeeded
 */
function unifyBinlit(pattern, term, G) {
  ...
  if ((ptag === 'i' || ptag === 'o') && Store.arity(pattern) === 1) {
    ...
    if (ptag === 'o') {
      if (value === 0n || (value & 1n) !== 0n) return false;
      G.push([tailPattern, Store.put1('binlit', value >> 1n)]);
      return true;
    }
    if (ptag === 'i') {
      if ((value & 1n) !== 1n) return false;
      G.push([tailPattern, Store.put1('binlit', value >> 1n)]);
      return true;
    }
  }
...
/**
 * Try to unify a pattern against a strlit term via equational normalization
 *
...
function unifyStrlit(pattern, term, G) {
  ...
  if (ptag === 'cons' && Store.arity(pattern) === 2) {
    if (str === '') return false;
    const firstChar = Store.put1('charlit', str.charCodeAt(0));
    const restStr = Store.put1('strlit', str.slice(1));
...
/**
 * Try to unify a pattern against an arrlit term via equational normalization
 *
 * arrlit([a,b,c]) vs acons(H, T) → H = a, T = arrlit([b,c])
 * arrlit([]) vs ae → success
 *
 * @param {number} pattern - Pattern term hash
 * @param {number} term - arrlit term hash
 * @param {Array} G - Goal list to push new constraints to
 * @returns {boolean} Whether equational normalization succeeded
 */
function unifyArrlit(pattern, term, G) {
  ...
  // Pattern: ae (empty array)
  if (ptag === 'atom' && Store.child(pattern, 0) === 'ae') {
    return elems.length === 0;
  }

  // Pattern: arrlit (element-wise) — same tag handled in caller

  // Pattern: acons(H, T) — decompose head + tail
 ...
 * Metavars have the dedicated 'metavar' tag — O(1) byte check, no string access.
...
/**
 * Check if term is a regular freevar (eigenvariable, not a metavar)
 * @param {number} h - Term hash
 * @returns {boolean}
 */
...
/**
 * Get the variable name from a freevar or metavar term
 * @param {number} h - Freevar/metavar hash
...
const unifyUF = (a, b, opts = {}) => {
  ...
  while (G.length) {
    ...
    // Equational normalization for binlit
    if (tag0 === 'binlit' || tag1 === 'binlit') {
      ...
      // Pattern vs binlit: equational normalization
      const [pat, lit] = tag0 === 'binlit' ? [t1, t0] : [t0, t1];
      if (!unifyBinlit(pat, lit, G)) return null;
      continue;
    }

    // Equational normalization for strlit
    if (tag0 === 'strlit' || tag1 === 'strlit') {
      ...
      // Pattern vs strlit: equational normalization
    ...
    // Equational normalization for arrlit
    if (tag0 === 'arrlit' || tag1 === 'arrlit') {
      // arrlit vs arrlit: element-wise unification
      if (tag0 === 'arrlit' && tag1 === 'arrlit') {
      ...
      // One side is arrlit, other is pattern
      const [pat, arr] = tag0 === 'arrlit' ? [t1, t0] : [t0, t1];
...
const match = (pattern, term, initialTheta = []) => {
  ...
  while (gLen > 0) {
    ...
    // arrlit vs arrlit: element-wise matching (before generic decompose — arrlit children are Uint32Arrays)
    if (tp === 'arrlit' && tt === 'arrlit') {
    ...
    // Equational normalization for binlit in match (inlined to avoid pair allocation)
    if (tt === 'binlit') {
      ...
      if ((ptag === 'i' || ptag === 'o') && Store.arity(p) === 1) {
        ...
        if (ptag === 'o') {
          if (value === 0n || (value & 1n) !== 0n) return null;
          _Gp[gLen] = tailPattern; _Gt[gLen] = Store.put1('binlit', value >> 1n); gLen++;
          continue;
        }
        if (ptag === 'i') {
          if ((value & 1n) !== 1n) return null;
          _Gp[gLen] = tailPattern; _Gt[gLen] = Store.put1('binlit', value >> 1n); gLen++;
          continue;
        }
      }
    ...
    // Equational normalization for arrlit in match
    if (tt === 'arrlit') {
      const elems = Store.getArrayElements(t);
      if (tp === 'atom' && Store.child(p, 0) === 'ae') {
        if (elems.length !== 0) return null;
        continue;
      }
      if (tp === 'acons' && Store.arity(p) === 2) {
        if (elems.length === 0) return null;
        const tailData = new Uint32Array(elems.length - 1);
        for (let i = 1; i < elems.length; i++) tailData[i - 1] = elems[i];
...
const matchIndexed = (pattern, term, theta, slots) => {
  ...
  while (gLen > 0) {
    ...
    // Equational normalization for arrlit (checked before tpId guard since atom ae has tpId=0)
    const ttId = Store.tagId(t);
    if (ttId === TAG_ARRLIT) {
      const tpId2 = Store.tagId(p);
      ...
      const elems = Store.getArrayElements(t);
      if (tpId2 === TAG.atom && Store.child(p, 0) === 'ae') {
        if (elems.length !== 0) return false;
        continue;
      }
      if (tpId2 === TAG_ACONS && Store.arity(p) === 2) {
        if (elems.length === 0) return false;
        const tailData = new Uint32Array(elems.length - 1);
        for (let i = 1; i < elems.length; i++) tailData[i - 1] = elems[i];
        _Gp[gLen] = Store.child(p, 0); _Gt[gLen] = elems[0]; gLen++;
    ...
    if (!tpId || !ttId) return false;

    // Same tag and arity — decompose (numeric comparison, no string alloc)
    const ap = Store.arity(p);
    if (tpId === ttId && ap === Store.arity(t)) {
      if (gLen + ap > _Gp.length) {
        const newSize = _Gp.length * 2;
        const ngp = new Array(newSize);
        const ngt = new Array(newSize);
    ...
    if (ttId === TAG.binlit) {
      const value = Store.child(t, 0);
      if (tpId === TAG.atom && Store.child(p, 0) === 'e') {
      ...
      if ((tpId === TAG.i || tpId === TAG.o) && Store.arity(p) === 1) {
        const tailPattern = Store.child(p, 0);
        if (tpId === TAG.o) {
          if (value === 0n || (value & 1n) !== 0n) return false;
          _Gp[gLen] = tailPattern; _Gt[gLen] = Store.put1('binlit', value >> 1n); gLen++;
    ...
    // Reverse equational normalization: binlit pattern vs structural term
    if (tpId === TAG.binlit) {
      const value = Store.child(p, 0);
      if (ttId === TAG.atom && Store.child(t, 0) === 'e') {
        if (value !== 0n) return false;
        continue;
      }
      if ((ttId === TAG.i || ttId === TAG.o) && Store.arity(t) === 1) {
        const tailTerm = Store.child(t, 0);
        if (ttId === TAG.o) {
          if (value === 0n || (value & 1n) !== 0n) return false;
          _Gp[gLen] = Store.put1('binlit', value >> 1n); _Gt[gLen] = tailTerm; gLen++;
          continue;
        }
        if (ttId === TAG.i) {
          if ((value & 1n) !== 1n) return false;
...
module.exports = {
  ...
  isFreevar,
  getVarName,
  // Equational normalization helpers (for testing)
  unifyBinlit,
  unifyStrlit,
  unifyArrlit
};
