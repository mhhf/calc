...
function unifyBinlit(pattern, term, G) {
  ...
  // Flat: i(X) or o(X)
  if ((patternNode.tag === 'i' || patternNode.tag === 'o') && patternNode.children.length === 1) {
    const tailPattern = patternNode.children[0];
    if (patternNode.tag === 'o') {
      if (value === 0n || value % 2n !== 0n) return false;
      G.push([tailPattern, Store.put('binlit', [value / 2n])]);
      return true;
    }
    if (patternNode.tag === 'i') {
...
function unifyStrlit(pattern, term, G) {
  ...
  // Flat: cons(H, T)
  if (patternNode.tag === 'cons' && patternNode.children.length === 2) {
    if (str === '') return false;
    const firstChar = Store.put('charlit', [str.charCodeAt(0)]);
    const restStr = Store.put('strlit', [str.slice(1)]);
    G.push([patternNode.children[0], firstChar]);
    G.push([patternNode.children[1], restStr]);
    return true;
  }
...
const match = (pattern, term, initialTheta = []) => {
  ...
  while (G.length) {
    ...
    // Ephemeral expansion for binlit in match
    if (nt.tag === 'binlit') {
      if (!unifyBinlit(p, t, G)) return null;
      continue;
    }
