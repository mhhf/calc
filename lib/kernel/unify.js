 ...
 * Legacy — new code should use equational theories instead.
 */
function unifyBinlit(pattern, term, G) {
  ...
  if (ptag === 'atom' && Store.child(pattern, 0) === 'e') return value === 0n;
  if (ptag === 'binlit') return Store.child(pattern, 0) === value;
  if ((ptag === 'i' || ptag === 'o') && Store.arity(pattern) === 1) {
    const tailPattern = Store.child(pattern, 0);
    if (ptag === 'o') {
      if (value === 0n || (value & 1n) !== 0n) return false;
      G.push([tailPattern, Store.put1('binlit', value >> 1n)]); return true;
    }
    if (ptag === 'i') {
...
/** Legacy strlit helper — new code should use equational theories. */
function unifyStrlit(pattern, term, G) {
  if (Store.tag(term) !== 'strlit') return false;
  const str = Store.child(term, 0);
  const ptag = Store.tag(pattern);
  if (!ptag) return false;
  if (ptag === 'atom' && Store.child(pattern, 0) === 'nil') return str === '';
  if (ptag === 'strlit') return Store.child(pattern, 0) === str;
  if (ptag === 'cons' && Store.arity(pattern) === 2) {
    if (str === '') return false;
    G.push([Store.child(pattern, 0), Store.put1('charlit', str.charCodeAt(0))]);
    G.push([Store.child(pattern, 1), Store.put1('strlit', str.slice(1))]);
    return true;
  }
  return false;
}
...
module.exports = {
  ...
  getVarName,
  setTheories,
  // Equational normalization helpers (legacy, for testing)
  unifyBinlit,
  unifyStrlit,
  unifyArrlit
};
