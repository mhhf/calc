...
const { TAG } = Store;
import { occurs } from './substitute.js';
import { defaultTheories, buildTheoryLookup } from './eq-theory.js';
...
function setTheories(theories) {
  ...
  _classTags = null; // recomputed lazily — tag names, not ids (survives reindexing)
}

// ── Theory value classes ─────────────────────────────────────────────
// A theory may declare `classTags`: the tag NAMES of its value class —
// the compact tag plus every structural form it rewrites through (e.g.
// binlit's {binlit, i, o, e}; ratlit's ℚ class). Over-approximation
// consumers (ci.js's conservative unifiability) read the union instead
// of hardcoding any calculus's constructor names (RES_0143 L4).
let _classTags = null;
function theoryClassTags() {
  if (!_classTags) {
    _classTags = new Set();
    for (const th of _theories) {
      for (const t of th.classTags || []) _classTags.add(t);
    }
  }
  return _classTags;
}
...
/**
 * The equational theory registered for a tag, or null. Read-only view of
 * the setTheories dispatch table — index layers (disc-tree) use it to keep
 * candidate lookup theory-aware (an index may never change WHICH matches
 * exist; TODO_0265 Phase 3, D13 corollary).
 */
function theoryFor(tagId) {
  return _rewriteFromTag[tagId] || null;
}

/**
 * THE index contract for cross-tag matching: can a fact tagged srcTagId
 * match a pattern headed dstTagId via an equational theory rewrite?
 * Every candidate-lookup index (fingerprint, disc-tree, first-arg buckets,
 * future ones) must either consult this or over-approximate — an index
 * that answers from raw tag equality hides matches the matcher would find
 * (the disc-tree did exactly that until TODO_0265 Phase 3). Enforced by
 * the strategy-vs-predicate-oracle differential test.
 */
function canCrossMatch(srcTagId, dstTagId) {
  const th = _rewriteFromTag[srcTagId];
  return th ? th.canRewrite(srcTagId, dstTagId) : false;
}
...
function occursInUF(v, h, uf) {
  ...
  // arrlit stores its elements out-of-band (getArrayElements), NOT as
  // Store.child()/arity() slots — the generic loop below would miss a metavar
  // nested in an array, admitting a cyclic substitution (m ↦ …[m]…). Descend
  // explicitly, mirroring the arrlit case in the main unify loop.
  if (Store.tagId(resolved) === TAG_ARRLIT) {
    const elems = Store.getArrayElements(resolved);
    for (let i = 0; i < elems.length; i++) {
      if (occursInUF(v, elems[i], uf)) return true;
    }
    return false;
  }
...
export { unify, match, matchIndexed, undoSave, undoRestore, undoDiscard, isMetavar, setTheories, theoryFor, canCrossMatch, theoryClassTags };
export default { unify, match, matchIndexed, undoSave, undoRestore, undoDiscard, isMetavar, setTheories, theoryFor, canCrossMatch, theoryClassTags };
