...
/**
 * Try to unify a pattern against an arrlit term via ephemeral expansion
 *
...
const unifyUF = (a, b, opts = {}) => {
  ...
  while (G.length) {
    ...
    // Ephemeral expansion for arrlit
...
const match = (pattern, term, initialTheta = []) => {
  ...
  while (gLen > 0) {
    ...
    // Ephemeral expansion for arrlit in match
    if (tt === 'arrlit') {
...
const matchIndexed = (pattern, term, theta, slots) => {
  ...
  while (gLen > 0) {
    ...
    // Ephemeral expansion for arrlit (checked before tpId guard since atom ae has tpId=0)
    ...
    // Reverse ephemeral expansion: binlit pattern vs structural term
    if (tpId === TAG.binlit) {
