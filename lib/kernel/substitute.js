...
const debruijnSubst = (bodyHash, index, replacement) => {
  ...
  // Every binder (exists/forall + the μMALL fixpoints mu/nu) increments the de
  // Bruijn depth. Read from the ONE kernel registry (Store.BINDER_TAGS) so a
  // future binder cannot silently corrupt nested-binder indices here.
  const BINDER_TAGS = Store.BINDER_TAGS;
  function go(h, depth) {
