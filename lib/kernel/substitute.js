...
//
// Default: ENABLED. Set `CALC_0216_GROUND_BIT=0` to disable (both the
// Store-side bookkeeping and the apply() short-circuit turn off together).
// See lib/kernel/store.js for measured cold-load impact.
const _GROUND_BIT_ENABLED = (typeof process === 'undefined')
  ? true
  : process.env.CALC_0216_GROUND_BIT !== '0';
const _isGround = Store.isGround;

// TODO_0216 Phase 2 (idea A) REMOVED. Map-indexed θ was a net loss on the
// cold-load path: avg θ size observed in production is ~2.7 (well below any
// reasonable threshold where Map construction pays for itself), so the always-
// present length-check branch just added overhead. See phase report.
...
const debruijnSubst = (bodyHash, index, replacement) => {
  ...
  // Fixpoint binders (μMALL, TODO_0009) increment the de Bruijn depth exactly
  // like exists/forall — omitting them here silently corrupts bound indices in
  // nested `mu X. … nu Y. …` bodies.
  const TAG_MU = Store.TAG.mu;
  const TAG_NU = Store.TAG.nu;
  function go(h, depth) {
    ...
    if (tid === TAG_EXISTS || tid === TAG_FORALL || tid === TAG_MU || tid === TAG_NU) {
...
export { sub, apply, applyIndexed, subCompiled, debruijnSubst, eq, copy, occurs };
export default { sub, apply, applyIndexed, subCompiled, debruijnSubst, eq, copy, occurs };
