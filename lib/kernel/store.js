  ...
  if (existing !== undefined) {
    if (tags[existing] === tagId && arities[existing] === 1) {
      ...
      if (typeof c0 === 'string') {
        ...
        if (BIGINT_TABLE[stored] === c0) return existing;
      } else {
...
function put2(tagName, c0, c1) {
  ...
  if (existing !== undefined) {
    if (tags[existing] === tagId && arities[existing] === 2) {
      ...
      else if (typeof c0 === 'bigint') { if (BIGINT_TABLE[child0[existing]] !== c0) match = false; }
      else { if (child0[existing] !== c0) match = false; }
      if (match) {
        if (typeof c1 === 'string') { if (stringTable.get(child1[existing]) !== c1) match = false; }
        else if (typeof c1 === 'bigint') { if (BIGINT_TABLE[child1[existing]] !== c1) match = false; }
  ...
  else if (typeof c0 === 'bigint') child0[id] = storeBigInt(c0);
  else child0[id] = c0;

  if (typeof c1 === 'string') child1[id] = stringTable.intern(c1);
  else if (typeof c1 === 'bigint') child1[id] = storeBigInt(c1);
  else child1[id] = c1;
