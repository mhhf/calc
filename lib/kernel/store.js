...
import { hashString, hashCombine, hashCombine2, hashBigInt } from '../hash.js';
// =============================================================================
// Tag Registry
// =============================================================================
...
// ⚠ Appending here shifts PRED_BOUNDARY and invalidates every serialized Store
// (out/ill.json, precompiled .bin caches). Batch new kernel tags into ONE commit
// and bump store-binary VERSION so stale caches self-reject (TODO_0265 Phase 1).
//
// till (timed ILL) kernel tags — TODO_0265 Phase 1 one-tag commit:
//   ratlit  — compact rational leaf: 2 bigint children (num, den), canonical
//             gcd-reduced / den > 0 / den = 1 stored as binlit (see
//             calculus/till/lib/ratlit-theory.js)
//   at      — stamped resource at(A, t): availability stamp t on formula A
//   after   — activation window lower bound after(E)
//   before  — validity deadline before(E)
// 'monad' is the ONE graded lax monad, monad(grade, body) — binary since
// the D6 merge-back (TODO_0157): ILL is the trivial-grade instance
// (unit = binlit 0), till fills the grade with delays. The former
// separate 'gmonad' tag is gone (store-binary VERSION 8).
['atom', 'freevar', 'metavar', 'tensor', 'loli', 'with', 'bang', 'one', 'type',
 ...
 'arrlit', 'acons', 'named_arg',
 'ratlit', 'at', 'after', 'before',
 // Fixpoint connectives (TODO_0009 rung 3, μMALL): mu = least fixed point
 // (positive), nu = greatest fixed point (negative). Unary binder terms
 // (body de-Bruijn-encoded, like exists/forall). Appended last — see the
 // VERSION-bump gotcha above (store-binary VERSION 8→9).
 'mu', 'nu'].forEach(registerTag);
...
// Binder connectives: unary terms whose single child is a body with a
// de-Bruijn-encoded bound variable (exists/forall + the μMALL fixpoints
// mu/nu). ONE source of truth for every binder-aware walk (debruijnSubst et
// al.) — a new binder registered above MUST be added here, or nested-binder
// de Bruijn indices silently corrupt (TODO_0009 audit 2026-09-11). All are
// built-in (below PRED_BOUNDARY), so this set is stable across clear/restore.
const BINDER_TAGS = new Set([TAG.exists, TAG.forall, TAG.mu, TAG.nu]);
...
BIGINT_CHILD_TAGS[TAG.ratlit] = 1; // both children (num, den) are bigints
...
LEAF_TAGS[TAG.ratlit] = 1;

// charlit: child is raw uint32 codepoint (not a term ref, not a string)
// No special table needed — stored directly in childBuf

// =============================================================================
// Ground-bit classification (TODO_0216 Phase 1, idea D)
// =============================================================================
//
// A term h is "ground" iff apply(h, θ) === h for every θ — i.e. no metavar
// appears anywhere under h. This property is structure-determined and can be
// cached: when we insert a new term via put/put1/put2/put3, all its children
// already live in the arena with their ground bits set (content-addressed
// allocation order). So we compute the parent bit from O(arity) child bits.
//
// The bit is read by apply() at every traversal step to short-circuit
// substitution on ground subtrees. Default: ENABLED. Set
// `CALC_0216_GROUND_BIT=0` to disable (safety escape hatch — both the
// write-side bookkeeping and the read-side short-circuit then become
// zero-cost). Measured impact on multisig_nocall_solc_symbolic cold load
// (N=15 subprocesses): −3% total, −5% on pair-rename inside fusion chains.
//
// `_GROUND_TAG_KIND` is a per-tag enum indexed by tagId:
//   0 = DERIVE       — ground iff all term-ref children are ground
//   1 = LEAF_GROUND  — atomic, always ground (atom, binlit, strlit, ...)
//   2 = NEVER_GROUND — substitution target (metavar, freevar): never ground
//
// NOTE on freevar: apply()'s θ-scan matches ANY hash key, including freevars
// (see tests/kernel-ops.test.js:43 which builds θ with freevar keys). Marking
// freevar as LEAF_GROUND would incorrectly short-circuit those substitutions.
// Treat freevars the same as metavars — both are substitution targets.
const _GROUND_BIT_ENABLED = (typeof process === 'undefined')
  ? true
  : process.env.CALC_0216_GROUND_BIT !== '0';
const _GK_DERIVE       = 0;
const _GK_LEAF_GROUND  = 1;
const _GK_NEVER_GROUND = 2;
const _GROUND_TAG_KIND = new Uint8Array(256);
_GROUND_TAG_KIND[TAG.atom]    = _GK_LEAF_GROUND;
_GROUND_TAG_KIND[TAG.freevar] = _GK_NEVER_GROUND; // apply() CAN substitute freevars
_GROUND_TAG_KIND[TAG.binlit]  = _GK_LEAF_GROUND;
_GROUND_TAG_KIND[TAG.strlit]  = _GK_LEAF_GROUND;
_GROUND_TAG_KIND[TAG.charlit] = _GK_LEAF_GROUND;
_GROUND_TAG_KIND[TAG.bound]   = _GK_LEAF_GROUND; // bound-var — bigint payload, no nested term
_GROUND_TAG_KIND[TAG.evar]    = _GK_LEAF_GROUND; // existential var slot — bigint payload
_GROUND_TAG_KIND[TAG.ratlit]  = _GK_LEAF_GROUND; // rational leaf — two bigint payloads
_GROUND_TAG_KIND[TAG.metavar] = _GK_NEVER_GROUND;
// Every other tag defaults to DERIVE.
...
// Ground-bit side-table (TODO_0216 Phase 1). 1 = term has no metavar anywhere
// under it, 0 = term has at least one metavar. Index 0 is the null sentinel
// (never read). Values beyond nextId are stale and must not be read.
let groundBits = new Uint8Array(termCapacity);
...
function growTerms() {
  ...
  const newGroundBits = new Uint8Array(newCap);
  newTags.set(tags);
  newArities.set(arities);
  newChildOff.set(childOff);
  newGroundBits.set(groundBits);
  tags = newTags;
  arities = newArities;
  childOff = newChildOff;
  groundBits = newGroundBits;
  termCapacity = newCap;
}

// Compute the ground bit for a freshly-allocated term. Callers have already
// inserted every child into the arena, so `groundBits[childTermRef]` is set.
// See the block comment at `_GROUND_TAG_KIND` for the classification rules.
function _computeGroundBitFromArrayChildren(elements) {
  for (let i = 0; i < elements.length; i++) {
    if (groundBits[elements[i]] !== 1) return 0;
  }
  return 1;
}
function _computeGroundBitFromTermChildren(off, nChildren) {
  for (let i = 0; i < nChildren; i++) {
    if (groundBits[childBuf[off + i]] !== 1) return 0;
  }
  return 1;
}
...
function put(tagName, children) {
  ...
  if (ARRAY_CHILD_TAGS[tagId] && children.length === 1 && children[0] instanceof Uint32Array) {
    ...
    // Ground iff every element hash is ground (TODO_0216 P1).
    if (_GROUND_BIT_ENABLED) {
      groundBits[id] = _computeGroundBitFromArrayChildren(children[0]);
    }
    return id;
  }
  ...
  // Ground-bit: leaf tags are constant; others derived from term-ref children.
  // Tags with STRING/BIGINT children are flagged _GK_LEAF_GROUND, so the
  // derivation loop below never sees string-interned indices treated as term
  // refs (the termCapacity sentinel at index 0 has groundBits[0] = 0 by default
  // but we never reach that branch for those tags).
  if (_GROUND_BIT_ENABLED) {
    const gk = _GROUND_TAG_KIND[tagId];
    if (gk === _GK_LEAF_GROUND) {
      groundBits[id] = 1;
    } else if (gk === _GK_NEVER_GROUND) {
      groundBits[id] = 0;
    } else {
      groundBits[id] = _computeGroundBitFromTermChildren(childOff[id], nChildren);
    }
  }
 ...
 * Hot path: binlit, freevar, bang construction.
 */
function put1(tagName, c0) {
  ...
  // Ground-bit: LEAF tags (binlit, freevar, ...) are constant; DERIVE tags with
  // a numeric term child (bang, monad, ...) inherit from that child.
  if (_GROUND_BIT_ENABLED) {
    const gk1 = _GROUND_TAG_KIND[tagId];
    groundBits[id] = gk1 === _GK_LEAF_GROUND  ? 1
                   : gk1 === _GK_NEVER_GROUND ? 0
                   : (typeof c0 === 'number' ? groundBits[c0] : 1);
  }
  return id;
}
...
function put2(tagName, c0, c1) {
  ...
  // Ground-bit: both numeric children must be ground; LEAF/NEVER tags bypass.
  if (_GROUND_BIT_ENABLED) {
    const gk2 = _GROUND_TAG_KIND[tagId];
    if (gk2 === _GK_LEAF_GROUND)  groundBits[id] = 1;
    else if (gk2 === _GK_NEVER_GROUND) groundBits[id] = 0;
    else {
      const g0 = typeof c0 === 'number' ? groundBits[c0] : 1;
      const g1 = typeof c1 === 'number' ? groundBits[c1] : 1;
      groundBits[id] = (g0 === 1 && g1 === 1) ? 1 : 0;
    }
  }
  return id;
}
...
function put3(tagName, c0, c1, c2) {
  ...
  // Ground-bit for arity-3: AND of three numeric-child bits (LEAF/NEVER bypass).
  if (_GROUND_BIT_ENABLED) {
    const gk3 = _GROUND_TAG_KIND[tagId];
    if (gk3 === _GK_LEAF_GROUND)  groundBits[id] = 1;
    else if (gk3 === _GK_NEVER_GROUND) groundBits[id] = 0;
    else {
      const g0 = typeof c0 === 'number' ? groundBits[c0] : 1;
      const g1 = typeof c1 === 'number' ? groundBits[c1] : 1;
      const g2 = typeof c2 === 'number' ? groundBits[c2] : 1;
      groundBits[id] = (g0 === 1 && g1 === 1 && g2 === 1) ? 1 : 0;
    }
  }
  return id;
}
...
/**
 * TODO_0216 Phase 1 (idea D): true iff no metavar appears anywhere under h.
 *
 * The bit is maintained O(1)-per-put (see put/put1/put2/put3) from the bits
 * of h's immediate children, which are already set because content-addressed
 * allocation is bottom-up. Reads are a single Uint8Array lookup.
 *
 * apply(h, θ) is a no-op on ground terms. substitute.js consults this bit
 * when CALC_0216_GROUND_BIT=1 to short-circuit the traversal.
 *
 * @param {number} id - Term index
 * @returns {boolean}
 */
function isGround(id) {
  if (typeof id !== 'number' || id < 1 || id >= nextId) return false;
  return groundBits[id] === 1;
}
...
// ─── Arena-prefix fingerprint (TODO_0278 A1) ────────────────────────────────
//
// A loaded calculus bundle IS the arena prefix as of load completion: hash
// meanings (sequential IDs) are only valid against that exact byte content.
// fingerprintMark() captures the prefix bounds cheaply; prefixDigest(mark)
// content-hashes the prefix lazily (memoized; the arena is append-only, so
// the prefix never changes under it). Orbit certificates carry the mark +
// digest and any consumer whose arena does not extend the same prefix
// refuses them. Pure JS (FNV-1a, two lanes) — browser-safe, non-crypto:
// use is fingerprinting, and a collision only re-enables an exact-signature
// comparison that must ALSO match.

function fingerprintMark() {
  return {
    n: nextId - 1, c: nextChildPos, s: stringTable.size(),
    b: bigIntTable.size(), a: nextArrayId, t: nextTag,
  };
}

const _fpMemo = new Map();
onReplace(() => _fpMemo.clear());

function _fnvInit() { return [0x811c9dc5 | 0, 0xcbf29ce4 | 0]; }
function _fnvByte(l, x) {
  l[0] = Math.imul(l[0] ^ (x & 0xff), 0x01000193);
  l[1] = Math.imul(l[1] ^ ((x >>> 8) ^ (x & 0xff)) & 0xff, 0x01000193);
}
function _fnvU32(l, x) {
  _fnvByte(l, x & 0xffff); _fnvByte(l, (x >>> 8) & 0xffff);
  _fnvByte(l, (x >>> 16) & 0xffff); _fnvByte(l, (x >>> 24) & 0xffff);
}
function _fnvStr(l, s) {
  _fnvU32(l, s.length);
  for (let i = 0; i < s.length; i++) _fnvU32(l, s.charCodeAt(i));
}

/** Content digest of the arena prefix bounded by `mark` (from
 *  fingerprintMark()). Returns a hex string, or null when the current arena
 *  is shorter than the mark (a different/younger bundle). */
function prefixDigest(mark) {
  const { n, c, s, b, a, t } = mark;
  if (!(n >= 0 && n <= nextId - 1 && c >= 0 && c <= nextChildPos &&
        s >= 0 && s <= stringTable.size() && b >= 0 && b <= bigIntTable.size() &&
        a >= 0 && a <= nextArrayId && t >= 0 && t <= nextTag)) return null;
  const key = `${n}.${c}.${s}.${b}.${a}.${t}`;
  const hit = _fpMemo.get(key);
  if (hit !== undefined) return hit;
  const l = _fnvInit();
  for (let i = 0; i < t; i++) _fnvStr(l, TAG_NAMES[i]);
  for (let i = 1; i <= n; i++) { _fnvByte(l, tags[i]); _fnvByte(l, arities[i]); }
  for (let i = 0; i < c; i++) _fnvU32(l, childBuf[i]);
  for (let i = 0; i < s; i++) _fnvStr(l, stringTable.get(i));
  for (let i = 0; i < b; i++) _fnvStr(l, bigIntTable.get(i).toString(16));
  for (let i = 0; i < a; i++) {
    const e = ARRAY_TABLE[i];
    _fnvU32(l, e.length);
    for (let j = 0; j < e.length; j++) _fnvU32(l, e.data[e.offset + j]);
  }
  const digest = (l[0] >>> 0).toString(16).padStart(8, '0') +
                 (l[1] >>> 0).toString(16).padStart(8, '0');
  _fpMemo.set(key, digest);
  return digest;
}
...
function restore(data) {
  ...
  // Recompute ground bits (TODO_0216 P1). The bit is structure-determined, so
  // we don't serialize it — we walk terms 1..nodeCount in allocation order
  // (which is topological: every child id is strictly less than its parent's).
  // This keeps the snapshot/binary format unchanged; H6 tests prove round-trip.
  // Skip when disabled — apply() won't read the array.
  if (!_GROUND_BIT_ENABLED) return;
  groundBits[0] = 0;
  for (let id = 1; id <= nodeCount; id++) {
    const t = tags[id];
    const gk = _GROUND_TAG_KIND[t];
    if (gk === _GK_LEAF_GROUND) { groundBits[id] = 1; continue; }
    if (gk === _GK_NEVER_GROUND) { groundBits[id] = 0; continue; }
    let g = 1;
    if (ARRAY_CHILD_TAGS[t]) {
      const entry = ARRAY_TABLE[childBuf[childOff[id]]];
      if (entry) {
        ...
        for (let i = entry.offset; i < end; i++) {
          if (groundBits[entry.data[i]] !== 1) { g = 0; break; }
        }
      }
    } else {
      const off = childOff[id];
      const a = arities[id];
      for (let i = 0; i < a; i++) {
        if (groundBits[childBuf[off + i]] !== 1) { g = 0; break; }
      }
    }
    groundBits[id] = g;
  }
...
export { put, put1, put2, put3, get, tag, tagId, children, child, rawChild, arity, isTerm, isTermChild, isGround, eq, clear, size, fingerprintMark, prefixDigest, snapshot, restore, TAG, TAG_NAMES, PRED_BOUNDARY, registerTag, getArrayElements, putArray, onClear, onReplace, LEAF_TAGS, BINDER_TAGS };
export default { put, put1, put2, put3, get, tag, tagId, children, child, rawChild, arity, isTerm, isTermChild, isGround, eq, clear, size, fingerprintMark, prefixDigest, snapshot, restore, TAG, TAG_NAMES, PRED_BOUNDARY, registerTag, getArrayElements, putArray, onClear, onReplace, LEAF_TAGS, BINDER_TAGS };
