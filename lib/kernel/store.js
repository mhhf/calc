...
['atom', 'freevar', 'tensor', 'loli', 'with', 'bang', 'one', 'type',
 ...
 'bound', 'exists', 'forall', 'evar', 'oplus', 'zero'].forEach(registerTag);
...
function matchesEntry(id, tagName, children) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (typeof c === 'string') {
      if (ID_TO_STRING[stored] !== c) return false;
    } else if (typeof c === 'bigint') {
...
function snapshot(metadata) {
  ...
  const snapStrings = ID_TO_STRING.slice(0, nextStringId);
...
function restore(data) {
  const { nodeCount, tags: dTags, arities: dArities,
          ...
          dedupHashes, strings, bigints, tagNames } = data;
  ...
  for (const s of strings) {
    STRING_TO_ID.set(s, nextStringId);
    ID_TO_STRING.push(s);
    nextStringId++;
  }
