...
import { copy, eq } from './substitute.js';
/**
...
function isAtomic(h) {
  ...
  return tag === 'atom' || tag === 'freevar' || tag === 'metavar';
}
 ...
 */
function loliOf(hash) {
  let h = hash;
  for (let i = 0; i < 20; i++) {
    const t = Store.tag(h);
    if (t === 'loli') return h;
    if (t === 'bang') { h = Store.child(h, 1); continue; }
    if (t === 'forall') { h = Store.child(h, 0); continue; }
    return null;
  }
  return null;
}
 ...
 * @param {Set<number>} externals - External freevar hashes from extFV
 * @returns {boolean} true if any external freevar was bound (result is unsound)
 */
function boundExt(theta, externals) {
...
export { copy, eq, freeVars, isAtomic, connective, tag, children, predHead, isPredTag, loliOf, rTensor, extFV, boundExt };
export default { copy, eq, freeVars, isAtomic, connective, tag, children, predHead, isPredTag, loliOf, rTensor, extFV, boundExt };
