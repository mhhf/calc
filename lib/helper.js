...
const foldCleanPreorder = function (tree, loc, prefix = "", last = true) {
  ...
  let string = tree.head[loc] = __prefix + clc.yellow(tree.head[loc]);
  // loc && (tree.head[loc] = __prefix + tree.head[loc]);
  Object.keys(tree.head).forEach(name => {
    if(name !== loc) {
      string += "\n" + prefix +  ((last || tree.children.length === 0) ? (tree.children.length === 0 && !last ? "" : "  ") : "│ ") + "│ " + name + ": " + tree.head[name];
    }
  })
  table.push(string);
