...
import { compiledSub, producePers, canonObject } from '../engine/state-ops.js';
import { fromObject, toObject } from '../engine/fact-set.js';
import { buildCanonicalizer, defaultTheories } from '../kernel/eq-theory.js';
import { clearBWCache } from '../engine/opt/backward-cache.js';
import { EMPTY_MATCH_OPTS } from '../engine/match.js';
import { resolveConn, factKeyOf, flattenAnte } from '../engine/formula-utils.js';
import { ratParts } from '../kernel/rat-term.js';
import { mul as _wMul, sub as _wSub } from '../rat.js';
import { mix32 as _mix32, thetaHash as _thetaHash, strHash as _strHash, sampleIndex } from '../engine/prf.js';
...
import { StampTable } from './labels.js';
import { packRef, refInner, refStamp } from '../engine/fact-set.js';
...
function buildTimedConfig(cc) {
  ...
  // ungrouped behavior).
  if (!g.values) {
    throw new Error('buildTimedConfig: cc.grades.values (label value algebra) is required — see THY_0024 / lib/timed/labels.js');
  }
  ...
  // The effective policy carries the label-interning constructor
  // (policy.stampTable): fact-set imports no timed code — the timed
  // layer hands it the table through the policy (RES_0143 M1).
  const policy = cc.factSetPolicy
    ? Object.freeze({ ...cc.factSetPolicy,
        labels: cc.factSetPolicy.labels || g.values, stampTable: StampTable })
    : Object.freeze({ labels: g.values, stampTable: StampTable, stampTag });
  ...
  return Object.freeze({
    ...
    withTag: rc.externalChoice || null, // external-choice guard (coalesce/isDead)
    productTag: rc.product || null,     // antecedent-spine walker (coalesce)
    unitTag: rc.unit || null,
    chooser: sched.chooser || 'random',
    seed: sched.seed || 0,
    cohort: sched.cohort || 'fifo',
    // Composed theory canonicalizer (state-canonicity invariant) —
    // config-derived like the engine's (index.js _compileOpts); gates on
    // rule.canonPatterns at fire, canonicalizes plain-object entry facts.
    canon: buildCanonicalizer([...defaultTheories, ...(cc.theories || [])]),
  });
}
...
function normalizeTimedState(input, tcfg) {
  ...
  // State-entry boundary: canonicalize plain facts (inner values under
  // at() wrappers included — canonicalize recurses; stamp halves are
  // value-interned by the label algebra either way).
  return fromObject(canonObject(plain.linear || {}, tcfg.canon),
                    canonObject(plain.persistent || {}, tcfg.canon), tcfg.policy);
}
...
function tryTimedMatch(rule, state, calc, matchOpts, tcfg, diag, baseA) {
  ...
  function search(i, partialA) {
    ...
    // cohort discipline. Activation joins every taken stamp.
    //
    // Whole-bind anchors ONCE per inner: later same-inner anchors are
    // SUFFIX takes (the spread starts at the anchor), and a suffix that
    // drops a cohort can have a strictly smaller activation — under a
    // function-valued merge (product stamps: an old cohort may carry the
    // dominant dist) or the lifo walk (the dropped cohort IS the max) —
    // so minimization would prefer a partial take and `!_W` would stop
    // binding the total (D4 violation, found 2026-09-04). Under fifo +
    // order-max merge every suffix contains the max stamp, so the
    // suffixes were equal-activation duplicates losing the FIFO
    // tie-break — which is why this was invisible until sill. Counted
    // takes (`!_k`) keep all anchors: choosing WHICH k copies is genuine
    // choice, and minimization picking the activation-least subset is
    // the semantics.
    const isCounted = (meta.countTake || 0) > 1 || meta.countVar;
    if (isCounted && !bodyIsAt) {
      const anchored = meta.countVar ? new Set() : null;
      for (let gi = 0; gi < n; gi++) {
        const i0 = lifo ? n - 1 - gi : gi;
        const ref0 = packRef(ib[i0], sb[i0]);
        if (availOf(i0, ref0) <= 0) continue;
        if (anchored) {
          if (anchored.has(ib[i0])) continue;
          anchored.add(ib[i0]);
        }
        const saved = undoSave();
        ...
        undoRestore(theta, saved);
        // Anchors of a DIFFERENT inner are genuine alternatives; for `!_k`
        // later same-inner anchors are alternative k-subsets (kept), for
        // `!_W` they are partial takes (skipped via `anchored` above).
      }
...
function fire(state, m, tcfg, mult = 1) {
  ...
  const _cpats = tcfg.canon ? rule.canonPatterns : null;
  for (let i = 0; i < pats.length; i++) {
    let h = compiledSub(pats[i], i, m.theta, m.slots, recipes, subApplyIdx);
    if (_cpats && _cpats.indexOf(pats[i]) >= 0) h = tcfg.canon(h);
    let count = 1;
      ...
      if (doCoalesce) {
        ...
        if (!eff.all) {
          isDead = (inner, sid) => {
            ...
            if ((tcfg.withTag && it === tcfg.withTag) ||
                (tcfg.implTag && it === tcfg.implTag)) return false;
...
function settleExplore(inputState, rules, opts = {}) {
  ...
  //
  // leafMeasure (opt-in, THY_0034): { unit, step(acc, m) } — a TRACE
  // MEASURE folded over the path's firings, reported on each leaf as
  // `measure`. This is where conservation (usage/cost) axes live: a
  // measure is a function of the world's event multiset — well-defined
  // because independent-swap reorderings preserve the multiset (L3) —
  // never a stamp axis (stamp values are broadcast to every output and
  // joined over reads, so a non-idempotent axis would double-count;
  // the cartesian/linear no-go). pathWeight is the multiplicative
  // instance of the same shape, kept separate because its woplus
  // division semantics feeds the T3 estimator.
  const LM = opts.leafMeasure || null;
  function step(state, depth, pathWeight, macc) {
    if (depth > DEPTH_CAP) {
    ...
    if (tied.length === 1 ||
      ...
      return _fireOrFork(state, m, depth, pathWeight, macc);
    }
    ...
    for (const m of sorted) {
      ...
      children.push({
        ...
        tree: _fireOrFork(branch, m, depth, pathWeight, macc),
      });
    }
  ...
  function _fireOrFork(state, m, depth, pathWeight, macc) {
    if (m.rule.weighted && m.rule.consequentAlts.length > 1) {
      const children = [];
      for (let i = 0; i < m.rule.consequentAlts.length; i++) {
        ...
        const branch = state.snapshot();
        _countFired();
        const mAlt = _withAlt(m, i);
        fire(branch, mAlt, tcfg);
        children.push({ weight: w, tree: step(branch, depth + 1, _wMul(pathWeight, w), LM ? LM.step(macc, mAlt) : macc) });
      }
      return { type: 'choice', rule: m.rule.name, activation: stamps.term(m.activation), children };
    }
    _countFired();
    fire(state, m, tcfg);
    return step(state, depth + 1, pathWeight, LM ? LM.step(macc, m) : macc);
  }
  ...
  const tree = step(root, 0, [1n, 1n], LM ? LM.unit : undefined);
  return { tree, leaves };
}
 ...
 *
 * With opts.leafMeasure, leaves and frontier entries carry `measure` —
 * but the dominance filter stays STAMP-ONLY: a leaf stamp-dominated yet
 * measure-better is dropped from `frontier`. A (stamp × measure) Pareto
 * set must be computed over `leaves` (measures bring their own order;
 * it is never derived from the stamp join — THY_0034).
 */
function settleFrontier(inputState, rules, opts = {}) {
  ...
  for (const e of entries) {
    ...
    if (!dominated) {
      ...
      if (e.leaf.measure !== undefined) out.measure = e.leaf.measure;
