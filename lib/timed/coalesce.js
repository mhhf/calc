...
function _anteObservers(ante, tcfg, out) {
  ...
  const flat = (x) => {
    const t = Store.tag(x);
    if (tcfg.productTag && t === tcfg.productTag) { flat(Store.child(x, 0)); flat(Store.child(x, 1)); return; }
    if (tcfg.unitTag && t === tcfg.unitTag) return;
  ...
  for (let t = 0; t < linear.maxTagId; t++) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if ((tcfg.withTag && it === tcfg.withTag) ||
          (tcfg.implTag && it === tcfg.implTag)) continue;
