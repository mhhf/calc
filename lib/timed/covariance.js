...
function analyze(rule, tcfg) {
  ...
  if (rule.delay && rule.delay.term !== undefined) {
    // Compound delay term (TODO_0285): per-slot shift degrees inside the
    // term are unresolved analysis — conservatively unsafe for rebase.
    fail('compound delay term (per-component shift degrees unanalyzed)');
  }
