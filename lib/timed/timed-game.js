...
import { factKeyOf, flattenAnte } from '../engine/formula-utils.js';
import { refInner } from '../engine/fact-set.js';
import { normalizeTimedState, tryTimedMatch, loliCandidates, INT_TAG } from './timed.js';

/** Leaves of a `&`-spine, left-to-right (nested & flattens — grouping is
 *  presentation). Shared by withProject and menuStatus. */
function _menuLeaves(menu, rc) {
  const leaves = [];
  (function spine(h) {
...
function _projectionEnabled(base, projected, horizon, ruleList, calc, matchOpts, tcfg, compileLoli) {
  ...
  // Labelled state (THY_0024): activations are stamp ids in state2's
  // table; consumed maps carry packed refs.
  const stamps = state2.linear.stamps;
  ...
  for (const m of cands) {
    if (stamps.cmp(m.activation, horizonId) > 0) continue;
    for (const hStr in m.consumed) {
      if (projKeys.has(factKeyOf(refInner(Number(hStr)), tcfg.expTag))) return true;
    }
  }
  return false;
}
...
function withProject(inputState, factHash, index, opts = {}) {
  ...
  if (!linear[key] && Store.tag(key) !== tcfg.stampTag) {
    const wrapped = Store.put(tcfg.stampTag, [key, unit]);
  ...
  if (fromPersistent) {
    ...
    const stamped = Store.tag(key) === tcfg.stampTag;
  ...
  const leaves = _menuLeaves(menu, rc);
  ...
  for (const h of flat.linear) {
    ...
    if (tcfg.expTag && Store.tag(inner) === tcfg.expTag) {
      ...
      if (Store.tag(g) !== INT_TAG) throw new Error('choose: count grade must be a ground integer');
      count = Number(Store.child(g, 0));
      inner = Store.child(inner, 1);
    }
    if (count === 0) continue;
    const out = Store.put(tcfg.stampTag, [inner, s]);
...
function menuStatus(inputState, factHash, ruleList, opts) {
  ...
  if (Store.tag(menu) === tcfg.stampTag) {
