 ...
 *
 * The relaxation tier grades its refusals (settle-optimality §8.4): the
 * result carries `tiedContention` — true iff EVERY dependent pair of
 * relaxation firings is co-activated. `certified: true` (no dependent
 * pairs) is T2's hypothesis; `tiedContention: true` alone is the
 * frontier-adequacy hypothesis (the committed worlds of settleExplore
 * are outcome-complete, so settleFrontier returns the TRUE Pareto
 * frontier — the tied-contention adequacy proposition). Both false is
 * the W-gap shape: a dependent pair at distinct activations, where an
 * unfocused derivation can strictly dominate every committed world.
 *
...
import { isGround } from '../engine/pattern-utils.js';
import { resolveDelayValue } from './timed.js';
...
function relaxationCertify(engineCalc, ruleList, tcfg, state, horizon, opts = {}) {
  ...
  while (changed) {
    ...
    for (const r of ruleList) {
      ...
      const attempt = () => {
        ...
        let done = a;
        if (r.delay) {
          let d = resolveDelayValue(r.delay, theta, slots);
  ...
  // pairwise independence over the relaxation's firing set. Each flagged
  // pair also records CO-ACTIVATION (equal activation under the committed
  // total order): if every dependent pair is co-activated the program is
  // TIED-CONTENDED, which is exactly the frontier-adequacy hypothesis
  // (settle-optimality §8.4) — settleExplore branches on every tied
  // conflict, so the committed worlds are outcome-complete even though T2
  // (which needs NO dependent pairs) is refused. `allCoActivated` is
  // tracked outside the witness list — the list is capped at 16, and a
  // tiedness verdict read off a truncated list would be unsound.
  const contended = [];
  let allCoActivated = true;
  const flag = (i, j, why) => {
    const co = cmp(firings[i].activation, firings[j].activation) === 0;
    if (!co) allCoActivated = false;
    if (contended.length < 16) {
      contended.push({ a: firings[i].rule, b: firings[j].rule, why, coActivated: co });
    }
  };
  ...
  return {
    ...
    tiedContention: contended.length === 0 || allCoActivated,
    method: 'relaxation',
...
function certifyContention(engineCalc, ruleList, tcfg, state, horizon, opts = {}) {
  ...
  if (st.ok) return { certified: true, tiedContention: true, method: 'structural' };
