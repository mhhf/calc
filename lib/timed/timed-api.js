...
import backward from '../engine/backchain.js';
  ...
  return {
    ...
    settleChunked: (state, T, execOpts = {}) =>
      timed.settleChunked(state, filterRules(execOpts), {
        ...
        // A chunk is an EXTENT (slice width), not a threshold: an
        // algebra whose parseStamp widens thresholds to a down-set
        // supplies parseExtent for widths (a widened width would absorb
        // the sentinel into fact stamps through the chunk accumulator);
        // scalar algebras need no distinction.
