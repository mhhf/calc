 ...
 *   floor(v)      — optional: largest integral value ⊑ v (rebase origin
 *                   shifts; absent → settle's opts.rebase throws loudly)
 */

// The packed-ref ENCODING (packRef/refInner/refStamp, STAMP_CAP fence)
// lives with the row representation in lib/engine/fact-set.js
// (RES_0143 M1); this module is the label INTERNING side, reaching the
// FactSet through the policy object (policy.stampTable = StampTable).
import { STAMP_BITS, STAMP_CAP } from '../engine/fact-set.js';
...
class StampTable {
  constructor(alg) {
    ...
    // ⊕ order-prune is NOT a slot (0284 audit): C2+C3 make cmp >= 0 the
    // unique sound B&B cut — a weaker cut is behavior-identical (the leaf
    // keeps strict-< anyway), a stronger one unsound. A declared slot is
    // a migration error, loudly.
    if (alg.prunes !== undefined) {
      throw new Error("StampTable: alg.prunes is not a slot — the order-class B&B cut is fixed at cmp(partial, best) >= 0 (soundness under C2+C3); remove the declaration");
    }
   ...
   *  dead against best b? Fixed at cmp(p, b) >= 0 on the float-fast id
   *  cmp — `>=` keeps the FIRST match at equal grade (the FIFO half of
   *  timed.js's invariant pair); the cut is contract-determined, not a
   *  slot (see the constructor fence). */
  prunes(p, b) {
    return this.cmp(p, b) >= 0;
  }
