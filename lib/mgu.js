// TODO - ocurrence check
const isFreevar = str => /\_Freevar$/.test(str)
const subs = require("./substitute.js");
const Calc = require("./calc.js");
const { profiler } = require("./profiler.js");
const { internNode } = require("./intern.js");
const { getStore } = require("./store.js");

const propagate = function (theta, new_theta) {
  profiler.count('mgu.propagate');
  new_theta.forEach(([k, v]) => {
    theta = theta.map(([k_, v_]) => [k_, subs(v_.copy(), k, v)])
  })
  return theta.concat(new_theta);
}

/**
 * O(1) equality check using content-addressed hashes
 */
function termsEqual(t0, t1) {
  // Same reference = equal
  if (t0 === t1) {
    profiler.count('mgu.eq.ref');
    return true;
  }

  // Both strings - direct compare
  if (typeof t0 === 'string' && typeof t1 === 'string') {
    return t0 === t1;
  }

  // One is string, one is not - not equal
  if (typeof t0 === 'string' || typeof t1 === 'string') {
    return false;
  }

  // Use content-addressed hash (O(1) equality)
  profiler.count('mgu.eq.hash');
  const h0 = internNode(t0).hash;
  const h1 = internNode(t1).hash;
  return h0 === h1;
}

/**
 * Most General Unifier
 *
 * @param {Array} G - List of unification goals [[t0, t1], ...]
 * @returns {Array|false} - Substitution theta or false if unification fails
 */
const mgu = function (G) {
  const endTime = profiler.time('mgu');
  profiler.count('mgu.call');
  let rules = Calc.db.rules;
  var theta = [];
  while (G.length > 0) {
    profiler.count('mgu.iteration');
    let job = G.pop();
    let t0 = job[0];
    let t1 = job[1];
    let isEqType = t0.id == t1.id;
    let isVarL = typeof t0 !== "string" && isFreevar(rules[t0.id].ruleName)
    let isVarR = typeof t1 !== "string" && isFreevar(rules[t1.id].ruleName)
    let isEq = termsEqual(t0, t1);  // O(1) via content-addressing
    let isString = typeof t0 === "string" || typeof t1 === "string"

    // console.log(`unifying ${t0} =?= ${t1}`);
    // console.log(`isEqType: ${isEqType} -- ${t0.id} ${t1.id}`);
    if(isEq) {
      // do notighg
    } else if(isVarL && isVarR) {
      // console.log(`  ?? doublevar`);
      G = [[t0, t1]].concat(G);
    } else if(isVarL) {
      // console.log(`  eleminate`);
      theta = propagate(theta, [[t0, t1]])
      G = G.map(([l, r]) => ([subs(l, t0, t1), subs(r, t0, t1)]))
    } else if(isVarR) {
      // console.log(`  swap`);
      G.push([t1, t0]);
    } else if(isEqType && !isString) {
      // console.log(`  decompose`);
      t0.vals
      .forEach((v, i) => {
        G.push([v, t1.vals[i]]);
      })
    } else {
      // console.log(`  fail`);
      profiler.count('mgu.fail');
      endTime();
      return false;
    }
    // console.log(`  G: \n${G.map(task => `    ${task}`).join("\n")}`);
  }
  endTime();
  return theta;
}

module.exports = mgu;
