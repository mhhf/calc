...
function buildParserFromTables(tables) {
  ...
  return function parse(input) {
    ...
    const parseAtom = () => {
      ...
      // Binder keywords: forall X Y Z. body / exists X. body (de Bruijn)
      // Supports multiple variables: forall X Y Z. body → forall(forall(forall(body)))
      if (binderDefs) {
        for (const [kw, tagName] of Object.entries(binderDefs)) {
          if (src.slice(pos, pos + kw.length) === kw &&
            ...
            const savedPos = pos;
            pos += kw.length; ws();
            // Collect one or more variable names before the dot
            const varNames = [];
            while (true) {
              const varMatch = src.slice(pos).match(/^[A-Z][a-zA-Z0-9_']*/);
              if (!varMatch) break;
              varNames.push(varMatch[0]);
              pos += varMatch[0].length; ws();
            }
            if (varNames.length > 0 && pos < src.length && src[pos] === '.') {
              pos++; ws();
              // Push all vars onto binder stack (outer first)
              for (const v of varNames) binderStack.push(v);
              const body = parseExpr(0);
              // Pop in reverse order
              for (let vi = varNames.length - 1; vi >= 0; vi--) binderStack.pop();
              // Wrap body in nested binder nodes (innermost first)
              let result = body;
              for (let vi = varNames.length - 1; vi >= 0; vi--) {
                result = Store.put(tagName, [result]);
              }
              return result;
            }
      ...
      // Number literals: 42, 0xFF, long hex → arrlit of byte binlits
      if (parseNumbers) {
        const numMatch = src.slice(pos).match(/^(0x[0-9a-fA-F]+|\d+)/);
        if (numMatch && (pos === 0 || !/[a-zA-Z_]/.test(src[pos - 1]))) {
          pos += numMatch[0].length; ws();
          const raw = numMatch[0];
          // Long hex (> 64 hex chars = 32 bytes) → arrlit of byte binlits
          if (raw.startsWith('0x') && raw.length - 2 > 64) {
            const hexStr = raw.slice(2);
            if (hexStr.length % 2 !== 0) throw new Error(`Parse error: odd-length hex literal '${raw}'`);
            const byteCount = hexStr.length / 2;
            const elems = new Uint32Array(byteCount);
            for (let bi = 0; bi < byteCount; bi++) {
              elems[bi] = Store.put('binlit', [BigInt(parseInt(hexStr.slice(bi * 2, bi * 2 + 2), 16))]);
            }
      ...
      if (pos < src.length && src[pos] === '{') {
        ...
        ws();
        if (!consume('}')) throw new Error('Parse error: expected }');
        return Store.put('monad', [inner]);
      }

      if (consume('[')) {
        const elems = [];
        if (pos < src.length && src[pos] !== ']') {
          elems.push(parseExpr(0));
          while (consume(',')) {
            elems.push(parseExpr(0));
          }
          // [A, B | REST] → acons(A, acons(B, REST))
          if (consume('|')) {
            const tail = parseExpr(0);
            if (!consume(']')) throw new Error('Parse error: expected ]');
            let result = tail;
      ...
      if (idMatch) {
        ...
        if (/[A-Z]/.test(name[0])) {
          ...
          // Free variable (metavar — unification variable)
          if (multiCharFV) {
      ...
      throw new Error(`Parse error at ${line}:${col}: unexpected '${src.slice(pos, pos + 10)}'`);
    };

    const result = parseExpr(0);
    ws();
    if (pos < src.length) {
      throw new Error(`Parse error at ${line}:${col}: unexpected '${src.slice(pos)}'`);
    }
    return result;
  };
}
