...
function buildParser(constructors, parserOpts = {}) {
  ...
  // application, multiCharFreevars, binders, ...) —
 ...
 * parser's elided-grade convention) drops information — render(monad(d,b))
...
function deriveRoles(constructors, polarity) {
  ...
  for (const [name, constr] of Object.entries(constructors)) {
    ...
    if (cat === 'multiplicative') {
      ...
      else if (arity === 0 && pol === 'negative') role = 'additiveUnit';  // ⊤, unit of &
      else if (arity === 0) role = 'additiveZero';                       // 0, unit of ⊕
      ...
      role = 'existential';
    } else if (cat === 'fixpoint' && arity === 1) {
      // μMALL fixed points (TODO_0009): lfp = μ (positive), gfp = ν (negative).
      // Consumed structurally by the tabling/GTC checkers (Store.TAG identity).
      if (pol === 'positive') role = 'lfp';
      else if (pol === 'negative') role = 'gfp';
    }
    if (role) {
      if (roles[role]) {
