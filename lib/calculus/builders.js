...
import { earleyGrammarFromTables, parserFromGrammar } from '../parser/earley-grammar.js';
 ...
 *
 * The computation role is derived from the constructors (category 'monad')
 * and threaded into the grammar so `{ expr }` builds the calculus's own
 * computation node — a graded (2-ary) computation additionally needs
 * parserOpts.gradeUnit (an in-process () => hash hook); without it the
 * grammar throws a clear error on `{ expr }` instead of silently building
 * ILL's unary monad (audit round 11, F1).
 */
function buildParser(constructors, parserOpts = {}) {
  const tables = parserTables(constructors);
  ...
  if (compEntry) {
    ...
    if (comp) tables.computation = comp;
  }
  if (parserOpts.gradeUnit) tables.gradeUnit = parserOpts.gradeUnit;
 ...
 * in engine/formula-utils.js) — the one shape every monad-aware site reads.
 * All other roles are bare connective names.
 */
