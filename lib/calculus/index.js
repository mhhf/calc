...
import generator from './meta-parser.js';
...
import { RESERVED_RULE_NAMES } from '../engine/reserved-preds.js';
  ...
  for (const rp of rulesPaths) {
    ...
    for (const name of Object.keys(next)) {
      if (RESERVED_RULE_NAMES.has(name)) {
        throw new Error(`rule '${name}' is a machinery-reserved rule name ` +
          `(synthesized by the prover and intercepted by the kernel — a calculus ` +
          `may not declare it; see lib/engine/reserved-preds.js)`);
      }
      if (name in rules) {
...
function deriveContextStructure(spec) {
  ...
  if (consumable.length < 1 || copySources.length > 1) {
    throw new Error(
      ...
      `zones [${zoneNames.join(', ')}] but no consumable zone ` +
      `(without contraction: [${consumable.join(', ')}]; with: ` +
      `[${copySources.join(', ')}]) — at least one consumable zone and ` +
      `at most one copy source are required`
    );
  }
  // Primary consumable zone = FIRST no-contraction zone in position order
  // (Set preserves first occurrence); further no-contraction zones are
  // AUX consumable zones (TODO_0285). Aux membership is wrapper-routed:
  // each aux zone needs a constructor whose @category names the zone,
  // and may not declare weakening or contraction (policies the engine
  // does not honor for aux zones are fenced loudly, not ignored).
  const consumableZone = consumable[0];
  const copySource = copySources[0] || null;
  const auxZones = consumable.slice(1);
  // Aux zone names double as @category values (the zone↔wrapper link), so
  // a zone named after a category the engine already interprets would make
  // its wrapper connective match OTHER machinery too ('monad' → default
  // monad_r/monad_l injection, 'exponential' → graded-prefix parsing,
  // 'comonad'/'quantifier'/... → deriveRoles). Fence the collision loudly.
  const RESERVED_CATEGORIES = new Set([
    'multiplicative', 'additive', 'exponential', 'monad', 'comonad', 'quantifier',
  ]);
  let wrapperZoneByTag = null;
  for (const zone of auxZones) {
    if (RESERVED_CATEGORIES.has(zone)) {
      throw new Error(
        `deriveContextStructure: aux zone '${zone}' collides with a ` +
        `reserved @category name — zone names must not shadow connective ` +
        `categories the engine interprets`
      );
    }
    if (properties[zone].weakening) {
      throw new Error(
        `deriveContextStructure: aux zone '${zone}' declares weakening — ` +
        `aux consumable zones must be linear-policy (unsupported policy ` +
        `would be silently unenforced)`
      );
    }
    let wrapper = null;
    for (const name in ctors) {
      if ((ctors[name].annotations || {}).category === zone) { wrapper = name; break; }
    }
    if (!wrapper) {
      throw new Error(
        `deriveContextStructure: aux zone '${zone}' has no wrapper ` +
        `constructor (a connective with @category ${zone}) — nothing ` +
        `could ever route to it`
      );
    }
    if (!wrapperZoneByTag) wrapperZoneByTag = {};
    wrapperZoneByTag[wrapper] = zone;
  }
  const record = {
    zones: [consumableZone, ...zoneNames.filter(z => z !== consumableZone)],
    properties,
    consumableZone,
    consumableZones: [consumableZone, ...auxZones],
    copySource,
    copyTarget: copySource ? consumableZone : null,
  };
  if (wrapperZoneByTag) record.wrapperZoneByTag = wrapperZoneByTag;
  return record;
}
