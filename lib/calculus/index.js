 ...
 * @param {Object} [opts] - { parser, grades }: parser options for BOTH the
 *   rules file and calculus.parse (graded/timed syntax), and the
 *   grade-algebra record for @grade side conditions (TODO_0265 6b).
...
function load(calcPath, rulesPath = null, opts = {}) {
  ...
  let rules = {};
  if (rulesPath) {
    rules = loadRules(rulesPath, spec.constructors, opts);
  }
...
function loadRules(rulesPath, constructors, opts = {}) {
  ...
  return parseRules2(text, parseFormula, { grades: opts.grades });
}
...
function buildCalculus(spec, rules = {}, opts = {}) {
  ...
  // call its graded monad anything (till: gmonad). The role record drives
  ...
  return {
    ...
    // Grade-algebra record for template-rule side conditions (TODO_0265
    // 6b, D13: the SAME record the timed scheduler reads); null for
    // ungraded calculi.
