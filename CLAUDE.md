...
│   ├── decimate.js      # Generic: decimation driver — ∃_ρ waves, lazy recursion, sample/exact/solve (calc.collapse; TODO_0297 P2/P3)
...
- Datasorts (fence B slices 1+2, TODO_0011 round-2 spec): `even <: lst.` with an UNDECLARED lhs introduces a datasort, defined by ordinary membership clauses — nullary heads (`warm/s: warm sea.`) or constructor heads with datasort premises on immediate subterm variables (`even/c: even (cons H T) <- odd T.`). The name gets a loader-synthesized dual role (sort atom + unary membership predicate, sound post-f7ec930a; `!even T` backward goals just work). Clauses compile to a top-down deterministic tree automaton; INSIDE MASSES are solved EXACTLY at load (priors.js solveMasses: SCC decomposition, linear systems by rational Gaussian elimination; `calc.masses`). Fences, each a named load error: f1 head = one depth-1 constructor pattern with distinct variables; f2 premises = unary datasort goals on head argument variables, each at most once, base-compatible; f3 one clause per (datasort, head) — determinism (masses double-count under overlap); f4 ≤1 recursive argument per head within its SCC (nonlinear = fence B″, algebraic masses); empty language (no base case) = load error; critical/supercritical mass systems (singular or negative solution) = load error. Conditioning a wave: static `exists X: even @w.` at the binder, or dynamic `!within E warm` facts (program declares `within: (x: base) -> (s: sort) -> type.`; finite domains only — recursive within = product states, fence B slice 3). Semantics is RESTRICTION, not renormalization (B4): surviving worlds keep their masses; the conditioned sampler draws heads ∝ ρ·Π m(child states) while the recorded mass factor stays ρ·bias, so importance ≡ m(S) for every seed on bias-free programs (B6, zero-variance — the telescoping per-draw factor is massFactor·drawTotal/drawWeight); child waves register at the automaton's child STATES; tokens carry the BINDER's sort name (`drawn cons even`, m4) and draw-check walks composite witnesses at child states. Pins: tests/engine/will-datasorts.test.js (worked example: m(even)=8/3, m(odd)=4/3, partition = m(lst)=4).
