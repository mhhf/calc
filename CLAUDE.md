...
│   ├── decimate.js      # Generic: decimation driver — collapse of suspended ∃-facts (calc.collapse, modes sample/exact/solve; TODO_0297 P2)
...
- Decimation (`calc.collapse`, TODO_0297 P2): an unresolved existential consequent settles to a SUSPENDED ∃-fact (`at(exists(body), t)`) — inert under plain settle (D4 opt-in). The driver opens it (fresh evar, conjuncts split at the stamp), re-settles so `!bias E c Q` rules bind the evar, computes posteriors = prior · Π distinct bias facts (M8), draws min-entropy-first (PRF, value-derived keys — never evar ids), `substituteEvar`s, repeats. Modes: `sample` (restart on contradiction, M9; exact importance accounting, T3), `exact` (full collapse tree, unnormalized masses — T1), `solve` (first-solution DFS). Wave sorts resolve from signature positions; no wave table is stored
