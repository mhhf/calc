...
└── (tested via)         # tests/engine/rill-modality.test.js — loads via rill. Guarded signals coinduct via the EXISTING GTC (νR = progress, ○ = syntactic guard); no gtc-check.js change. ○-elimination / temporal cut = the whole-context tick (DONE); frontier: graded ○ (fold tick into gill's grade frame) and ⊤
