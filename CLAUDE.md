...
├── rill.rules           # The single ○R rule (next_r) — PROMOTION-SHAPED (G ; |- ○A <- G ; |- A, empty linear context): the temporal no-carry-forward discipline (a linear resource consumed now cannot advance; only persistent/signal resources do). NO ○-left rule ⇒ non-collapse (○a ⊬ a, a ⊬ ○a). Loaded as the LIST [ill.rules, fill.rules, rill.rules]
...
└── (tested via)         # tests/engine/rill-modality.test.js — loads via rill. Guarded signals coinduct via the EXISTING GTC (νR = progress, ○ = syntactic guard); no gtc-check.js change. Frontier: no ○-elimination (whole-context tick / temporal cut) and no ⊤ yet
