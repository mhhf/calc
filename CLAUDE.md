...
**Backward prover** (L1-L4): kernel.js → generic.js → focused.js → strategy/ (manual, auto). Zone names come from `calculus.contextStructure`, DERIVED from the family's `@position_modes` + `@structural` declarations (consumableZone/copySource + per-zone exchange/contraction/weakening; two-zone default `DEFAULT_CONTEXT_STRUCTURE` in `lib/kernel/sequent.js` for bare calculi) — no `'linear'`/`'cartesian'` literals in lib/ logic (TODO_0086). AUX consumable zones (TODO_0285): further no-contraction zones are admitted, WRAPPER-ROUTED (membership = the connective whose `@category` names the zone; missing wrapper or aux weakening = loud load error); the search/kernel thread ONE union pool (`Seq.consumablePool`) — columns are router-materialized views (THY_0033), so DECLARING a zone is calculus data (the pool plumbing itself was the one-time P4 change; it is zone-count-agnostic). sill's `loc` (zone: located) is the instance.
...
├── sill.calc            # @extends gill (surface inherited); place index sort (opaque — no arithmetic), loc `#1 @@ #2` @category located (= the located zone's wrapper), 4-ary seq @position_modes "cartesian linear located linear" + per-position @structural rules — the zones-as-data acceptance artifact: declaring a zone is calculus DATA (the P4 pool plumbing in the kernel/prover is one-time and zone-count-agnostic; no per-zone engine edits). loc is UNPOLARIZED (synthetic atom, like at/drawn); per-fiber linearity is automatic (place is part of the fact hash)
...
calculus/rill/           # rill — reactive ILL (TODO_0203, THY_0043): fill + the ○ next-time modality. The SECOND fork in the μMALL chain — firewalls the soundness-subtle temporal/FRP machinery from fill's audited μMALL core, as fill firewalls μMALL from ILL. Signals □A = νX.(A & ○X), streams ◇A = μX.(A ⊕ ○X)
├── rill.calc            # @extends fill (surface inherited: ILL + mu/nu); declares ONLY circle (○) — unary, @category modality (a FRESH category → deriveRoles gives it no role, so the cyclic engine is untouched), @polarity positive, @ascii "O #1"
├── rill.rules           # The single ○ rule (next_r) — the whole-context TICK (@tick): G ; ○Δ |- ○C <- G ; Δ |- C, fires only when succ is ○C AND every linear formula is ○-wrapped, strips one ○ from each, persistent G passes through. Empty-Δ case = promotion-shaped ○R (intro); non-empty case = ○-elimination / temporal cut (applicative ○(a⊸b),○a⊢○b; lax-monoidal ○a,○b⊢○(a*b)). One rule = intro+elim, still NON-COLLAPSING (○a ⊬ a: succ not ○; a ⊬ ○a: ctx not ○-wrapped). @tick is a whole-sequent transform: bypassed in focused.js (applyAndRecurse + K-twin), fully RE-DERIVED in kernel.js (spec.tick case — all-○ pool, premise = stripped ctx, no smuggled persistent; adversarially fenced). Loaded as the LIST [ill.rules, fill.rules, rill.rules]
├── calculus-config.js   # Composes fill's config field-by-field (family/init/theories/backward/ffi/compose/EVM domain referenced); rill-OWN: connectives (rill.calc chain), loader.buildParser (○ prefix operator), compile.cacheEpoch 'rill'
├── lib/forward-parser.js # loadRill (rill.calc + [ill.rules, fill.rules, rill.rules]) + the .ill parser (fill's operators + the ○ prefix, auto-derived from @ascii — ○ is NOT a binder)
├── index.js             # rill facade (mirrors fill/index.js): load/precompile bound to rillConfig + loadRill/proveString/parseSequent
└── (tested via)         # tests/engine/rill-modality.test.js — loads via rill. Guarded signals coinduct via the EXISTING GTC (νR = progress, ○ = syntactic guard); no gtc-check.js change. ○-elimination / temporal cut = the whole-context tick (DONE). ○ is STRONG monoidal for ⊗ (○(a*b)⊣⊢○a*○b — same resource multiset; the tick leftover re-wrap is linear accounting, fenced no-dup/create/discard). Frontier: ⊤

calculus/grill/          # grill — graded μMALL (TODO_0284/0009, THY_0044): gill (grades) + fill (μ/ν) in ONE calculus. @extends gill + the fill μ/ν surface; deriveRoles arms lfp/gfp beside gill's grade roles with NO engine change. The first grade×fixpoint COMBINATION brick: graded coinductive signals νX.(a&!!_0 X), graded inductive streams, grade arithmetic under fixpoints — all kernel+GTC verified. calculus-config.js composes gillConfig field-by-field (grill-OWN: connectives from grill.calc chain, loadGrillSequent via makeSequentLoader with a grill-bound theory + auto-derived μ/ν binders — NO explicit binders map to the sequent loader, else `mu A` breaks; the FORWARD parser DOES take {mu,nu}; cacheEpoch 'grill'). Tested via tests/engine/grill-graded-munu.test.js
calculus/trill/          # trill — graded REACTIVE μMALL (THY_0044 roadmap item 3): grill + the ○ next-time modality, i.e. ALL THREE unification axes (grades × μ/ν × ○) in one calculus. To grill what rill is to fill — the ○ fork. Composes with ZERO engine work (the whole-context @tick is grade- and fixpoint-agnostic): graded ○-guarded signals νX.(a & ○(!!_d X)), graded temporal streams μX.(a ⊕ ○(!!_d X)), grade commutes over a tick (○(!!_0 a)⊢!!_0(○a)) — all kernel+GTC verified; ○ still non-collapsing; coinductive weakening / grade manufacture refused; cut admissible (fuzz-cut). trill.calc @extends grill + circle (○); trill.rules = the @tick next_r; calculus-config.js mirrors grill's (composes gillConfig, loadTrillSequent via makeSequentLoader over [gill.rules, grill.rules, trill.rules] + trill-bound theory, ○ prefix auto-derived, cacheEpoch 'trill'). Tested via tests/engine/trill-graded-reactive.test.js
...
- Prover `opts.exhaustive` (THY_0042 §4): the default focused search is COMMITTED-CHOICE — at every `if (result) return result` it takes the first locally-successful result, incomplete for additive don't-know nondeterminism under a linear-resource constraint (a leftover from a wrong `with_l1`/`with_l2` pick only fails the emptiness check at the ROOT, too late to backtrack — e.g. `a & I ⊢ I`, weakening `!a ⊢ I`). `opts.exhaustive` switches to a SEPARATE continuation-passing driver (`searchCoreK`/`applyAndRecurseK` in focused.js) that threads the root emptiness constraint back as a success continuation, so it backtracks into the choice. It IMPLIES loop detection (else exponential on coinductive/unprovable goals). The committed path is byte-identical when the flag is off (ILL/EVM pay nothing); soundness is unchanged (kernel + GTC still verify the returned tree). fill/rill-only / experimental — never set on the ILL EVM path.
...
- State canonicity (theory-eq sweep, doc/def/0046): `===` on hashes is REPRESENTATION equality; value equality is theory-relative (`binlit 3` ≠ hash `i(i(e))` but theory-equal). Live states hold only canonical facts because every fact-entry boundary canonicalizes: production (compile-time `canonPatterns` gate — class constructor over a variable, e.g. `tok (i X)`; unflagged rules pay nothing), plain-object entry (`canonObject`), loli materialization, clause outputs (resolve-all/tiers/tabling). Engine internals may key on hashes UNDER this invariant; certifiers/checkers never trust it — they compare modulo the theories (certify-confluence `_thEq`). strlit has no canonical form (string facts can split rows; matching stays correct). Pins: tests/engine/state-canonicity.test.js
...
- Trace measures (THY_0034): conservation axes (fuel/cost, ⊔=+) NEVER ride stamps — stamp values are broadcast/joined/re-emitted (cartesian), conserved values are linear (the broadcast no-go; paper §8.5). Accounting = `settleExplore`/`settleFrontier` opt `leafMeasure: {unit, step(acc, m)}` (leaves + frontier entries carry `measure`; `m.altIndex` on woplus alts); gating = fuel as linear tokens; preference = chooser; timing feedback = term-computed delays. Frontier dominance stays STAMP-ONLY — a (stamp × measure) Pareto set is computed over `leaves`, never `frontier`
...
- Product stamps (sill, TODO_0285): the stamp VALUE is a (time, dist) pair interned as ONE StampTable id (packRef unchanged); committed settle runs the LEX total order (time primary, dist tie-break) — the Pareto frontier lives ONLY in `settleFrontier` (explore + join-derived dominance filter). Scalar grades embed as dist-0 and REIFY back to scalar terms (canonical collapse); scalar horizons widen to the down-set (T, ∞). `@(T ~ D)` pair grades are a declared `@grammar grade` template (grade-chain emission); compound variable delays compile as `{ term }` and θ-substitute at fire. accelerate/coalesce/rebase reject sill loudly (the algebra omits scale/floorDiv/floor, and coalescing is fenced on the function-valued merge — a componentwise join reads past-stamp components their order position doesn't carry); ∞ (tinf) is DIST-only — an infinite time half is fenced at parse; a 0-time dist-growing loop advances the lex frontier and evades maxInstantSteps — maxSteps catches it. Certification is clause-only via spatial.sill's pair clauses + the `fire` config `join` predicate (componentwise joins may equal no single input)
...
- `tools/fuzz-gtc.js` — checkGTC soundness fuzzer (TODO_0009): valid cyclic-proof back-edges accepted, mutants (5 unsoundness classes) rejected (`node tools/fuzz-gtc.js [--count N] [--seed N]`; 500-trial run in test:heavy)
- `tools/fuzz-cut.js` — cut-admissibility fuzzer across ill/fill/gill/grill (THY_0044 §4): if `Γ⊢A` and `A,Δ⊢C` are cut-free provable then `Γ,Δ⊢C` is, kernel-verified — the generic `cut` is admissible per instance, incl. coinductive/graded-coinductive cut (`node tools/fuzz-cut.js [--count N] [--seed N]`; 60-trial run in test:heavy; deterministic battery in tests/engine/cut-admissibility.test.js)
