...
function matchLoli(h, state, calc, matchOpts = EMPTY_MATCH_OPTS) {
  ...
  const comp = rc && rc.computation;
  const bodyInner = comp && Store.tag(body) === comp.tag
    ? Store.child(body, comp.bodyIdx) : body;
  ...
  for (let ti = 0; ti < linearTriggers.length; ti++) {
    ...
    // the pattern's own head tag decides the group (atom-headed → atom
    // path even when a same-named predicate tag exists globally)
    const pTag = pred ? Store.tagId(pattern) : -1;
    const trigTagId = pTag >= Store.PRED_BOUNDARY ? pTag : -1;
    let found = false;

    const candidates = trigTagId >= 0
      ? state.linear.group(trigTagId)
      : pred ? state.groupForPredTag(0, pred) : state.groupForPred(pred);
  ...
  if (persistentTriggers.length > 0) {
    // provePersistent is always present in a factory-built matchOpts: the
    // generic layer's baseline stateProvePersistent (from buildGenericProtocol)
    // is the factory default, overridden by the composition root when FFI is
    // enabled. No defensive fallback — the contract guarantees presence.
    const proved = matchOpts.provePersistent(
