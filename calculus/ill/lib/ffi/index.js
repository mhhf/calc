...
register('arithmetic.min', arithmetic.min);
register('arithmetic.max', arithmetic.max);
...
// Register explicit rational operations (TODO_0265 Phase 1; clause
// definitions in calculus/till/prelude/rat.ill — inert unless that file is
// loaded, since no ILL program emits q-predicates)
register('rat.qplus', ratFFI.qplus);
register('rat.qsub', ratFFI.qsub);
register('rat.qmul', ratFFI.qmul);
register('rat.qdiv', ratFFI.qdiv);
register('rat.qlt', ratFFI.qlt);
register('rat.qle', ratFFI.qle);
register('rat.qeq', ratFFI.qeq);
register('rat.qneq', ratFFI.qneq);
register('rat.qeq_bool', ratFFI.qeq_bool);
register('rat.qmin', ratFFI.qmin);
register('rat.qmax', ratFFI.qmax);

// Numeric-tower dispatch (TODO_0011 §3 — the collapsed names): ONE handler
// per shared operation — bin fast path first (fails advisorily on rational
// args: binToInt(ratlit) = null), rational fallback second (total on the
// tower: bins coerce to n/1). Coherent on the overlap by the collapse law,
// so try-both is sound; a bin-FALSE comparison retries rationally and gets
// the same answer. Only calculi whose ffi META points here use these
// (till); ILL's meta keeps the bin-only handlers.
const numDispatch = (binFn, ratFn) => (args) => {
  const r = binFn(args);
  return (r && r.success) ? r : ratFn(args);
};
register('num.plus', numDispatch(arithmetic.plus, ratFFI.qplus));
register('num.mul', numDispatch(arithmetic.mul, ratFFI.qmul));
register('num.lt', numDispatch(arithmetic.lt, ratFFI.qlt));
register('num.le', numDispatch(arithmetic.le, ratFFI.qle));
register('num.eq', numDispatch(arithmetic.eq, ratFFI.qeq));
register('num.neq', numDispatch(arithmetic.neq, ratFFI.qneq));
register('num.eq_bool', numDispatch(arithmetic.eq_bool, ratFFI.qeq_bool));
register('num.min', numDispatch(arithmetic.min, ratFFI.qmin));
register('num.max', numDispatch(arithmetic.max, ratFFI.qmax));
...
const defaultMeta = {
  ...
  min: { ffi: 'arithmetic.min', mode: '+ + -' },
  max: { ffi: 'arithmetic.max', mode: '+ + -' },
  ...
  qplus: { ffi: 'rat.qplus', mode: '+ + -', multiModal: true },
  qsub: { ffi: 'rat.qsub', mode: '+ + -' },
  qmul: { ffi: 'rat.qmul', mode: '+ + -' },
  qdiv: { ffi: 'rat.qdiv', mode: '+ + -' },
  qlt: { ffi: 'rat.qlt', mode: '+ +' },
  qle: { ffi: 'rat.qle', mode: '+ +' },
  qeq: { ffi: 'rat.qeq', mode: '+ +' },
  qneq: { ffi: 'rat.qneq', mode: '+ +' },
  qeq_bool: { ffi: 'rat.qeq_bool', mode: '+ + -' },
...
export { register, get, has, registry, mode, arithmetic, memory, calldata, array, convert, defaultMeta, parsedModes, getModes, getModeMeta };
export default { register, get, has, registry, mode, arithmetic, memory, calldata, array, convert, defaultMeta, parsedModes, getModes, getModeMeta };
