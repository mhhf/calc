 ...
 */
/**
 * FFI groundness: "ground enough for FFI dispatch".
 * Differs from pattern-utils.isGround (structural groundness):
 *   - arrlit is always ground here (FFI treats containers as opaque)
 *   - invalid terms return false (not true)
 * See SE-6 in workload audit for rationale.
 */
function isGround(h) {
  const t = Store.tag(h);
  if (!t) return false;
  ...
  if (t === 'freevar') return true; // eigenvariable — ground for FFI dispatch

  if (t === 'atom') return true;

  // Arrlit/trie: concrete container — ground for FFI dispatch regardless of element groundness.
  // FFI ops (arr_get, arr_set, arr_len, trie_get, trie_set) operate on the container, not element values.
  if (t === 'arrlit' || t === 'tn') return true;
...
export { binToInt, intToBin, strToHash, hashToStr, charToHash, hashToChar, isGround };
export default { binToInt, intToBin, strToHash, hashToStr, charToHash, hashToChar, isGround };
