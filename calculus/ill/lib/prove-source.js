 ...
 *
 * Payload: `forward-trace/v2` (skeleton + leaves, lazy per-leaf traces).
 *
...
function proveSymex({ body, absImports, calcName, profile, key, cacheDir, opts }) {
  ...
  let initialState;
  try {
    // EVM-normalizing decomposition (code→arrlit→semantic) — symex queries
    // may carry bytecode facts; a no-op for states without them.
    initialState = mde.normalizeQuery(queryHash);
  } catch (e) {
...
export { proveSource, proveSubtree, extractSymexLeafTrace, hashKey, parseHeader, resolveImport, SANDBOX_ROOT, _resetCache };
export default { proveSource, proveSubtree, extractSymexLeafTrace, hashKey, parseHeader, resolveImport, SANDBOX_ROOT, _resetCache };
