...
function deriveZkRuleSpecs(calculus, zkTags) {
  ...
  for (const [ruleName, rule] of Object.entries(calculus.rules)) {
    ...
    const premises = [];
    if (d.modeShift) {
      // monad_r: mode shift → 1 premise (the BODY child, 1 since the D6
      // binary merge) with lax=1
      premises.push({ succedent: { Child: 1 }, lax: 1 });
 ...
 *
 * @param {Object} term - GenericTerm (from extractTerm or guidedTerm)
...
function generateWitness(term, sequent, opts = {}) {
  ...
  function emitGroundDecomp(bodyHash) {
    ...
    if (tag === 'tensor') {
      ...
      const g = Store.child(bodyHash, 0);   // grade (bang(grade, A))
      const bc1 = Store.child(bodyHash, 1); // inner formula
      chipRows('absorption').push([1, bodyHash, g, bc1]);
      addFormulaLookup(bodyHash);
    } else if (tag === 'one') {
  ...
  function walk(_t, _nonceIn, _lax, _goal, _delta) {
    ...
    switch (rule) {
      ...
      case 'bang_r': {
        const hash = goal;
        const g = Store.child(hash, 0);  // grade (bang(grade, A))
        const c1 = Store.child(hash, 1); // inner formula
        const n0 = ++nonceCtr;
        chipRows('bang_r').push([1, hash, g, c1, nonceIn, lax, n0]);
        addFormulaLookup(hash);
        t = t.subterms[0]; nonceIn = n0; goal = c1;
        continue;
      }
      ...
      case 'monad_r': {
        const hash = goal;
        const g = Store.child(hash, 0);  // unit grade (D6: monad(grade, body))
        const c1 = Store.child(hash, 1); // body
        const n0 = ++nonceCtr;
        chipRows('monad_r').push([1, hash, g, c1, nonceIn, lax, n0]);
        addFormulaLookup(hash);
        // monad_r forces lax=1 for inner proof
        const inner = t.evidence || t.subterms[0];
        t = inner; nonceIn = n0; lax = 1; goal = c1;
        continue;
      }
      ...
      case 'dereliction': {
        ...
        const g = Store.child(hash, 0);  // grade (bang(grade, A))
        const c1 = Store.child(hash, 1); // inner formula
        chipRows('bang_l').push([1, hash, g, c1]);
        addFormulaLookup(hash);
        deltaRemove(delta, hash);
        deltaAdd(delta, c1);
        t = t.subterms[0]; continue;
      ...
      case 'absorption': {
        ...
        const g = Store.child(hash, 0);  // grade (bang(grade, A))
        const c1 = Store.child(hash, 1); // inner formula
        chipRows('absorption').push([1, hash, g, c1]);
        addFormulaLookup(hash);
        deltaRemove(delta, hash);
        t = t.subterms[0]; continue;
      }
      ...
      case 'monad_l': {
        ...
        const g = Store.child(hash, 0);  // unit grade (D6: monad(grade, body))
        const c1 = Store.child(hash, 1); // body
        chipRows('monad_l').push([1, hash, g, c1]);
        addFormulaLookup(hash);
        deltaRemove(delta, hash);
        deltaAdd(delta, c1);
        t = t.subterms[0]; continue;
      ...
      case 'copy': {
        ...
        if (customChips.size > 0 && Store.isTerm(hash) && Store.tag(hash) === 'loli') {
          ...
          if (Store.isTerm(monadH) && Store.tag(monadH) === 'monad') {
            ...
            const goalOut = rTensor(allDeltaFacts);
      ...
      case 'loli_match': {
        ...
        const groundBody = rTensor(groundFacts);
        const groundMonad = Store.put('monad', [monadUnit(), groundBody]);
        ...
        chipRows('monad_l').push([1, groundMonad, monadUnit(), groundBody]);
...
function resolveRule(term) {
  ...
  if (rule === 'bang_l') {
    ...
    if (term.subterms.length === 1 && term.principal != null) {
      const inner = Store.child(term.principal, 1); // bang(grade, A) — inner formula is child 1
...
export { generateWitness, generateChunkedTreeWitness, deriveZkTags, deriveZkRuleSpecs, extractUint256PredMeta, bigintToLimbs, computeAdditionCarries, computeIncrementCarries, computeMultiplicationCarries };
export default { generateWitness, generateChunkedTreeWitness, deriveZkTags, deriveZkRuleSpecs, extractUint256PredMeta, bigintToLimbs, computeAdditionCarries, computeIncrementCarries, computeMultiplicationCarries };
