 ...
 * (from rwTrace) and produces per-chip trace rows for the flat
...
function generateFlatWitness(trace, sequent, opts = {}) {
  ...
  const canonConsRomEntries = new Map(); // consHash → { canonCons, lookups }
  let substIdCtr = opts.substIdStart || 1;
  ...
  for (const step of trace) {
    ...
    if (!isLoli) {
      ...
      const groundAnt = rTensor(consumed);
      const groundCons = rTensor(step.produced);
      const groundMonad = Store.put('monad', [monadUnit(), groundCons]);
    ...
    groundCons = step.produced.length === 0 ? oneHash : rTensor(step.produced);
    ...
    if (isLoli) {
      ...
      consHash = Store.child(monadHash, 1);   // original consequent body pattern
      // Ground tensor of consumed (for antecedent spine verification)
      groundAnt = consumed.length === 0 ? oneHash : rTensor(consumed);
      ...
      // Verified via CANON_CONS_BUS lookup against preprocessed ROM.
      const bodyLeaves = flattenTensor(consHash);
      canonCons = bodyLeaves.length === 0 ? oneHash : rTensor(bodyLeaves);
      // Track canon_cons ROM entry (keyed by cons_hash)
      const ccExisting = canonConsRomEntries.get(consHash);
      if (ccExisting) {
        ccExisting.lookups++;
      } else {
        canonConsRomEntries.set(consHash, { canonCons, lookups: 1 });
      }
    } else {
      antHash = consumed.length === 0 ? oneHash : rTensor(consumed);
      consHash = groundCons; // compiled: cons_hash = ground_cons (both right-associated)
      monadHash = Store.put('monad', [monadUnit(), consHash]);
      antSpine = buildSpineIntermediates(consumed, MAX_CONSUMED);
    }
    ...
    row.push(monadHash);
    row.push(isLoli ? 0 : 1);  // auxiliary: active * (1 - is_loli)
    ...
    row.push(canonCons);   // canonical body form (verified via CANON_CONS_BUS)
  ...
  // Format: [cons_hash, canon_cons, is_active, num_lookups]
  const canon_cons_rom = [];
  for (const [consHash, entry] of canonConsRomEntries) {
    canon_cons_rom.push([consHash, entry.canonCons, 1, entry.lookups]);
  }
  ...
  return {
    ...
    canon_cons_rom,
    tags: zkTags,
    constants: { one_hash: oneHash, monad_unit_hash: monadUnit() },
    max_ctx_size: Math.max(flatInit.length, flatFinal.length),
    _substIdCtr: substIdCtr, // internal: next available substId (for chunking)
  };
}
...
function generateChunkedFlatWitness(trace, sequent, opts = {}) {
  ...
  let nextSubstId = 1; // Global substId counter across chunks (avoids recycling)
  const chunks = [];
  ...
  for (let start = 0; start < trace.length; start += maxRows) {
    ...
    const chunk = generateFlatWitness(segment, sequent, {
      ...
      substIdStart: nextSubstId,
    });
    chunks.push(chunk);

    // Next chunk's init = this chunk's final context
    currentLinear = chunk.chips.flat_final.map(row => row[1]);
    nextSubstId = chunk._substIdCtr; // Continue from where this chunk left off
  }

  // PV normalization: all chunks must have the same max_ctx_size
  // so that FlatInitChip and FlatFinalChip produce identical PV counts → constant VK.
  const maxCtxSize = Math.max(...chunks.map(c => c.max_ctx_size));
  for (const chunk of chunks) {
    chunk.max_ctx_size = maxCtxSize;
  }

  // Full-program ROMs: union of all entries across chunks.
  // Preprocessed data (entries without num_lookups) becomes identical across
  // chunks → same Merkle commitment → same VK contribution.
  normalizeRoms(chunks);

  return chunks;
}

/**
 * Normalize ROM entries across chunks so preprocessed data is program-static.
 *
 * For each ROM type (formula, gamma, freevar), builds the union of all
 * entries across chunks. Each chunk gets the full union with per-chunk
 * num_lookups (0 for entries not used by that chunk). This ensures
 * identical preprocessed Merkle commitments → identical VK.
 */
function normalizeRoms(chunks) {
  // FormulaRom: [hash, tag, c0, c1, is_active, num_lookups]
  // Key: hash (unique — same hash always has same tag/c0/c1)
  ...
  // Key: full tuple (substId:freevarHash:groundValue) since substIds are chunk-local
  ...
  // CanonConsRom: [cons_hash, canon_cons, is_active, num_lookups]
  // Key: cons_hash (deterministically maps to canon_cons)
  const ccUnion = new Map();
  const ccLookups = [];
  for (const chunk of chunks) {
    const cl = new Map();
    for (const row of (chunk.canon_cons_rom || [])) {
      if (!ccUnion.has(row[0])) {
        ccUnion.set(row[0], [row[0], row[1], row[2]]);
      }
      cl.set(row[0], row[3]);
    }
    ccLookups.push(cl);
  }
  const ccKeys = [...ccUnion.keys()].sort((a, b) => a - b);
  for (let i = 0; i < chunks.length; i++) {
    chunks[i].canon_cons_rom = ccKeys.map(h => [
      ...ccUnion.get(h), ccLookups[i].get(h) || 0,
    ]);
  }
}

export { generateFlatWitness, generateChunkedFlatWitness, MAX_CONSUMED, MAX_PRODUCED };
export default { generateFlatWitness, generateChunkedFlatWitness, MAX_CONSUMED, MAX_PRODUCED };
