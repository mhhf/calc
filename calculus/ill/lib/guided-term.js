 ...
 */
function guidedTerm(trace, rfTerm) {
  let term = rfTerm;
...
function buildStepTerm(step, continuation) {
  ...
  // structure. loliOf peels bang/forall wrappers to find the loli node.
  const loliHash = isLoli ? step.loliHash : loliOf(rule.hash);
  if (!loliHash) {
  ...
  const antecedentProof = antProof(
  ...
  const groundMonadBody = rTensor(groundLinear);
  const groundMonadic = Store.put('monad', [monadUnit(), groundMonadBody]);
  ...
  const consumed = linLeaves(groundAntecedent);
  ...
  if (isLoli) {
    ...
    let inner2 = consqDecomp(groundLinear, continuation);
    ...
    // Phase 6-6a: persistentGoals annotation for predicate verification.
    // Each ground persistent goal (e.g., inc(2,3)) is extracted for in-circuit
    // arithmetic constraints in PredicateRomAir. Apply subApplyIdx to resolve
    // freevars in the evidence goals to their ground values.
    const persistentGoals = (persistentEvidence || [])
      .filter(ev => ev && ev.goal)
      .map(ev => subApplyIdx(ev.goal, theta, slots));
    inner = {
      rule: 'copy', principal: groundLoli, subterms: [inner2],
      consumed, produced: groundLinear, continuation, persistentGoals
    };
  }
 ...
 * provePersistent, used to fill the promotion() subterms.
 */
function antProof(patternHash, theta, slots, evidence) {
  const tag = Store.tag(patternHash);

  if (tag === 'tensor') {
    const left = antProof(Store.child(patternHash, 0), theta, slots, evidence);
    const right = antProof(Store.child(patternHash, 1), theta, slots, evidence);
    return { rule: 'tensor_r', principal: null, subterms: [left, right] };
  }

  if (tag === 'bang') {
    const inner = Store.child(patternHash, 1);
 ...
 *
 * Three resolution methods, matching provePersistent' three levels:
 ...
 * built by rTensor. Each tensor_l peels off the leftmost fact,
 * leaving it in delta for subsequent steps to consume.
 */
function consqDecomp(groundFacts, continuation) {
  ...
  for (let i = groundFacts.length - 2; i >= 0; i--) {
    ...
    const tensorHash = rTensor(remaining);
    inner = { rule: 'tensor_l', principal: tensorHash, subterms: [inner] };
  }
  return inner;
}

// Self-register as the bridge's guided-profile term builder: importing
// this module IS opting the ILL realization in (the generic bridge holds
// only a registration slot — TODO_0296 P2).
registerGuidedTerm(guidedTerm);

export { guidedTerm, loliOf, rTensor };
export default { guidedTerm, loliOf, rTensor };
