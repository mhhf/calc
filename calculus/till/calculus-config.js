...
// Connective + sort tables DERIVED from till.calc (kit.js — one source of
// truth: a connective exists iff declared there with @category; the former
// hand-written duplicate is how tables drift). tensor/loli/one/bang share
// ILL's store tags (one Store, shared numeric prelude); monad is till's
// own 2-ary monad (D6 — ILL's unary {A} untouched). The literal fences
// are the only till-specific sort data JS may hold: a numeral's
// membership in a refinement sort is a VALUE check (nonneg at 'delay',
// integral at 'count', [0,1] at 'weight'); sort EDGES live in logic files
// (prelude/rat.ill), never here.
const _fp = (h) => ratParts(h);
const { connectives: tillConnectives, sorts: tillSorts } = makeCalcTables(TILL_CALC, {
  fences: {
...
const tillGrades = {
  ...
  // (the StampTable's fixed cmp ≥ 0 cut), mass is never summed. The
  // measure class ('sum'/'sample') arrives with gill/will, never here.
...
const TILL_FFI_META = _face.META;

// ── Theory engine (TODO_0273, kit.js): discharges template theory
// premises. A `<- !qsub F E H` line in till.rules is a GOAL over the
// numeric theory (prelude/rat.ill clauses are the semantics; the FFI face
// is the terminating decision procedure — FFI principle). prove() returns
// [[metavar, value], …] — canonical and ground — or null: underivable ⇒
// the rule is simply inapplicable, which is how grade fences hold (qsub
// is checked; Grade Preservation, THY_0022).
const tillTheory = makeTheory({
  preludeFile: path.join(import.meta.dirname, 'prelude/rat.ill'),
  META: TILL_FFI_META,
  getConfig: () => tillCalculusConfig,
});
