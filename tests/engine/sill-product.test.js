...
`, sillConfig),
  ...
  it('!_W binds the TOTAL under product stamps; activation is the cw-join', () => {
    // Regression (suffix-anchor bug, 2026-09-04): the counted spread
    // anchored at every cohort; the suffix dropping g@(1~5) had activation
    // (2,3) ⊏lex the full take's cw-join (2,5), so minimization preferred
    // the partial take and !_W fired per-cohort with W=1 (D4 violation).
    const calc = loadTmp(`
g: type.
outw: bin -> type.
allg: !_W g -o { outw W }.
`, sillConfig);
    const r = calc.settle({
      linear: {
        [at(atom('g'), tpair(putRat(1n, 1n), putRat(5n, 1n)))]: 1,
        [at(atom('g'), tpair(putRat(2n, 1n), putRat(3n, 1n)))]: 1,
      }, persistent: {},
    }, '10');
    assert.equal(r.events.length, 1, 'one whole-bind firing, not per-cohort');
    assert.equal(r.events[0].theta[0], Store.put('binlit', [2n]), 'W = total');
    const key = at(Store.put('outw', [Store.put('binlit', [2n])]), tpair(putRat(2n, 1n), putRat(5n, 1n)));
    assert.equal(r.state.linear[key], 1, 'stamped at the componentwise join (2 ~ 5)');
  });

  it('lex frontier REGRESSES under whole-bind even choice-free (L2× witness, settle-optimality §8.4)', () => {
    // rt activates at join((2,3),(2,0)) = (2,3); allg (whole-bind) at
    // join((1,5),(2,3)) = (2,5) — no tie, ever (choice-free). rt fires
    // first and consumes g@(2~3); allg's forced join then DROPS to (1,5)
    // ⊏lex (2,3): the frontier regresses. The scalar rescue (L2's
    // choice-freedom disjunct) is C4-dependent — the join of lex-bounded
    // stamps can lex-exceed the bound only when merge ≠ order max — so it
    // does not lift to the product algebra. This is the executable
    // boundary witness for the whole-bind-free hypothesis in T1×.
    const calc = loadTmp(`
g: type.
h: type.
done1: type.
...
`, sillConfig);
    const r = calc.settle({
      linear: {
        [at(atom('g'), tpair(putRat(1n, 1n), putRat(5n, 1n)))]: 1,
        [at(atom('g'), tpair(putRat(2n, 1n), putRat(3n, 1n)))]: 1,
        [at(atom('h'), tpair(putRat(2n, 1n), putRat(0n, 1n)))]: 1,
      }, persistent: {},
    }, '10');
    assert.deepEqual(r.events.map((e) => e.rule), ['rt', 'allg']);
    const acts = r.events.map((e) => v.parse(e.activation));
    assert.deepEqual(acts[0], pv(2, 1, 3, 1), 'rt at (2,3)');
    assert.deepEqual(acts[1], pv(1, 1, 5, 1), 'allg at (1,5) — lex-below the fired frontier');
  });
