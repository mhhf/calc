...
const { unify: originalUnify } = require('../../lib/v2/kernel/unify');
const { apply: originalSubApply } = require('../../lib/v2/kernel/substitute');
const { buildIndex } = require('../../lib/mde/prove');
...
async function main() {
  const calc = await mde.load([path.join(__dirname, 'fixtures/bin.mde')]);
