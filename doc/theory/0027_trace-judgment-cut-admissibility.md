...
unique_contribution: "First cut-admissibility result for a measure-weighted existential quantifier (constructor priors ρ : s → ℚ≥0) in a linear sequent calculus, with EXACT weight conservation — no relaxation order, no expectation, no threshold. The mechanism is new AT THE THEOREM LEVEL (2026-09-01 adversarial novelty audit): the run trace is not a judgment annotation but a definable zone of linear draw-token hypotheses, making weight a function of the ENDSEQUENT — no prior system computes probability from which hypotheses remain in the endsequent (idea-level relatives to cite and distinguish: PRISM's msw switches are persistent/memoized facts with no proof theory; Dahlqvist–Kozen's randomness-as-linear-resource is denotational only; time/error credits are anonymous additive budgets; nominal fresh names carry no weight). The provenance connection is also sharpened: where Green–Tannen sum monomials over proof trees, here each proof carries exactly ONE monomial — its endsequent's token multiset — because tokens are linear. Three structural consequences not in the literature: (1) ghost-draw = weakening restricted to the token class, forced by cut reduction (tape-linear systems are not closed under cut reduction) and excluded from canonical counting exactly like ordinary weakening under focusing; (2) no promotion through a draw — !R's empty-linear-zone condition, applied to tokens, is precisely what makes contraction-driven duplication and weakening-driven erasure trace-safe; (3) identity expansion fails at ∃_ρ — a proof-theoretic no-cloning: a committed draw can be passed along whole but not deconstructed and re-derived."
references:
