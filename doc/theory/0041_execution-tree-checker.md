...
Facts are compared **modulo theory**: every fact the checker touches
(initial, produced, consumed, recorded terminal) is routed through the
composed theory canonicalizer (in the TCB), so a checker never rests on
representation equality — two theory-equal facts (`i(i(e))` and `binlit 3`)
collapse to one representative. Full certification is `valid && !unsupported`
(mirroring the backward kernel's `valid && !unverified`): a rule shape the
checker cannot re-derive — a **grade-0-specialized / fused** rule, which
binds antecedent positions at compile time outside the runtime θ — is
reported `unsupported`, never `error`; branding a valid exploration unsound
would be the one thing a soundness checker must not do. Such programs are
certified per-path by the guided profile (`buildGuidedTerm`/`checkTerm`)
instead.
