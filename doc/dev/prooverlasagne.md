...
3. **Rule specs hardcoded in the prover.** `buildRuleSpecs()` in `focused/prover.js` manually defines `makePremises` functions for each ILL rule (~170 lines of hand-coded JS). Meanwhile, the *same rules* are already declared in `calculus/ill.rules` and parsed into descriptors. The `.rules` file is the source of truth — but the prover ignores it and reimplements the rules from scratch. See [section 3.1](#31-the-rules-pipeline-what-already-exists) for the full analysis of what's generated vs what's hardcoded.
...
```
ill.calc + ill.rules
       │ (rules2-parser: sequent notation → flat descriptors)
       ▼
calculus/index.js:loadRules()
       │ (build calculus object)
       ▼
calculus object = {
...
| `rules[name].descriptor` | `rules2-parser.js:parseRules2` | Flat descriptor: connective, side, arity, contextSplit, contextFlow, premises |
| `rules[name].numPremises` | `rules2-parser.js:parseRules2` | Premise count |
| `parser`, `renderer` | `calculus/index.js:buildParser/Renderer` | From `@ascii`/`@latex`/`@prec` annotations |

**What's still hardcoded (not generated from `.rules`):**

| What | Where | Notes |
|---|---|---|
| Identity axiom logic | `focused/prover.js:tryIdentity` | Unification-based matching — moves to L2 |
| Context threading | `focused/prover.js:prove` | Hodas-Miller lazy delta passing — moves to L2 |

`makePremises` generation is **RESOLVED** — `rule-interpreter.js` (251 lines) reads rule descriptors and produces `{ specs, alternatives }`. Handles structural rules, axiom rules, alternatives (dereliction/absorption). 41 tests, all 13 ILL rules verified.
...
- **Composability.** L4 strategies should have functional, stateless interfaces so they can be composed in the future. Isabelle's tactical combinators (`THEN`, `ORELSE`, `REPEAT`) compose tactics algebraically — our L4 strategies should be composable the same way. Not implementing combinators now (premature), but keeping L4 APIs as pure `(state) → result` functions makes future composition straightforward.

- **Async/parallel.** Sledgehammer (Isabelle) races multiple provers in parallel, first success wins, then reconstructs the proof locally. Our L4 strategies could similarly be raced via `Promise.race([auto, generic, ...])`. Deferred — but L4 strategies should be async-capable for this reason. The key Sledgehammer insight: external provers act as "relevance filters," identifying *which lemmas* matter; a trusted local prover then uses those lemmas.
...
### 3.6.1 Open Question: Lax Monad as Backward/Forward Integration

CLF/Celf/LolliMon integrate forward and backward chaining via the lax monad `{A}`: entering the monad switches from backward (L2/L3) to forward (L4c), exiting at quiescence. This would make L4c a **subroutine** of L2 rather than a peer strategy:

```
backward search → encounters {A} → mode switch → forward engine runs → quiescence → resume backward
```

**Impact on layer design:**
- L2 recognizes `monad_r` as a mode-switching rule, calls L4c internally
- L4c becomes callable from L2, not just from L4 strategy level
- Ceptre stages (named rule subsets that each run to quiescence, with inter-stage transitions) are an alternative structuring mechanism for forward chaining

This is a **major open question** that may restructure the L2/L4c boundary. Significant research needed before implementation — see [[clf-celf-ceptre]], [[execution-trees-metaproofs]], [[muMALL-fixed-points]]. For now, L4c is a standalone strategy. The monad integration should be studied alongside μMALL fixed points, as both extend the logic itself.
...
|---|---|---|---|
| Lax monad `{A}` as backward/forward mode switch | **HIGH** | needs-research | CLF/Celf/LolliMon integrate via monad. May restructure L2/L4c boundary. Study alongside μMALL. See §3.6.1, [[clf-celf-ceptre]] |
| Analysis layer / metaproofs | MEDIUM | needs-research | Property verification over execution trees: conservation, safety, reachability, deadlock-freedom. Extends L4d. See [[execution-trees-metaproofs]] |
| Generic structural connective interpretation per family | MEDIUM | todo | Different families (LNL, display, adjoint) have different structural connective sets. The interpreter needs to be parameterized by family |
| Dual representation elimination (seq.linear vs delta) | MEDIUM | needs-benchmarks | Two representations of linear resources costs ~25% runtime. L2 should use multiset as single source of truth. Benchmark before changing |
| Ceptre stages | LOW | needs-research | Named rule subsets that each run to quiescence, with inter-stage transitions. Alternative to flat forward chaining. Study after μMALL. See [[clf-celf-ceptre]] §3.3 |
| Forward/backward state unification | LOW | needs-benchmarks | Only pursue if sharing saves significant code without hurting forward throughput |
| LCF-style abstract types in JS | LOW | deferred | WeakMap + closures could simulate ML's abstract types. Not urgent |
| ~~Generic structural interpreter for `.rules` ASTs~~ | ~~HIGH~~ | **DONE** | `rule-interpreter.js` (251 lines). 41 tests |
...
- [[prover-optimization]] — dual representation, benchmarks, optimization catalog
- [[clf-celf-ceptre]] — lax monad, forward chaining, Ceptre stages
- [[execution-trees-metaproofs]] — branching execution, metaproofs, μMALL
